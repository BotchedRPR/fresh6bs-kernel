...
#else
static inline struct sb_pqueue *sb_pq_create(unsigned int flag, unsigned int size, sb_cmp_func cmp)
{ return ERR_PTR(SB_PQUEUE_DISABLE); }
static inline void sb_pq_destroy(struct sb_pqueue *pq) {}

static inline int sb_pq_pop(struct sb_pqueue *pq, sb_data *data)
{ return SB_PQUEUE_DISABLE; }
static inline int sb_pq_push(struct sb_pqueue *pq, unsigned int idx, sb_data data)
{ return SB_PQUEUE_DISABLE; }

static inline bool sb_pq_empty(struct sb_pqueue *pq)
{ return true; }
static inline void sb_pq_clear(struct sb_pqueue *pq)
{}
...
{ return SB_PQUEUE_DISABLE; }
static inline int sb_pq_get_data(struct sb_pqueue *pq, unsigned int idx, sb_data *data)
{ return SB_PQUEUE_DISABLE; }

static inline int sb_pq_top(struct sb_pqueue *pq, sb_data *data)
{ return SB_PQUEUE_DISABLE; }
