...
#define SB_MISC_DISABLE	(-3664)
#if IS_ENABLED(CONFIG_SB_NOTIFY)
struct sb_misc *sb_misc_init(struct device *dev, const char *path, sb_event_func cb_ioctl);
int sb_misc_exit(struct sb_misc *misc);
#else
static inline struct sb_misc *sb_misc_init(struct device *dev, const char *path, sb_event_func cb_ioctl)
