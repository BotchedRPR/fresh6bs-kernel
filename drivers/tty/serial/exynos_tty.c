...
int skip_packets = 1000;	// skip patchram data
#define PTI_COMMAND			0x01
#define PTI_EVENT			0x04
#define CMD_COMPLETE_EVENT	0x0E
#define CMD_STATUS_EVENT	0x0F
				...
				unsigned char* trace_buf, int len)
{
	...
	if (ourport->dbg_mode & UART_DBG_MODE)
	{
		...
		if (dir == 0 ) {
			/* Allow only PTIs of command and event */
			if (trace_buf[0] == PTI_COMMAND)
				pr_err("[TX] %02X %02X %02X\n", trace_buf[0], trace_buf[1], trace_buf[2]);
		} else if (dir == 1) {
			if (trace_buf[0] == PTI_EVENT) {
				if (trace_buf[1] == CMD_STATUS_EVENT || trace_buf[1] == CMD_COMPLETE_EVENT)
					pr_err("[RX] %02X %02X %02X\n", trace_buf[0], trace_buf[1], trace_buf[2]);
			}
		}
		return;
	}
	...
	while (max_count-- > 0) {
		...
		if (unlikely(uerstat & S3C2410_UERSTAT_ANY)) {
			...
			if (uerstat & S3C2410_UERSTAT_FRAME) {
				port->icount.frame++;
				pr_err("[tty] uerstat & S3C2410_UERSTAT_FRAME! (%d)\n", port->icount.frame);
			}
			if (uerstat & S3C2410_UERSTAT_OVERRUN) {
				port->icount.overrun++;
				pr_err("[tty] uerstat & S3C2410_UERSTAT_OVERRUN! (%d\n", port->icount.overrun);
			}
		...
		if (ourport->uart_logging || ourport->dbg_mode & UART_DBG_MODE)
...
static int exynos_serial_probe(struct platform_device *pdev)
{
	...
	if (ourport->uart_logging == 1) {
		...
		if (!ourport->uart_local_buf.buffer) {
			...
			pr_err("Failed to alloc uart_local_buf\n");
			return -ENOMEM;
		}
