...
static void s2m_set_thermal_charging_current(struct s2m_chg_manager_info *battery, int *charging_current)
{
	...
	thermal_limit_level = battery->pdata->thermal_limit_level[battery->thermal_limit - 1];
	thermal_limit = battery->thermal_limit;
...
static int s2m_ifconn_handle_notification(struct s2m_chg_manager_info *battery, unsigned long action, void *data)
{
	struct ifconn_notifier_template *ifconn_info = (struct ifconn_notifier_template *)data;
	muic_attached_dev_t attached_dev = (muic_attached_dev_t)ifconn_info->event;
	int cable_type = 0;
...
static int s2m_ifconn_handle_pd_notification(struct notifier_block *nb, unsigned long action, void *data)
{
	struct s2m_chg_manager_info *battery = container_of(nb, struct s2m_chg_manager_info, ifconn_pd_nb);

	return s2m_ifconn_handle_notification(battery, action, data);
}

static int s2m_ifconn_handle_mu_notification(struct notifier_block *nb, unsigned long action, void *data)
{
	struct s2m_chg_manager_info *battery = container_of(nb, struct s2m_chg_manager_info, ifconn_mu_nb);
...
#endif
#if IS_ENABLED(CONFIG_IFCONN_NOTIFIER)
	ifconn_notifier_register(&battery->ifconn_pd_nb,
				 s2m_ifconn_handle_pd_notification,
				 IFCONN_NOTIFY_BATTERY,
				 IFCONN_NOTIFY_MANAGER_PDIC);
	ifconn_notifier_register(&battery->ifconn_mu_nb,
				 s2m_ifconn_handle_mu_notification,
				 IFCONN_NOTIFY_BATTERY,
