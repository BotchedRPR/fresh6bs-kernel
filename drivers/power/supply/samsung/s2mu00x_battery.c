...
#include <linux/power/samsung/s2mu00x_battery.h>
...
#include <linux/misc/samsung/muic/muic_notifier.h>
#endif
...
static int set_charging_current(struct s2mu00x_battery_info *battery)
{
	...
	if (battery->topoff_current != topoff_current) {
		...
		if (!psy)
			return -EINVAL;
		ret = power_supply_set_property(psy, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_CURRENT_FULL, &value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = battery->pdata->chg_full_vcell;
		break;
...
#if IS_ENABLED(CONFIG_HV_MUIC_S2MU004_PE)
		...
		unsigned long action, void *data)
{
	...
	if (attached_dev == ATTACHED_DEV_OTG_MUIC) {
		if (!strcmp(cmd, "ATTACH")) {
			...
			if (!psy)
				return -EINVAL;
			ret = power_supply_set_property(psy, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_CHARGE_OTG_CONTROL, &value);
			if (ret < 0)
				pr_err("%s: Fail to execute property\n", __func__);
