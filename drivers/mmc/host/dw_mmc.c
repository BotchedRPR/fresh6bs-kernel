...
static void dw_mci_sicd_setting(struct mmc_host *mmc, bool set)
{
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct dw_mci *host = slot->host;
...
inline u32 dw_mci_calc_hto_timeout(struct dw_mci *host)
{
	...
	if (!host->extended_tmout)
		...
		max_time = SDMMC_DATA_EXT_TMOUT_MAX_CNT * SDMMC_DATA_EXT_TMOUT_CRT / (host_clock / 1000);
	...
	if (target_timeout < max_time) {
		...
		if (!host->extended_tmout)
			max_ext_time = SDMMC_DATA_TMOUT_EXT_MULTI / (host_clock / 1000);
		else
			max_ext_time = SDMMC_DATA_EXT_TMOUT_EXT_MULTI / (host_clock / 1000);
	...
	if (!host->extended_tmout) {
		tmout_value &= ~(SDMMC_DATA_TMOUT_EXT_MAX_CNT << SDMMC_DATA_TMOUT_EXT_SHIFT);
		tmout_value |= ((ext_cnt + SDMMC_DATA_TMOUT_EXT_ADD) << SDMMC_DATA_TMOUT_EXT_SHIFT);
	} else {
		tmout_value &= ~(SDMMC_DATA_EXT_TMOUT_EXT_MAX_CNT << SDMMC_DATA_EXT_TMOUT_EXT_SHIFT);
		tmout_value |= ((ext_cnt + SDMMC_DATA_EXT_TMOUT_EXT_ADD) << SDMMC_DATA_EXT_TMOUT_EXT_SHIFT);
	}
...
inline u32 dw_mci_calc_timeout(struct dw_mci *host)
{
	...
	if (!host->extended_tmout) {
		/* normal time out check */
		if (!(mci_readl(host, BLOCK_DMA_FOR_CI) & (0x1 << 29))) {
			temp = mci_readl(host, BLOCK_DMA_FOR_CI);
			temp |= (0x1 << 29);
			mci_writel(host, BLOCK_DMA_FOR_CI, temp);
		}
		/* value = (TMOUT[31:11] * 8 + 0xffffff * (TMOUT[10:8]-1)) * cclk_in */

		/* time = TMOUT[31:11] * 8 * cclk_in */
		max_time = SDMMC_DATA_TMOUT_MAX_CNT * SDMMC_DATA_TMOUT_CRT / (host_clock / 1000);

		/* if over max_time, calculate TMOUT[10:8] */
		if (target_timeout > max_time) {
			/* calculate TMOUT[10:8] : time = 0xffffff * (TMOUT[10:8]-1) * cclk_in */
			max_ext_time = SDMMC_DATA_TMOUT_EXT_MULTI / (host_clock / 1000);
			ext_cnt = target_timeout / max_ext_time;
			target_timeout -= (max_ext_time * ext_cnt);
		}

		/* calculate TMOUT[31:11] */
		count = (target_timeout * (host_clock / 1000)) / SDMMC_DATA_TMOUT_CRT;
		ext_cnt += SDMMC_DATA_TMOUT_EXT_ADD;
		/* adjust over value */
		if (count > SDMMC_DATA_TMOUT_MAX_CNT)
			count = SDMMC_DATA_TMOUT_MAX_CNT;
		if (ext_cnt > SDMMC_DATA_TMOUT_EXT_MAX_CNT)
			ext_cnt = SDMMC_DATA_TMOUT_EXT_MAX_CNT;

		ret = ((count << SDMMC_DATA_TMOUT_SHIFT)
			| (ext_cnt << SDMMC_DATA_TMOUT_EXT_SHIFT)
			| SDMMC_RESP_TMOUT);
	} else {
		/* extended time out check */
		if ((mci_readl(host, BLOCK_DMA_FOR_CI) & (0x1 << 29))) {
			temp = mci_readl(host, BLOCK_DMA_FOR_CI);
			temp &= ~(0x1 << 29);
			mci_writel(host, BLOCK_DMA_FOR_CI, temp);
		}
		/* value = (TMOUT[31:12] * 16 + 0x1000000 * (TMOUT[11:8]-1)) * cclk_in */

		/* time = TMOUT[31:12] * 16 * cclk_in */
		max_time = SDMMC_DATA_EXT_TMOUT_MAX_CNT * SDMMC_DATA_EXT_TMOUT_CRT / (host_clock / 1000);

		/* if over max_time, calculate TMOUT[11:8] */
		if (target_timeout > max_time) {
			/* calculate TMOUT[11:8] : time = 0x1000000 * (TMOUT[11:8]-1) * cclk_in */
			max_ext_time = SDMMC_DATA_EXT_TMOUT_EXT_MULTI / (host_clock / 1000);
			ext_cnt = target_timeout / max_ext_time;
			target_timeout -= (max_ext_time * ext_cnt);
		}

		/* calculate TMOUT[31:12] */
		count = (target_timeout * (host_clock / 1000)) / SDMMC_DATA_EXT_TMOUT_CRT;

		ext_cnt += SDMMC_DATA_EXT_TMOUT_EXT_ADD;
		/* adjust over value */
		if (count > SDMMC_DATA_EXT_TMOUT_MAX_CNT)
			count = SDMMC_DATA_EXT_TMOUT_MAX_CNT;
		if (ext_cnt > SDMMC_DATA_EXT_TMOUT_EXT_MAX_CNT)
			ext_cnt = SDMMC_DATA_EXT_TMOUT_EXT_MAX_CNT;

		ret = ((count << SDMMC_DATA_EXT_TMOUT_SHIFT)
			| (ext_cnt << SDMMC_DATA_EXT_TMOUT_EXT_SHIFT)
			| SDMMC_RESP_TMOUT);
	}
...
#if IS_ENABLED(CONFIG_SEC_MMC_FEATURE)
void dw_mci_cmdq_err_check(struct mmc_host *mmc, u32 cmd_error, u32 data_error, u32 status)
{
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct dw_mci *host = slot->host;
	const struct dw_mci_drv_data *drv_data = host->drv_data;

	if (drv_data && drv_data->cq_err_check)
		drv_data->cq_err_check(cmd_error, data_error, status ? status : mci_readl(host, RESP0));
}
#endif
...
static int dw_mci_emmc_pwr_control(struct dw_mci *host, unsigned int power_mode)
{
	...
	switch (power_mode) {
	case MMC_POWER_ON:
		if (pdata->emmc_pwr.vcc_off_time == -1LL)
			goto resume;

		now = ktime_get();
		discharge_period = ktime_to_ms(
			ktime_sub(now, pdata->emmc_pwr.vcc_off_time));
		if (mmc->card && mmc_card_suspended(mmc->card) &&
			discharge_period < pdata->emmc_pwr.dis_charge) {
			dev_info(host->dev, "%s: need to give delay: discharge_period only = %lld\n",
					__func__, discharge_period);
			mdelay(pdata->emmc_pwr.dis_charge - discharge_period);
		}

		pdata->emmc_pwr.vcc_off_time = -1LL;
resume:
		...
		if (pdata->pinctrl && pdata->clk_drive_base)
			pinctrl_select_state(pdata->pinctrl, pdata->clk_drive_base);

		break;
	case MMC_POWER_OFF:
		if (!(mmc->caps2 & MMC_CAP2_FULL_PWR_CYCLE)) {
			dev_info(host->dev, "%s : Skip mmc turn off", __func__);
			goto vcc_off;
		}

		if (pdata->pinctrl && pdata->clk_drive_pdn)
			pinctrl_select_state(pdata->pinctrl, pdata->clk_drive_pdn);
...
vcc_off:
		...
		pdata->emmc_pwr.vcc_off_time = ktime_get();
		break;
	}
...
__releases(&host->lock) __acquires(&host->lock)
{
	...
	if (drv_data->req_err_check)
		drv_data->req_err_check(host, mrq);
...
static void dw_mci_cmdq_pre_setting(struct mmc_host *mmc, bool set)
{
	struct dw_mci_slot *slot = mmc_priv(mmc);
	struct dw_mci *host = slot->host;
...
#endif
	.cmdq_pre_setting = dw_mci_cmdq_pre_setting,
#if IS_ENABLED(CONFIG_MMC_DW_DEBUG)
...
#endif
	.sicd_setting = dw_mci_sicd_setting,
#if IS_ENABLED(CONFIG_SEC_MMC_FEATURE)
	.err_check = dw_mci_cmdq_err_check,
#endif
};
...
//            mmc->pm_flags |= MMC_PM_IGNORE_PM_NOTIFY;
	...
	if (mmc->caps2 & MMC_CAP2_CQE) {
		...
		cq_host = cqhci_pltfm_init(to_platform_device(host->dev));
