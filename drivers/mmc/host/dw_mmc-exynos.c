...
static void dw_mci_exynos_config_hs400(struct dw_mci *host, u32 timing)
{
	...
	if (timing == MMC_TIMING_MMC_HS400) {
		...
		if (host->slot->mmc->ios.enhanced_strobe)
...
static int dw_mci_exynos_parse_dt(struct dw_mci *host)
{
	...
	if (IS_ERR(priv->clk_drive_pdn))
		...
		dev_info(host->dev, "use gpio control in suspend, resume\n");
	...
	case 0:
		...
		if (!(of_property_read_u32(np, "emmc-pwr-suspend", &priv->emmc_pwr.mmc_pwr_ctrl))) {
				/* init vcc-off timestamp */
				priv->emmc_pwr.vcc_off_time = -1LL;
				if (of_property_read_u32(np, "emmc-dis-charge-time", &priv->emmc_pwr.dis_charge))
					priv->emmc_pwr.dis_charge = 20;
				/* enable once to prevent power off use_cnt 0 regulators */
				priv->emmc_pwr.vemmc = devm_regulator_get_optional(host->dev, "vemmc");
				if (!IS_ERR(priv->emmc_pwr.vemmc)) {
					ret = regulator_enable(priv->emmc_pwr.vemmc);
					if (ret) {
						dev_err(host->dev, "vemmc regulators enable failed\n");
					}
				} else {
					dev_err(host->dev, "vemmc regulator initialization failed\n");
				}

				priv->emmc_pwr.vqemmc = devm_regulator_get_optional(host->dev, "vqemmc");
				if (!IS_ERR(priv->emmc_pwr.vqemmc)) {
					ret = regulator_enable(priv->emmc_pwr.vqemmc);
					if (ret) {
						dev_err(host->dev, "vqemmc regulators enable failed\n");
					}
				} else {
					dev_err(host->dev, "vqemmc regulator initialization failed\n");
				}
				dev_info(host->dev, "vemmc, vqemmc registration completed\n");
			}
		break;
	case 1:
		/* dwmmc1 : SDIO    */
...
#if defined(CONFIG_BCM43013) || defined(CONFIG_BCM43012)
		if (id == 2) {
			/* In case of Merlot, dwmmc2 is SDIO for Synaptics Wi-Fi chipset */
...
#if IS_ENABLED(CONFIG_SEC_MMC_FEATURE)
static void dw_mci_exynos_req_err_check(struct dw_mci *host,
		struct mmc_request *mrq)
{
	if (host->ch_id == MMC_CARD || host->ch_id == SD_CARD)
		if (host->slot && host->slot->mmc)
			mmc_sd_sec_log_req_err(host->slot->mmc, mrq);
}

static void dw_mci_exynos_cq_err_check(u32 cmd_error, u32 data_error, u32 status)
{
	mmc_sec_log_cq_err(cmd_error, data_error, status);
}
#endif
...
#if IS_ENABLED(CONFIG_SEC_MMC_FEATURE)
	.req_err_check = dw_mci_exynos_req_err_check,
	.cq_err_check = dw_mci_exynos_cq_err_check,
#endif
...
static int dw_mci_exynos_probe(struct platform_device *pdev)
{
	...
	if (host->slot && host->slot->mmc) {
		if (host->ch_id == MMC_CARD)
			mmc_sec_set_features(host->slot->mmc);
	}
#endif
