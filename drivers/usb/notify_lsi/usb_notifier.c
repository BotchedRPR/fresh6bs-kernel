...
#if IS_ENABLED(CONFIG_USB_OTG_SW_SWITCH)
...
#define USB_LDOCONTROL_EXYNOS9830	4
	...
	struct wakeup_source *switch_wakelock;
#endif
	...
	if (np) {
		...
		if (pdev) {
			pr_info("%s: get the %s platform_device\n",
				__func__, pdev->name);
			ret = dwc3_exynos_vbus_event(&pdev->dev, state);
			if (!ret)
				goto end;
			else
				pr_info("%s: dwc3_exynos_vbus_event error return %d\n",
						__func__, ret);
		}
	}
...
static void check_usb_id_state(int state)
{
	...
	if (np) {
		...
		if (pdev) {
			pr_info("%s: get the %s platform_device\n",
					__func__, pdev->name);
			ret = dwc3_exynos_id_event(&pdev->dev, state);
			if (!ret)
				goto end;
			else
				pr_info("%s: dwc3_exynos_id_event error return %d\n",
						__func__, ret);
		}
	}
	pr_err("%s: failed to get the platform_device\n", __func__);
...
#if IS_ENABLED(CONFIG_USB_OTG_SW_SWITCH)
			o_notify->muic_hw_switch = OTG_NOTI_ATTACH;
#else
...
#if IS_ENABLED(CONFIG_USB_OTG_SW_SWITCH)
				o_notify->muic_hw_switch = OTG_NOTI_DETACH;
#else
...
#if IS_ENABLED(CONFIG_USB_OTG_SW_SWITCH)
static void otg_disabled_func(struct timer_list *t)
{
	struct usb_notifier_platform_data *pdata = container_of(t,
						 struct usb_notifier_platform_data,
						 otg_disabled_timer);
	struct otg_notify *o_notify = get_otg_notify();

	if (!o_notify->muic_hw_switch) {
		pr_info("%s: otg connection timeout!!\n", __func__);
		o_notify->muic_sw_switch = SW_SWITCH_OFF;
	}

	cancel_delayed_work(&pdata->usbid_work);
#if 0
	if (pdata->switch_wakelock->active) {
		pr_info("%s: wake unlock!!\n", __func__);
		__pm_relax(pdata->switch_wakelock);
	}
#endif
}
...
{
	struct usb_notifier_platform_data *pdata = container_of(w,
						struct usb_notifier_platform_data,
						usbid_work.work);
	struct otg_notify *o_notify = get_otg_notify();
	union power_supply_propval value;
	static int pre_notify;

	if (o_notify->muic_sw_switch == SW_SWITCH_OFF) {
		pr_info("%s: sw switch off\n", __func__);
		del_timer_sync(&pdata->otg_disabled_timer);
		exynos_set_host(0);
	...
	if (pre_notify != o_notify->muic_hw_switch) {

		if (o_notify->muic_hw_switch == OTG_NOTI_ATTACH) {
			pr_info("%s: hw switch attached\n", __func__);
			value.intval = true;
			exynos_set_host(1);
		} else {
			pr_info("%s: hw switch detached\n", __func__);
			mod_timer(&pdata->otg_disabled_timer, jiffies + OTG_DISABLED_DELAY_TIME);
			value.intval = false;
			exynos_set_host(0);
		}

		pr_info("%s: change otg_hw_switch state! [%08X->%08X]\n", __func__, pre_notify, o_notify->muic_hw_switch);

		pre_notify = o_notify->muic_hw_switch;
	} else {
		goto restart;
	}

restart:
	schedule_delayed_work(&pdata->usbid_work, msecs_to_jiffies(500));
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	char value[32];
	struct otg_notify *o_notify = get_otg_notify();
	struct usb_notifier_platform_data *pdata = dev->platform_data;;
	...
	if (!strncmp(value, "enabled", 6)) {
		if(o_notify->muic_sw_switch == SW_SWITCH_ON){
			pr_info("%s: otg switch already enabled!\n", __func__);
			return size;
		}
		pr_info("%s: otg switch enabled\n", __func__);
		o_notify->muic_sw_switch = SW_SWITCH_ON;

		__pm_stay_awake(pdata->switch_wakelock);
		timer_setup(&pdata->otg_disabled_timer, otg_disabled_func, 0);
		pdata->otg_disabled_timer.expires = jiffies + OTG_DISABLED_DELAY_TIME;
		//pdata->otg_disabled_timer.function = otg_disabled_func;

		add_timer(&pdata->otg_disabled_timer);
	...
	schedule_delayed_work(&pdata->usbid_work, msecs_to_jiffies(0));
...
static int usb_notifier_probe(struct platform_device *pdev)
{
	...
	wakeup_source_register(&pdev->dev, "otg_sw_switch");
#endif
