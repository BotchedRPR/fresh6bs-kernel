				   ...
				   int *bypass)
{
	...
	if (g_xhci_exynos->hcd->self.root_hub == hdev)
		main_hcd = 1;

	user_scenario = xhci_exynos_scenario_info();
	pr_info("%s: scenario = %d, main = %d, pm = %d\n", __func__,
		user_scenario, main_hcd, xhci_exynos_pm_state);
	if (user_scenario == AUDIO_MODE_IN_CALL && main_hcd)
		*bypass = 1;
	else
		*bypass = 0;
