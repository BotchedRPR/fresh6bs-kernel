			  ...
			  const struct usb_ctrlrequest *creq)
{
	...
	spin_lock_irqsave(&ffs->ev.waitq.lock, flags);
	if (!wq_has_sleeper(&ffs->ev.waitq)) {
		spin_unlock_irqrestore(&ffs->ev.waitq.lock, flags);
		pr_err("usb: %s no waiter for setup packet control req %02x.%02x v%04x i%04x l%04x\n",
			__func__, creq->bRequestType, creq->bRequest,
			le16_to_cpu(creq->wValue), le16_to_cpu(creq->wIndex), le16_to_cpu(creq->wLength));
		return -ENODATA;
	}
	ffs->ev.setup = *creq;
