...
static int p9222_normal_full_charge(struct p9222_charger_data *charger)
{
	...
	sb_vote_set(charger->apm_vote, VOTER_WPC_CONTROL, true, WPC_APM_VOUT_OFF);
	msleep(200);
	sb_vote_set(charger->ldo_vote, VOTER_WPC_CONTROL, true, WPC_LDO_OFF);
...
static int p9222_normal_re_charge(struct p9222_charger_data *charger)
{
	...
	sb_vote_set(charger->ldo_vote, VOTER_WPC_CONTROL, false, WPC_LDO_ON);
	sb_vote_set(charger->apm_vote, VOTER_WPC_CONTROL, true, WPC_APM_VOUT_ON);
	...
	sb_vote_set(charger->apm_vote, VOTER_WPC_CONTROL, false, WPC_APM_NONE);
...
static int p9222_set_phm_vote(void *data, sb_data v)
{
...
static int p9222_set_apm_vote(void *data, sb_data v)
{
	struct p9222_charger_data *charger = data;
	...
	pr_info("%s: v = %lld, old_mode = 0x%x, new_mode = 0x%x, ret = %d, charge_mode = %d\n",
		__func__, v, old_mode, ap_mode, ret, charge_mode);
	return ret;
}

static int p9222_set_ldo_vote(void *data, sb_data v)
{
	struct p9222_charger_data *charger = data;
	bool acok = false;
	int ret = 0;
...
static int p9222_set_vm_phm(void *pdata, int event, int en, sb_data data)
{
	...
	ret = sb_vote_set(charger->phm_vote, event, en, data);

	return !ret;
}

static int p9222_get_vm_phm(void *pdata, sb_data *data)
{
	struct p9222_charger_data *charger = pdata;
...
static int p9222_monitor_work(struct p9222_charger_data *charger)
{
	...
	if (!p9222_is_phm(charger)) {
		if (p9222_check_darkzone_state(charger, vrect, vout)) {
			p9222_send_eop(charger, POWER_SUPPLY_EXT_HEALTH_UNDERVOLTAGE);
			...
			sb_vote_set(charger->apm_vote, VOTER_CABLE, true, WPC_APM_WAKE_UP);
		...
		const union power_supply_propval *val)
{
	...
	{
		sb_data apm_result = WPC_APM_NONE;

		sb_vote_get_result(charger->apm_vote, &apm_result);
		if (apm_result == WPC_APM_WAKE_UP)
			sb_vote_set(charger->apm_vote, VOTER_CABLE, true, WPC_APM_NORMAL);
		else
			sb_vote_refresh(charger->apm_vote);
	}
		break;
...
static void p9222_wpc_det_work(struct work_struct *work)
{
	...
	if (wc_w_state) {
		...
		sb_vote_set(charger->phm_vote, VOTER_WPC_CS100, false, WPC_PHM_OFF);
		sb_vote_set(charger->phm_vote, VOTER_WPC_ALIGN_CHK, false, WPC_PHM_OFF);
		sb_vote_set(charger->apm_vote, VOTER_TEST, false, WPC_APM_NONE);
		sb_vote_set(charger->apm_vote, VOTER_WPC_CONTROL, false, WPC_APM_NONE);
		sb_vote_set(charger->apm_vote, VOTER_CABLE, false, WPC_APM_NONE);
		sb_vote_set(charger->ldo_vote, VOTER_WPC_CONTROL, false, WPC_LDO_ON);
	}
...
static void p9222_curr_measure_work(struct work_struct *work)
{
	...
	sb_vote_set(charger->apm_vote, VOTER_TEST, true, WPC_APM_TEST);
...
static const struct sb_wrl_op p9222_wrl_op = {
	.set_vm_phm = p9222_set_vm_phm,
	.get_vm_phm = p9222_get_vm_phm,
};

static int p9222_charger_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct p9222_charger_data *charger;
	...
	charger->phm_vote = sb_vote_init(VN_WPC_PHM, SB_VOTE_TYPE_MAX, VOTER_MAX,
		false, sb_voter_name, p9222_set_phm_vote, NULL, charger);
	...
	sb_vote_set_pri(charger->phm_vote, VOTER_WPC_ID_CHK, VOTE_PRI_9);
	sb_vote_set_pri(charger->phm_vote, VOTER_WPC_DET, VOTE_PRI_10);


	charger->apm_vote = sb_vote_init(VN_WPC_APM, SB_VOTE_TYPE_MAX, VOTER_MAX,
		WPC_APM_NONE, sb_voter_name, p9222_set_apm_vote, NULL, charger);
	if (IS_ERR_OR_NULL(charger->apm_vote)) {
	...
	charger->ldo_vote = sb_vote_init(VN_WPC_LDO, SB_VOTE_TYPE_MAX, VOTER_MAX,
		WPC_LDO_ON, sb_voter_name, p9222_set_ldo_vote, NULL, charger);
	...
	sb_wrl_register(&p9222_wrl_op, charger);
...
err_ldo_vote:
	sb_vote_destroy(charger->apm_vote);
err_apm_vote:
