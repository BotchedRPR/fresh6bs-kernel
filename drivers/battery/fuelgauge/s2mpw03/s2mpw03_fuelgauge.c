...
static void s2mpw03_runtime_dumpdone(struct s2mpw03_fuelgauge_data *fuelgauge)
{
	...
	/* charger off */
	sb_vote_set_f(VN_CHG_EN, VOTER_S2MPW03_CONTROL, true, SB_CHG_MODE_CHARGING_OFF);
	...
	/* charger on */
	sb_vote_set_f(VN_CHG_EN, VOTER_S2MPW03_CONTROL, false, SB_CHG_MODE_CHARGING_OFF);
...
static void s2mpw03_fg_adjust_capacity_max(
	struct s2mpw03_fuelgauge_data *fuelgauge, int curr_raw_soc)
{
	int diff = 0;
...
static void s2mpw03_fg_calculate_dynamic_scale(
		struct s2mpw03_fuelgauge_data *fuelgauge, int capacity, bool scale_by_full)
{
	...
	if (!fuelgauge->is_end_charge & scale_by_full)
		capacity = 100;
	...
	pr_info("%s: %d is used for capacity_max, capacity(%d), is_end_charge(%d)\n",
		__func__, fuelgauge->capacity_max, capacity, fuelgauge->is_end_charge);
...
static int s2mpw03_fg_update_bat_param(struct s2mpw03_fuelgauge_data *fuelgauge, int step)
{
	...
	if ((param0 != bat_data->model_param1[0]) ||
		...
		/* charger on */
		sb_vote_set_f(VN_CHG_EN, VOTER_S2MPW03_CONTROL, false, false);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		if (fuelgauge->pdata->capacity_calculation_type &
			...
			s2mpw03_fg_adjust_capacity_max(fuelgauge, val->intval);
			s2mpw03_fg_get_scaled_capacity(fuelgauge, val);

			if (val->intval > 1010) {
				pr_info("%s: scaled capacity (%d)\n", __func__, val->intval);
				s2mpw03_fg_calculate_dynamic_scale(fuelgauge, 100, false);
			}
		}
#else
			if (fuelgauge->pdata->capacity_calculation_type &
	...
	enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (val->intval == POWER_SUPPLY_STATUS_FULL)
			fuelgauge->is_end_charge = true;
		else
			fuelgauge->is_end_charge = false;
		pr_info("%s: is_end_charge (%d)\n", __func__, fuelgauge->is_end_charge);
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL:
		if (fuelgauge->pdata->capacity_calculation_type &
				SEC_FUELGAUGE_CAPACITY_TYPE_DYNAMIC_SCALE){
#if defined(CONFIG_UI_SOC_PROLONGING)
			s2mpw03_fg_calculate_dynamic_scale(fuelgauge, val->intval, true);
#else
...
static int s2mpw03_fuelgauge_probe(struct platform_device *pdev)
{
	...
	fuelgauge->factory_mode = sb_get_factory_mode();
...
static void s2mpw03_fuelgauge_shutdown(struct platform_device *pdev)
{
	...
	pr_info("%s: S2MPW03 Fuelgauge driver shutdown\n", __func__);
}
...
static int __init s2mpw03_fuelgauge_init(void)
{
	...
	return platform_driver_register(&s2mpw03_fuelgauge_driver);
}
