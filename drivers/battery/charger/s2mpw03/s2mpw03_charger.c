...
struct s2mpw03_charger_platform_data {
	int chg_float_voltage;
	bool check_topoff_wa;
	char *charger_name;
...
struct current_table gcur_table[72];
...
static void s2mpw03_make_current_table(void)
{
	...
	struct current_table temp_table[72];
	int div_cur, i;
	u8 idx_table[702] = { 0 };
	u8 x, y;
...
static struct current_table *s2mpw03_find_current_data(unsigned int chg_cur)
{
	struct current_table *pret = &gcur_table[0];
	int start, end, mid = 0;
...
static void s2mpw03_test_read(struct s2mpw03_charger_data *charger)
{
	...
	/* print Shipmode CTRL 1  - Last Bit Should be 0 or 1*/
	s2mpw03_read_reg(charger->client, S2MPW03_CHG_REG_SHIP_MODE_CTRL1, &data);
	sprintf(str+strlen(str), "%x:%02x ", S2MPW03_CHG_REG_SHIP_MODE_CTRL1, data);

	/* print Shipmode CTRL 2 - data[6:4] should be 100  */
	s2mpw03_read_reg(charger->client, S2MPW03_CHG_REG_SHIP_MODE_CTRL2, &data);
	sprintf(str+strlen(str), "%x:%02x ", S2MPW03_CHG_REG_SHIP_MODE_CTRL2, data);
		...
		int float_voltage)
{
	...
	pr_err("%s: float_voltage %d & top off event [%d]\n",
...
static void s2mpw03_set_fast_charging_current(struct s2mpw03_charger_data *charger,
		int charging_current)
{
	struct current_table *pret = NULL;
	...
	pr_err("%s: chg_curr = %d, data = %d, add_sts = %d, add_val = %d\n",
...
static int s2mpw03_get_current_eoc_setting(struct s2mpw03_charger_data *charger)
{
	...
	pr_err("%s: data(0x%x), top-off current	%d\n", __func__, data, eoc_current[data]);
...
static int s2mpw03_set_in2bat_vote(void *data, sb_data v)
{
...
/* Battery Shipmode Setting Function*/
static void s2mpw03_set_shipmode_setting(struct s2mpw03_charger_data *charger, int enable)
{
	enable = (enable > 0) ? 1 : 0;
	if (enable) {
		pr_err("%s: Setting Shipmode DB Time to 32S\n", __func__);
		s2mpw03_update_reg(charger->client, S2MPW03_CHG_REG_SHIP_MODE_CTRL2,
			SHIPMODE_DB_TIME_32S << CHG_SHIP_MODE_CTRL2_SHIFT, CHG_SHIP_MODE_CTRL2_MASK);
	}
	/* enable Shipmode Use - Causes device to power off after 32s after VBUS is disconnected*/
	s2mpw03_update_reg(charger->client, S2MPW03_CHG_REG_SHIP_MODE_CTRL1,
		enable << CHG_SHIP_MODE_CTRL1_SHIFT, CHG_SHIP_MODE_CTRL1_MASK);
}
...
static bool s2mpw03_chg_init(struct s2mpw03_charger_data *charger)
{
	...
	charger->factory_mode = sb_get_factory_mode();
...
static bool s2mpw03_check_topoff_wa(int topoff, bool check_topoff_wa)
{
	...
	avg_cur = value.intval;

	value.intval = SEC_BATTERY_CURRENT_MA;
	psy_do_property("battery", get,
		POWER_SUPPLY_PROP_CURRENT_NOW, value);
	now_cur = value.intval;

	/* topoff w/a*/
	return (now_cur > 0) && (now_cur <= (topoff / 2)) &&
		(avg_cur > 0) && (avg_cur <= (topoff / 2));
}

static bool s2mpw03_check_topoff_status(u8 chg_sts1, u8 chg_sts3)
{
...
{
	u8 chg_sts1 = 0, chg_sts3 = 0, pmic_sts = 0;
	int ret = 0;
	...
	if (s2mpw03_check_topoff_status(chg_sts1, chg_sts3) ||
			s2mpw03_check_topoff_wa(charger->topoff_current, charger->pdata->check_topoff_wa))
...
static int s2mpw03_get_charging_health(struct s2mpw03_charger_data *charger)
{
	...
	if (is_wireless_type(charger->cable_type)) {
		...
		if (value.intval == POWER_SUPPLY_EXT_HEALTH_UNDERVOLTAGE)
...
static int s2mpw03_get_shipmode_setting(struct s2mpw03_charger_data *charger)
{
	int ret;
	u8 shipmode_setting;

	ret = s2mpw03_read_reg(charger->client, S2MPW03_CHG_REG_SHIP_MODE_CTRL1, &shipmode_setting);
	if (ret < 0) {
		pr_info("%s: SHIPMODE_CTRL1 Read Failed ret = %d\n", __func__, ret);
	} else if (shipmode_setting & CHG_SHIP_MODE_CTRL1_MASK) {
		ret = 1;
		pr_info("%s: Forced Shipmode Test ret = %d\n", __func__, ret);
	} else {
		ret = 0;
		pr_info("%s: Forced Shipmode not used ret = %d\n", __func__, ret);
	}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_SHIPMODE_TEST:
		val->intval = s2mpw03_get_shipmode_setting(charger);
		break;
	default:
		return -EINVAL;
	}
...
/*	int previous_cable_type = charger->cable_type; */
	...
	case POWER_SUPPLY_EXT_PROP_CHARGING_ENABLED:
		...
		pr_info("%s: %s\n", __func__,
			sb_chg_mode_name[charger->charge_mode]);
	...
	case POWER_SUPPLY_EXT_PROP_CHARGE_OTG_CONTROL:
		...
		break;
	case POWER_SUPPLY_EXT_PROP_SHIPMODE_TEST:
		s2mpw03_set_shipmode_setting(charger, val->intval);
		break;
		...
		struct s2mpw03_charger_platform_data *pdata)
{
	...
	pdata->check_topoff_wa = of_property_read_bool(np, "battery,check_topoff_wa");
	pr_info("%s : check_topoff_wa(%d)\n", __func__, pdata->check_topoff_wa);
...
static int s2mpw03_charger_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: S2MPW03 Charger driver probe\n", __func__);
	...
	charger->in2bat_vote = sb_vote_init(VN_IN2BAT, SB_VOTE_TYPE_MIN, VOTER_MAX,
		true, sb_voter_name, s2mpw03_set_in2bat_vote, NULL, charger);
	if (IS_ERR_OR_NULL(charger->in2bat_vote)) {
		dev_err(s2mpw03->dev, "%s: failed to init in2bat_vote\n", __func__);
		ret = -ENOMEM;
		goto err_in2bat_vote;
	}
	sb_vote_set_pri(charger->in2bat_vote, VOTER_S2MPW03_CONTROL, VOTE_PRI_10);
	...
	pr_info("%s: S2MPW03 charger driver loaded OK\n", __func__);
...
//	power_supply_unregister(&charger->psy_battery);
	sb_vote_destroy(charger->in2bat_vote);
err_in2bat_vote:
err_parse_dt:
err_parse_dt_nomem:
...
static void s2mpw03_charger_shutdown(struct platform_device *pdev)
{
	struct s2mpw03_charger_data *charger = platform_get_drvdata(pdev);

#if 0
	free_irq(charger->irq_chg, charger);
#endif
	free_irq(charger->irq_tmrout, charger);
	s2mpw03_muic_unregister(charger->muic);

	s2mpw03_enable_charger_switch(charger, true);
	sb_vote_set(charger->in2bat_vote, VOTER_S2MPW03_CONTROL, true, 1);
