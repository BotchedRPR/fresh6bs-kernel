...
static void s2mpw03_muic_free_irqs(struct s2mpw03_muic *muic)
{
	...
	FREE_IRQ(muic->irq_acokf, muic);
	FREE_IRQ(muic->irq_acokr, muic);
}
...
EXPORT_SYMBOL(s2mpw03_init_muic);
...
void s2mpw03_muic_unregister(struct s2mpw03_muic *muic)
{
	...
	cancel_delayed_work_sync(&muic->init_work);
	cancel_delayed_work_sync(&muic->attach_work);
	cancel_delayed_work_sync(&muic->detach_work);

	destroy_workqueue(muic->wqueue);
}
EXPORT_SYMBOL(s2mpw03_muic_unregister);

static int __init s2mpw03_muic_init(void)
{
	pr_info("%s:\n", __func__);
	return 0;
}
module_init(s2mpw03_muic_init);

static void __exit s2mpw03_muic_exit(void)
{
	pr_info("%s:\n", __func__);
}
module_exit(s2mpw03_muic_exit);
