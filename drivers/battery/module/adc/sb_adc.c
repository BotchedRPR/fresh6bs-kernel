...
#define SB_ADC_CELL_NAME	"#sb-adc-cells"
#define SB_ADC_CELL_COUNT	2
#define IO_CHANNEL_NAMES	"io-channel-names"
...
finish_sysfs:
	sysfs->adc = adc;
	/* add sysfs node */
	sb_sysfs_add_attrs(adc->name, PSY_BATTERY_NAME, sysfs->attr, idx);
...
static void del_adc(struct sb_adc *adc)
{
	...
	if (adc->sysfs != NULL) {
		sb_sysfs_remove_attrs(adc->name);
		adc->sysfs->adc = NULL;
	}
...
{
	if (IS_ERR_OR_NULL(adc) || (data == NULL))
		return -EINVAL;
...
{
	if (IS_ERR_OR_NULL(adc) || (value == NULL))
		return -EINVAL;
...
{
	struct sb_adc_param_list *param_list;
	struct of_phandle_args args;
	struct sb_adc_param *param;
	struct sb_adc *adc;
	int mapind, nummaps = 0;
	int ret;
	...
	do {
		ret = of_parse_phandle_with_args(np, name,
			SB_ADC_CELL_NAME, nummaps, NULL);
	...
	param_list = kzalloc(sizeof(struct sb_adc_param_list), GFP_KERNEL);
	if (!param_list)
		return ERR_PTR(-ENOMEM);

	param_list->name = name;
	param_list->count = nummaps;
	INIT_LIST_HEAD(&param_list->list);

	param = kcalloc(nummaps, sizeof(struct sb_adc_param), GFP_KERNEL);
	if (!param) {
		ret = -ENOMEM;
	...
	for (mapind = 0; mapind < nummaps; mapind++) {
		ret = of_parse_phandle_with_args(np, name,
			SB_ADC_CELL_NAME, mapind, &args);

		if (ret)
			goto err_get_param;

		if (args.args_count != SB_ADC_CELL_COUNT) {
			ret = -ENODEV;
			goto err_get_param;
		}

		mutex_lock(&adc_lock);
		adc = find_adc(args.np->name);
		if (adc == NULL) {
			adc_log("name = %s is null!!\n", args.np->name);
		} else {
			param[mapind].adc = adc;
			param[mapind].thresh = args.args[0];
			param[mapind].flag = args.args[1];
			list_add(&param[mapind].list, &param_list->list);

			adc_log("get adc (name = %s, threshold = %d, flag = %d)\n",
				param[mapind].adc->name, param[mapind].thresh, param[mapind].flag);
		}
		mutex_unlock(&adc_lock);
	}
...
EXPORT_SYMBOL(sb_adc_get_param_list);

void sb_adc_free_param_list(struct sb_adc_param_list *param_list)
{
	struct sb_adc_param *param;

	if (IS_ERR_OR_NULL(param_list))
		return;

	if (list_empty(&param_list->list))
		goto free_param_list;

	param = list_first_entry(&param_list->list, struct sb_adc_param, list);
	kfree(param);

free_param_list:
	kfree(param_list);
}
EXPORT_SYMBOL(sb_adc_free_param_list);

static bool check_param(struct sb_adc_param *param)
{
	int value = 0;
	...
	mutex_lock(&adc_lock);
	value = get_value(param->adc);
	mutex_unlock(&adc_lock);

	switch (param->flag) {
	case SB_ADC_FLAG_EQ:
		return (value == param->thresh);
	case SB_ADC_FLAG_NE:
		return (value != param->thresh);
	case SB_ADC_FLAG_LT:
		return (value < param->thresh);
...
unsigned int sb_adc_check_param_list(struct sb_adc_param_list *param_list)
{
	...
	list_for_each_entry(param, &param_list->list, list) {
...
EXPORT_SYMBOL(sb_adc_check_param_list);
...
static int adc_it_dummy(void *pdata, int *data)
{
	*data = -1;
	return 0;
}
...
static int parse_dt(struct device *dev)
{
	...
	prop = of_find_property(dev->of_node, IO_CHANNEL_NAMES, NULL);
	if (!prop)
		return -EINVAL;
	if (!prop->value)
		return -ENODATA;
	...
	for (; p < end; p += l) {
		...
		if (IS_ERR_OR_NULL(iio)) {
			adc_log("%s is not found iio channel, create by dummy\n", p);
			adc = sb_adc_add(p, dev, adc_it_dummy);
		} else {
