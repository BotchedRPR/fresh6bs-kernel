...
#if 0 // IS_ENABLED(CONFIG_SEC_EXT) build error
		...
		if (sscanf(buf, "%10d\n", &x) == 1) {
			...
			if (x == is_slate_mode(battery)) {
				...
				sec_bat_set_current_event(battery,
					...
					SEC_BAT_CURRENT_EVENT_SLATE);
				sb_wrl_set_vm_phm(VOTER_SLATE, true, WPC_PHM_ON);
				sb_vote_set(battery->chgen_vote, VOTER_SLATE, true, SB_CHG_MODE_BUCK_OFF);
				...
				sb_vote_set(battery->chgen_vote, VOTER_SLATE,
					false, 0);
				sb_wrl_set_vm_phm(VOTER_SLATE, false, WPC_PHM_OFF);
				dev_info(battery->dev,
...
{
	return sb_sysfs_add_attrs("event_attr", PSY_BATTERY_NAME,
		sysfs_event_attrs, ARRAY_SIZE(sysfs_event_attrs));
}
module_init(sysfs_event_init);

MODULE_DESCRIPTION("Samsung Battery Event Sysfs");
