...
EXPORT_SYMBOL(sec_bat_init_eng_data);

static bool sec_bat_set_charge_test(struct sec_battery_info *battery)
{
	...
	if (eng_data->charge_test_input > 0) {
		...
		sb_vote_set(battery->input_vote, VOTER_TEST, true, eng_data->charge_test_input);
		is_changed = true;
	}
	if (eng_data->charge_test_fcc > 0) {
		pr_info("%s : fcc charge test %d\n", __func__, eng_data->charge_test_fcc);
		sb_vote_set(battery->fcc_vote, VOTER_TEST, true, eng_data->charge_test_fcc);
		is_changed = true;
	}
	if (eng_data->charge_test_topoff > 0) {
		pr_info("%s : topoff charge test %d\n", __func__, eng_data->charge_test_topoff);
		sb_vote_set(battery->topoff_vote, VOTER_TEST, true, eng_data->charge_test_topoff);
		is_changed = true;
	}
...
{
	return sb_sysfs_add_attrs("battery_eng", PSY_BATTERY_NAME,
		sysfs_eng_attrs, ARRAY_SIZE(sysfs_eng_attrs));
}
module_init(sysfs_device_init);

MODULE_DESCRIPTION("Samsung Battery ENG Sysfs");
