				...
				const char *buf, size_t count)
{
	...
	case BATT_READ_RAW_SOC:
		break;
	case BATT_TYPE:
		snprintf(battery->batt_type, sizeof(battery->batt_type) - 1, "%s", buf);
...
{
	return sb_sysfs_add_attrs("battery_attr", PSY_BATTERY_NAME,
		sysfs_battery_attrs, ARRAY_SIZE(sysfs_battery_attrs));
}
module_init(sysfs_battery_init);

MODULE_DESCRIPTION("Samsung Battery Sysfs");
