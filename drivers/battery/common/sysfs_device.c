				...
				const char *buf, size_t count)
{
	...
	case BATT_CAPACITY_MAX:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			dev_err(battery->dev,
					"%s: BATT_CAPACITY_MAX(%d), fg_reset(%d)\n", __func__, x, sb_get_fg_reset());
			if (!sb_get_fg_reset() && !battery->store_mode) {
				value.intval = x;
...
{
	return sb_sysfs_add_attrs("device_attr", PSY_BATTERY_NAME,
		sysfs_device_attrs, ARRAY_SIZE(sysfs_device_attrs));
}
module_init(sysfs_device_init);

MODULE_DESCRIPTION("Samsung Battery Device Sysfs");
