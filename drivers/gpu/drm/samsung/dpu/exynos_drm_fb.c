...
static void exynos_drm_fb_set_offsets(struct drm_framebuffer *fb)
{
	 ...
	 */
	if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SBWC(SBWC_MOD_LOSSLESS, 0),
				fb->modifier) ||
			has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SBWC(SBWC_MOD_NONE, 0),
				fb->modifier)) {
		fb->offsets[1] = Y_SIZE_SBWC(fb->width, fb->height, is_10bpc);
	} else {
		...
		const struct drm_format_info *info)
{
	...
	if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SBWC(SBWC_MOD_LOSSY, 0),
				      ...
				      const struct drm_plane_state *plane_state)
{
	...
	if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SAJC(0), fb->modifier) ||
		...
		has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SBWC(SBWC_MOD_LOSSLESS, 0),
				fb->modifier) ||
		has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SBWC(SBWC_MOD_LOSSY, 0),
				fb->modifier))
		...
		struct drm_atomic_state *old_state)
{
	...
	for_each_oldnew_crtc_in_state(old_state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		if (diff_msec >= default_vblank_timeout)
			pr_warn("[CRTC:%d:%s] vblank wait timed out!!!(%lld msec / %lld msec)\n",
...
static void
exynos_drm_atomic_commit_modeset_disables(struct drm_atomic_state *old_state,
...
void exynos_atomic_commit_tail(struct drm_atomic_state *old_state)
{
	...
	exynos_drm_atomic_commit_modeset_disables(old_state, &hibernation_crtc_mask,
					&disabling_crtc_mask);
