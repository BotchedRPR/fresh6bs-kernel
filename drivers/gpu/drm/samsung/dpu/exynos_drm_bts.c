...
static u64 dpu_bts_get_resol_clock(struct decon_device *decon)
{
	...
	/*
	 * For 1H Time in Video Mode,
	 * vclk equation need to consider vblank
	 * vclk = hres * (vres + vblank) * fps
	 */

	if (decon->config.mode.op_mode == DECON_VIDEO_MODE)
		yres += decon->config.vblank;
...
void dpu_bts_set_bus_qos(const struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (is_version(&decon->config.version, 7, 0)) {
		/*
		 * wqhd(+) high fps case
		 *  - INT min freq : 400Mhz
		 *  - MIF min freq : 546Mhz
...
static void dpu_bts_dp_qos_update(struct decon_device *decon)
{
	...
	DPU_DEBUG_BTS(decon, "dp pixelclock(%u)\n", dp_pixelclock);
}
