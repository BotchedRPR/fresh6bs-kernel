...
#define DISP_V_MARGIN	20	/* 20% considering porch or guard time */
		...
		const struct drm_display_mode *mode)
{
	...
	if ((decon->config.svsync_time * NSEC_PER_USEC) > one_fr_t) {
		...
		partial_err("svsync_time(%dusec) is over one frame time(%dusec)\n",
		...
		const struct drm_display_mode *mode, int y1)
{
	...
	if (y1 >= svsync_y1_max) {
		partial_debug("partial y1(%d) is over svsync_y1_max(%d)\n",
				y1, svsync_y1_max);
		return false;
	} else
		return true;
		...
		const struct drm_rect *req, struct drm_rect *r)
{
	...
	if (!exynos_partial_check_svsync(partial, mode, r->y1)) {
		partial_debug("changed full: after checking svsync_time\n");
		return -EINVAL;
	}
		...
		const struct drm_rect *partial_r)
{
	...
	struct rcd_device *rcd = partial->exynos_crtc->rcd;
#endif
...
#if IS_ENABLED(CONFIG_EXYNOS_DMA_RCD)
	if (rcd && rcd->funcs)
		rcd->funcs->set_partial(rcd, partial_r);
#endif
...
exynos_partial_register(struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (decon->emul_mode) {
		partial_info("partial update disabled in emul-mode\n");
		return NULL;
	}
