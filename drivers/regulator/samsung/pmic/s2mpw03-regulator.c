...
static ssize_t revision_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct s2mpw03_dev *s2mpw03 = dev_get_drvdata(dev);
	int ret = 0;

	ret = snprintf(buf, 7, "EVT%u.%u\n", s2mpw03->pmic_rev, s2mpw03->pmic_ver);

	return ret;
}

static ssize_t b2_forced_pwm_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	struct s2mpw03_dev *s2mpw03 = dev_get_drvdata(dev);
	u8 reg_data;
	...
	/* BUCK2_CTRL Address = 0x118 */
	ret = s2mpw03_read_reg(s2mpw03->pmic, 0x18, &reg_data);
	if (ret)
		return ret;

	/* MODE_B2 = BUCK2_CTRL[3:2] (10 = Auto mode (Default), 11 = Force PWM) */
	ret = snprintf(buf, sizeof(buf), "%d\n", (reg_data >> 2) & 0x01);
	return ret;
}

static ssize_t b2_forced_pwm_store(struct device *dev, struct device_attribute *attr,
			    const char *buf, size_t size)
{
	 ...
	 * MODE_B2 = BUCK2_CTRL[3:2] (10 = Auto mode (Default), 11 = Force PWM)
	 */
	ret = s2mpw03_update_reg(s2mpw03->pmic, 0x18, value << 2, 0x01 << 2);
	if (ret < 0)
		return ret;
...
static DEVICE_ATTR_RW(b2_forced_pwm);

static struct attribute *s2mpw03_pmic_sysfs_attrs[] = {
	&dev_attr_revision.attr,
	&dev_attr_b2_forced_pwm.attr,
	NULL
};
static const struct attribute_group s2mpw03_pmic_sysfs_attr_group = {
	.attrs = s2mpw03_pmic_sysfs_attrs,
};
#endif
...
#if IS_ENABLED(CONFIG_SEC_PM)
	struct device *s2mpw03_pmic_sysfs;
#endif
...
#if IS_ENABLED(CONFIG_SEC_PM)
	s2mpw03_pmic_sysfs = sec_device_create(iodev, "pmic");
	if (IS_ERR(s2mpw03_pmic_sysfs))
		return -ENODEV;

	ret = sysfs_create_group(&s2mpw03_pmic_sysfs->kobj, &s2mpw03_pmic_sysfs_attr_group);
	if (ret) {
		sec_device_destroy(s2mpw03_pmic_sysfs->devt);
		return -ENODEV;
	}
#endif
