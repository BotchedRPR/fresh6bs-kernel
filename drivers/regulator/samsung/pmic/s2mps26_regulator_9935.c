...
#include <linux/samsung/pmic/s2mps26.h>
#include <linux/samsung/pmic/s2mps26-regulator-9935.h>
...
int sub_pmic_read_reg(struct i2c_client *i2c, uint8_t reg, uint8_t *val)
{
	...
	return s2mps26_read_reg(i2c, reg, val);
}
...
static int s2mps26_pmic_probe(struct platform_device *pdev)
{
	...
	ret = s2mps26_write_reg(s2mps26->iodev->pmic2, 0x5F, 0x00);
	if (ret)
		pr_err("%s: Failed to set off time delay\n", __func__);
