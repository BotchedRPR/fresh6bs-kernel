				...
				unsigned int target_freq, int relation)
{
	...
	case CPUFREQ_RELATION_C:
		index = cpufreq_table_find_index_ac(policy, target_freq);
		break;
	}
						...
						unsigned int relation)
{
	...
	if (cpumask_empty(&active_cpus)) {
		pr_info("%s: there is no active cpus in policy%d\n",
				__func__, cpumask_first(&domain->cpus));
		return -ENODEV;
	}

	policy = cpufreq_cpu_get(cpumask_any(&active_cpus));
	if (!policy) {
		pr_info("%s: failed get cpufreq policy\n", __func__);
		return -ENODEV;
	}
...
static unsigned int exynos_cpufreq_get(unsigned int cpu)
{
	...
	freq = resolve_freq(policy, freq, CPUFREQ_RELATION_C);
