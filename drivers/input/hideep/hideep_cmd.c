			...
			struct device_attribute *attr, char *buf)
{
	...
	for (ii = 0; ii < ts->vr_info->num_tx; ii++) {
		for (jj = 0; jj < ts->vr_info->num_rx; jj++) {
			...
			memset(temp, 0x00, 10);
			snprintf(temp, 10, "%d,", ts->rawdata[ii * ts->vr_info->num_rx + jj]);
			strlcat(buf, temp, PAGE_SIZE);
		}
	}
...
static DEVICE_ATTR(rawdata, 0444, rawdata_show, NULL);
...
static void run_cm_driver_read(void *device_data)
{
	struct sec_cmd_data *sec = (struct sec_cmd_data *)device_data;
	struct hideep_t *ts = container_of(sec, struct hideep_t, sec);
	char buff[SEC_CMD_STR_LEN] = {0};
	...
	ret = ts->hideep_api->i2c_read(ts, HIDEEP_CMD_CONNECTION_RESULT_REG, data, 8 * 11 * 2);
	if (ret < 0) {
		input_err(true, &ts->client->dev, "%s: connection result read failed\n", __func__);
		goto err_cm_driver_read;
	}
	...
	ret = 0;
	for (ii = 0; ii < 11; ii++) {
		pr_cont("[sec_input:hiddep]:[%02d]", ii);
		for (jj = 0; jj < ts->vr_info->num_rx; jj++)
			pr_cont(" %d", sdata[ii * ts->vr_info->num_rx + jj]);
		pr_cont("\n");
	}

	snprintf(buff, sizeof(buff), "%s", sdata[0] == 1 ? "OK" : "NG");
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
	sec->cmd_state = SEC_CMD_STATUS_OK;
...
err_cm_driver_read:
	...
	sec_cmd_set_cmd_result(sec, buff, strnlen(buff, sizeof(buff)));
	if (sec->cmd_all_factory_state == SEC_CMD_STATUS_RUNNING)
		sec_cmd_set_cmd_result_all(sec, buff, strnlen(buff, sizeof(buff)), "CM_DRIVER");
	sec->cmd_state = SEC_CMD_STATUS_FAIL;
...
static struct sec_cmd sec_cmds[] = {
	...
	{SEC_CMD("run_cm_driver_read", run_cm_driver_read),},
