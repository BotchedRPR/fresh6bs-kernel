...
static struct ztm620_info *g_ztm620_info;

static bool ztm620_regulator_on(struct ztm620_info *info);
static bool ztm620_regulator_off(struct ztm620_info *info);
...
static void run_wheel_read(void *device_data);
static void get_wheel(void *device_data);
#endif
#endif
...
static s32 ztm620_write_reg(struct i2c_client *client, u16 reg, u16 value)
{
	...
	ret = ztm620_write_data(client, reg, (u8 *)&value, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:failed to write reg 0x%04x. ret:%d\n",
			__func__, reg, ret);
		return I2C_FAIL;
	}
...
static s32 ztm620_write_cmd(struct i2c_client *client, u16 reg)
{
	...
	if (ret < 0) {
		info->comm_err_cnt++;
		dev_err(&client->dev, "%s:failed to send reg 0x%04x. ret:%d\n",
			__func__, reg, ret);
		return ret;
	}
...
static int ztm620_read_finger_cnt(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	s32 ret, val = 0;

	ret = ztm620_read_data(info->client, ZTM620_FINGER_CNT_REG, (u8 *)&val, 2);
	if (ret < 0) {
		dev_err(&client->dev,
			"%s:Failed to read ZTM620_FINGER_CNT_REG register\n", __func__);
		return -1;
	}
...
static int  ztm620_send_deep_sleep_cmd(struct ztm620_info *info)
{
	...
	if (ret) {
		dev_err(&client->dev,
			"%s:failed to write ZTM620_DEEP_SLEEP_CMD\n",
			__func__);
		return -1;
	}
...
static int  ztm620_write_wakeup_cmd(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	int ret, i;
	...
	for (i= 0; i < INIT_RETRY_CNT; i++) {
		...
		if (ret) {
			dev_err(&client->dev,
				"%s:failed to write ZTM620_WAKEUP_CMD\n", __func__);
		}
...
static int ztm620_set_optional_mode(struct ztm620_info *info, bool force)
{
...
static void ztm620_set_ta_status(struct ztm620_info *info)
{
	...
	ret = ztm620_set_optional_mode(info, false);
	if (ret != I2C_SUCCESS)
		dev_err(&client->dev, "%s:failed to set optional_mode\n", __func__);
}
...
static bool ztm620_ts_read_coord(struct ztm620_info *info)
{
	...
	if (info->touch_info.event_flag == 0 || info->touch_info.status == 0) {
		ret = ztm620_set_optional_mode(info, false);
		if (ret < 0) {
			dev_err(&client->dev, "%s:error set_optional_mode.\n", __func__);
			return false;
		}
	...
	ret = ztm620_set_optional_mode(info, false);
	if (ret < 0) {
		dev_err(&client->dev, "%s:error set optional_mode.\n", __func__);
		return false;
	}
...
retry_power_sequence:
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed to send power sequence(0xc004)\n", __func__);
		goto fail_power_sequence;
	}
	usleep_range(DELAY_FOR_POST_TRANSCATION, DELAY_FOR_POST_TRANSCATION);
...
int ztm620_regulator_init(struct ztm620_info *info)
{
	...
	if (info->avdd == NULL) {
		...
		ret = regulator_set_voltage(info->avdd, 3000000, 3000000);
		if (ret)
			dev_err(&client->dev,
...
static int ztm620_gpio_reset(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	struct zxt_ts_platform_data	*pdata = info->pdata;
	int ret = 0;
...
static int ztm620_mode_change_reset(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	struct zxt_ts_platform_data	*pdata = info->pdata;
	int ret = 0;
...
static int ztm620_hard_reset(struct ztm620_info *info)
{
	...
	ret = ztm620_gpio_reset(info);
	if (ret) {
		dev_err(&client->dev, "%s:Failed to ztm620_gpio_reset\n", __func__);
		goto err_out;
	}
...
static bool ztm620_init_touch(struct ztm620_info *info, bool fw_skip)
{
	...
	info->wet_mode = 0;
	info->noise_mode = 0;
#if 0 //for bringup stage,not load firmware
	if (info->fw_data == NULL) {
...
retry_init:
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read ZTM620_HW_ID\n", __func__);
		goto fail_init;
	}
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM620_FIRMWARE_VERSION[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm620_read_data(client, ZTM620_MINOR_FW_VERSION, (u8 *)&cap->fw_minor_version, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM620_MINOR_FW_VERSION[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm620_read_data(client, ZTM620_DATA_VERSION_REG, (u8 *)&cap->reg_data_version, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM620_DATA_VERSION_REG[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm620_read_data(client, ZTM620_EEPROM_INFO_REG, (u8 *)&chip_eeprom_info, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed ZTM620_EEPROM_INFO_REG[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm620_check_need_upgrade(info, cap->fw_version,
	...
	if (ret && !fw_skip) {
		...
		if (!ret) {
			dev_err(&client->dev, "%s:Failed ztm620_upgrade_firmware[%d]\n", __func__, ret);
			goto fail_init;
		}
		dev_info(&client->dev, "%s:Upgrade complete\n", __func__);
		...
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:Failed INT_ENABLE_FLAG[%d]\n", __func__, ret);
			goto fail_init;
		}
	...
	if (ret) {
		...
		ret = ztm620_write_reg(client, ZTM620_INT_ENABLE_FLAG, 0);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:Failed ZTM620_INT_ENABLE_FLAG[%d]\n", __func__, ret);
			goto fail_init;
		}
	}
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM620_X_RESOLUTION[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm620_write_reg(client, ZTM620_Y_RESOLUTION, (u16)pdata->y_resolution + ABS_PT_OFFSET);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM620_Y_RESOLUTION[%d]\n", __func__, ret);
		goto fail_init;
	}

	if (pdata->orientation != -1) {
		ret = ztm620_write_reg(client, ZTM620_COORD_ORIENTATION, (u16)pdata->orientation);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:Failed ZTM620_COORD_ORIENTATION[%d]\n", __func__, ret);
			goto fail_init;
		}
	}
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM620_SUPPORTED_FINGER_NUM[%d]\n", __func__, ret);
		goto fail_init;
	}
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM620_INITIAL_TOUCH_MODE[%d]\n", __func__, ret);
		goto fail_init;
	}

	ret = ztm620_write_reg(client, ZTM620_TOUCH_MODE, info->touch_mode);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:Failed ZTM620_TOUCH_MODE[%d]\n", __func__, ret);
		goto fail_init;
	}
	...
	ret = ztm620_set_optional_mode(info, true);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to set optional_mode\n", __func__);
		goto fail_init;
	}
	...
	for (i = 0; i < 10; i++) {
		ret = ztm620_write_cmd(client, ZTM620_CLEAR_INT_STATUS_CMD);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:failed to write ZTM620_CLEAR_INT_STATUS_CMD[%d]\n", __func__, i);
		}
		usleep_range(10, 10);
	}
...
static int ztm620_hard_reset_recovery(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
...
static bool ztm620_mini_init_touch(struct ztm620_info *info)
{
	...
	for (i = 0; i < INIT_RETRY_CNT; i++) {
		...
		if (ret >= 0) {
			if (chip_check_sum == ZTM620_CHECK_SUM) {
				...
				dev_err(&client->dev, "%s: Invalid chip_check_sum [0x%04x]\n", __func__, chip_check_sum);
		} else
			dev_err(&client->dev, "%s:Failed read CHECKSUM_RESULT register\n", __func__);
		msleep(1);
	}

	if (i == INIT_RETRY_CNT) {
		dev_err(&client->dev, "%s:Failed retry CHECKSUM_RESULT register\n", __func__);
		goto fail_mini_init;
	}
	...
	ret = ztm620_write_reg(client, ZTM620_X_RESOLUTION, (u16)(pdata->x_resolution + ABS_PT_OFFSET));
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM620_X_RESOLUTION\n", __func__);
		goto fail_mini_init;
	}

	ret = ztm620_write_reg(client, ZTM620_Y_RESOLUTION, (u16)(pdata->y_resolution + ABS_PT_OFFSET));
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM620_Y_RESOLUTION\n", __func__);
		goto fail_mini_init;
	}
	...
	if (pdata->orientation != -1) {
		ret = ztm620_write_reg(client, ZTM620_COORD_ORIENTATION, (u16)pdata->orientation);
		if (ret != I2C_SUCCESS) {
			dev_err(&client->dev, "%s:failed to write ZTM620_COORD_ORIENTATION\n", __func__);
			goto fail_mini_init;
		}
	}
	...
	ret = ztm620_write_reg(client, ZTM620_SUPPORTED_FINGER_NUM, (u16)MAX_SUPPORTED_FINGER_NUM);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM620_SUPPORTED_FINGER_NUM\n", __func__);
		goto fail_mini_init;
	}

	ret = ztm620_write_reg(client, ZTM620_INITIAL_TOUCH_MODE, TOUCH_POINT_MODE);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev, "%s:failed to write ZTM620_INITIAL_TOUCH_MODE\n", __func__);
		goto fail_mini_init;
	}
	...
	ret = ztm620_read_data(client, 0x0308, (u8 *)&chip_check_sum, 2);
	if (ret >= 0) {
		dev_err(&client->dev, "%s: 0x0308:[0x%04x]\n", __func__, chip_check_sum);
	} else {
		dev_err(&client->dev, "%s:Failed to read 0x0308 register\n", __func__);
		goto fail_mini_init;
	}

	ret = ztm620_read_data(client, 0x0115, (u8 *)&chip_check_sum, 2);
	if (ret >= 0) {
		dev_err(&client->dev, "%s: 0x0115:[0x%04x]\n", __func__, chip_check_sum);
	} else {
		dev_err(&client->dev, "%s:Failed to read 0x0115 register\n", __func__);
		goto fail_mini_init;
	}
...
fail_mini_init:
	...
	ret = ztm620_hard_reset_recovery(info);
	if (ret) {
		dev_err(&client->dev, "%s:Failed ztm620_hard_reset_recovery\n", __func__);
		return false;
	}
...
static void  ztm620_send_wakeup_event(struct ztm620_info *info)
{
	...
	input_mt_report_slot_state(info->input_dev,
					MT_TOOL_FINGER, 1);
	input_report_abs(info->input_dev, ABS_MT_POSITION_X, 1);
	input_report_abs(info->input_dev, ABS_MT_POSITION_Y, 1);
...
static irqreturn_t ztm620_touch_work(int irq, void *data)
{
...
#endif
	int i = 0, ret = 0, t;
	u32 x, y, z, w, maxX, maxY, hold_time;
	...
	if (ret) {
		dev_err(&client->dev, "%s:Failed to occupy work lock. work:[%d] gesture:[%d] enabled:[%d]\n",
	...
	if (info->gesture_enable) {
		...
		if (ret)
			...
			dev_err(&client->dev, "%s:Can't detect wakeup event (0x%04x)\n",
				__func__, info->touch_info.status);
		...
		if (ret) {
			...
			if (ret) {
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
				dev_dbg(&client->dev, "%s:[%d][P] x=%3d y=%3d w=%d\n",
					__func__, i, x, y, info->wet_mode);
#else
				dev_dbg(&client->dev, "%s:[%d][P] w = %d\n",
				...
				if (info->debug_enable)
					dev_info(&client->dev, "%s:[%d][M] x=%3d y=%3d w=%d\n",
						__func__, i, x, y, info->wet_mode);
				else
					dev_dbg(&client->dev, "%s:[%d][M] x=%3d y=%3d w=%d\n",
						__func__, i, x, y, info->wet_mode);
#else
				dev_dbg(&client->dev, "%s:[%d][M] p = %d\n",
...
#else
				dev_dbg(&client->dev, "%s:[%d][R][0x%02x%02x%02x]\n",
	...
	if (zinitix_bit_test(info->touch_info.status, BIT_WHEEL_EVENT)) {
		...
		if (zinitix_bit_test(info->touch_info.finger_cnt, WHEEL_BIT_CW)) {
			dev_dbg(&client->dev, "%s:BIT_WHEEL_CW[0x%04x][0x%04x]\n",
					__func__, info->touch_info.status, info->touch_info.finger_cnt);
			input_report_rel(bezel_info->input_dev, REL_WHEEL, BZ_CW);
			input_report_rel(bezel_info->input_dev, REL_X, BZ_CW);
			input_sync(bezel_info->input_dev);
			bezel_info->dqa_cw_cnt++;
		} else if (zinitix_bit_test(info->touch_info.finger_cnt, WHEEL_BIT_CCW)) {
			dev_dbg(&client->dev, "%s:BIT_WHEEL_CCW[0x%04x][0x%04x]\n",
					__func__, info->touch_info.status, info->touch_info.finger_cnt);
			input_report_rel(bezel_info->input_dev, REL_WHEEL, BZ_CC);
			input_report_rel(bezel_info->input_dev, REL_X, BZ_CC);
			input_sync(bezel_info->input_dev);
			bezel_info->dqa_ccw_cnt++;
		} else if (zinitix_bit_test(info->touch_info.finger_cnt, WHEEL_BIT_PRESS)) {
			dev_dbg(&client->dev, "%s:WHEEL_BIT_PRESS[0x%04x][0x%04x]\n",
					__func__, info->touch_info.status, info->touch_info.finger_cnt);
			input_report_rel(bezel_info->input_dev, REL_WHEEL, BZ_LV);
			input_report_rel(bezel_info->input_dev, REL_X, BZ_LV);
			input_sync(bezel_info->input_dev);
		} else if (zinitix_bit_test(info->touch_info.finger_cnt, WHEEL_BIT_RELEASE)) {
			dev_dbg(&client->dev, "%s:WHEEL_BIT_RELEASE[0x%04x][0x%04x]\n",
					__func__, info->touch_info.status, info->touch_info.finger_cnt);
			input_report_rel(bezel_info->input_dev, REL_WHEEL, BZ_RT);
			input_report_rel(bezel_info->input_dev, REL_X, BZ_RT);
			input_sync(bezel_info->input_dev);
		} else {
			bezel_info->dqa_err_cnt++;
			dev_info(&client->dev, "%s:Unknown event[0x%04x][0x%04x]\n",
					__func__, info->touch_info.status, info->touch_info.finger_cnt);
		}
	}
#endif
...
static int ztm620_resume(struct device *dev)
{
	...
	ret = ztm620_write_wakeup_cmd(info);
	if (ret)
		dev_err(&client->dev,
			"%s:Failed wake cmd\n", __func__);
...
static void ztm620_input_close(struct input_dev *dev)
{
	...
	ret = ztm620_mode_change_recovery(info);
	if (ret)
		dev_err(&client->dev,
	...
	ret = ztm620_read_finger_cnt(info);
	if (ret < 0) {
		dev_err(&client->dev,
			"%s:Failed ztm620_read_finger_cnt ret:[%d] kernel:[%d]\n", __func__, ret, info->finger_event_cnt);
	} else
		dev_info(&client->dev, "%s:finger_cnt ic:[%d] kernel:[%d]\n", __func__, ret, info->finger_event_cnt);
...
#if SUPPORTED_TOUCH_WHEEL
	{TSP_CMD("run_wheel_read", run_wheel_read),},
	{TSP_CMD("get_wheel", get_wheel),},
#endif
};
...
#endif
#if 0  //for avoid bring up compile error
static void fw_update(void *device_data)
{
	struct ztm620_info *info = (struct ztm620_info *)device_data;
	struct i2c_client *client = info->client;
...
static int read_fw_ver_bin(struct ztm620_info *info, u32 *version)
{
	const struct firmware *tsp_fw = NULL;
	u16 project_id, fw_version, ic_name, module_ver;
	char fw_path[ZINITIX_MAX_FW_PATH];
	int ret;
	...
	ic_name = (u16)(info->fw_data[68] | (info->fw_data[69] << 8));
	module_ver = (u16)(info->fw_data[48] | (info->fw_data[49] << 8));
	project_id = (u16)((info->fw_data[56] & 0x0f) | ((info->fw_data[52] & 0x0f) << 4));
	fw_version = (u16)(info->fw_data[60] | (info->fw_data[61] << 8));

	release_firmware(tsp_fw);
	info->fw_data = NULL;

	*version = (u32)(ic_name << 24) | (project_id << 16) |\
			(module_ver << 8) | fw_version;
...
static void get_fw_ver_bin(void *device_data)
{
	...
	ret = read_fw_ver_bin(info, &version);
	if (ret) {
		dev_err(&info->client->dev,
	...
	snprintf(finfo->cmd_buff + length, sizeof(finfo->cmd_buff) - length, "%08X", version);
	set_cmd_result(finfo, finfo->cmd_buff,
			strnlen(finfo->cmd_buff, sizeof(finfo->cmd_buff)));
	finfo->cmd_state = OK;
...
static int read_fw_ver_ic(struct ztm620_info *info, u32 *version)
{
	struct i2c_client *client = info->client;
	u16 ic_name, project_id, module_ver, fw_version, major_ver, minor_ver;
	int ret;
	...
	ret = ztm620_read_data(client, ZTM620_CHIP_REVISION, (u8 *)&ic_name, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read ic_name\n", __func__);
		return -1;
	}

	ret = ztm620_read_data(client, ZTM620_FIRMWARE_VERSION, (u8 *)&major_ver, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read major_ver\n", __func__);
		return -1;
	}

	ret = ztm620_read_data(client, ZTM620_MINOR_FW_VERSION, (u8 *)&minor_ver, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read minor_ver\n", __func__);
		return -1;
	}

	ret = ztm620_read_data(client, ZTM620_HW_ID, (u8 *)&module_ver, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read model_version\n", __func__);
		return -1;
	}

	ret = ztm620_read_data(client, ZTM620_DATA_VERSION_REG, (u8 *)&fw_version, 2);
	if (ret < 0) {
		dev_err(&client->dev, "%s:Failed to read fw_version\n", __func__);
		return -1;
	}

	project_id = ((major_ver & 0xf) << 4) | (minor_ver & 0xf);
	*version = (u32)(ic_name << 24) | (project_id << 16) |\
			(module_ver << 8) | fw_version;
...
static void get_fw_ver_ic(void *device_data)
{
	...
	ret = read_fw_ver_ic(info, &version);
	if (ret) {
		dev_err(&client->dev, "%s:Failed read_fw_ver_ic[%d]\n", __func__, ret);
		goto out;
	}
...
static void run_wheel_read(void *device_data)
{
	...
	ret = ztm620_set_touchmode(info, TOUCH_WHEEL_MODE);
	if (ret) {
	...
	ret = get_raw_data(info, (u8 *)raw_data->wheel_data, 2);
	if (!ret) {
		dev_err(&client->dev, "%s:Failed to read raw data\n", __func__);
		finfo->cmd_state = FAIL;
		snprintf(finfo->cmd_buff, sizeof(finfo->cmd_buff), "NG");
	...
	for (i = 0; i < info->cap_info.x_node_num; i++) {
		...
		for (j = 0; j < info->cap_info.y_node_num; j++) {
			...
			pr_cont(" %5d", raw_data->wheel_data[offset]);
		}
		pr_cont("\n");
	}
...
static void get_wheel(void *device_data)
{
	...
	for (i=0; i < y_num; i++) {
		sprintf(tmp, "%d",  raw_data->wheel_data[i + (y_num * y_node)]);
		strncat(finfo->cmd_buff, tmp, strlen(tmp));
		if (i < y_num-1)
			strncat(finfo->cmd_buff, " ", strlen(" "));
	}
...
#if SUPPORTED_TOUCH_WHEEL
static ssize_t ztm620_store_wheel_enable(struct device *dev,
				  ...
				  const char *buf, size_t size)
{
	...
	dev_info(&client->dev, "%s:wheel mode[%s]\n", __func__, enable ? "on":"off");
...
static ssize_t ztm620_show_wheel_enable(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	...
	status = !!zinitix_bit_test(status, DEF_OPTIONAL_MODE_WHEEL_ON_BIT);
			...
			struct device_attribute *attr, char *buf)
{
	...
	status = !!zinitix_bit_test(status, DEF_OPTIONAL_MODE_SENSITIVE_BIT);
...
static int ztm620_store_gesture_on(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	int ret = 0, i;
	u16 gesture_state = 0;
...
static int ztm620_store_gesture_off(struct ztm620_info *info)
{
	struct i2c_client *client = info->client;
	int ret;
	...
	ret = ztm620_mini_init_touch(info);
	if (!ret) {
		dev_err(&client->dev, "%s:Failed to resume[%d]\n", __func__, ret);
		return -1;
	}
...
gesture_retry:
	...
	if (enable) {
		ret = ztm620_store_gesture_on(info);
		if (ret) {
			dev_err(&client->dev, "%s:failed to enable gesture mode\n", __func__);
			if (retry < INIT_RETRY_CNT) {
				dev_err(&client->dev, "%s:Excute hard reset routine[%d/%d]\n",
		...
		ret = ztm620_store_gesture_off(info);
		if (ret) {
			dev_err(&client->dev, "%s:failed to disable gesture mode\n", __func__);
			if (retry < INIT_RETRY_CNT) {
				dev_err(&client->dev, "%s:Excute hard reset routine[%d/%d]\n",
						__func__, retry, INIT_RETRY_CNT);
				ret = ztm620_hard_reset_recovery(info);
	...
	dev_info(&client->dev, "%s:[%s]\n", __func__, info->gesture_enable ? "on":"off");
					...
					char *buf)
{
	...
	if (info->work_state == SUSPEND) {
		dev_err(&client->dev, "%s:device is in suspend[%d]\n", __func__,info->work_state);
		return -EIO;
	}
...
static ssize_t ztm620_show_fw_ver_ic(struct device *dev, struct device_attribute *devattr, char *buf)
{
	...
	snprintf(buffer + length, sizeof(buffer) - length, "%08X", version);
...
#endif
#if SUPPORTED_TOUCH_WHEEL
static DEVICE_ATTR(wheel, S_IRUGO | S_IWUSR | S_IWGRP,
			ztm620_show_wheel_enable, ztm620_store_wheel_enable);
#endif
...
#else
	factory_ts_dev = device_create(tsp_sec_class, NULL, 0, info, "tsp");
	if (unlikely(!factory_ts_dev)) {
		dev_err(&info->client->dev, "%s:ailed to create factory dev\n", __func__);
		ret = -ENODEV;
		goto err_create_device;
					...
					struct zxt_ts_platform_data *pdata)
{
	...
	ret = of_property_read_s32(np, "zinitix,orientation", &pdata->orientation);
	if (ret < 0) {
		pdata->orientation = -1;
	}
	...
	if (ret < 0) {
		pdata->avdd_ldo = 0;
		dev_err(dev, "%s:failed to get  avdd_regulator %d\n", __func__, ret);
	}
...
static void ztm620_debug_dwork(struct work_struct *work)
{
	struct ztm620_info *info = container_of(work,
				struct ztm620_info, debug_dwork.work);
	struct i2c_client *client = info->client;
...
#if SUPPORTED_TOUCH_WHEEL
static ssize_t bezel_show_dqa_cw_cnt(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	...
	dev_info(&client->dev, "%s: factory_mode=[%d]\n", __func__, ddata->dqa_cw_cnt);
	ddata->dqa_cw_cnt = 0;

	return length;
}

static ssize_t bezel_show_dqa_ccw_cnt(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	struct ztm_bezel_ddata *ddata = info->bezel_info;
	...
	length = sprintf(buf,"%d\n", ddata->dqa_ccw_cnt);
	dev_info(&client->dev, "%s: factory_mode=[%d]\n", __func__, ddata->dqa_ccw_cnt);
	ddata->dqa_ccw_cnt = 0;

	return length;
}

static ssize_t bezel_show_dqa_err_cnt(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct ztm620_info *info = dev_get_drvdata(dev);
	struct i2c_client *client = info->client;
	struct ztm_bezel_ddata *ddata = info->bezel_info;
	...
	length = sprintf(buf,"%d\n", ddata->dqa_err_cnt);
	dev_info(&client->dev, "%s: factory_mode=[%d]\n", __func__, ddata->dqa_err_cnt);
	ddata->dqa_err_cnt = 0;

	return length;
}

static struct device_attribute dev_attr_show_dqa_cw_cnt =
		__ATTR(cw_cnt, S_IRUGO,bezel_show_dqa_cw_cnt, NULL);
static struct device_attribute dev_attr_show_dqa_ccw_cnt =
		__ATTR(ccw_cnt, S_IRUGO,bezel_show_dqa_ccw_cnt, NULL);
static struct device_attribute dev_attr_show_dqa_err_cnt =
		__ATTR(err_cnt, S_IRUGO,bezel_show_dqa_err_cnt, NULL);

static struct attribute *bezel_attributes[] = {
 	&dev_attr_show_dqa_cw_cnt.attr,
	&dev_attr_show_dqa_ccw_cnt.attr,
	&dev_attr_show_dqa_err_cnt.attr,
 	NULL,
};

static struct attribute_group bezel_attr_group = {
	.attrs = bezel_attributes,
};

static int ztm620_bezel_sysfs_init(struct ztm620_info *info)
{
	struct ztm_bezel_ddata *ddata = info->bezel_info;
	struct i2c_client *client = info->client;
	int ret;
	...
	if (IS_ERR(ddata->rotary_dev)) {
		dev_err(&client->dev, "Failed to create sec_device_create\n");
		return -1;
	}
#else
	ddata->rotary_dev = device_create(tsp_sec_class, NULL, 0, info, BEZEL_NAME);
	if (IS_ERR(ddata->rotary_dev)) {
		dev_err(&client->dev, "Failed to create sec_device_create\n");
		return -1;
	}
...
#endif
	dev_set_drvdata(ddata->rotary_dev, info);

	ret = sysfs_create_group(&ddata->rotary_dev->kobj, &bezel_attr_group);
	if (ret) {
		dev_err(&client->dev, "%s: Failed to create sysfs group\n", __func__);
#if IS_ENABLED(CONFIG_DRV_SAMSUNG)		
		sec_device_destroy(ddata->rotary_dev->devt);
#else
		device_destroy(tsp_sec_class, ddata->rotary_dev->devt);
#endif
 		return -1;
	}
...
static int ztm620_setup_bezel_input(struct ztm620_info *info)
{
	struct ztm_bezel_ddata *ddata = info->bezel_info;
	struct i2c_client *client = info->client;
	int ret;
	...
	ret = ztm620_bezel_sysfs_init(info);
	if (ret) {
		dev_err(&client->dev, "%s:failed ztm620_bezel_sysfs_init[%d]\n",\
			__func__, ret);
		return -EPERM;
	}
...
static int ztm620_probe(struct i2c_client *client, const struct i2c_device_id *i2c_id)
{
	...
	if (IS_ERR(tsp_sec_class)) {
		dev_err(&client->dev, "Failed to create class tsp_sec_class\n");
		return -1;
	}
	...
	if (!pdata) {
		dev_err(&client->dev, "%s:Failed to allocate bezel_info\n", __func__);
		goto err_bezel_alloc;
	}

	ret = ztm620_setup_bezel_input(info);
	if (ret) {
		dev_err(&client->dev, "%s: Failed bezel device setup.\n", __func__);
	...
	if (lpcharge) {
		...
		INIT_DELAYED_WORK(&info->debug_dwork, ztm620_debug_dwork);
