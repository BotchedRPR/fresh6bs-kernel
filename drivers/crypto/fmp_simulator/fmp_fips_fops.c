...
static void fmp_fips_hash_deinit(struct hash_data *hdata)
{
	if (hdata->hmac != NULL) {
		...
		kfree_sensitive(hdata->hmac);
		return;
	}

	if (hdata->sha != NULL) {
		memset(hdata->sha, 0x00, sizeof(*hdata->sha));
		kfree_sensitive(hdata->sha);
		return;
	}
}
...
static int fmp_test_crypt(struct exynos_fmp *fmp, struct fmp_test_data *fdata,
...
out_err_fail:
	for (k = 0; k < 1000; k++)
		kfree_sensitive(Ct[k]);
	kfree_sensitive(Ct);

out_err_mem_ct:
	for (k = 0; k < 1000; k++)
		kfree_sensitive(Pt[k]);
	kfree_sensitive(Pt);
out_err_mem_pt:
	free_page((unsigned long)data);
out_unlock:
...
error_hash:
	...
	kfree_sensitive(ses_new->sg);
	kfree_sensitive(ses_new->pages);
error_cipher:
	kfree_sensitive(ses_new);
	return ret;
}
...
static inline void fmp_destroy_session(struct fmp_fips_info *info, struct csession *ses_ptr)
{
	...
	kfree_sensitive(ses_ptr->pages);
	kfree_sensitive(ses_ptr->sg);
	mutex_unlock(&ses_ptr->sem);
	mutex_destroy(&ses_ptr->sem);
	kfree_sensitive(ses_ptr);
}
...
int fmp_fips_release(struct inode *inode, struct file *file)
{
	...
	fmp = info->fmp;
	fmp->result.overall = setOverall;
	dev_info(fmp->dev, "recovered result.overall (%d)\n", fmp->result.overall);
