...
static int set_drexbts(void __iomem *base, struct bts_stat *stat)
{
	...
	if (stat->drex_on) {
		__raw_writel(stat->drex->write_flush_config_0, base + WRITE_FLUSH_CONFIG_0);
		__raw_writel(stat->drex->write_flush_config_1, base + WRITE_FLUSH_CONFIG_1);

		for (i = 0; i < MAX_QOS + 1; i++)
			__raw_writel(stat->drex->drex_timeout[i],
					base + QOS_TIMER_0 + (4 * i));

		for (i = 0; i < VC_TIMER_TH_NR; i++)
			__raw_writel(stat->drex->vc_timer_th[i],
					base + VC_TIMER_TH_0 + (4 * i));

		__raw_writel(stat->drex->cutoff_con, base + CUTOFF_CON);
		__raw_writel(stat->drex->brb_cutoff_con, base + BRB_CUTOFF_CON);

	//	__raw_writel(stat->emergent_config_r, base + EMERGENT_CONFIG_R);
	//	__raw_writel(stat->emergent_config_w, base + EMERGENT_CONFIG_W);
	}

	if (stat->drex_pf_on) {
		__raw_writel(stat->drex_pf->pf_token_control, base + PF_TOKEN_CONTROL);
		__raw_writel(stat->drex_pf->pf_token_threshold0, base + PF_TOKEN_THRESHOLD0);

		for (i = 0; i < PF_TIMER_NR; i++)
			__raw_writel(stat->drex_pf->pf_qos_timer[i],
...
static int get_drexbts(void __iomem *base, struct bts_stat *stat)
{
	...
	if (stat->drex_on) {
		stat->drex->write_flush_config_0 = __raw_readl(base + WRITE_FLUSH_CONFIG_0);
		stat->drex->write_flush_config_1 = __raw_readl(base + WRITE_FLUSH_CONFIG_1);

		for (i = 0; i < MAX_QOS + 1; i++)
			stat->drex->drex_timeout[i] =
					__raw_readl(base + QOS_TIMER_0 + (4 * i));

		for (i = 0; i < VC_TIMER_TH_NR; i++)
			stat->drex->vc_timer_th[i] =
					__raw_readl(base + VC_TIMER_TH_0 + (4 * i));

		stat->drex->cutoff_con = __raw_readl(base + CUTOFF_CON);
		stat->drex->brb_cutoff_con = __raw_readl(base + BRB_CUTOFF_CON);

		//stat->emergent_config_r = __raw_readl(base + EMERGENT_CONFIG_R);
		//stat->emergent_config_w = __raw_readl(base + EMERGENT_CONFIG_W);

	}

	if (stat->drex_pf_on) {
		stat->drex_pf->pf_token_control = __raw_readl(base + PF_TOKEN_CONTROL);
		stat->drex_pf->pf_token_threshold0 = __raw_readl(base + PF_TOKEN_THRESHOLD0);

		for (i = 0; i < PF_TIMER_NR; i++)
			stat->drex_pf->pf_qos_timer[i] =
					__raw_readl(base + PF_QOS_TIMER_0 + (4 * i));
	}

	return 0;
}
#if defined(QMAX_THRESHOLD_R) && defined(QMAX_THRESHOLD_W) && defined(DEFAULT_QMAX_RD_TH)
...
static int set_ipbts_urgent(void __iomem *base, struct bts_stat *stat)
{
	...
	__raw_writel(stat->qurgent_th_r, base + TIMEOUT_CNT_R0);
	__raw_writel(stat->qurgent_th_w, base + TIMEOUT_CNT_W0);
...
static int get_ipbts_urgent(void __iomem *base, struct bts_stat *stat)
{
	...
	stat->qurgent_th_r = __raw_readl(base + TIMEOUT_CNT_R0);
	stat->qurgent_th_w = __raw_readl(base + TIMEOUT_CNT_W0);
		...
		unsigned int index)
{
	...
	__raw_writel(stat->drex->drex_timeout[index], base + QOS_TIMER_0 + (4 * index));
		...
		unsigned int index)
{
	...
	__raw_writel(stat->drex->vc_timer_th[index], base + VC_TIMER_TH_0 + (4 * index));
...
static int set_drexbts_emergent_config(void __iomem *base, struct bts_stat *stat)
{
	if (!base || !stat)
		return -EINVAL;
...
static int get_drexbts_emergent_config(void __iomem *base, struct bts_stat *stat)
{
	if (!base || !stat)
		return -EINVAL;
...
static int set_drexbts_pf_token(void __iomem *base, struct bts_stat *stat)
{
	if (!base || !stat)
		return -EINVAL;
...
static int get_drexbts_pf_token(void __iomem *base, struct bts_stat *stat)
{
	if (!base || !stat)
		return -EINVAL;
...
#if defined(RREQ_THRT_CON) && defined(RREQ_THRT_MO_P2)
static int set_drexbts_pf_rreq_thrt_con(void __iomem *base, struct bts_stat *stat)
{
	if (!base || !stat)
		return -EINVAL;

	__raw_writel(stat->drex_pf->pf_rreq_thrt_con, base + RREQ_THRT_CON);
...
static int get_drexbts_pf_rreq_thrt_con(void __iomem *base, struct bts_stat *stat)
{
	...
	stat->drex_pf->pf_rreq_thrt_con = __raw_readl(base + RREQ_THRT_CON);
		...
		unsigned int index)
{
	...
	__raw_writel(stat->drex_pf->pf_qos_timer[index], base + PF_QOS_TIMER_0 + (4 * index));
...
int register_btsops(struct bts_info *info)
{
	...
	case IP_BTS:
		...
		info->ops->set_emergent_config = NULL;
		info->ops->get_emergent_config = NULL;
		info->ops->set_pf_token = NULL;
		info->ops->get_pf_token = NULL;
	...
	case DREX_BTS:
		...
		info->ops->set_emergent_config = set_drexbts_emergent_config;
		info->ops->get_emergent_config = get_drexbts_emergent_config;
		info->ops->set_pf_token = set_drexbts_pf_token;
		info->ops->get_pf_token = get_drexbts_pf_token;
		info->ops->get_pf_rreq_thrt_con = NULL;
		info->ops->set_pf_rreq_thrt_con = NULL;
		info->ops->get_allow_mo_for_region = NULL;
		info->ops->set_allow_mo_for_region = NULL;
