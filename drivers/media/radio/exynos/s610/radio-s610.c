...
/*
 * drivers/media/radio/exynos/s610/radio-s610.c
 *
	...
	switch (ctrl->id) {
		...
		case V4L2_CID_S610_CH_BAND:
			...
			if (radio->radio_region == 2) {
				...
				if (s610_radio_freq_is_inside_of_the_band(payload, radio->radio_region))
...
static __poll_t s610_radio_fops_poll(struct file *file,
...
{
	struct device_node *np = NULL;
	struct i2c_client *client15;
	struct s2mpu15_dev *s2mpu15;

	np = of_find_compatible_node(NULL, NULL, "samsung,s2mpu15mfd");
	if (!np) {
		dev_err(radio->dev, "PMIC s2mpu15mfd device is not available\n");
		return NULL;
	}
	client15 = of_find_i2c_device_by_node(np);
	if (!client15) {
		dev_err(radio->dev, "PMIC s2mpu15mfd i2s is not available\n");
		return NULL;
	}
	s2mpu15 = i2c_get_clientdata(client15);
	radio->i2c_main1 = s2mpu15->pmic1;
	radio->i2c_main2 = s2mpu15->pmic2;

	return np;
}

#define ADDR_LDO16M	0xB8
#define ADDR_LDO27M_CTRL 0xCF
#define ADDR_LDO27M_OUT  0xD0
int fm_ldo_enable(struct s610_radio *radio)
{
	u8 reg16m, reg27m;
	int ret;

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO16M, &radio->ldo16m);
	dev_info(radio->dev, "%s:1) read S2MPU15_PM_LDO16_CTRL: %xh\n", __func__, radio->ldo16m);

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, &radio->ldo27m_ctrl);
	dev_info(radio->dev, "%s:1) read S2MPU15_PM_LDO27_CTRL: %xh\n", __func__, radio->ldo27m_ctrl);

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO27M_OUT, &radio->ldo27m_out);
	dev_info(radio->dev, "%s:1) read S2MPU15_PM_LDO27_OUT: %xh\n", __func__, radio->ldo27m_out);

	/* LDO16M 0xE0 */
	ret = s2mpu15_write_reg(radio->i2c_main1, ADDR_LDO16M, 0xE0);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	/* ADDR_LDO27M_CTRL 0xD2 */
	ret = s2mpu15_write_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, 0xD2);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	/* ADDR_LDO27M_OUT 0x80 */
	ret = s2mpu15_write_reg(radio->i2c_main1, ADDR_LDO27M_OUT, 0x80);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO16M, &reg16m);
	dev_info(radio->dev, "%s:2) read S2MPU15_PM_LDO16_CTRL: %xh\n", __func__, reg16m);

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, &reg27m);
	dev_info(radio->dev, "%s:2) read S2MPU15_PM_LDO27_CTRL: %xh\n", __func__, reg27m);

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO27M_OUT, &reg27m);
	dev_info(radio->dev, "%s:2) read S2MPU15_PM_LDO27_OUT: %xh\n", __func__, reg27m);

	return 0;
}

int fm_ldo_disable(struct s610_radio *radio)
{
	u8 reg16m, reg27m;
	int ret;

	/* LDO16M 0xD8 */
	ret = s2mpu15_write_reg(radio->i2c_main1, ADDR_LDO16M, radio->ldo16m);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	/* ADDR_LDO27M_CTRL 0xD2 */
	ret = s2mpu15_write_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, radio->ldo27m_ctrl);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	/* ADDR_LDO27M_OUT 0x80 */
	ret = s2mpu15_write_reg(radio->i2c_main1, ADDR_LDO27M_OUT, radio->ldo27m_out);
	if (ret < 0) {
		dev_err(radio->dev, "%s: failed to write PMIC main register\n", __func__);
		return -1;
	}
	...
	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO16M, &reg16m);
	dev_info(radio->dev, "%s: read S2MPU15_PM_LDO16_CTRL: %xh\n", __func__, reg16m);

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO27M_CTRL, &reg27m);
	dev_info(radio->dev, "%s: read S2MPU15_PM_LDO27_CTRL: %xh\n", __func__, reg27m);

	s2mpu15_read_reg(radio->i2c_main1, ADDR_LDO27M_OUT, &reg27m);
	dev_info(radio->dev, "%s: read S2MPU15_PM_LDO27_OUT: %xh\n", __func__, reg27m);
...
#ifdef USE_PMIC_RW
	if (!exynos_pmic_parse_dt(radio)) {
		dev_err(&pdev->dev,
			"Failed to get pmic dt !!!!!\n");
	}
#endif /* USE_PMIC_RW */
...
MODULE_DESCRIPTION("Driver for S620 FM Radio in Exynos8535");
MODULE_LICENSE("GPL");
