...
void mfc_rm_load_balancing(struct mfc_ctx *ctx, int load_add)
{
	...
	list_for_each_entry(tmp_ctx, &dev->ctx_list, list) {
		...
		if (tmp_ctx->op_core_type == MFC_OP_CORE_ALL) {
			...
			if (IS_MULTI_MODE(tmp_ctx)) {
				...
				core->total_mb += (tmp_ctx->weighted_mb / dev->num_core);
				mfc_ctx_debug(3, "[RMLB] ctx[%d] fix load both core\n",
						tmp_ctx->num);
				...
				mfc_ctx_debug(3, "[RMLB] ctx[%d] fix load single core-%d\n",
						tmp_ctx->num, core->id);
	...
	list_for_each_entry(tmp_ctx, &dev->ctx_list, list)
		mfc_ctx_debug(3, "[RMLB] MFC-%d) ctx[%d] %s %dx%d %dfps %s, load: %d%%, op_core_type: %d, op_mode: %d(stream: %d)\n",
	...
	for (i = 0; i < dev->num_core; i++)
		mfc_ctx_debug(3, "[RMLB] >> MFC-%d total load: %d%%\n", i,
...
int __mfc_rm_check_real_time_resource(struct mfc_ctx *ctx)
{
	...
	for (i = 0; i < MFC_NUM_CONTEXTS; i++) {
		if (!dev->ctx[i])
			continue;
		tmp_ctx = dev->ctx[i];
		if ((tmp_ctx->num != ctx->num) && (tmp_ctx->rt != MFC_RT)) {
			mfc_ctx_debug(3, "[RM][c:%d] This is not RT (%d)\n",
	...
	for (i = 0; i < dev->num_core; i++)
		max_mb += dev->core[i]->core_pdata->max_mb;

	mfc_ctx_debug(3, "[RM] The total RT mb is %d / %d\n", total_mb, max_mb);
...
void mfc_rm_update_real_time(struct mfc_ctx *ctx)
{
	...
	mfc_ctx_debug(2, "[PRIO] update rt: %d -> %d, operating fps: %d, prio: %d->%d\n",
	...
	if ((new_rt != ctx->rt) || (new_prio != ctx->prio)) {
		...
		if (core) {
			ret = core->sched->change_prio_work(core, ctx, new_rt, new_prio);
		} else {
