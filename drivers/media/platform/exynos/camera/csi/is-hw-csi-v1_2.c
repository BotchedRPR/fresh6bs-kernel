		...
		u32 vc, struct is_frame_cfg *cfg, u32 hwformat, u32 dummy_pixel)
{
	...
	val = is_hw_get_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_FMT]);
	...
	if (dma_stg_mode == CSIS_REG_DMA_STG_LEGACY_MODE) {
		...
		is_hw_set_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_FMT], val);

		val = is_hw_get_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_RESOL]);
		val = is_hw_set_field_value(val, &csi_vcdma_fields[CSIS_F_DMA_N_HRESOL], cfg->width + dummy_pixel);
		is_hw_set_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_RESOL], val);
	} else if (dma_stg_mode == CSIS_REG_DMA_STG_PACKET_MODE) {
		is_hw_set_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_FMT], val);
	}

	is_hw_set_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_FCNTSEQ], 0);
...
{
	u32 val = is_hw_get_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_FCNTSEQ]);

	csi_hw_s_frameptr(vc_reg, vc, number, false);

	is_hw_set_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_ADDR1 + number], (u32)addr);

	val |= 1 << number;
	is_hw_set_reg(vc_reg, &csi_vcdma_regs[CSIS_R_DMA0_FCNTSEQ], val);
}
