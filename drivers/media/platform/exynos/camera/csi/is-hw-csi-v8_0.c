	...
	u32 vc, struct is_vci_config *config, u32 width, u32 height, bool potf)
{
	...
	if (CSIS_PIXEL_PER_CLK == CSIS_PPC_8)
		pixel_mode = CSIS_PIXEL_MODE_OCTA;
	else if (CSIS_PIXEL_PER_CLK == CSIS_PPC_4)
		pixel_mode = CSIS_PIXEL_MODE_QUAD;
	else if (CSIS_PIXEL_PER_CLK == CSIS_PPC_2)
		pixel_mode = CSIS_PIXEL_MODE_DUAL;
	else
		pixel_mode = CSIS_PIXEL_MODE_SING;
		...
		u32 vc, struct is_frame_cfg *cfg, u32 hwformat, u32 dummy_pixel)
{
	...
	val = is_hw_set_field_value(val, &csi_dmax_chx_fields[CSIS_DMAX_CHX_F_HRESOL],
								ALIGN(cfg->width, CSIS_PIXEL_PER_CLK));
		...
		u32 height)
{
	...
	width = ALIGN(width, CSIS_PIXEL_PER_CLK); /* Align with 8px for 8PPC */
	...
	is_hw_set_field(base_reg, &csi_ebuf_regs[offset],
			...
			width / CSIS_PIXEL_PER_CLK); /* cyles */
