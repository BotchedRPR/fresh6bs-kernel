...
int csi_hw_g_dma_irq_src_vc(u32 __iomem *base_reg,
	struct csis_irq_src *src, u32 vc_phys, bool clear)
{
	u32 dma_src, vc;
	...
	/* Not each VC[i] */
	src->err_id[0] |= (dma_src & BIT(CSIS_INT_DMA_FIFO_FULL)) ?
		BIT(CSIS_ERR_DMA_DMAFIFO_FULL) : 0;
	src->err_id[0] |= (dma_src & BIT(CSIS_INT_DMA_ABORT_DONE)) ?
		BIT(CSIS_ERR_DMA_ABORT_DONE) : 0;
	src->err_id[0] |= (dma_src & BIT(CSIS_INT_DMA_LASTDATA_ERROR)) ?
		BIT(CSIS_ERR_DMA_LASTDATA_ERROR) : 0;
	src->err_id[0] |= (dma_src & BIT(CSIS_INT_DMA_LASTADDR_ERROR)) ?
		BIT(CSIS_ERR_DMA_LASTADDR_ERROR) : 0;
...
void csi_hw_s_bns_ch(u32 __iomem *reg, u32 ch)
{
	...
	u32 bns_mux_addr = 0x1725020C;

	if (CALL_PSMC_OPS(psmc_ops, call,
			SMC_CMD_REG, SMC_REG_ID_SFR_W(bns_mux_addr), ch, 0))
		err("[SMC] bns mux setting failed, addr(0x%x) val(%d)\n",
			bns_mux_addr, ch);
}
...
void csi_hw_s_dma_input_mux(void __iomem *mux_regs,
	u32 wdma_index, bool bns_en, u32 bns_dma_mux_val, u32 csi_ch, int otf_ch)
{
	struct pablo_smc_ops *psmc_ops = pablo_get_smc_ops();
	int ret;
	u32 mux_val;
	u32 mux_addr[5] = {0x17250240, 0x17250244, 0x17250248, 0x1725024C, 0x17250250};
	...
	ret = CALL_PSMC_OPS(psmc_ops, call,
			SMC_CMD_REG, SMC_REG_ID_SFR_W(mux_addr[wdma_index]), mux_val, 0);
	if (ret) {
		err("[SMC] WDMA ch(%d) mux set fail, addr(0x%x) ret(%d) val(%d)\n",
			wdma_index, mux_addr[wdma_index], ret, mux_val);
		return;
	}
...
void csi_hw_s_init_input_mux(void __iomem *mux_regs, u32 index)
{
	...
	ret = CALL_PSMC_OPS(psmc_ops, call,
			SMC_CMD_REG, SMC_REG_ID_SFR_W(mux_addr[index]), 0xFFFFFFFF, 0);
	if (ret)
		err("[SMC] dma_ch(%d) imux set fail ret(%d)\n", index, ret);
}
