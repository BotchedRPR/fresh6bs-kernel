...
static int sensor_module_check_match_seq(struct is_device_sensor *device,
		struct exynos_platform_is_module *pdata, struct i2c_client *client)
{
	int i, j;
	int idx_prop = 0;
	...
	for (i = 0; i < pdata->num_of_match_groups; i++) {
		...
		for (j = 0; j < pdata->num_of_match_entry[i]; j++) {
			...
			client->addr = (slave_addr & 0xFFFF);
			if (reg_type == 2) {
				if (data_type == 2) {
					ret = device->ixc_ops->read16(client, reg, &val16);
					if (ret < 0) {
					...
					ret = device->ixc_ops->read8(client, reg, &val8);
					if (ret < 0) {
						info(" I2C ERROR %d", ret);
						continue;
					}
				...
				ret = device->ixc_ops->addr8_read8(client, reg, &val8);
				if (ret < 0) {
					info(" I2C ERROR %d", ret);
					continue;
				}
...
static int sensor_module_probe_i2c(struct platform_device *pdev)
{
	int ret = 0;
	struct is_core *core;
	...
	ixc_client = (struct i2c_client *)device->client;
	device->ixc_ops = pablo_get_i2c();
	...
	if (device->pdata->i2c_dummy_enable) {
		...
		match_result = sensor_module_check_match_seq(device, pdata, ixc_client);
		...
		if (pdata->cis_np) {
			...
			cis_client = i2c_new_client_device(ixc_client->adapter, cis_info);
		}
		...
		if (pdata->af_np) {
			...
			/* FIXME: error handling */
			af_client = i2c_new_client_device(ixc_client->adapter, af_info);
		}
		...
		if (pdata->eeprom_np) {
			...
			/* FIXME: error handling */
			eeprom_client = i2c_new_client_device(ixc_client->adapter, eeprom_info);
		}
		...
		if (pdata->ois_np) {
			...
			/* FIXME: error handling */
			ois_client = i2c_new_client_device(ixc_client->adapter, ois_info);
		}
	}
