						...
						struct ae_param *adj_dgain)
{
	...
	if (sensor_peri->sensor_interface.cis_mode == ITF_CIS_SMIA_WDR) {
		...
		is_sensor_peri_compensate_gain_for_ext_br(device, setting->exposure,
								&adj_again->val,
								&adj_dgain->val);
		adj_again->short_val = adj_again->val;
		adj_dgain->short_val = adj_dgain->val;
	}
...
int is_sensor_ctl_update_exposure_duration(struct is_device_sensor *device,
					u32 *dm_index,
					struct ae_param expo,
					u64 frame_duration)
{
	...
	if (expo.long_val != 0 && expo.short_val != 0) {
		...
		sensor_peri->cis.cur_sensor_uctrl.dynamicFrameDuration = frame_duration;
		...
		for (i = 0; i < loop_cnt; i++) {
			...
			sensor_peri->cis.expecting_sensor_udm[(dm_index[0] + i) % EXPECT_DM_NUM].dynamicFrameDuration =
				sensor_peri->cis.cur_sensor_uctrl.dynamicFrameDuration;
		}
	} else {
		for (i = 0; i < loop_cnt; i++) {
			...
			sensor_peri->cis.expecting_sensor_udm[(dm_index[0] + i) % EXPECT_DM_NUM].dynamicFrameDuration =
				sensor_peri->cis.expecting_sensor_udm[dm_index[1]].dynamicFrameDuration;
		}
	}
...
void is_sensor_ctl_frame_evt(struct is_device_sensor *device)
{
	...
	FIMC_BUG_VOID(!device);
	FIMC_BUG_VOID(!device->subdev_module);
	...
	if ((module_ctl->valid_sensor_ctrl == true) ||
		...
		/* If S/W frame sync activated, compensate slave's frameration */
		if ((sensor_peri->cis.dual_sync_type == DUAL_SYNC_TYPE_SW)
			&& (sensor_peri->cis.dual_sync_mode == DUAL_SYNC_SLAVE))
			is_sensor_peri_compensate_duration_for_frame_sync(device, vsync_count);
		...
		if (CALL_OISOPS(sensor_peri->mcu->ois, ois_get_active)) {
			...
			if (ret < 0) {
				err("[%s] v4l2_subdev_call(ois_mode_change, mode:%d) is fail(%d)",
					module->sensor_name, sensor_peri->mcu->ois->ois_mode, ret);
				goto p_err;
			}

			ret = CALL_OISOPS(sensor_peri->mcu->ois, ois_set_coef, sensor_peri->subdev_mcu, sensor_peri->mcu->ois->coef);
			if (ret < 0) {
				err("[%s] v4l2_subdev_call(ois_set_coef, coef:%d) is fail(%d)",
					module->sensor_name, sensor_peri->mcu->ois->coef, ret);
				goto p_err;
			}
...
int is_sensor_ctl_adjust_sync(struct is_device_sensor *device, u32 sync)
{
	...
	if (!device->subdev_module) {
		err("device->subdev_module is null");
		ret = -EINVAL;
		goto p_err;
	}
