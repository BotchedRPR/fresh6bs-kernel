...
static const u32 *sensor_imx754_dualsync_slave;
static u32 sensor_imx754_dualsync_slave_size;

static const u32 *sensor_imx754_dualsync_single;
static u32 sensor_imx754_dualsync_single_size;

static u32 sensor_imx754_multicam_slave;
...
int sensor_imx754_cis_init(struct v4l2_subdev *subdev)
{
	...
	sensor_imx754_cis_data_calculation(sensor_imx754_pllinfos[setfile_index], cis->cis_data);
...
int sensor_imx754_cis_set_global_setting(struct v4l2_subdev *subdev)
{
	...
	ret = sensor_cis_set_registers(subdev, sensor_imx754_global, sensor_imx754_global_size);
	if (ret < 0) {
...
int sensor_imx754_cis_update_crop_region(struct v4l2_subdev *subdev)
{
	...
	ret |= cis->ixc_ops->read16(client, 0x0408, &x_start);
	ret |= cis->ixc_ops->read16(client, 0x040A, &y_start);

	x_start += delta_x;
	y_start += delta_y;

	ret |= cis->ixc_ops->write16(client, 0x0408, x_start);
	ret |= cis->ixc_ops->write16(client, 0x040A, y_start);
...
int sensor_imx754_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	if (mode >= sensor_imx754_max_setfile_num) {
		err("invalid mode(%d)!!", mode);
		ret = -EINVAL;
		goto p_err;
	}

	sensor_imx754_cis_data_calculation(sensor_imx754_pllinfos[mode], cis->cis_data);

	cis->mipi_clock_index_cur = CAM_MIPI_NOT_INITIALIZED;

	I2C_MUTEX_LOCK(cis->i2c_lock);

	ret = sensor_cis_set_registers(subdev, sensor_imx754_setfiles[mode], sensor_imx754_setfile_sizes[mode]);
	if (ret < 0) {
		err("[%d]sensor_imx754_set_registers fail!!", cis->id);
		goto p_err_i2c_unlock;
	}
...
int sensor_imx754_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	/* Sensor stream on */
	if (sensor_imx754_multicam_slave == 1) /* Mirror Flip */
		cis->ixc_ops->write16(client, 0x0100, 0x0102);
	else
...
int sensor_imx754_cis_set_analog_gain(struct v4l2_subdev *subdev, struct ae_param *again)
{
	...
	/* Short analog gain */
	ret = cis->ixc_ops->write16(client, 0x0204, short_gain);
	if (ret < 0)
		goto p_err_i2c_unlock;
				...
				struct device *cdev, enum ixc_type type)
{
	...
	if (sensor_imx754_multicam_slave == 1)
	...
	if (mclk_freq_khz == 19200) {
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_imx754_dualsync_slave = sensor_imx754_dual_slave_settings_A_19p2;
			sensor_imx754_dualsync_slave_size = ARRAY_SIZE(sensor_imx754_dual_slave_settings_A_19p2);
			sensor_imx754_dualsync_single = sensor_imx754_dual_single_settings_A_19p2;
			sensor_imx754_dualsync_single_size = ARRAY_SIZE(sensor_imx754_dual_single_settings_A_19p2);
			...
			sensor_imx754_dualsync_slave = sensor_imx754_dual_slave_settings_B_19p2;
			sensor_imx754_dualsync_slave_size = ARRAY_SIZE(sensor_imx754_dual_slave_settings_B_19p2);
			sensor_imx754_dualsync_single = sensor_imx754_dual_single_settings_B_19p2;
			sensor_imx754_dualsync_single_size = ARRAY_SIZE(sensor_imx754_dual_single_settings_B_19p2);
		...
		if (strcmp(setfile, "default") == 0 ||
			...
			sensor_imx754_dualsync_slave = sensor_imx754_dual_slave_settings_A;
			sensor_imx754_dualsync_slave_size = ARRAY_SIZE(sensor_imx754_dual_slave_settings_A);
			sensor_imx754_dualsync_single = sensor_imx754_dual_single_settings_A;
			sensor_imx754_dualsync_single_size = ARRAY_SIZE(sensor_imx754_dual_single_settings_A);
			...
			sensor_imx754_dualsync_slave = sensor_imx754_dual_slave_settings;
			sensor_imx754_dualsync_slave_size = ARRAY_SIZE(sensor_imx754_dual_slave_settings);
			sensor_imx754_dualsync_single = sensor_imx754_dual_single_settings;
			sensor_imx754_dualsync_single_size = ARRAY_SIZE(sensor_imx754_dual_single_settings);
...
static const struct i3c_device_id sensor_cis_imx754_idt_i3c[] = {
	I3C_DEVICE(0, 0, NULL), /* TBD : manufacturer id, part id, driver-data */
	{ /* sentinel */ },
};
...
#ifdef MODULE
module_driver(sensor_cis_imx754_driver_i3c,i3c_i2c_driver_register, i3c_i2c_driver_unregister, &sensor_cis_imx754_driver_i2c)
#else
static int __init sensor_cis_imx754_init(void)
{
	int ret;

	ret = i2c_add_driver(&sensor_cis_imx754_driver_i2c);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_imx754_driver_i2c.driver.name, ret);

	ret = i3c_driver_register_with_owner(&sensor_cis_imx754_driver_i3c, THIS_MODULE);
	if (ret)
		err("failed to add %s driver: %d\n",
			sensor_cis_imx754_driver_i3c.driver.name, ret);
