...
int sensor_gh1_cis_log_status(struct v4l2_subdev *subdev)
{
	...
	ret = cis->ixc_ops->read16(client, 0x00DC, &data16);
	if (unlikely(!ret)) pr_info("0x00DC(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x00EA, &data16);
	...
	ret = cis->ixc_ops->read16(client, 0x0202, &data16);
	if (unlikely(!ret)) pr_info("0x0202(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0204, &data16);
	if (unlikely(!ret)) pr_info("0x0204(0x%x)\n", data16);
	else goto i2c_err;
	ret = cis->ixc_ops->read16(client, 0x0340, &data16);
