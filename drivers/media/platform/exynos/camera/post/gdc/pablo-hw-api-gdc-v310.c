...
#include "pablo-hw-reg-gdc-v310.h"
#include <linux/smc.h>
...
u32 camerapp_hw_gdc_sw_reset(struct pablo_mmio *pmio)
{
	...
	/* GDC should be set to non-secure after reset  : D_TZPC_MCSC : 0x1561_0000 */
	/* TODO : Need to Confirm for NEUS */
	smc_ret = exynos_smc(0x82000410, 0x1, 0x15610204, 0x2);
	if (smc_ret)
...
{
	GDC_SET_R(pmio, &gdc_regs[GDC_R_GDC_INT_STATUS], 0x0003);
}
 ...
 */
	...
	if (d_frame->extra == COMP) {
		output_stride_lum_w_2bit = (u32)(SBWC_HEADER_STRIDE(out_dma_width));
		output_stride_chroma_w_2bit = (u32)(SBWC_HEADER_STRIDE(out_dma_width));
 ...
 */
	...
	GDC_SET_F(pmio, &gdc_regs[GDC_R_WAXI_STRIDE_HEADER_LUM],
		&gdc_fields[GDC_F_WRITE_STRIDE_HEADER_LUMA], output_stride_lum_w_2bit);
	GDC_SET_F(pmio, &gdc_regs[GDC_R_WAXI_STRIDE_HEADER_CHROMA],
		&gdc_fields[GDC_F_WRITE_STRIDE_HEADER_CHROMA], output_stride_chroma_w_2bit);
...
/*
 * lossy compression rate : 2~4
 * 8bit : 2~3, 10bit : 2~4
 * 2 : 64 byte(8bit 50%, 10bit 40%)
 * 3 : 96 byte(8bit 75%, 10bit 60%)
 * 4 : 128 byte(10bit 80%)
 */
	...
	struct gdc_frame *s_frame, struct gdc_frame *d_frame)
{
	if (d_frame->extra == COMP) {
		...
		&gdc_fields[GDC_F_DST_COMPRESSOR], 1);
	} else if (d_frame->extra == COMP_LOSS) {
		GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
		&gdc_fields[GDC_F_DST_COMPRESSOR], 2);
		/* Need to set Comp_ratio */
		GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
		&gdc_fields[GDC_F_WDMA_COMP_LOSSY_BYTE32NUM], 2);
	} else {
		GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
		&gdc_fields[GDC_F_DST_COMPRESSOR], 0);
	}
}
...
void camerapp_hw_gdc_status_read(struct pablo_mmio *pmio)
{
	...
	isPixelBusy = GDC_GET_F(pmio,
		&gdc_regs[GDC_R_GDC_DEBUG_BUSY], &gdc_fields[GDC_F_PXCBUSYLUM]);
	isPortBusy = GDC_GET_F(pmio,
		&gdc_regs[GDC_R_GDC_DEBUG_BUSY], &gdc_fields[GDC_F_PORT0BUSY]);
