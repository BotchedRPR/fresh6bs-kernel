...
#include "pablo-hw-reg-gdc-v401.h"
#include <linux/smc.h>
...
{
	GDC_SET_R(pmio, &gdc_regs[GDC_R_GDC_INT_STATUS], 0x003F);
}
	...
	struct gdc_frame *s_frame, struct gdc_frame *d_frame)
{
	...
	u32 output_stride_lum_w_header, output_stride_chroma_w_header;
 ...
 */
	...
	output_stride_lum_w_header = 0;
	output_stride_chroma_w_header = 0;
	...
	/* calculate write DMA stride size for SBWC */
	if (d_frame->extra == COMP) {
		output_stride_lum_w_header = (u32)(SBWC_HEADER_STRIDE(out_dma_width));
		output_stride_chroma_w_header = (u32)(SBWC_HEADER_STRIDE(out_dma_width));
		if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_8BIT) {
			output_stride_lum_w = (u32)(SBWC_8B_STRIDE(out_dma_width));
			output_stride_chroma_w = (u32)(SBWC_8B_STRIDE(out_dma_width));
		} else if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_10BIT) {
			output_stride_lum_w = (u32)(SBWC_10B_STRIDE(out_dma_width));
			output_stride_chroma_w = (u32)(SBWC_10B_STRIDE(out_dma_width));
		}
	} else if (d_frame->extra == COMP_LOSS) {
		/* Assume that lossy compression rate is 8bit 50%, 10bit 60% */
		if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_8BIT) {
			output_stride_lum_w = (u32)(SBWCL_8B_STRIDE(out_dma_width, GDC_LOSSY_COMP_RATE_50));
			output_stride_chroma_w = (u32)(SBWCL_8B_STRIDE(out_dma_width, GDC_LOSSY_COMP_RATE_50));
		} else if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_10BIT) {
			output_stride_lum_w = (u32)(SBWCL_10B_STRIDE(out_dma_width, GDC_LOSSY_COMP_RATE_60));
			output_stride_chroma_w = (u32)(SBWCL_10B_STRIDE(out_dma_width, GDC_LOSSY_COMP_RATE_60));
		}
	} else if ((d_frame->gdc_fmt->pixelformat == V4L2_PIX_FMT_NV12M_P010)
 ...
 */
	...
	GDC_SET_F(pmio, &gdc_regs[GDC_R_WAXI_STRIDE_HEADER_LUM],
		&gdc_fields[GDC_F_WRITE_STRIDE_HEADER_LUMA], output_stride_lum_w_header);
	GDC_SET_F(pmio, &gdc_regs[GDC_R_WAXI_STRIDE_HEADER_CHROMA],
		&gdc_fields[GDC_F_WRITE_STRIDE_HEADER_CHROMA], output_stride_chroma_w_header);
	...
	struct gdc_frame *s_frame, struct gdc_frame *d_frame)
{
	...
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_DST_HEADER_BASE_LUM],
		&gdc_fields[GDC_F_GDC_DST_HEADER_BASE_LUM], d_frame->addr.y_2bit);
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_DST_HEADER_BASE_CHROMA],
		&gdc_fields[GDC_F_GDC_DST_HEADER_BASE_CHROMA], d_frame->addr.cbcr_2bit);
...
{
	if (d_frame->extra == COMP) {
		GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
		&gdc_fields[GDC_F_DST_COMPRESSOR], COMP);
	} else if (d_frame->extra == COMP_LOSS) {
		GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
		&gdc_fields[GDC_F_DST_COMPRESSOR], COMP_LOSS);
		/* Need to set Comp_ratio */
		if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_8BIT) {
			/* 8bit : 50% */
			GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
			&gdc_fields[GDC_F_WDMA_COMP_LOSSY_BYTE32NUM], 2);
		} else if (d_frame->pixel_size == CAMERA_PIXEL_SIZE_10BIT) {
			/* 10bit : 60% */
			GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
			&gdc_fields[GDC_F_WDMA_COMP_LOSSY_BYTE32NUM], 3);
		}
	} else {
		GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_COMPRESSOR],
		&gdc_fields[GDC_F_DST_COMPRESSOR], NONE);
	}
}
...
int camerapp_hw_get_sbwc_constraint(u32 pixelformat, u32 width, u32 height, u32 type)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(type)) {
		gdc_info("Compressor decorder is not supported\n");
		return -EINVAL;
	}
...
void camerapp_hw_gdc_update_param(struct pablo_mmio *pmio, struct gdc_dev *gdc)
{
	...
	/* gdc boundary option setting */
	GDC_SET_F(pmio, &gdc_regs[GDC_R_GDC_BOUNDARY_OPTION],
		&gdc_fields[GDC_F_GDC_BOUNDARY_OPTION], 1);
...
void camerapp_hw_gdc_status_read(struct pablo_mmio *pmio)
{
	...
	wdmaStatus = GDC_GET_F(pmio,
		&gdc_regs[GDC_R_GDC_DEBUG_BUSY], &gdc_fields[GDC_F_WDMA_BUSY]);
	rdmaStatus = GDC_GET_F(pmio,
		&gdc_regs[GDC_R_GDC_DEBUG_BUSY], &gdc_fields[GDC_F_RDMA_BUSY]);

	pr_info("[GDC_Status] = ReqCntLum(%x) HitCntLum(%x)",
		ReqCntLum, HitCntLum);
	pr_info("[GDC_Status] = ReqCntLum(%x) HitCntLum(%x)\
		cacheStatus0(%x) hit(%x) miss(%x) WDMA(%x) RDMA(%x)\n",
...
void camerapp_hw_gdc_votf_enable(struct pablo_mmio *pmio, u8 rw)
{
	if (rw) {
		/* read */
		GDC_SET_R(pmio, &gdc_regs[GDC_R_GDC_VOTF], 1);
	} else {
		/* write */
		GDC_SET_R(pmio, &gdc_regs[GDC_R_GDC_VOTF], 0x18);
		GDC_SET_R(pmio, &gdc_regs[GDC_R_GDC_BUS_USER_INFO_RDMA], 0);
		GDC_SET_R(pmio, &gdc_regs[GDC_R_GDC_BUS_USER_INFO_WDMA], 0x40);
	}
}
