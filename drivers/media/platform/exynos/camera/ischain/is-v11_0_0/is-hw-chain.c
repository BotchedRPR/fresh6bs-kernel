...
static void is_hw_s2mpu_cfg(void)
{
	...
	reg = ioremap(0x17280054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);


	/* CSTAT: SYSMMU_D_CSTAT_S2 */
	reg = ioremap(0x17760054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* BYRP: SYSMMU_BRP_S0_S2 */
	reg = ioremap(0x17450054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* RGBP: SYSMMU_RGBP_S0_S2 */
	reg = ioremap(0x17850054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* MCFP/MCSC: SYSMMU_MCFP_S0_S2 */
	reg = ioremap(0x17C90054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* MCFP/MCSC: SYSMMU_MCFP_S1_S2	 */
	reg = ioremap(0x17CC0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);

	/* YUVP: SYSMMU_YUVP_S0_S2 */
	reg = ioremap(0x17A90054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);


	/* LME: SYSMMU_LME_S0_S2 */
	reg = ioremap(0x184C0054, 0x4);
	writel(0xFF, reg);
	iounmap(reg);
}
#endif
...
static struct is_llc_way_num is_llc_way[IS_LLC_SN_END] = {
	...
	/* UHD scenario : VOTF 0.5MB, MCFP 2.5MB */
	[IS_LLC_SN_UHD].votf = 1,
	[IS_LLC_SN_UHD].mcfp = 5,
	/* 8K scenario : VOTF 1MB, MCFP 1.5MB */
	[IS_LLC_SN_8K].votf = 2,
	[IS_LLC_SN_8K].mcfp = 3,
