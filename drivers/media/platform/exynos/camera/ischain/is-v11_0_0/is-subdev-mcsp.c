...
static u32 is_ischain_mxp_get_sbwc_type(struct is_fmt *fmt)
{
	u32 hw_sbwc = fmt->sbwc_type;
	...
	int index)
{
	...
	if (mcs_output->dma_cmd != (node->request & BIT(0))) {
		...
		if (pindex == PARAM_MCS_OUTPUT5) {
			...
			pablo_set_static_dvfs(device, "HF", IS_DVFS_SN_END);
		}
	}
	...
	/* SBWC Check */
	hw_sbwc = is_ischain_mxp_get_sbwc_type(fmt);
