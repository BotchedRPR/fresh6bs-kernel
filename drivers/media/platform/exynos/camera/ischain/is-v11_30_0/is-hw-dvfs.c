			...
			struct exynos_platform_is *pdata)
{
	...
	for (device_id = 0; device_id < IS_SENSOR_COUNT; device_id++) {
		...
		if (cfg->votf) {
			...
			t_clk_cstat = cfg->width * cfg->height * cfg->max_fps / IS_DVFS_OTF_PPC / MHZ;
...
/* LME, GDC, VRA */
static u32 _get_m2m_lv(struct is_device_ischain *device, struct exynos_platform_is *pdata)
{
	struct is_device_sensor *ids;
	struct is_device_ischain *idi;
	struct is_sensor_cfg *cfg;
	...
	for (device_id = 0; device_id < IS_SENSOR_COUNT; device_id++) {
		...
		/* 0.45 PPC */
		clk_m2m += w * h * max_fps / 45 * 100 / MHZ
			* sw_margin.numerator / sw_margin.denominator;
	}

	for (lv = IS_DVFS_LV_END - 1; lv >= 0; lv--) {
		clk = pdata->qos_tb[IS_DVFS_M2M][lv][IS_DVFS_VAL_FRQ];
	...
	dbg_dvfs(1, "[DVFS] clk(m2m:%d MHz) lv(%d)\n", device, clk_m2m, lv_m2m);
...
void is_hw_dvfs_get_qos_throughput(u32 *qos_thput)
{
	...
	qos_thput[IS_DVFS_M2M] = PM_QOS_M2M_THROUGHPUT;
}
