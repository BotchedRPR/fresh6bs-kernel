...
static void is_hw_s2mpu_cfg(void)
{
	...
	/* SYSMMU_D0_CSTAT_S2 */
	reg = ioremap(0x154F0000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D1_CSTAT_S2 */
	reg = ioremap(0x155C0000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D0_RGBP_S2 */
	reg = ioremap(0x15650000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D1_RGBP_S2 */
	reg = ioremap(0x15680000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D2_RGBP_S2	 */
	reg = ioremap(0x156B0000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D3_RGBP_S2	 */
	reg = ioremap(0x156E0000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D_YUVP_S2 */
	reg = ioremap(0x14A90000, 0x4);
	writel(0x0, reg);
	iounmap(reg);

	/* SYSMMU_D0_M2M_S2 */
	reg = ioremap(0x12CF0000, 0x4);
	writel(0x0, reg);
	iounmap(reg);
...
int is_hw_check_changed_chain_id(struct is_hardware *hardware, u32 hw_id, u32 instance)
{
	...
	case DEV_HW_PAF0:
	case DEV_HW_PAF1:
		for (hw_idx = DEV_HW_PAF0; hw_idx <= DEV_HW_PAF1; hw_idx++) {
	...
	case DEV_HW_3AA0:
	case DEV_HW_3AA1:
		for (hw_idx = DEV_HW_3AA0; hw_idx <= DEV_HW_3AA1; hw_idx++) {
			if (test_bit(hw_idx, &hardware->hw_map[instance]) && hw_idx != hw_id) {
				mapped_hw_id = hw_idx;
				break;
			}
