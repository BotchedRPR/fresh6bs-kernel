...
int is_hw_camif_cfg(void *sensor_data)
{
	...
	if (IS_ENABLED(SECURE_CAMERA_FACE) && IS_ENABLED(PROTECT_SYSREG_IS)
		&& (sensor->ex_scenario == IS_SCENARIO_SECURE)) {
...
void is_hw_chain_probe(void *core)
{
	...
	is_ispc_video_probe(&i_video[PABLO_VIDEO_PROBE_I0C], core, IS_VIDEO_I0T_NUM, 0);
	is_ispp_video_probe(&i_video[PABLO_VIDEO_PROBE_I0P], core, IS_VIDEO_I0G_NUM, 0);
	...
	/* VRA */
	is_vra_video_probe(&i_video[PABLO_VIDEO_PROBE_VRA], core, IS_VIDEO_VRA_NUM, 0);
}
