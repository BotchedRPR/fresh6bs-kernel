	...
	IS_DECLARE_PMAP(pmap))
{
	...
	if ((queue->framecfg.hw_pixeltype & PIXEL_TYPE_SIZE_MASK) == CAMERA_PIXEL_SIZE_14BIT)
	...
	IS_DECLARE_PMAP(pmap))
{
	...
	if (test_bit(IS_ISCHAIN_OFFLINE_REPROCESSING, &device->state) &&
		...
		if (sensor_config->width != 0 && sensor_config->height != 0) {
			otf_input->width = sensor_config->width;
			otf_input->height = sensor_config->height;
		}
	}
	...
	set_bit(PARAM_3AA_VDMA1_INPUT, pmap);
