	...
	IS_DECLARE_PMAP(pmap))
{
	...
	set_bit(PARAM_YPP_OTF_OUTPUT, pmap);
	...
	set_bit(PARAM_YPP_STRIPE_INPUT, pmap);
	...
	IS_DECLARE_PMAP(pmap))
{
	...
	if (!frame) {
		...
		set_bit(PARAM_YPP_CONTROL, pmap);
	}
	...
	set_bit(PARAM_YPP_DMA_INPUT, pmap);
	...
	struct camera2_node *node)
{
	...
	if (IS_ENABLED(LOGICAL_VIDEO_NODE)) {
		...
		for (n = 0; n < CAPTURE_NODE_MAX; n++) {
			...
			targetAddr = NULL;
			ret = is_hw_get_capture_slot(lvn_frame, &targetAddr,
					NULL, sframe->id);
			if (ret) {
