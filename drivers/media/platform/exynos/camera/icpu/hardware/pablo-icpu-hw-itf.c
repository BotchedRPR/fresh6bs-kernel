...
void pablo_icpu_hw_force_powerdown(struct icpu_platform_data *pdata)
{
	...
	HW_OPS(force_powerdown, pdata->sysctrl_reg_base, pdata->cstore_reg_base);
...
bool pablo_icpu_hw_fw_mem_alloc_boot(struct icpu_platform_data *pdata)
{
	if (!pdata)
		return 0;

	return pdata->fw_mem_alloc_boot;
}
KUNIT_EXPORT_SYMBOL(pablo_icpu_hw_fw_mem_alloc_boot);

u32 pablo_icpu_hw_fw_mem_size(struct icpu_platform_data *pdata)
{
	if (!pdata)
		return 0;

	return pdata->fw_mem_size;
}
KUNIT_EXPORT_SYMBOL(pablo_icpu_hw_fw_mem_size);
...
int pablo_icpu_hw_probe(void *pdev)
{
	...
	pdata->fw_mem_alloc_boot = of_property_read_bool(np, "fw-mem-alloc-boot");
	of_property_read_u32(np, "fw-mem-size", &pdata->fw_mem_size);
	ICPU_INFO("fw-mem-alloc-boot %s, size=<0x%x>", pdata->fw_mem_alloc_boot ? "true" : "false",
			pdata->fw_mem_size);
