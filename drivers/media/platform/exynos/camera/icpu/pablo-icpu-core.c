...
int pablo_icpu_boot(void)
{
	...
	pdata = dev_get_platdata(&core->pdev->dev);
	if (!pdata)
		return -ENOMEM;

#if defined(CONFIG_PM)
	ret = pm_runtime_get_sync(&core->pdev->dev);
	if (ret < 0) {
	...
	ret = pablo_icpu_debug_config((void *)core->fw_kva, core->fw_dmabuf->size);
	if (ret) {
		ICPU_ERR("icpu debug config fail, kva:0x%x, size:%d",
				core->fw_kva, core->fw_dmabuf->size);
		return ret;
	}

	/* dest : upper 32bit of 36bit address */
	pablo_icpu_hw_set_base_address(pdata, core->fw_dva >> 4);
...
static void __hw_panic_handler(void)
{
	...
	if (pm_runtime_suspended(&core->pdev->dev))
		return;
...
static void __teardown_core(struct platform_device *pdev)
{
	...
	if (pablo_icpu_hw_fw_mem_alloc_boot(pdata))
...
static int pablo_icpu_probe(struct platform_device *pdev)
{
	...
	ret = is_mem_init(&core->mem, core->pdev);
	if (ret) {
	...
	if (pablo_icpu_hw_fw_mem_alloc_boot(pdata)) {
		ret = init_firmware_mem(core, pablo_icpu_hw_fw_mem_size(pdata));
		if (ret)
			goto probe_fail;
	}
	config_firmware(pablo_icpu_hw_fw_mem_alloc_boot(pdata));
