...
static int __clear_messages(struct msg_box *box)
{
	...
	for (i = 0; i < MESSAGE_MAX_COUNT; i++) {
		if (box->msg[i].buf)
			ICPU_INFO("free dma_buf, size(%d), kva(%x), dva(%x)",
					box->msg[i].buf->size, box->msg[i].buf->kva, box->msg[i].buf->iova);

		if (box->msg[i].buf)
			CALL_VOID_BUFOP(box->msg[i].buf, free, box->msg[i].buf);
...
static int __check_buffer_param(char *str, long *dva_msb, long *dva_lsb, struct is_priv_buf **dma_buf)
{
	int ret;
	const char *open_tk = "[";
	const char *close_tk = "]";
	...
	if (buffer_type) {
		mem = &get_icpu_core()->mem;
		buf = CALL_PTR_MEMOP(mem, alloc, mem->priv, size, NULL, 0);
		if (buf == NULL) {
			ICPU_ERR("dma_buf alloc fail\n");
			return -ENOMEM;
		}
		*dma_buf = buf;
		buf_dva = CALL_BUFOP(buf, dvaddr, buf);

		*dva_msb = buf_dva >> 4;
		*dva_lsb = buf_dva & 0xF;
		buf_kva = CALL_BUFOP(buf, kvaddr, buf);
		memcpy((void *)buf_kva, (void *)data, strlen(data));

		if (!IS_ENABLED(ICPU_IO_COHERENCY))
			CALL_BUFOP(buf, sync_for_device, buf,
					0, size, DMA_TO_DEVICE);
		ret = size;
		ICPU_INFO("dma_buf size(%d), kva(%x), dva(%x), data(0x%x, 0x%x)",
				size, buf_kva, buf_dva, *dva_msb, *dva_lsb);
	}
...
static int __set_msg(const char *val, const struct kernel_param *kp)
{
	...
	long data[16];
	struct msg_box *box = __get_msg_box_by_scenario(__icpu.scenario);
	struct is_priv_buf *dma_buf =  NULL;
	size_t val_len;
	...
	if (num_data > 16) {
		ICPU_ERR("Max num of data is 16 but, %d\n", num_data);
		vfree(pbuf);
		return -1;
	}
