...
static int is_votf_alloc_internal_buffer(struct is_group *group, u32 width, u32 height)
{
	...
	bpp = is_subdev_internal_g_bpp(leader, sensor_cfg);

	is_subdev_internal_get_sbwc_type(leader, &sbwc_type, &lossy_byte32num);

	ret = is_subdev_internal_s_format(leader, width, height, bpp, sbwc_type, lossy_byte32num,
					NUM_OF_VOTF_BUF, "VOTF");
	if (ret) {
		merr("is_subdev_internal_s_format is fail(%d)", leader, ret);
		return ret;
	}

	ret = is_subdev_internal_start(leader);
	if (ret) {
		merr("subdev internal start is fail(%d)", leader, ret);
		return ret;
	}
	...
	ret = is_subdev_internal_s_format(subdev_pdaf,
					pd_width, pd_height, 16, sbwc_type, lossy_byte32num,
					NUM_OF_VOTF_BUF, "VOTF");
	if (ret) {
		merr("is_subdev_internal_s_format is fail(%d)", subdev_pdaf, ret);
		goto err_subdev_pdaf_s_format;
	}

	ret = is_subdev_internal_start(subdev_pdaf);
	if (ret) {
		merr("subdev_pdaf internal start is fail(%d)", subdev_pdaf, ret);
...
err_subdev_pdaf_s_format:
	ret_err = is_subdev_internal_stop(leader);
	if (ret_err)
...
static int is_votf_free_internal_buffer(struct is_group *group)
{
	...
	ret = is_subdev_internal_stop(leader);
	if (ret)
		merr("subdev internal stop is fail(%d)", leader, ret);
	...
	ret = is_subdev_internal_stop(subdev_pdaf);
	if (ret)
		merr("subdev internal stop is fail(%d)", subdev_pdaf, ret);
...
KUNIT_EXPORT_SYMBOL(is_votf_create_link_sensor);
...
int is_votf_destroy_link_sensor(struct is_group *group)
{
	...
	if (group->id >= GROUP_ID_MAX) {
		mgerr("group ID is invalid(%d)", group, group, group->id);
...
KUNIT_EXPORT_SYMBOL(is_votf_destroy_link_sensor);
...
KUNIT_EXPORT_SYMBOL(is_votf_get_frame);
...
KUNIT_EXPORT_SYMBOL(is_votf_register_framemgr);
