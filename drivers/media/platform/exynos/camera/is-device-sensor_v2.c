	...
	u32 position, struct is_module_enum **module)
{
	...
	*module = NULL;
	...
	if (mindex >= mmax) {
		*module = NULL;
		minfo("module(%d) is not found, position(%d)", device, sensor_id, position);
		ret = -EINVAL;
	}
...
static int is_sensor_start(struct is_device_sensor *device)
{
	...
	bool use_fasten_ae = !sensor_only && IS_ENABLED(USE_FASTEN_AE);
	...
	is_set_static_dvfs(ischain, true, use_fasten_ae);
...
p_err_itf_stream_on:
	...
	is_set_static_dvfs(ischain, false, use_fasten_ae);
		...
		struct is_subdev *subdev)
{
	...
	if (test_bit(IS_SUBDEV_INTERNAL_USE, &subdev->state)) {
		ctrl.id = SENSOR_IOCTL_G_VC1_FRAMEPTR + (subdev->id - ENTRY_SSVC1);
		ret = v4l2_subdev_call(device->subdev_csi, core, ioctl, SENSOR_IOCTL_CSI_G_CTRL, &ctrl);
		if (ret) {
			mserr("SENSOR_IOCTL_G_VC1_FRAMEPTR(g_ctrl) is fail(%d)", subdev, subdev, ret);
			return -EINVAL;
		}
	...
	struct is_frame *frame)
{
	...
	if (frame->shot) {
		 ...
		 */
		if (IS_ENABLED(CRTA_CALL) || CHK_MODECHANGE_SCN(frame->shot->ctl.aa.captureIntent)) {
	...
	unsigned int notification)
{
	...
	for (i = CSI_VIRTUAL_CH_1; i < CSI_VIRTUAL_CH_MAX; i++) {
		...
		ctrl.id = SENSOR_IOCTL_G_VC1_FRAMEPTR + (i - 1);
		ret = v4l2_subdev_call(device->subdev_csi, core, ioctl, SENSOR_IOCTL_CSI_G_CTRL, &ctrl);
		if (ret) {
			err("csi_g_ctrl fail");
			framemgr_x_barrier_irqr(framemgr, 0, flags);
...
static int is_sensor_probe(struct platform_device *pdev)
{
	...
	for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
		...
		snprintf(name, IS_STR_LEN, "VC%d", vc);
		is_subdev_probe(&device->ssvc[vc], device_id, ENTRY_SSVC0 + vc,
				name, &is_subdev_ssvc_ops);

		clear_bit(IS_SUBDEV_OPEN, &device->ssvc[vc].state);
		clear_bit(IS_SUBDEV_START, &device->ssvc[vc].state);
	}
...
int is_sensor_get_frame_type(u32 instance)
{
	...
	ids = idi->sensor;
	if (!ids) {
		err("failed to get devcie sensor(%d)", instance);
		return -ENODEV;
	}
...
KUNIT_EXPORT_SYMBOL(is_sensor_get_frame_type);
			  ...
			  struct v4l2_ext_controls *ctrls)
{
	...
	case V4L2_CID_IS_G_OFFLINE_DATA:
		...
		pkv_dma_buf_vunmap(dbuf, (void *)off_data.data_desc.base_kvaddr);
...
int is_sensor_register_itf(struct is_device_sensor *device)
{
	...
	return pablo_lib_register_sensor_itf(&sensor_peri->sensor_interface);
}
