...
EXPORT_SYMBOL_GPL(pablo_get_core_async);
	...
	u32 type, u32 scenario, ulong smc_cmd)
{
	...
	case SMC_SECCAM_PREPARE:
		ret = CALL_PSMC_OPS(psmc_ops, call, SMC_SECCAM_PREPARE, 0, 0, 0);
		if (ret) {
	...
	case SMC_SECCAM_UNPREPARE:
		...
		ret = CALL_PSMC_OPS(psmc_ops, call, SMC_SECCAM_UNPREPARE, 0, 0, 0);
		if (ret != 0) {
...
void is_print_frame_dva(struct is_subdev *subdev)
{
	...
	if (test_bit(IS_SUBDEV_START, &subdev->state) && framemgr) {
		...
		for (j = 0; j < framemgr->num_frames; ++j) {
			...
			if (shot)
				is_hw_print_target_dva(&framemgr->frames[j], subdev->instance);
		}
	}
}
...
static void __is_fault_handler(struct device *dev)
{
	...
	for (i = 0; i < IS_SENSOR_COUNT; i++) {
		...
		if (test_bit(IS_SENSOR_OPEN, &sensor->state) && framemgr) {
			...
			for (sd_index = 0; sd_index < CSI_VIRTUAL_CH_MAX; sd_index++)
				is_print_frame_dva(&sensor->ssvc[sd_index]);
		}
	}
	...
	for (i = 0; i < IS_STREAM_COUNT; i++) {
		...
		/* BYRP */
		subdev = &ischain->group_byrp.leader;
		is_print_frame_dva(subdev);
		...
		/* RGBP */
		subdev = &ischain->group_rgbp.leader;
		is_print_frame_dva(subdev);

		/* MCFP */
		subdev = &ischain->group_mcfp.leader;
		is_print_frame_dva(subdev);

		/* YUVP */
		subdev = &ischain->group_yuvp.leader;
		is_print_frame_dva(subdev);
...
static int is_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		if (ret
		    && !IS_ENABLED(CONFIG_PABLO_KUNIT_TEST)
	...
	pablo_hw_chain_info_probe(&core->hardware);
	...
	ret = is_hardware_probe(&core->hardware, &core->interface,
				&core->interface_ischain,
				&core->resourcemgr.mem, core->pdev);
	if (ret) {
	...
	if (IS_ENABLED(SECURE_CAMERA_IRIS) || IS_ENABLED(SECURE_CAMERA_FACE)) {
		...
		if (IS_ENABLED(SECURE_CAMERA_FACE_SEQ_CHK))
			...
			ret = CALL_PSMC_OPS(psmc_ops, call,
					SMC_SECCAM_SETENV, SECURE_CAMERA_CH, SECURE_CAMERA_HEAP_ID, 0);
		...
		if (IS_ENABLED(SECURE_CAMERA_FACE_SEQ_CHK))
			...
			ret = CALL_PSMC_OPS(psmc_ops, call,
					SMC_SECCAM_INIT, mem_info_addr, mem_info_size, 0);
		...
		if (IS_ENABLED(SECURE_CAMERA_FACE_SEQ_CHK))
			...
			ret = CALL_PSMC_OPS(psmc_ops, call,
					SMC_SECCAM_INIT_NSBUF, mem_info_addr, mem_info_size, 0);
...
static int is_driver_init(void)
{
	...
	ret = platform_driver_register(is_sensor_get_platform_driver());
	if (ret) {
		err("is_sensor_driver register failed(%d)", ret);
		return ret;
	}

	ret = platform_driver_register(pablo_paf_pdp_get_platform_driver());
	if (ret) {
	...
	ret = platform_driver_register(is_camif_dma_get_platform_driver());
	if (ret) {
		err("is_camif_dma_driver register failed(%d)", ret);
		return ret;
	}

	ret = platform_driver_register(pablo_camif_subblks_get_platform_driver());
	if (ret) {
...
void is_driver_exit(void)
{
	...
	platform_driver_unregister(pablo_paf_pdp_get_platform_driver());
	platform_driver_unregister(is_sensor_get_platform_driver());
