...
#define mdbg_adt(level, fmt, instance, args...) \
	dbg_common((is_get_debug_param(IS_DEBUG_PARAM_HW)) >= (level), "[%d][ADT]", fmt, instance, ##args)
#define minfo_adt(fmt, instance, args...) \
	info_common("[%d][ADT]", fmt, instance, ##args)
#define mwarn_adt(fmt, instance, args...) \
	warn_common("[%d][ADT][WRN]%d:", fmt "\n", instance, __LINE__, ##args)
#define merr_adt(fmt, instance, args...) \
	err_common("[%d][ADT][ERR]%s:%d:", fmt "\n", instance, __func__, __LINE__, ##args)
...
#if IS_ENABLED(CONFIG_PABLO_SELF_TEST)
