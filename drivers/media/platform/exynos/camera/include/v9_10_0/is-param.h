...
#define VOTF_IP_ISPLP			(0)
#define VOTF_IP_ISPHQ			(1)
#define VOTF_IP_DCP			(2)
#define VOTF_IP_CIP1			(4)
#define VOTF_IP_CIP2			(3)

#define VOTF_AXI_ID_DCP_MASTER_IN	(0)
#define VOTF_AXI_ID_DCP_SALVE_IN	(1)
#define VOTF_AXI_ID_CIP1_MASTER_IN	(0)
#define VOTF_AXI_ID_CIP1_SALVE_IN	(1)
#define VOTF_AXI_ID_CIP2_DISPARITY_IN	(0)

#define MCSC_OUTPUT_DS		MCSC_OUTPUT5
#define MCSC_INPUT_HF		MCSC_OUTPUT5

typedef u32 pdma_addr_t;
...
struct taa_param {
	...
	struct param_dma_output		efd_output;	/* Early FD */
	struct param_dma_output		mrg_output;	/* mrg_out*/
	struct param_dma_output		ods_output;	/* ORBDS */
	struct param_dma_output		lds_output;	/* LMEDS */
	struct param_dma_output		hf_output;	/* High Frequency */
	struct param_stripe_input	stripe_input;
};
#endif
...
struct taa_param_set {
	...
	pdma_addr_t			output_dva_before_bds[IS_MAX_PLANES];
	pdma_addr_t			output_dva_after_bds[IS_MAX_PLANES];
	pdma_addr_t			output_dva_mrg[IS_MAX_PLANES];
	pdma_addr_t			output_dva_efd[IS_MAX_PLANES];
#if defined(ENABLE_ORBDS)
	pdma_addr_t			output_dva_orbds[IS_MAX_PLANES];
#endif
#if defined(ENABLE_LMEDS)
	pdma_addr_t			output_dva_lmeds[IS_MAX_PLANES];
#endif
#if IS_ENABLED(USE_HF_INTERFACE)
	pdma_addr_t			output_dva_hf[IS_MAX_PLANES];
#endif
#if IS_ENABLED(LOGICAL_VIDEO_NODE)
	pdma_addr_t			output_dva_drcgrid[IS_MAX_PLANES];
#endif
...
struct isp_param_set {
	...
	pdma_addr_t			input_dva[IS_MAX_PLANES];
	pdma_addr_t			input_dva_tnr_prev[IS_MAX_PLANES];
	pdma_addr_t			input_dva_tnr_wgt[IS_MAX_PLANES];
#if IS_ENABLED(USE_MCFP_MOTION_INTERFACE)
	pdma_addr_t			input_dva_motion[IS_MAX_PLANES];
	u64				input_kva_motion[IS_MAX_PLANES]; /* For debugging */
#endif
#if defined(ENABLE_RGB_REPROCESSING)
	pdma_addr_t			input_dva_rgb[IS_MAX_PLANES];
	pdma_addr_t			input_dva_noise[IS_MAX_PLANES];
#endif
#if defined(ENABLE_SC_MAP)
	pdma_addr_t			input_dva_scmap[IS_MAX_PLANES];
#endif
#if IS_ENABLED(USE_MCFP_MOTION_INTERFACE)
	pdma_addr_t			input_dva_drcgrid[IS_MAX_PLANES];
#endif
#if IS_ENABLED(USE_YPP_INTERFACE)
	pdma_addr_t			output_dva_yuv[IS_MAX_PLANES];
	pdma_addr_t			output_dva_rgb[IS_MAX_PLANES];
#else
	pdma_addr_t			output_dva_chunk[IS_MAX_PLANES];
	pdma_addr_t			output_dva_yuv[IS_MAX_PLANES];
#endif
	pdma_addr_t			output_dva_tnr_wgt[IS_MAX_PLANES];
	pdma_addr_t			output_dva_tnr_prev[IS_MAX_PLANES];
#if IS_ENABLED(USE_YPP_INTERFACE)
	pdma_addr_t			output_dva_nrds[IS_MAX_PLANES];
	pdma_addr_t			output_dva_noise[IS_MAX_PLANES];
	pdma_addr_t			output_dva_hf[IS_MAX_PLANES];
	pdma_addr_t			output_dva_drc[IS_MAX_PLANES];
	pdma_addr_t			output_dva_hist[IS_MAX_PLANES];
#endif
#if defined(ENABLE_RGB_REPROCESSING)
	pdma_addr_t			output_dva_noise_rep[IS_MAX_PLANES];
#endif
...
struct ypp_param_set {
	...
	pdma_addr_t			input_dva[IS_MAX_PLANES];
	pdma_addr_t			input_dva_lv2[IS_MAX_PLANES];
	pdma_addr_t			input_dva_noise[IS_MAX_PLANES];
	pdma_addr_t			input_dva_drc[IS_MAX_PLANES];
	pdma_addr_t			input_dva_hist[IS_MAX_PLANES];
	u32				mode;
	u32				reserved[4];
	u32				tnr_mode;
	u32				instance_id;
...
struct lme_param_set {
	...
	pdma_addr_t			cur_input_dva[IS_MAX_PLANES];
	pdma_addr_t			prev_input_dva[IS_MAX_PLANES];
#if IS_ENABLED(ENABLE_LME_PREV_KVA)
	u64				prev_input_kva[IS_MAX_PLANES];
#endif
	pdma_addr_t			output_dva[IS_MAX_PLANES];			/* H/W MV dva */
