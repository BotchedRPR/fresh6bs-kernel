...
#define CAMERA2_MAX_MF_STILL_FRAME_NUM		10
...
#define CHK_MODECHANGE_SCN(captureIntent)	\
	...
	|| (captureIntent == AA_CAPTURE_INTENT_STILL_CAPTURE_REMOSAIC_LLHDR_DYNAMIC_SHOT) \
...
enum is_subscenario_id {
	...
	ISS_SUB_SCENARIO_CROPPED_REMOSAIC_CAPTURE_SINGLE = 21,                      /* 21: Remosaic cropped zoom capture single (HDR Auto) */
	ISS_SUB_SCENARIO_MERGED_STILL_REMOSAIC_CAPTURE_MFHDR = 22,                  /* 22: Remosaic MFHDR Capture ( HDR Auto/Off ) */
	...
	ISS_SUB_SCENARIO_AUTO_NIGHT_CAPTURE = 100,                                  /* 100: Auto Super night capture */
...
enum aa_capture_intent {
	...
	AA_CAPTURE_INTENT_STILL_CAPTURE_CROPPED_REMOSAIC_DYNAMIC_SHOT           = 123,  /* Not used, Will be removed */
	AA_CAPTURE_INTENT_STILL_CAPTURE_REMOSAIC_SINGLE_FLASH                   = 124,
	AA_CAPTURE_INTENT_STILL_CAPTURE_CROPPED_REMOSAIC_SINGLE                 = 125,  /* Not used, Will be removed */
	AA_CAPTURE_INTENT_STILL_CAPTURE_SHORT_REF_LLHDR_DYNAMIC_SHOT            = 126,
	AA_CAPTURE_INTENT_STILL_CAPTURE_PROCESSED_REMOSAIC_SINGLE               = 127,  /* Not used, Will be removed */
	AA_CAPTURE_INTENT_STILL_CAPTURE_PROCESSED_REMOSAIC_DYNAMIC_SHOT         = 128,  /* Not used, Will be removed */
	AA_CAPTURE_INTENT_STILL_CAPTURE_PROCESSED_REMOSAIC_MFHDR_DYNAMIC_SHOT   = 129,  /* Not used, Will be removed */
	AA_CAPTURE_INTENT_STILL_CAPTURE_PROCESSED_REMOSAIC_SINGLE_FLASH         = 130,  /* Not used, Will be removed */
...
struct camera2_aa_ctl {
	...
	uint32_t			vendor_captureExtraInfo;
	uint32_t			vendor_zoomLockState;
	uint32_t			vendor_reserved[15];
};
...
struct camera2_tnr_info {
	uint32_t	bestTnrScore;

	uint32_t	sensorToMeDsCropAreaOffsetX;
	uint32_t	sensorToMeDsCropAreaOffsetY;
	uint32_t	sensorToMeDsCropAreaWidth;
	uint32_t	sensorToMeDsCropAreaHeight;

	float		meDsLv0Ratio;
	float		meDsLv1Ratio;

	uint32_t	meDsLv0OutputWidth;
	uint32_t	meDsLv0OutputHeight;
	uint32_t	meDsLv1OutputWidth;
	uint32_t	meDsLv1OutputHeight;

};

struct camera2_me_rta_result {
	uint32_t	finalMeConfidence;
	uint32_t	meConfidence;
	uint32_t	localMotionLevel;

	uint32_t	lmcGridNumX;
	uint32_t	lmcGridNumY;
	uint32_t	lmcBlockWidth;
	uint32_t	lmcBlockHeight;

	float		processedGyroAngle[3];
	float		processedOisAngle[2];

	uint32_t	mcType;
	uint32_t	gmcGridDx[7][9];
	uint32_t	gmcGridDy[7][9];

	uint32_t	reserved[10];
};

struct camera2_me_rta_capture_result {
	uint32_t	frameNumOfReferenceFrame;
	uint32_t	tnrScoreOfReferenceFrame;
	uint32_t	motionIndexOfReferenceFrame;
	int32_t		brightnessValueOfReferenceFrame;

	uint32_t	numOfCandidateFrames;
	uint32_t	frameNumOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	uint32_t	tnrScoreOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	uint32_t	motionIndexOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	int32_t		brightnessValueOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];

	uint32_t	finalMeConfidenceOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	uint32_t	meConfidenceOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	uint32_t	localMotionLevelOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	float		avgGridDistanceOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];
	uint32_t	gyroMagnitudeOfCandidateFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];

	uint32_t	numOfMergeFrames;
	uint32_t	orderOfMergeFrames[CAMERA2_MAX_MF_STILL_FRAME_NUM];

	uint32_t	reserved[10];
};

struct camera2_tnr_udm {
	struct camera2_tnr_info			tnrInfo;

	uint32_t				meRtaInput[100];

	struct camera2_me_rta_result		meRtaResult;
	struct camera2_me_rta_capture_result	meRtaCaptureResult;
};
...
#if IS_ENABLED(USE_MERTA_GYRO_INFO)
...
#if IS_ENABLED(USE_DDK_INTF_CAP_META)
struct camera2_shot_cap {
	struct camera2_cap_meta	cap_meta;
	/*magic : 23456789 */
	uint32_t		magicNumber;
};
#endif
