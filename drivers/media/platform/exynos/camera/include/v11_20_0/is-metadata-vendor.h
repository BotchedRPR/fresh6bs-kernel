...
enum sensor_colorfilterarrangement {
	...
	SENSOR_COLORFILTERARRANGEMENT_MONO,
	SENSOR_COLORFILTERARRANGEMENT_NIR
};
...
enum capture_state {
	...
	CAPTURE_STATE_PICKING_DELAY_0 = 1,
	CAPTURE_STATE_PICKING_DELAY_1 = 2,
	CAPTURE_STATE_PICKING_DELAY_2 = 3,
...
enum aa_moire_trigger
{
	AA_MOIRE_TRIGGER_OFF = 0,
	AA_MOIRE_TRIGGER_ON,
};
...
struct camera2_aa_ctl {
	...
	char				vendor_multiFrameEvList[24];
	uint32_t			vendor_captureExtraInfo;
	uint32_t			vendor_zoomLockState;
	enum aa_moire_result		vendor_moireResult;
	uint32_t 			vendor_displayP3Enabled;
...
struct camera2_dm {
	...
	struct camera2_blacklevel_dm		shortBlacklevel;
...
struct camera2_rta_udm {
	uint32_t vsLength;
	uint32_t vendorSpecific[53];

	uint32_t vs2Length;
	uint32_t vendorSpecific2[85];
};
...
struct camera2_tnr_info {
	...
	int32_t 	sensorToMeDsCropAreaOffsetX;
	int32_t 	sensorToMeDsCropAreaOffsetY;
...
struct camera2_segmentationInfo_uctl
{
	uint8_t segmentationMap[256];
	uint32_t dataSize;
	uint32_t version;
	uint32_t resultType;
...
struct camera2_node {
	 ...
	 * result : [ 0] 0: buf0 is invalid, 1: buf0 is valid
	 *          [ 1] 0: buf1 is invalid, 1: buf1 is valid
	 *          ...
	 *          [31] 0: buf31 is invalid, 1: buf31 is valid
	 */
	uint32_t			mode;
