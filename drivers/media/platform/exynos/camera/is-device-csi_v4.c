		...
		struct is_frame *frame, u32 index, u32 vc)
{
	...
	csi_hw_s_multibuf_dma_addr(wdma->regs_ctl, wdma->regs_vc[vc],
				vc, index, frame->dvaddr_buffer[0]);
}
...
static inline dma_addr_t csi_get_dva(struct is_frame *frame, int idx, int vc)
{
	...
	if (IS_ENABLED(USE_SENSOR_GROUP_LVN)) {
		sframe = &frame->cap_node.sframe[vc];
		dvaddr = sframe->dva[idx];
	} else {
	...
	int enable)
{
	...
	ret = csi_s_fro(csi, device, csi->otf_ch);
	if (ret) {
	...
	/* BNS configuration */
	if (csi->bns && csi->otf_ch >= 0)
		pablo_camif_bns_cfg(csi->bns, sensor_cfg, csi->otf_ch);
	...
	if (test_bit(IS_GROUP_OTF_OUTPUT, &group->state) &&
		...
		if (mcb) {
			...
			set_bit(csi->otf_ch, &mcb->active_ch);
		...
		if (ebuf) {
			...
			mutex_lock(&ebuf->lock);
			csi_hw_s_ebuf_enable(ebuf->regs, true, csi->otf_ch, EBUF_AUTO_MODE,
					ebuf->num_of_ebuf, ebuf->fake_done_offset);
			mutex_unlock(&ebuf->lock);

			/* ebuf support 2 vc */
			csi_hw_s_cfg_ebuf(ebuf->regs, csi->otf_ch,
				CSI_VIRTUAL_CH_0,
				sensor_cfg->input[CSI_VIRTUAL_CH_0].width,
				sensor_cfg->input[CSI_VIRTUAL_CH_0].height);
	...
	struct is_device_csi *csi)
{
	...
	if (test_bit(IS_GROUP_OTF_OUTPUT, &group->state) &&
		...
		if (mcb) {
			mutex_lock(&mcb->lock);
			clear_bit(csi->otf_ch, &mcb->active_ch);
			if (!mcb->active_ch)
		...
		if (ebuf) {
			mutex_lock(&ebuf->lock);
			csi_hw_s_ebuf_enable(ebuf->regs, false, csi->otf_ch, EBUF_AUTO_MODE,
					ebuf->num_of_ebuf, ebuf->fake_done_offset);
			mutex_unlock(&ebuf->lock);
