...
static inline void is_hw_cstat_frame_end(struct is_hw_ip *hw_ip, u32 instance, bool frame_done)
{
	...
	if (test_and_clear_bit(HW_SUSPENDING, &hw_ip->state)) {
		...
		enum is_cstat_subdev_id s_id)
{
	...
	msdbg_hw(2, "[F%d][%s] cache invalidate: kva %p, size: %lu", instance, hw_ip,
...
static int is_hw_cstat_isr1(u32 hw_id, void *data)
{
	...
	if (!test_bit(HW_CONFIG, &hw_ip->state) || test_bit(HW_SUSPEND, &hw_ip->state)) {
		msinfo_hw("[INT1] Ignore ISR before init_shot 0x%lx 0x%lx\n",
...
static int is_hw_cstat_isr2(u32 hw_id, void *data)
{
	...
	if (!test_bit(HW_CONFIG, &hw_ip->state) || test_bit(HW_SUSPEND, &hw_ip->state)) {
		msinfo_hw("[INT2] Ignore ISR before init_shot 0x%x\n",
				instance, hw_ip, src);
		atomic_dec(&hw->isr_run_count);
		wake_up(&hw->isr_wait_queue);
...
static int __nocfi is_hw_cstat_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = CALL_HW_HELPER_OPS(hw_ip, open, instance, &hw->lib[instance],
				LIB_FUNC_CSTAT);
	if (ret)
	...
	mem = CALL_HW_CHAIN_INFO_OPS(hw_ip->hardware, get_iommu_mem, GROUP_ID_3AA0);
	...
	for (sd_id = 0; sd_id < IS_CSTAT_SUBDEV_NUM; sd_id++) {
		...
		pablo_internal_subdev_probe(sd, instance, ENTRY_INTERNAL, mem,
					is_cstat_subdev_name[sd_id]);

		sd->width = info.w;
		sd->height = info.h;
		sd->num_planes = 1;
		sd->num_batch = 1;
		sd->num_buffers = info.num;
		sd->bits_per_pixel = info.bpp;
		sd->memory_bitwidth = info.bpp;
			...
			bool flag, u32 f_type)
{
	...
	ret = CALL_HW_HELPER_OPS(hw_ip, init, instance, &hw->lib[instance],
				(u32)flag, f_type, LIB_FUNC_CSTAT);
	if (ret)
		return ret;
...
err_dma_create:
	CALL_HW_HELPER_OPS(hw_ip, deinit, instance, &hw->lib[instance]);
...
static int is_hw_cstat_disable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	CALL_HW_HELPER_OPS(hw_ip, disable, instance, &hw->lib[instance]);
		...
		struct cstat_param_set *p_set)
{
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_vhist",
			hw_ip, frame, 0, 1,
		...
		struct cstat_param_set *p_set)
{
	...
	for (sd_id = 0; sd_id < IS_CSTAT_SUBDEV_NUM; sd_id++) {
		sd = &hw->subdev[sd_id];
		if (!test_bit(PABLO_SUBDEV_ALLOC, &sd->state)) {
			mswarn_hw("[%s] Not in START state 0x%lx", instance, hw_ip,
					sd->name, sd->state);
			continue;
		}
...
shot_fail_recovery:
	if (do_lib_shot)
		CALL_HW_HELPER_OPS(hw_ip, restore, instance, &hw->lib[instance]);
...
static int is_hw_cstat_get_meta(struct is_hw_ip *hw_ip, struct is_frame *frame,
	ulong hw_map)
{
	struct is_hw_cstat *hw_cstat = (struct is_hw_cstat *)hw_ip->priv_info;

	return CALL_HW_HELPER_OPS(hw_ip, get_meta, frame->instance,
				frame, &hw_cstat->lib[frame->instance]);
}
	...
	u32 instance, u32 fcount, u32 size, ulong addr)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, get_cap_meta, instance,
				&hw_cstat->lib[instance], fcount, size, addr);
}
...
static int is_hw_cstat_load_setfile(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, load_setfile, instance, &hw_cstat->lib[instance]);
}
	...
	u32 instance, ulong hw_map)
{
	...
	ret = CALL_HW_HELPER_OPS(hw_ip, apply_setfile, instance, &hw->lib[instance], scenario);
	if (ret)
		return ret;

	ret = CALL_HW_HELPER_OPS(hw_ip, load_cal_data, instance, &hw->lib[instance]);
	if (ret)
		return ret;
...
static int is_hw_cstat_delete_setfile(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, delete_setfile, instance, &hw_cstat->lib[instance]);
}
...
static int is_hw_cstat_restore(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, restore, instance, &hw_cstat->lib[instance]);
}
...
static int is_hw_cstat_sensor_start(struct is_hw_ip *hw_ip, u32 instance)
{
	 ...
	 */
	if (hw->input == DMA || !IS_ENABLED(CSTAT_DDK_LIB_CALL))
		return 0;
...
static int is_hw_cstat_notify_timeout(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, notify_timeout, instance, &hw->lib[instance]);
}
