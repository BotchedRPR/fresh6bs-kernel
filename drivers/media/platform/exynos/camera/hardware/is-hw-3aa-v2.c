	...
	struct param_dma_output *dma_out, u32 *dma_out_dva, u32 instance, u32 fcount)
{
	...
	*dma_out_dva = frame->dvaddr_buffer[0];
	...
	msdbg_hw(2, "[F%d][%s] index:%d dva %pad kva 0x%lx size %d\n", instance, hw_ip,
			fcount, sd->name,
...
static inline void __is_hw_3aa_frame_end(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (hw->param_set[instance].otf_input.source == OTF_INPUT_PAF_RDMA_PATH)
...
static int is_hw_3aa_isr(u32 id, void *data)
{
	...
	if (!test_bit(HW_CONFIG, &hw_ip->state)) {
		msinfo_hw("[INT] Ignore ISR before HW config 0x%x\n",
	...
	if (!test_bit(HW_OPEN, &hw_ip->state) || !test_bit(HW_RUN, &hw_ip->state)) {
		mserr_hw("[F%d]id %d, invalid HW state 0x%lx, status 0x%x\n", instance, hw_ip,
...
static int is_hw_csis_dma_isr(u32 id, void *data)
{
	...
	if (!test_bit(HW_CONFIG, &hw_ip->state)) {
		msinfo_hw("[DMA] Ignore ISR before HW config 0x%x\n",
				instance, hw_ip, status);
		goto out;
	}

	if (!test_bit(HW_OPEN, &hw_ip->state)) {
		mserr_hw("[F%d] %s invalid HW state 0x%lx status 0x%x\n", instance, hw_ip,
...
static int __nocfi is_hw_3aa_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	msdbg_hw(2, "[%s] core: 0x%lx context0: 0x%lx, zsl: 0x%lx, strp: 0x%lx\n",
		instance, hw_ip, __func__,
		(ulong)hw_ip->regs[REG_SETA], (ulong)hw_ip->regs[REG_EXT1],
		(ulong)hw_ip->regs[REG_EXT2], (ulong)hw_ip->regs[REG_EXT3]);
...
static int is_hw_3aa_enable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	for (i = 0; i < max_num; i++) {
		struct is_hw_ip *hw_ip_3aa = CALL_HW_CHAIN_INFO_OPS(hardware, get_hw_ip,
								DEV_HW_3AA0 + i);
	...
	u32 instance)
{
	...
	if (param_set->otf_input.source != OTF_INPUT_PAF_RDMA_PATH) {
	...
	ulong hw_map)
{
	...
	if (frame->type == SHOT_TYPE_INTERNAL) {
		...
		CALL_HW_OPS(hw_ip->hardware, dma_cfg, "3xc",
		...
		CALL_HW_OPS(hw_ip->hardware, dma_cfg, "3xp",
			hw_ip, frame, b_idx, frame->num_buffers,
		...
		CALL_HW_OPS(hw_ip->hardware, dma_cfg, "mrg",
			hw_ip, frame, b_idx, frame->num_buffers,
...
#if IS_ENABLED(LOGICAL_VIDEO_NODE)
		CALL_HW_OPS(hw_ip->hardware, dma_cfg, "txdrg",
			hw_ip, frame, b_idx, frame->num_buffers,
		...
		CALL_HW_OPS(hw_ip->hardware, dma_cfg, "efd",
			hw_ip, frame, 0, 1,
...
#if defined(ENABLE_ORBDS)
		CALL_HW_OPS(hw_ip->hardware, dma_cfg, "ods",
			hw_ip, frame, 0, 1,
	...
	u32 instance, u32 fcount, u32 size, ulong addr)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, get_cap_meta, instance,
				&hw_3aa->lib[instance], fcount, size, addr);
}
