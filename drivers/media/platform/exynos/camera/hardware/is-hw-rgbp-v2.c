...
static int is_hw_rgbp_handle_interrupt0(u32 id, void *context)
{
	...
	status = rgbp_hw_g_int0_state(hw_ip->pmio, true,
			(hw_ip->num_buffers & 0xffff), &hw_rgbp->irq_state[RGBP_INTR0])
			& rgbp_hw_g_int0_mask(hw_ip->pmio);
	...
	if (rgbp_hw_is_occurred0(status, INTR0_FRAME_END_INT)) {
		rgbp_hw_s_clock(hw_ip->pmio, false);
...
static int is_hw_rgbp_handle_interrupt1(u32 id, void *context)
{
	...
	status = rgbp_hw_g_int1_state(hw_ip->pmio, true,
			(hw_ip->num_buffers & 0xffff), &hw_rgbp->irq_state[RGBP_INTR1])
			& rgbp_hw_g_int1_mask(hw_ip->pmio);
...
static int is_hw_rgbp_finish(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	res = rgbp_hw_wait_idle(hw_ip->pmio);
	if (res)
...
static int is_hw_rgbp_set_cmdq(struct is_hw_ip *hw_ip, u32 instance, u32 set_id, u32 num_buffers, struct c_loader_buffer *clb)
{
	...
	/*
	 * TODO : enable corex
	ret = rgbp_hw_s_corex_update_type(hw_ip->pmio, set_id, COREX_COPY);
	if (ret)
		return ret;
	*/

	if (clb)
		rgbp_hw_s_cmdq(hw_ip->pmio, set_id, num_buffers, clb->header_dva, clb->num_of_headers);
	else
		rgbp_hw_s_cmdq(hw_ip->pmio, set_id, num_buffers, 0, 0);
...
static int __nocfi is_hw_rgbp_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	hw_rgbp->pb_c_loader_payload = CALL_PTR_MEMOP(hw_ip->mem, alloc, hw_ip->mem->priv,
							0x8000, NULL, 0);
	...
	hw_rgbp->pb_c_loader_header = CALL_PTR_MEMOP(hw_ip->mem, alloc,
			...
			bool flag, u32 f_type)
{
	...
	for (idx = 0; idx < rdma_max_cnt; idx++) {
		ret = rgbp_hw_rdma_create(&hw_rgbp->rdma[idx], hw_ip->pmio, idx);
		if (ret) {
	...
	for (idx = 0; idx < wdma_max_cnt; idx++) {
		ret = rgbp_hw_wdma_create(&hw_rgbp->wdma[idx], hw_ip->pmio, idx);
		if (ret) {
							...
							u32 instance, u32 id, u32 set_id, struct is_frame *dma_frame)
{
	...
	if (cmd == DMA_OUTPUT_COMMAND_ENABLE) {
		...
		rgbp_hw_s_sbwc(hw_ip->pmio, set_id, sbwc_en);
	}
...
static int __is_hw_rgbp_bypass(struct is_hw_ip *hw_ip, u32 set_id)
{
	...
	rgbp_hw_s_block_bypass(hw_ip->pmio, set_id);
	...
	u32 instance, const struct is_frame *frame)
{
	...
	rgbp_hw_s_grid_cfg(hw_ip->pmio, &grid_cfg);
	...
	u32 instance, u32 rgbp_strip_start_pos, const struct is_frame *frame, u32 set_id)
{
	...
	rgbp_hw_s_chain_src_size(hw_ip->pmio, set_id, in_width, in_height);
	rgbp_hw_s_chain_dst_size(hw_ip->pmio, set_id, out_width, out_height);

	rgbp_hw_s_dtp_output_size(hw_ip->pmio, set_id, in_width, in_height);
	...
	rgbp_hw_s_cout_fifo(hw_ip->pmio, set_id, param_set->otf_output.cmd);

	rgbp_hw_s_yuvsc_enable(hw_ip->pmio, set_id, yuvsc_enable, yuvsc_bypass);

	rgbp_hw_s_yuvsc_dst_size(hw_ip->pmio, set_id, yuvsc_size.dst_h_size, yuvsc_size.dst_v_size);

	hratio = ((u64)yuvsc_size.input_h_size << 20) / yuvsc_size.dst_h_size;
	vratio = ((u64)yuvsc_size.input_v_size << 20) / yuvsc_size.dst_v_size;

	rgbp_hw_s_yuvsc_scaling_ratio(hw_ip->pmio, set_id, hratio, vratio);

	/* set_crop */
	rgbp_hw_s_crop(hw_ip->pmio, in_width, in_height, out_width, out_height);
	...
	if (param_set->dma_output_hf.cmd == DMA_OUTPUT_VOTF_ENABLE) {
		...
		upsc_enable = 0x1;
		rgbp_hw_s_upsc_dst_size(hw_ip->pmio, set_id, upsc_size.dst_h_size, upsc_size.dst_v_size);

		hratio = (upsc_size.input_h_size << 20) / upsc_size.dst_h_size;
		vratio = (upsc_size.input_v_size << 20) / upsc_size.dst_v_size;

		rgbp_hw_s_upsc_scaling_ratio(hw_ip->pmio, set_id, hratio, vratio);
		rgbp_hw_s_upsc_coef(hw_ip->pmio, set_id, hratio, vratio);
		...
		decomp_enable = 0x1;
		decomp_bypass = 0x0;
		rgbp_hw_s_decomp_size(hw_ip->pmio, set_id, upsc_size.dst_h_size, upsc_size.dst_v_size);

		rgbp_hw_s_votf(hw_ip->pmio, set_id, 0x1, 0x1);
	} else
		rgbp_hw_s_votf(hw_ip->pmio, set_id, 0x0, 0x1);

hf_disable:
	rgbp_hw_s_upsc_enable(hw_ip->pmio, set_id, upsc_enable, upsc_bypass);
	rgbp_hw_s_decomp_enable(hw_ip->pmio, set_id, decomp_enable, decomp_bypass);

	if (!IS_ENABLED(IRTA_CALL))
		gamma_bypass = 0x1;

	/* set gamma */
	rgbp_hw_s_gamma_enable(hw_ip->pmio, set_id, gamma_enable, gamma_bypass);
	...
	ulong hw_map)
{
	...
	cmd_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "rgbp", hw_ip,
	...
	cmd_output_hf = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "rgbphf", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_output_sf = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "rgbpsf", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_output_yuv = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "rgbpyuv", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_output_rgb = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "rgbprgb", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	rgbp_hw_s_clock(hw_ip->pmio, true);

	rgbp_hw_s_core(hw_ip->pmio, frame->num_buffers, set_id,
	...
	if (param_region->rgbp.control.strgen == CONTROL_COMMAND_START) {
		...
		rgbp_hw_s_strgen(hw_ip->pmio, set_id);
	}
	...
	struct is_interface_ischain *itfc, int id, const char *name)
{
	...
	hw_ip->pmio_reg_seqs = vzalloc(sizeof(struct pmio_reg_seq) * rgbp_hw_g_reg_cnt());
