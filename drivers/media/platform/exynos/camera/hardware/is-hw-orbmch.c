...
IS_TIMER_FUNC(__is_hw_orbmch_reset)
{
	...
	if (frame) {
		...
		CALL_HW_OPS(hw_ip->hardware, frame_done, hw_ip, frame, -1, IS_HW_CORE_END, IS_SHOT_UNPROCESSED, false);
	} else
	...
	struct is_frame *frame, u32 instance, struct pablo_internal_subdev *subdev)
{
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "mch_prev_desc_in", hw_ip,
			frame, cur_idx, frame->num_buffers,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
			param_set->prev_output_dva,
			mchxsTargetAddress);
	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "mch_prev_desc_in_k", hw_ip,
			frame, cur_idx,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
	...
	u32 instance, u32 status)
{
	...
	if (IS_ENABLED(ORBMCH_DDK_LIB_CALL)) {
		...
		CALL_HW_OPS(hardware, frame_done, hw_ip, NULL, -1, IS_HW_CORE_END, IS_SHOT_SUCCESS, true);
...
static int is_hw_orbmch_shot(struct is_hw_ip *hw_ip, struct is_frame *frame, ulong hw_map)
{
	...
	/* DMA settings */
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "orb_cur_in", hw_ip,
				frame, cur_idx, frame->num_buffers,
				&param_set->dma.cur_input_cmd,
				param_set->dma.input_plane,
				param_set->cur_input_dva,
				frame->dvaddr_buffer);
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "mch_mv_out", hw_ip,
			frame, cur_idx, frame->num_buffers,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
			param_set->output_dva,
			frame->orbxcTargetAddress);
	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "mch_pure_out", hw_ip,
			frame, cur_idx,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
			param_set->output_kva_motion_pure,
			frame->orbxcKTargetAddress);
	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "mch_proc_out", hw_ip,
			frame, cur_idx,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
	...
	u32 instance, enum ShotErrorType done_type)
{
	...
	return CALL_HW_OPS(hw_ip->hardware, frame_done, hw_ip, frame, -1, IS_HW_CORE_END, done_type, false);
}
