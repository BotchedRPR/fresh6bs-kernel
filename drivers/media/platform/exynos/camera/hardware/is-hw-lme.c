...
IS_TIMER_FUNC(is_hw_frame_end_timer)
{
	...
	CALL_HW_OPS(hardware, frame_done, hw_ip, NULL, -1, 0,
	...
	ulong hw_map)
{
	...
	if (is_reprocessing) {
		...
		cmd_cur_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lems", hw_ip,
		...
		cmd_prev_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lmes_prev", hw_ip,
				frame, cur_idx, frame->num_buffers,
		...
		CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "lmes_prev", hw_ip,
			frame, cur_idx,
			&param_set->dma.prev_input_cmd,
			param_set->dma.input_plane,
	...
	cmd_output = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lmec", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "lmec", hw_ip,
			frame, cur_idx,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "lmem", hw_ip,
			frame, cur_idx,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
