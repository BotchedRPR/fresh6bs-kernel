	...
	struct is_frame **prev_frame, struct is_frame **cur_frame, const char *data_type_name)
{
	...
	*prev_frame = &internal_framemgr->frames[1];
	...
	msdbg_hw(3, "[F:%d] mcfp prev %s rdma addr(%pad), cur %s rdma addr(%pad)\n",
		instance, hw_ip,
		frame->fcount,
...
static void __is_hw_isp_update_param(struct is_hw_ip *hw_ip, struct is_param_region *p_region,
...
static int isp_hw_get_buf_info(enum is_hw_isp_buf_id buf_id,
				struct pablo_internal_subdev *sd,
				struct isp_param_set *param_set)
{
	u32 width = param_set->otf_input.width;
	u32 height = param_set->otf_input.height;
	u32 payload_size, header_size;

	switch (buf_id) {
	case ISP_INTERNAL_BUF_IMG:
		if (!CHK_VIDEOTNR_MODE(param_set->tnr_mode))
			return -EINVAL;
		...
		sd->num_batch = 1;
		sd->num_buffers = 2;
		sd->bits_per_pixel = 13;
		sd->memory_bitwidth = 13;
		payload_size = is_hw_dma_get_payload_stride(
			COMP, sd->bits_per_pixel, width,
			0, 0, MCFP_COMP_BLOCK_WIDTH, MCFP_COMP_BLOCK_HEIGHT) *
			DIV_ROUND_UP(height, MCFP_COMP_BLOCK_HEIGHT);
		header_size = is_hw_dma_get_header_stride(width, MCFP_COMP_BLOCK_WIDTH, 32) *
			DIV_ROUND_UP(height, MCFP_COMP_BLOCK_HEIGHT);
		sd->size[0] = payload_size + header_size;
		break;
	case ISP_INTERNAL_BUF_WGT:
		if (!(CHK_MFSTILL_MODE(param_set->tnr_mode)
		    || CHK_VIDEOTNR_MODE(param_set->tnr_mode)))
			return -EINVAL;

		sd->width = width / 2;
		sd->height = height / 2 + 4;
		sd->num_planes = 1;
		sd->num_batch = 1;
		sd->num_buffers = 2;
		sd->bits_per_pixel = 8;
...
{
	int ret;
	enum is_hw_isp_buf_id buf_id;
	struct pablo_internal_subdev *sd;
	struct is_hw_isp *hw = (struct is_hw_isp *)hw_ip->priv_info;

	for (buf_id = 0; buf_id < ISP_INTERNAL_BUF_MAX; buf_id++) {
		sd = &hw->subdev[instance][buf_id];

		if (test_bit(PABLO_SUBDEV_ALLOC, &sd->state))
			continue;

		if (isp_hw_get_buf_info(buf_id, sd, param_set))
			continue;
...
static void __is_hw_isp_free_buffer(u32 instance, struct is_hw_ip *hw_ip)
{
	enum is_hw_isp_buf_id buf_id;
	struct is_hw_isp *hw_isp = (struct is_hw_isp *)hw_ip->priv_info;
	struct pablo_internal_subdev *sd;
	...
	for (buf_id = 0; buf_id < ISP_INTERNAL_BUF_MAX; buf_id++) {
		sd = &hw_isp->subdev[instance][buf_id];
	...
	u32 instance, struct is_frame *frame)
{
	...
	dma_addr_t ixtTargetAddress[IS_MAX_PLANES] = {0, };
	dma_addr_t ixvTargetAddress[IS_MAX_PLANES] = {0, };
	dma_addr_t ixgTargetAddress[IS_MAX_PLANES] = {0, };
	dma_addr_t ixwTargetAddress[IS_MAX_PLANES] = {0, };
	u32 cur_idx, buf_i;
	...
	if (iq_config->mixer_en) {
		...
		ret = __is_hw_isp_get_mcfp_internal_frame(hw_ip, instance, frame,
				&hw_isp->subdev[instance][ISP_INTERNAL_BUF_WGT],
		...
		if (iq_config->still_en) {
			 ...
			 */
			ret = __is_hw_isp_get_mcfp_internal_frame(hw_ip, instance, frame,
				&hw_isp->subdev[instance][ISP_INTERNAL_BUF_IMG],
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "tnr_prev_in", hw_ip,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "tnr_wgt_in", hw_ip,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "tnr_prev_out", hw_ip,
		frame, cur_idx, frame->num_buffers,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "tnr_wgt_out", hw_ip,
		frame, 0, frame->num_buffers,
...
static int is_hw_isp_enable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	mem = CALL_HW_CHAIN_INFO_OPS(hw_ip->hardware, get_iommu_mem, GROUP_ID_ISP0);
	for (buf_id = 0; buf_id < ISP_INTERNAL_BUF_MAX; buf_id++)
		pablo_internal_subdev_probe(&hw_isp->subdev[instance][buf_id],
					instance, ENTRY_INTERNAL, mem,
					isp_internal_buf_name[buf_id]);
...
static int is_hw_isp_disable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	__is_hw_isp_free_buffer(instance, hw_ip);
...
static int is_hw_isp_shot(struct is_hw_ip *hw_ip, struct is_frame *frame, ulong hw_map)
{
	...
	__is_hw_isp_update_param(hw_ip, p_region, param_set, frame->pmap, instance);

	/* DMA settings */
	cur_idx = frame->cur_buf_index;

	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "ixs", hw_ip,
		frame, cur_idx, frame->num_buffers,
...
#if IS_ENABLED(SOC_TNR_MERGER)
#if IS_ENABLED(USE_MCFP_MOTION_INTERFACE)
	cmd_mv_in = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "motion", hw_ip,
		frame, cur_idx, frame->num_buffers,
	...
	if (CHECK_NEED_KVADDR_ID(IS_VIDEO_IMM_NUM))
		CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "motion", hw_ip,
			frame, cur_idx,
			&param_set->motion_dma_input.cmd,
			param_set->motion_dma_input.plane,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "drcgrid", hw_ip,
		frame, cur_idx, frame->num_buffers,
...
shot_fail_recovery:
	CALL_HW_HELPER_OPS(hw_ip, restore, instance, &hw_isp->lib[instance]);
	...
	u32 instance, u32 fcount, u32 size, ulong addr)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, get_cap_meta, instance,
				&hw_isp->lib[instance], fcount, size, addr);
}
...
int is_hw_isp_restore(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, restore, instance, &hw_isp->lib[instance]);
}
