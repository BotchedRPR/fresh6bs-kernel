...
int is_hw_vra_frame_end_final_out_ready_done(struct is_lib_vra *lib_vra)
{
	...
	ret = CALL_HW_OPS(hardware, frame_done, hw_ip, NULL, -1,
...
static int is_hw_vra_set_param(struct is_hw_ip *hw_ip,
	struct is_region *region, IS_DECLARE_PMAP(pmap), u32 instance,
	ulong hw_map)
{
	int ret;
	struct is_hw_vra *hw_vra;
	...
	ret = is_hw_vra_update_param(hw_ip, param, pmap, instance, fcount);
	if (ret) {
	...
	struct is_frame *frame, u32 instance, enum ShotErrorType done_type)
{
	...
	if (test_bit_variables(hw_ip->id, &frame->core_flag))
		ret = CALL_HW_OPS(hardware, frame_done, hw_ip, frame, wq_id, output_id,
				done_type, false);
