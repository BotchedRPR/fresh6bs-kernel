...
static int is_hw_yuvp_handle_interrupt0(u32 id, void *context)
{
	...
	status = yuvp_hw_g_int_state0(hw_ip->pmio, true,
			(hw_ip->num_buffers & 0xffff), &hw_yuvp->irq_state[YUVP0_INTR])
			& yuvp_hw_g_int_mask0(hw_ip->pmio);
	...
	if (yuvp_hw_is_occurred0(status, INTR_FRAME_END)) {
		yuvp_hw_s_clock(hw_ip->pmio, false);
		...
		if (unlikely(is_get_debug_param(IS_DEBUG_PARAM_YUVP)))
			yuvp_hw_print_chain_debug_cnt(hw_ip->pmio);
	}
...
static int is_hw_yuvp_handle_interrupt1(u32 id, void *context)
{
	...
	status = yuvp_hw_g_int_state1(hw_ip->pmio, true,
			(hw_ip->num_buffers & 0xffff), &hw_yuvp->irq_state[YUVP1_INTR])
			& yuvp_hw_g_int_mask1(hw_ip->pmio);
...
static void is_hw_yuvp_prepare(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	yuvp_hw_s_init(hw_ip->pmio);

	seed = is_get_debug_param(IS_DEBUG_PARAM_CRC_SEED);
	if (unlikely(seed))
		yuvp_hw_s_crc(hw_ip->pmio, seed);
}
...
static int is_hw_yuvp_finish(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = yuvp_hw_wait_idle(hw_ip->pmio);
	if (ret)
			       ...
			       u32 num_buffers, struct c_loader_buffer *clb)
{
	...
	ret = yuvp_hw_s_corex_update_type(hw_ip->pmio, set_id);
	if (ret)
		return ret;

	if (clb)
		yuvp_hw_s_cmdq(hw_ip->pmio, set_id, num_buffers, clb->header_dva, clb->num_of_headers);
	else
		yuvp_hw_s_cmdq(hw_ip->pmio, set_id, num_buffers, 0, 0);
...
static int __nocfi is_hw_yuvp_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	hw_yuvp->pb_c_loader_payload = CALL_PTR_MEMOP(hw_ip->mem,
						alloc,
						hw_ip->mem->priv,
						0x16000,
						NULL,
	...
	hw_yuvp->pb_c_loader_header = CALL_PTR_MEMOP(hw_ip->mem, alloc,
					hw_ip->mem->priv, 0x4000,
					NULL, 0);
			...
			bool flag, u32 f_type)
{
	...
	for (dma_id = 0; dma_id < rmda_max_cnt; dma_id++) {
		ret = yuvp_hw_rdma_create(&hw_yuvp->rdma[dma_id], hw_ip->pmio, dma_id);
		if (ret) {
	...
	for (dma_id = 0; dma_id < wdma_max_cnt; dma_id++) {
		ret = yuvp_hw_wdma_create(&hw_yuvp->wdma[dma_id], hw_ip->pmio, dma_id);
		if (ret) {
...
static int __is_hw_yuvp_bypass(struct is_hw_ip *hw_ip, u32 set_id)
{
	...
	yuvp_hw_s_block_bypass(hw_ip->pmio, set_id);
	...
	struct yuvp_param_set *param_set, u32 enable, u32 start_pos, u32 set_id)
{
	...
	yuvp_hw_s_strip(hw_ip->pmio, set_id, enable,
	...
	enum yuvp_input_path_type input_path, enum yuvp_chain_demux_type output_path)
{
	...
	ret = yuvp_hw_s_yuvnr_size(hw_ip->pmio, set_id, strip_start_pos, frame_width,
	...
	ret = yuvp_hw_s_sharpen_size(hw_ip->pmio, set_id, strip_start_pos, frame_width,
	...
	yuvp_hw_s_size(hw_ip->pmio, set_id, width, height, strip_enable);
err:
	return ret;
}
	...
	u32 set_id, enum yuvp_input_path_type *input_path, enum yuvp_chain_demux_type *output_path)
{
	...
	if (param_set->dma_input.cmd == DMA_INPUT_COMMAND_ENABLE) {
		*input_path = YUVP_INPUT_RDMA;
		yuvp_hw_s_mux_dtp(hw_ip->pmio, set_id, YUVP_MUX_DTP_RDMA_YUV);
	} else {
		*input_path = YUVP_INPUT_OTF;
		yuvp_hw_s_cinfifo(hw_ip->pmio, set_id);
		yuvp_hw_s_mux_dtp(hw_ip->pmio, set_id, YUVP_MUX_DTP_CINFIFO);
	}
	yuvp_hw_s_input_path(hw_ip->pmio, set_id, *input_path);
	...
	*output_path = 0;
	...
	if (param_set->otf_output.cmd == DMA_INPUT_COMMAND_ENABLE) {
		*output_path |= YUVP_DEMUX_DITHER_COUTFIFO_0;
		yuvp_hw_s_coutfifo(hw_ip->pmio, set_id);
		yuvp_hw_s_output_path(hw_ip->pmio, set_id, 1);
	} else {
		yuvp_hw_s_output_path(hw_ip->pmio, set_id, 0);
	}
	...
	*output_path |= YUVP_DEMUX_YUVNR_YUV422TO444;
	...
	yuvp_hw_s_demux_enable(hw_ip->pmio, set_id, *output_path);
}
	...
	struct is_frame *frame, u32 fcount, u32 instance)
{
	...
	cmd_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "yuvp", hw_ip,
	...
	cmd_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "yuvpout", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_input_drc = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "ypdga", hw_ip,
			frame, cur_idx, frame->num_buffers,
	 ...
	 */
	cmd_input_clahe = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "ypclahe", hw_ip,
	...
	cmd_input_scmap = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "scmap", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_output_svhist = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "ypsvhist", hw_ip,
			frame, 0, frame->num_buffers,
	...
	ulong hw_map)
{
	...
	yuvp_hw_s_clock(hw_ip->pmio, true);

	yuvp_hw_s_core(hw_ip->pmio, frame->num_buffers, set_id);
	...
	if (param_set->stripe_input.repeat_scenario == PABLO_REPEAT_YUVP_CLAHE) {
		...
		yuvp_hw_s_svhist_bypass(hw_ip->pmio, set_id, 1);
		hw_yuvp->config.clahe_bypass = 1;
		yuvp_hw_s_clahe_bypass(hw_ip->pmio, set_id, 1);
	}
	...
	if (param_region->yuvp.control.strgen == CONTROL_COMMAND_START) {
		...
		yuvp_hw_s_strgen(hw_ip->pmio, set_id);
	}
