	...
	ulong hw_map)
{
	...
	if ((!frame_done)
		|| (!test_bit(ENTRY_3AC, &frame->out_flag) && !test_bit(ENTRY_3AP, &frame->out_flag)
			...
			&& !test_bit(ENTRY_3AO, &frame->out_flag) && !test_bit(ENTRY_3AL, &frame->out_flag)
			&& !test_bit(ENTRY_MEXC, &frame->out_flag) && !test_bit(ENTRY_ORBXC, &frame->out_flag)))
	...
	/* DMA settings */
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "3xs",
	...
	cmd_before_bds = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "3xc",
		hw_ip, frame, cur_idx, frame->num_buffers,
		&param_set->dma_output_before_bds.cmd,
	...
	cmd_after_bds = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "3xp",
		hw_ip, frame, cur_idx, frame->num_buffers,
		&param_set->dma_output_after_bds.cmd,
	...
	cmd_efd = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "efd",
		hw_ip, frame, cur_idx, frame->num_buffers,
	...
	cmd_mrg = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "mrg",
		hw_ip, frame, cur_idx, frame->num_buffers,
...
#if IS_ENABLED(LOGICAL_VIDEO_NODE)
	cmd_drc_grid = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "txdrg",
		hw_ip, frame, cur_idx, frame->num_buffers,
...
#endif
#if defined(ENABLE_ORBDS)
	cmd_orb_ds = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "ods",
		hw_ip, frame, cur_idx, frame->num_buffers,
...
#endif
#if defined(ENABLE_LMEDS)
	cmd_lme_ds = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lds",
		hw_ip, frame, cur_idx, frame->num_buffers,
...
#endif
#if IS_ENABLED(USE_HF_INTERFACE)
	cmd_hf = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "thf",
		hw_ip, frame, cur_idx, frame->num_buffers,
