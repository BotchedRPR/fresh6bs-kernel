...
static int is_hw_byrp_handle_interrupt0(u32 id, void *context)
{
	...
	status = byrp_hw_g_int0_state(hw_ip->pmio, true,
			(hw_ip->num_buffers & 0xffff), &hw_byrp->irq_state[BYRP_INT0])
			& byrp_hw_g_int0_mask(hw_ip->pmio);
	...
	if (byrp_hw_is_occurred(status, INTR_FRAME_START)) {
		...
		/* byrp_hw_dump(hw_ip->mmio_base); */
	}

	if (byrp_hw_is_occurred(status, INTR_FRAME_END)) {
		msdbg_hw(2, "[F:%d]F.E\n", instance, hw_ip, hw_fcount);

		byrp_hw_s_clock(hw_ip->pmio, false);
...
static int is_hw_byrp_handle_interrupt1(u32 id, void *context)
{
	...
	status = byrp_hw_g_int1_state(hw_ip->pmio, true,
			(hw_ip->num_buffers & 0xffff), &hw_byrp->irq_state[BYRP_INT1])
			& byrp_hw_g_int1_mask(hw_ip->pmio);
...
static int is_hw_byrp_finish(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = byrp_hw_wait_idle(hw_ip->pmio);
	if (ret)
	...
	enum cmdq_setting_mode setting_mode, struct c_loader_buffer *clb)
{
	...
	case CTRL_MODE_DMA_DIRECT:
		byrp_hw_s_cmdq(hw_ip->pmio, set_id, num_buffers, clb->header_dva, clb->num_of_headers);
		break;
	case CTRL_MODE_APB_DIRECT:
		byrp_hw_s_cmdq(hw_ip->pmio, set_id, num_buffers, 0, 0);
		break;
	default:
...
static int __nocfi is_hw_byrp_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	hw_byrp->pb_c_loader_payload = CALL_PTR_MEMOP(hw_ip->mem,
						alloc,
						hw_ip->mem->priv,
						0x8000,
						NULL,
	...
	hw_byrp->pb_c_loader_header = CALL_PTR_MEMOP(hw_ip->mem, alloc,
			...
			bool flag, u32 f_type)
{
	...
	for (i = 0; i < hw_byrp->rdma_max_cnt; i++) {
		ret = byrp_hw_rdma_create(&hw_byrp->rdma[i], hw_ip->pmio, i);
		if (ret) {
	...
	for (i = 0; i < hw_byrp->wdma_max_cnt; i++) {
		ret = byrp_hw_wdma_create(&hw_byrp->wdma[i], hw_ip->pmio, i);
		if (ret) {
	...
	u32 instance, u32 id, struct is_crop *dma_crop_cfg, u32 set_id)
{
	...
	byrp_hw_g_input_param(param_set, instance, id, &dma_input, &input_dva);
	if (!input_dva) {
...
static int __is_hw_byrp_bypass(struct is_hw_ip *hw_ip, u32 set_id)
{
	...
	byrp_hw_s_block_bypass(hw_ip->pmio, set_id);
	...
	u32 instance, const struct is_frame *frame, struct is_crop *bcrop_cfg, u32 set_id)
{
	...
	if (use_bcrop_rgb) {
		pipe_w = rdma_width;
		pipe_h = rdma_height;
		byrp_hw_s_mpc_size(hw_ip->pmio, set_id, rdma_width, rdma_height);
		byrp_hw_s_bcrop_size(hw_ip->pmio, set_id, BYRP_BCROP_BYR,
					0, 0, rdma_width, rdma_height);
		byrp_hw_s_bcrop_size(hw_ip->pmio, set_id, BYRP_BCROP_RGB,
		...
		if (strip_enable) {
			...
			byrp_hw_s_bcrop_size(hw_ip->pmio, set_id, BYRP_BCROP_ZSL,
		...
		pipe_w = bcrop_w;
		pipe_h = bcrop_h;
		byrp_hw_s_mpc_size(hw_ip->pmio, set_id, bcrop_w + bcrop_x * 2, pipe_h + bcrop_h * 2);
		byrp_hw_s_bcrop_size(hw_ip->pmio, set_id, BYRP_BCROP_BYR,
					bcrop_x, bcrop_y, bcrop_w, bcrop_h);
		byrp_hw_s_bcrop_size(hw_ip->pmio, set_id, BYRP_BCROP_RGB,
					0, 0, bcrop_w, bcrop_h);
		byrp_hw_s_bcrop_size(hw_ip->pmio, set_id, BYRP_BCROP_ZSL, 0, 0, pipe_w, pipe_h);
	}

	byrp_hw_s_chain_size(hw_ip->pmio, set_id, pipe_w, pipe_h);
	byrp_hw_s_disparity_size(hw_ip->pmio, set_id, &size_config);
	...
	ulong hw_map)
{
	...
	cmd_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "byrp_img", hw_ip,
	...
	cmd_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "byrp_hdr", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_output = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "byrp_wdma_befor_wbg", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	cmd_output = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "byrp_wdma_after_wbg", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	byrp_hw_s_clock(hw_ip->pmio, true);

	byrp_hw_s_core(hw_ip->pmio, frame->num_buffers, set_id, param_set);
	...
	if (param_region->byrp.control.strgen == CONTROL_COMMAND_START) {
		...
		byrp_hw_s_strgen(hw_ip->pmio, set_id);
	}
