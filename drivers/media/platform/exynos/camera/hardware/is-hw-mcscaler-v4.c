...
static int is_hw_mcsc_handle_interrupt(u32 id, void *context)
{
	...
	is_scaler_get_input_status(hw_ip->pmio, hw_ip->id, &hl, &vl);
	/* read interrupt status register (sc_intr_status) */
	mask = is_scaler_get_intr_mask(hw_ip->pmio, hw_ip->id);
	status = is_scaler_get_intr_status(hw_ip->pmio, hw_ip->id);
	status = (~mask) & status;

	is_scaler_clear_intr_src(hw_ip->pmio, hw_ip->id, status);
	...
	if (is_scaler_intr_occurred0(status, INTR_FRAME_END)) {
		is_scaler_set_clock(hw_ip->pmio, false);
...
static int is_hw_mcsc_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	hw_mcsc->pb_c_loader_payload = CALL_PTR_MEMOP(hw_ip->mem,
						alloc,
						hw_ip->mem->priv,
						0x8000,
						NULL,
	...
	hw_mcsc->pb_c_loader_header = CALL_PTR_MEMOP(hw_ip->mem, alloc,
	...
	if (unlikely(test_bit(MCSC_DBG_DTP_OTF, &debug_mcsc)))
		mcsc_hw_s_dtp(hw_ip->pmio, MCSC_DTP_OTF, 1, MCSC_DTP_COLOR_BAR,
			0, 0, 0, MCSC_DTP_COLOR_BAR_BT601);
	else
		mcsc_hw_s_dtp(hw_ip->pmio, MCSC_DTP_OTF, 0, MCSC_DTP_BYPASS,
			0, 0, 0, MCSC_DTP_COLOR_BAR_BT601);

	if (unlikely(test_bit(MCSC_DBG_DTP_RDMA, &debug_mcsc)))
		mcsc_hw_s_dtp(hw_ip->pmio, MCSC_DTP_RDMA, 1, MCSC_DTP_COLOR_BAR,
			0, 0, 0, MCSC_DTP_COLOR_BAR_BT601);
	else
		mcsc_hw_s_dtp(hw_ip->pmio, MCSC_DTP_RDMA, 0, MCSC_DTP_BYPASS,
			0, 0, 0, MCSC_DTP_COLOR_BAR_BT601);
			...
			bool flag, u32 f_type)
{
	...
	for (dma_id = 0; dma_id < hw_mcsc->rdma_max_cnt; dma_id++) {
		ret = mcsc_hw_rdma_create(&hw_mcsc->rdma[dma_id], hw_ip->pmio, dma_id);
		if (ret) {
	...
	struct param_mcs_input *input)
{
	...
	cmd_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "mcsc_in", hw_ip, frame,
	...
	struct mcs_param *param, struct is_frame *frame, u32 instance)
{
	...
	if (hf_param->dma_cmd == DMA_OUTPUT_COMMAND_DISABLE || !rdma_addr[0]) {
		...
		is_scaler_set_djag_hf_enable(hw_ip->pmio, false, false);
	...
	ret = is_scaler_set_hf_config(dma, hw_ip->pmio,
	...
	is_scaler_get_djag_strip_config(hw_ip->pmio, 0, &djag_stripe_pre_dst_x, &djag_stripe_start_pos_x);
	...
	ulong hw_map)
{
	...
	/* enable clock */
	is_scaler_set_clock(hw_ip->pmio, true);
...
config:
	...
	CALL_HW_OPS(hw_ip->hardware, dbg_trace, hw_ip, frame->fcount, DEBUG_POINT_ADD_TO_CMDQ);
	is_scaler_start(hw_ip->pmio, hw_ip->id, frame->num_buffers, clb.header_dva, clb.num_of_headers);
...
int is_hw_mcsc_clear_interrupt(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	is_scaler_clear_intr_all(hw_ip->pmio, hw_ip->id);
	is_scaler_disable_intr(hw_ip->pmio, hw_ip->id);
	is_scaler_mask_intr(hw_ip->pmio, hw_ip->id,
...
{
	return CALL_HW_OPS(hw_ip->hardware, frame_done, hw_ip, frame, -1,
	...
	u32 instance)
{
	...
	is_scaler_set_input_source(hw_ip->pmio, hw_ip->id, input->dma_cmd + 1, false);
	...
	struct param_mcs_output *output, struct param_stripe_input *stripe_input, u32 output_id, u32 instance)
{
	...
	is_scaler_set_poly_src_size(hw_ip->pmio, output_id, src_pos_x, src_pos_y,
	...
	if (use_out_crop) {
		...
		is_scaler_set_poly_strip_config(hw_ip->pmio, output_id, temp_stripe_pre_dst_x, temp_stripe_start_pos_x);
		...
		if (!post_en) {
			...
			is_scaler_set_poly_out_crop_size(hw_ip->pmio, output_id, otcrop_pos_x, 0, otcrop_width, poly_dst_height);
	...
	if (!IS_ENABLED(SKIP_SETFILE_LOAD) && !test_bit(MCSC_DBG_SKIP_SETFILE, &debug_mcsc))
		is_scaler_set_poly_scaler_coef(hw_ip->pmio, output_id, hratio, vratio,
			sc_coefs, 2, sensor_position);

	is_scaler_set_poly_round_mode(hw_ip->pmio, output_id, round_mode_en);
	is_scaler_set_poly_dst_size(hw_ip->pmio, output_id,
		poly_dst_width, poly_dst_height);
	...
	struct param_mcs_output *output, struct param_stripe_input *stripe_input, u32 output_id, u32 instance)
{
	...
	if (!poly_otcrop_en && use_out_crop) {
		...
		if (is_scaler_get_poly_strip_enable(hw_ip->pmio, output_id))
			is_scaler_get_poly_strip_config(hw_ip->pmio,
					output_id,
		...
		is_scaler_set_post_strip_config(hw_ip->pmio, output_id, temp_stripe_pre_dst_x, temp_stripe_start_pos_x);
		...
		is_scaler_set_post_out_crop_size(hw_ip->pmio, output_id, otcrop_pos_x, 0, otcrop_width, dst_height);
	...
	if (!IS_ENABLED(SKIP_SETFILE_LOAD) && !test_bit(MCSC_DBG_SKIP_SETFILE, &debug_mcsc))
		is_scaler_set_post_scaler_coef(hw_ip->pmio, output_id,
	...
	u32 output_id, u32 instance)
{
	...
	if (poly_otcrop_en){
		is_scaler_get_poly_out_crop_size(hw_ip->pmio, output_id, &scaled_width, &scaled_height);
		output->width = scaled_width;
		output->height = scaled_height;
	} else if (post_otcrop_en){
		is_scaler_get_post_out_crop_size(hw_ip->pmio, output_id, &scaled_width, &scaled_height);
		output->width = scaled_width;
		output->height = scaled_height;
	} else
#endif
	...
	is_scaler_set_wdma_format(&hw_mcsc->wdma[output_id], hw_ip->pmio, hw_ip->id, output_id, plane, img_format);
	...
	is_scaler_set_wdma_sbwc_config(&hw_mcsc->wdma[output_id], hw_ip->pmio,
		output, output_id,
	...
	u32 output_id, u32 instance)
{
	...
	if (hw_mcsc->tune_set[instance]) {
		...
		is_scaler_set_b_c(hw_ip->pmio, output_id,
			setfile->sc_base[yuv].y_offset, setfile->sc_base[yuv].y_gain);
		is_scaler_set_h_s(hw_ip->pmio, output_id,
		...
		is_hw_bchs_range(hw_ip->pmio, output_id, yuv);
		msdbg_hw(2, "YUV range set default settings\n", instance, hw_ip);
	}
	is_hw_bchs_clamp(hw_ip->pmio, output_id, yuv, sc_bchs);
#else
		is_hw_bchs_range(hw_ip->pmio, output_id, yuv);
		is_hw_bchs_clamp(hw_ip->pmio, output_id, yuv, sc_bchs);
#endif
