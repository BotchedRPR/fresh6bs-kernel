...
struct is_hw_ip *get_mcsc_hw_ip(struct is_hw_ip *hw_ip)
{
	...
	return CALL_HW_CHAIN_INFO_OPS(&hw_ip->hardware, get_hw_ip, hw_id);
}
	...
	int done_type)
{
	...
	for (out_id = MCSC_OUTPUT0; out_id < cap->max_output; out_id++) {
		...
		if (test_bit(out_f_id, &hw_frame->out_flag)) {
			ret = CALL_HW_OPS(hardware, frame_done, hw_ip, frame,
	...
	u32 instance, enum ShotErrorType done_type)
{
	...
	if (test_bit_variables(hw_ip->id, &frame->core_flag))
		ret = CALL_HW_OPS(hardware, frame_done, hw_ip, frame, -1, IS_HW_CORE_END,
				done_type, false);
	...
	u32 instance)
{
	...
	core_num = GET_CORE_NUM(DEV_HW_MCSC1);
	hw_ip = CALL_HW_CHAIN_INFO_OPS(hardware, get_hw_ip, DEV_HW_MCSC1);
	if (hw_ip && frame->shot) {
		hardware->ni_udm[core_num][index] = frame->shot->udm.ni;
		msdbg_hw(2, "set_ni: [F:%d], %d,%d,%d -> %d,%d,%d\n",
				instance, hw_ip, frame->fcount,
