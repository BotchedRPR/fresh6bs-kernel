...
#include "sfr/is-sfr-cstat-v1_20.h"
...
const static struct cstat_hw_dma cstat_dmas[CSTAT_DMA_NUM] = {
	...
	{CSTAT_DMA_NONE,	CSTAT_REG_CNT,			"DMA_NONE"}, /* Not existing DMA */
...
void cstat_hw_s_corex_type(void __iomem *base, u32 type)
{
	...
	if (type == COREX_IGNORE) {
		cstat_hw_wait_corex_idle(base);
		mask = BIT_MASK(INTR1_CSTAT_COREX_END_INT_0);
		CSTAT_SET_R_DIRECT(base, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT1_CLEAR, mask);
		CSTAT_SET_R_DIRECT(base, CSTAT_R_FRO_INT0_CLEAR, mask);
	}
}
...
void cstat_hw_s_corex_delay(void __iomem *base, struct cstat_time_cfg *time_cfg)
{
	...
	dbg_hw(3, "[CSTAT]%s: vvalid %d req_vvalid %d clk %ld fro_en %d\n", __func__,
			time_cfg->vvalid,
			time_cfg->req_vvalid,
			time_cfg->clk,
			time_cfg->fro_en);
...
static void cstat_hw_s_ds_lme0(void __iomem *base, bool en, struct cstat_ds_cfg *cfg)
{
	u32 val;
...
static void cstat_hw_s_ds_lme1(void __iomem *base, bool en, struct cstat_ds_cfg *cfg)
{
	u32 val;
...
void cstat_hw_s_fro(void __iomem *base, u32 num_buffer, unsigned long grp_mask)
{
	...
	if (prev_fro_en != fro_en) {
		...
		if (fro_en)
			info_hw("[CSTAT] FRO ON. num_buffer %d grp_mask 0x%X\n",
					num_buffer, grp_mask);
		/* fro core reset */
...
u32 cstat_hw_is_occurred(ulong state, enum cstat_event_type type)
{
	...
	switch (type) {
	case CSTAT_FS:
		mask = BIT_MASK(INTR1_CSTAT_FRAME_START);
		break;
	case CSTAT_LINE:
		mask = BIT_MASK(INTR1_CSTAT_FRAME_INT_ON_ROW_COL_INFO);
		break;
	case CSTAT_FE:
		mask = BIT_MASK(INTR1_CSTAT_FRAME_END);
		break;
	case CSTAT_RGBY:
		mask = BIT_MASK(INTR1_CSTAT_RGBYHIST_OUT_LAST_PIXEL_ON_OUTPUT);
		break;
	case CSTAT_COREX_END:
		mask = BIT_MASK(INTR1_CSTAT_COREX_END_INT_0);
		break;
	case CSTAT_ERR:
		mask = (BIT_MASK(INTR1_CSTAT_IRQ_CORRUPTED_INTERRUPT) |
			BIT_MASK(INTR1_CSTAT_COREX_ERROR_INT) |
			BIT_MASK(INTR1_CSTAT_SDC_ERROR) |
			BIT_MASK(INTR1_CSTAT_LIC_CONTROLLER_OVERFLOW_ERROR) |
			BIT_MASK(INTR1_CSTAT_LIC_CONTROLLER_ROW_ERROR) |
			BIT_MASK(INTR1_CSTAT_C_LOADER_AND_COREX_OVERLAP) |
			BIT_MASK(INTR1_CSTAT_CINFIFO_INPUT_PROTOCOL_ERROR) |
			BIT_MASK(INTR1_CSTAT_CINFIFO_PIXEL_CNT_ERROR) |
			BIT_MASK(INTR1_CSTAT_CINFIFO_OVERFLOW_ERROR) |
			BIT_MASK(INTR1_CSTAT_CINFIFO_OVERLAP_ERROR));
		break;
	case CSTAT_CDAF:
		mask = BIT_MASK(INTR2_CSTAT_CDAF_DONE);
		break;
	case CSTAT_THSTAT_PRE:
		mask = BIT_MASK(INTR2_CSTAT_THSTAT_PRE_DONE);
		break;
	case CSTAT_THSTAT_AWB:
		mask = BIT_MASK(INTR2_CSTAT_THSTAT_AWB_DONE);
		break;
	case CSTAT_THSTAT_AE:
		mask = BIT_MASK(INTR2_CSTAT_THSTAT_AE_DONE);
		break;
	default:
		mask = 0;
		break;
...
void cstat_hw_print_err(char *name, u32 instance, u32 fcount, ulong src1, ulong src2)
{
	...
	err = find_first_bit(&src1, INTR1_CSTAT_MAX);
	while (err < INTR1_CSTAT_MAX) {
		switch (err) {
		case INTR1_CSTAT_IRQ_CORRUPTED_INTERRUPT:
		case INTR1_CSTAT_COREX_ERROR_INT:
		case INTR1_CSTAT_SDC_ERROR:
		case INTR1_CSTAT_LIC_CONTROLLER_OVERFLOW_ERROR:
		case INTR1_CSTAT_LIC_CONTROLLER_ROW_ERROR:
		case INTR1_CSTAT_C_LOADER_AND_COREX_OVERLAP:
		case INTR1_CSTAT_CINFIFO_INPUT_PROTOCOL_ERROR:
		case INTR1_CSTAT_CINFIFO_PIXEL_CNT_ERROR:
		case INTR1_CSTAT_CINFIFO_OVERFLOW_ERROR:
		case INTR1_CSTAT_CINFIFO_OVERLAP_ERROR:
		...
		err = find_next_bit(&src1, INTR1_CSTAT_MAX, err + 1);
	}
}
...
static inline enum format_type cstat_hw_get_dma_type(int id, u32 hw_format, u32 hw_order, u32 pixel_size)
{
	switch (id) {
	case CSTAT_W_FDPIG:
		if (hw_format == DMA_INOUT_FORMAT_RGB && pixel_size == 24)
			return DMA_FMT_YUV;
		else
			return DMA_FMT_RGB;
	default:
		return DMA_FMT_BAYER;
	}
}
		...
		struct cstat_param_set *param, u32 num_buffers, int disable)
{
	...
	case CSTAT_W_LMEDS1:
		...
		cfg.addrs = param->output_dva_lem_ds1;
		img_flag = true;
		break;
	case CSTAT_W_CDS0:
	...
	dma_type = cstat_hw_get_dma_type(dma->id, hw_format, hw_order, pixel_size);
	...
	if (ret || (stride_1p & 0x1)) {
		err_hw("[CSTAT][%s] invalid wdma_cfg: format %d stride_1p %d",
				dma->name, format, stride_1p);
		goto skip_dma;
	}
	...
	/* ret |= CALL_DMA_OPS(dma, dma_set_bus_info, bus_info); */
	if (ret) {
		err_hw("[CSTAT][%s] Fail to set dma. format(%d) dma_type(%d), mono_mode(%d), ret(%d)",
					dma->name, format, dma_type, mono_mode, ret);
		goto skip_dma;
	}
		...
		struct cstat_size_cfg *size_cfg)
{
	...
	case CSTAT_CROP_MENR:
		cstat_hw_s_crop_menr(base, crop, p_set, size_cfg);
		break;
	default:
		break;
	};
...
static void cstat_hw_g_edge_score(void __iomem *base, void *data)
{
	u32 *score = (u32 *)data;
...
int cstat_hw_g_stat_data(void __iomem *base, u32 stat_id, void *data)
{
	...
	case CSTAT_STAT_EDGE_SCORE:
		func_g_stat = cstat_hw_g_edge_score;
		break;
	default:
...
void cstat_hw_s_dma_cfg(struct cstat_param_set *p_set, struct is_cstat_config *conf)
{
	...
	p_set->cdaf.cmd = !(conf->cdaf_bypass || conf->cdaf_mw_bypass);
	if (p_set->cdaf.cmd) {
		p_set->cdaf.width = conf->cdaf_mw_x * 48; /* 48 bytes/grid */
		p_set->cdaf.height = conf->cdaf_mw_y;

		if (!p_set->cdaf.width || !p_set->cdaf.height) {
			p_set->cdaf.cmd = DMA_OUTPUT_COMMAND_DISABLE;
			warn_hw("[%d][CSTAT][F%d] Invalid CDAF Multi Window size %dx%d",
				p_set->instance_id, p_set->fcount,
				p_set->cdaf.width, p_set->cdaf.height);
		}

		dbg_hw(3, "[%d][CSTAT]set_config-%s:[F%d] cdaf %dx%d\n",
				p_set->instance_id,
				__func__,
				p_set->fcount,
				conf->cdaf_mw_x,
