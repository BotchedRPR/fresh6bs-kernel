...
/* VOTF */
static void is_scaler_set_rdma_votf_enable(void __iomem *base_addr, u32 votf_en, u32 cache_hint)
{
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_STAT_RDMAHF_VOTF_EN],
		&mcsc_fields[MCSC_F_STAT_RDMAHF_VOTF_EN], votf_en);
	is_hw_set_field(base_addr, &mcsc_regs[MCSC_R_STAT_RDMAHF_BUSINFO],
		&mcsc_fields[MCSC_F_STAT_RDMAHF_BUSINFO], (cache_hint << 4));
}
	...
	u32 output_id, u32 fmt, u32 plane, u32 dma_idx, u32 width, u32 height)
{
	...
	is_hw_set_reg(base_addr, &mcsc_regs[MCSC_R_YUV_HWFC_INDEX_RESET], 1);
...
{
	is_scaler_set_rdma_votf_enable(base_addr, votf_enable, RDMAHF_LLC_CACHE_HINT);
