...
#include "sfr/is-sfr-yuvp-v2_0.h"
...
unsigned int yuvp_hw_is_occurred0(unsigned int status, enum yuvp_event_type type)
{
	...
	case INTR_SETTING_DONE:
		mask = 1 << INTR1_YUVP_SETTING_DONE_INT;
		break;
	case INTR_ERR:
...
int yuvp_hw_s_reset(void *base)
{
	...
	while (YUVP_GET_R(base, YUVP_R_SW_RESET)) {
	...
	u32 in_crop_size_x, u32 cache_hint, u32 *sbwc_en, u32 *payload_size)
{
	...
	switch (dma->id) {
	case YUVP_WDMA_SVHIST:
		dma_output = &param_set->dma_output_svhist;
		width = 1024; //dma_output->width;
		height = vhist_grid_num; //dma_output->height;
		img_flag = false;
		break;
	default:
		err_hw("[YUVP] invalid dma_id[%d]", dma->id);
...
int yuvp_hw_wdma_create(struct is_common_dma *dma, void *base, u32 input_id)
{
	...
	switch (input_id) {
	case YUVP_WDMA_SVHIST:
		dma->reg_ofs = YUVP_R_STAT_WDMASVHIST_EN;
		dma->field_ofs = YUVP_F_STAT_WDMASVHIST_EN;
		available_bayer_format_map = 0X77 & IS_BAYER_FORMAT_MASK;
		snprintf(name, PATH_MAX, "YUVP_WDMA_SVHIST");
		break;
	default:
		err_hw("[YUVP] invalid input_id[%d]", input_id);
		ret = SET_ERROR;
	...
	struct yuvp_param_set *param_set)
{
	switch (id) {
	case YUVP_WDMA_SVHIST:
		*output_dva = param_set->output_dva_svhist;
...
void yuvp_hw_s_cmdq(void *base, u32 set_id, u32 num_buffers, dma_addr_t clh, u32 noh)
{
	...
	for (i = 0; i < num_buffers; ++i) {
		...
		if (clh && noh) {
			YUVP_SET_F(base, YUVP_R_CMDQ_QUE_CMD_H, YUVP_F_CMDQ_QUE_CMD_BASE_ADDR, DVA_36BIT_HIGH(clh));
...
void yuvp_hw_s_block_bypass(void *base, u32 set_id)
{
	...
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_RGB_SVHIST_BYPASS, 1);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_RGB_DGF_BYPASS, 1);
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_YUV_CNR_BYPASS, 1);
}
...
{
	YUVP_SET_R(base + GET_COREX_OFFSET(set_id), YUVP_R_RGB_SVHIST_BYPASS, enable);
}
...
void yuvp_hw_s_crc(void *base, u32 seed)
{
	...
	YUVP_SET_F(base, YUVP_R_RGB_DRCDIST_STREAM_CRC,
			YUVP_F_RGB_DRCDIST_CRC_SEED, seed);
	YUVP_SET_F(base, YUVP_R_YUV_CNR_STREAM_CRC,
			  YUVP_F_YUV_CNR_CRC_SEED, seed);
	YUVP_SET_F(base, YUVP_R_RGB_DGF_STREAM_CRC,
			  YUVP_F_RGB_DGF_CRC_SEED, seed);

}
KUNIT_EXPORT_SYMBOL(yuvp_hw_s_crc);

const struct pmio_field_desc *yuvp_hw_g_field_descs(void)
{
	return yuvp_field_descs;
}

unsigned int yuvp_hw_g_num_field_descs(void)
{
	return ARRAY_SIZE(yuvp_field_descs);
}

const struct pmio_access_table *yuvp_hw_g_access_table(int type)
{
	switch (type) {
	case 0:
		return &yuvp_volatile_ranges_table;
	case 1:
		return &yuvp_wr_noinc_ranges_table;
	default:
		return NULL;
	};
...
void yuvp_hw_init_pmio_config(struct pmio_config *cfg)
{
	...
	cfg->max_register = YUVP_R_YUV_CNR_STREAM_CRC;
	cfg->num_reg_defaults_raw = (YUVP_R_YUV_CNR_STREAM_CRC >> 2) + 1;
	cfg->phys_base = 0x17A40000;
...
{
	static int cnt = 0;
	u32 val, reg = YUVP_R_RGB_CCM_SEG0_CCM_VECTORS_0;
	int num = 200, k;
