...
#include "sfr/is-sfr-mcfp-v11_0.h"
...
#define info_mcfp(fmt, args...) \
		info_common("[MCPF]", fmt, ##args)
#define info_mcfp_ver(fmt, args...) \
		info_common("[MCPF](v11.0)", fmt, ##args)
...
int mcfp_hw_wait_idle(struct pablo_mmio *base)
{
	...
	while (!idle) {
		...
		if (try_cnt >= MCFP_TRY_COUNT) {
			...
			mcfp_hw_dump(pmio_get_base(base));
			ret = -ETIME;
			break;
		}
	...
	struct is_mcfp_config *config)
{
	...
	case MCFP_RDMA_CUR_IN_WGT:
		/* TO DO : 4bit LF weight + 4bit HF weight */
		if (config->wgt_bit == 4) {
...
int mcfp_hw_rdma_create(struct is_common_dma *dma, struct pablo_mmio *base, u32 input_id)
{
...
int mcfp_hw_wdma_create(struct is_common_dma *dma, struct pablo_mmio *base, u32 input_id)
{
	ulong available_bayer_format_map;
	int ret = 0;
	const char *name;
...
void mcfp_hw_s_block_bypass(struct pablo_mmio *base, u32 set_id)
{
	...
	MCFP_SET_F(base, MCFP_R_YUV_GAMMARGB_BYPASS, MCFP_F_YUV_GAMMARGB_BYPASS, 1);
	MCFP_SET_F(base, MCFP_R_YUV_RGBTOYUV_BYPASS, MCFP_F_YUV_RGBTOYUV_BYPASS, 1);
...
{
	if (wgt_shift_bit) {
		MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_WGTSHIFTERRDMA_BYPASS, 0);
		MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_WGTSHIFTERWDMA_BYPASS, 0);
		MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_WGTSHIFTERRDMA_WGT_LSHIFT, wgt_shift_bit);
		MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_WGTSHIFTERWDMA_WGT_RSHIFT, wgt_shift_bit);
	} else {
		MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_WGTSHIFTERRDMA_BYPASS, 1);
		MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_WGTSHIFTERWDMA_BYPASS, 1);
	}
}
