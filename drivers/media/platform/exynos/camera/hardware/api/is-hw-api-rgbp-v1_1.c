...
u32 *rgbp_hw_g_output_dva(struct rgbp_param_set *param_set, u32 instance, u32 id, u32 *cmd)
{
	u32 *output_dva = NULL;
...
int rgbp_hw_rdma_create(struct is_common_dma *dma, void __iomem *base, u32 input_id)
{
	...
	case RGBP_RDMA_CL:
		...
		break;
	case RGBP_RDMA_RGB_EVEN:
		base_reg = base + rgbp_regs[RGBP_RDMA_RGB_EVEN].sfr_offset;
		...
		break;
	case RGBP_RDMA_RGB_ODD:
		base_reg = base + rgbp_regs[RGBP_RDMA_RGB_ODD].sfr_offset;
		/* is_rgbp_rgb_format: 0,1,2,3,4,5,6,8,9,10,11,12,13 */
		available_bayer_format_map = 0x3F3F;
