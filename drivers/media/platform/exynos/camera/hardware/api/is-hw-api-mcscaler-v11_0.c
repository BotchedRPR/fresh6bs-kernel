	...
	u32 output_id, struct mcsc_v_coef *v_coef_cfg, int plane)
{
	...
	u32 reg_offset = 8, field_offset = 4, scaler_offset, scaler_field_offset;
	...
	u32 output_id, struct mcsc_v_coef *v_coef_cfg, int plane)
{
	...
	u32 reg_offset = 8, field_offset = 16, scaler_offset;
...
u32 is_scaler_get_payload_size(struct param_mcs_output *output, u8 plane)
{
	...
	if (comp_sbwc_en) {
		...
		stride = is_hw_dma_get_payload_stride(
			...
			comp_64b_align, COMP_LOSSYBYTE32NUM_32X4_U8,
		...
		dbg_hw(3, "width %d, height %d, strid %d, payload_size %d\n",
			output->full_output_width, height, stride, payload_size);
		dbg_hw(3, "comp_sbwc_en %d, comp_64b_align %d\n",
			comp_sbwc_en, comp_64b_align);
	}
	...
	case COMP_LOSS:
		...
		*strip_header_offset_in_byte = is_hw_dma_get_header_stride(
		...
		dbg_hw(2, "sbwc_type %d, pixelsize %d, start_pos_x %d, \
			comp_64b_align %d, lossy_byte32num %d, \
