...
#include "sfr/is-sfr-rgbp-v1_20.h"
...
void rgbp_hw_s_cout_fifo(void *base, u32 set_id, bool enable)
{
	...
	RGBP_SET_F(base, RGBP_R_YUV_COUTFIFO_AUTO_VBLANK, RGBP_F_YUV_COUTFIFO_AUTO_VBLANK, 0);
...
int rgbp_hw_rdma_create(struct is_common_dma *dma, void *base, u32 input_id)
{
...
static int rgbp_hw_s_rgb_wdma_format(void *base, u32 rgb_format)
{
	u32 ret;
	u32 converted_format;
...
int rgbp_hw_wdma_create(struct is_common_dma *dma, void *base, u32 input_id)
{
	ulong available_bayer_format_map;
	int ret;
	char *name;
	...
	case RGBP_WDMA_SF:
		...
		break;
	case RGBP_WDMA_RGB_EVEN:
		dma->reg_ofs = RGBP_R_RGB_WDMAREPRGBEVEN_EN;
		dma->field_ofs = RGBP_F_RGB_WDMAREPRGBEVEN_EN;
		/* is_rgbp_rgb_format: 0,1,2,3,4,5,6,8,9,10,11,12,13 */
		available_bayer_format_map = 0x3F3F;
...
void rgbp_hw_s_cmdq(void *base, u32 set_id, u32 num_buffers, dma_addr_t clh, u32 noh)
{
	...
	for (i = 0; i < num_buffers; ++i) {
		...
		if (clh && noh) {
			RGBP_SET_F(base, RGBP_R_CMDQ_QUE_CMD_H, RGBP_F_CMDQ_QUE_CMD_BASE_ADDR, clh);
...
void rgbp_hw_s_yuvsc_src_size(void *base, u32 set_id, u32 h_size, u32 v_size)
{
	/* Register removed from v1.1 */
}

void is_scaler_get_yuvsc_src_size(void *base, u32 set_id, u32 *h_size, u32 *v_size)
{
	/* Register removed from v1.1 */
}
KUNIT_EXPORT_SYMBOL(is_scaler_get_yuvsc_src_size);

void rgbp_hw_s_yuvsc_input_size(void *base, u32 set_id, u32 h_size, u32 v_size)
{
	/* Register removed from v1.1 */
}

void rgbp_hw_s_sc_input_size(void *base, u32 set_id, u32 width, u32 height)
{
	/* Register removed from v1.1 */
}
KUNIT_EXPORT_SYMBOL(rgbp_hw_s_sc_input_size);

void rgbp_hw_s_sc_src_size(void *base, u32 set_id, u32 width, u32 height)
{
	/* Register removed from v1.1 */
}
...
void rgbp_hw_s_sc_output_crop_size(void *base, u32 set_id, u32 width, u32 height)
{
	/* Register removed from v1.1 */
}
...
void rgbp_hw_s_yuvsc_out_crop_size(void *base, u32 set_id, u32 h_size, u32 v_size)
{
}
...
void yuvsc_set_poly_scaler_h_coef(void *base, u32 set_id, u32 h_sel)
{
	int index;
...
void yuvsc_set_poly_scaler_v_coef(void *base, u32 set_id, u32 v_sel)
{
	int index;
...
void rgbp_hw_s_upsc_src_size(void *base, u32 set_id, u32 h_size, u32 v_size)
{
	/* Register removed from v1.1 */
}
KUNIT_EXPORT_SYMBOL(rgbp_hw_s_upsc_src_size);

void is_scaler_get_upsc_src_size(void *base, u32 set_id, u32 *h_size, u32 *v_size)
{
	/* Register removed from v1.1 */
}
KUNIT_EXPORT_SYMBOL(is_scaler_get_upsc_src_size);

void rgbp_hw_s_upsc_input_size(void *base, u32 set_id, u32 h_size, u32 v_size)
{
	/* Register removed from v1.1 */
}
...
void rgbp_hw_s_upsc_out_crop_size(void *base, u32 set_id, u32 h_size, u32 v_size)
{
	/* Register removed from v1.1 */
}
KUNIT_EXPORT_SYMBOL(rgbp_hw_s_upsc_out_crop_size);

void rgbp_hw_s_ds_y_upsc_us(void *base, u32 set_id, u32 set)
{
	/* Register removed from v1.1 */
}
...
void rgbp_hw_s_decomp_iq(void *base, u32 set_id)
{
...
void rgbp_hw_s_grid_cfg(void *base, struct rgbp_grid_cfg *cfg)
{
	...
	val = 0;
	val = RGBP_SET_V(base, val, RGBP_F_RGB_LUMASHADING_BINNING_X, cfg->binning_x);
	val = RGBP_SET_V(base, val, RGBP_F_RGB_LUMASHADING_BINNING_Y, cfg->binning_y);
	...
	val = 0;
	val = RGBP_SET_V(base, val, RGBP_F_RGB_LUMASHADING_CROP_RADIAL_X, cfg->crop_radial_x);
	val = RGBP_SET_V(base, val, RGBP_F_RGB_LUMASHADING_CROP_RADIAL_Y, cfg->crop_radial_y);
...
const struct pmio_field_desc *rgbp_hw_g_field_descs(void)
{
	return rgbp_field_descs;
}

unsigned int rgbp_hw_g_num_field_descs(void)
{
	return ARRAY_SIZE(rgbp_field_descs);
}
...
void rgbp_hw_init_pmio_config(struct pmio_config *cfg)
{
	...
	cfg->max_register = RGBP_R_YUV_SC_STREAM_CRC;
	cfg->num_reg_defaults_raw = (RGBP_R_YUV_SC_STREAM_CRC >> 2) + 1;
	cfg->phys_base = 0x15700000;
