...
KUNIT_EXPORT_SYMBOL(pdp_hw_get_bpc_reorder_sram_size);
...
int pdp_hw_s_input_mux(struct is_pdp *pdp, bool on)
{
	int ret = 0;
	...
	if (IS_ENABLED(CONFIG_PDP_V5_0)) {
		...
		u32 mux_val = on ?
			pdp->mux_val[pdp->csi_ch] + NUM_CSI_LINK * (pdp->img_vc / NUM_DMA_VC) : 0x3F;

		ret = CALL_PSMC_OPS(psmc_ops, call,
				SMC_CMD_REG, SMC_REG_ID_SFR_W(mux_addr[pdp->id]), mux_val, 0);
		if (ret) {
			err_hw("[SMC] PDP(%d) input mux set fail, addr(0x%x) ret(%d)\n",
				pdp->id, mux_addr[pdp->id], ret);
			return -EINVAL;
		}
	} else {
