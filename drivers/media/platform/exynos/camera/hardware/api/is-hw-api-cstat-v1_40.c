...
#include "sfr/is-sfr-cstat-v1_40.h"
...
u32 cstat_hw_is_occurred(ulong state, enum cstat_event_type type)
{
	...
	case CSTAT_ERR:
		...
		break;
	case CSTAT_LIC_ERR:
		mask = (BIT_MASK(INTR1_CSTAT_LIC_CONTROLLER_OVERFLOW_ERROR) |
			BIT_MASK(INTR1_CSTAT_LIC_CONTROLLER_ROW_ERROR));
		break;
	case CSTAT_CDAF:
...
void cstat_hw_s_dma_cfg(struct cstat_param_set *p_set, struct is_cstat_config *conf)
{
	...
	if (p_set->dma_output_lme_ds0.cmd && conf->lmeds_bypass) {
		...
		warn_hw("[%d][CSTAT][F%d] bypass LMEDS0", p_set->instance_id, p_set->fcount);
