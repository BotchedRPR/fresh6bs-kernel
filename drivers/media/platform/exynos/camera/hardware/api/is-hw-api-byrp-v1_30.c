...
#include "sfr/is-sfr-byrp-v1_30.h"
#include "pmio.h"
...
static void byrp_hw_s_path(struct pablo_mmio *base, u32 set_id, bool dng_en)
{
	u32 val = 0;
	u32 output_1 = 0x1;
...
int byrp_hw_rdma_create(struct is_common_dma *dma, struct pablo_mmio *base, u32 dma_id)
{
...
int byrp_hw_wdma_create(struct is_common_dma *dma, struct pablo_mmio *base, u32 dma_id)
{
	ulong available_bayer_format_map;
	int ret;
	char *name;
...
void byrp_hw_s_block_bypass(struct pablo_mmio *base, u32 set_id)
{
	...
	BYRP_SET_F(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_DESPECKLE_BYPASS,
		BYRP_F_BYR_DESPECKLE_BYPASS, 0x1);
}
...
void byrp_hw_s_block_crc(struct pablo_mmio *base, u32 set_id, u32 seed)
{
	...
	BYRP_SET_F(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_DESPECKLE_STREAM_CRC,
		BYRP_F_BYR_DESPECKLE_CRC_SEED, seed);
...
void byrp_hw_s_bitmask(struct pablo_mmio *base, u32 set_id, bool enable, u32 bit_in, u32 bit_out)
{
...
void byrp_hw_s_pixel_order(struct pablo_mmio *base, u32 set_id, u32 pixel_order)
{
	...
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_DESPECKLE_EXPOSURE_ORDER, pixel_order);
}
...
void byrp_hw_s_disparity_size(struct pablo_mmio *base, u32 set_id, struct is_hw_size_config *size_config)
{
	...
	start_x = size_config->sensor_calibrated_width - size_config->bcrop_width;
	start_y = size_config->sensor_calibrated_height - size_config->bcrop_height;
	if (start_x < 0) {
		err_hw("[BYRP] invalid start_x(%d) sensor width(%d) bcrop width(%d)",
			start_x, size_config->sensor_calibrated_width, size_config->bcrop_width);
		start_x = 0;
	}
	if (start_y < 0) {
		err_hw("[BYRP] invalid start_y(%d) sensor height(%d) bcrop height(%d)",
			start_y, size_config->sensor_calibrated_height, size_config->bcrop_height);
		start_y = 0;
	}

	BYRP_SET_F(base, BYRP_R_BYR_DESPECKLE_RADIAL_COMPENSATION, BYRP_F_BYR_DESPECKLE_SENSOR_WIDTH,
		size_config->sensor_calibrated_width);
	BYRP_SET_F(base, BYRP_R_BYR_DESPECKLE_RADIAL_COMPENSATION_1, BYRP_F_BYR_DESPECKLE_SENSOR_HEIGHT,
		size_config->sensor_calibrated_height);
	BYRP_SET_F(base, BYRP_R_BYR_DESPECKLE_MEMORY_SPECKLES_CORR, BYRP_F_BYR_DESPECKLE_START_X, start_x);

	val = 0;
	val = BYRP_SET_V(base, val, BYRP_F_BYR_DESPECKLE_START_Y, start_y);
	val = BYRP_SET_V(base, val, BYRP_F_BYR_DESPECKLE_CROP_WIDTH, size_config->bcrop_width);
	BYRP_SET_R(base, BYRP_R_BYR_DESPECKLE_MEMORY_SPECKLES_CORR_1, val);

	val = 0;
	val = BYRP_SET_V(base, val, BYRP_F_BYR_DESPECKLE_CROP_HEIGHT, size_config->bcrop_height);
	val = BYRP_SET_V(base, val, BYRP_F_BYR_DESPECKLE_BPR_MIRROR_X_EN, 0);
	val = BYRP_SET_V(base, val, BYRP_F_BYR_DESPECKLE_BPR_MIRROR_Y_EN, 0);
	BYRP_SET_R(base, BYRP_R_BYR_DESPECKLE_MEMORY_SPECKLES_CORR_2, val);
...
void byrp_hw_s_strgen(struct pablo_mmio *base, u32 set_id)
{
	...
	/* start frame asap for byrp single test */
	BYRP_SET_F(base, GET_COREX_OFFSET(set_id) + BYRP_R_IP_USE_CINFIFO_NEW_FRAME_IN,
		BYRP_F_IP_USE_CINFIFO_NEW_FRAME_IN, 1);
...
void byrp_hw_s_core(struct pablo_mmio *base, u32 num_buffers, u32 set_id, struct byrp_param_set *param_set)
{
	bool cinfifo_en, coutfifo_en, bitmask_en, mono_mode_en, dng_en;
	u32 bit_in, bit_out;
	...
	byrp_hw_s_path(base, set_id, dng_en);
...
void byrp_hw_init_pmio_config(struct pmio_config *cfg)
{
	...
	cfg->max_register = BYRP_R_BYR_DESPECKLE_STREAM_CRC;
	cfg->num_reg_defaults_raw = (BYRP_R_BYR_DESPECKLE_STREAM_CRC >> 2) + 1;
	cfg->phys_base = 0x15500000;
