...
#include "sfr/is-sfr-mcsc-v11_20.h"
#include "is-hw.h"
...
void is_scaler_stop(struct pablo_mmio *base, u32 hw_id)
{
	MCSC_SET_F(base, MCSC_R_IP_PROCESSING, MCSC_F_IP_PROCESSING, 0);
}
...
void is_scaler_clear_shadow_ctrl(struct pablo_mmio *base, u32 hw_id)
{
	/* Not support */
}
...
int mcsc_hw_rdma_create(struct is_common_dma *dma, struct pablo_mmio *base, u32 input_id)
{
...
int mcsc_hw_wdma_create(struct is_common_dma *dma, struct pablo_mmio *base, u32 input_id)
{
	ulong available_bayer_format_map;
	ulong available_yuv_format_map;
	ulong available_rgb_format_map;
...
void is_scaler_set_djag_enable(struct pablo_mmio *base, u32 djag_enable)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_ENABLE, djag_enable);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_EZPOST_ENABLE, djag_enable);
...
void is_scaler_set_djag_input_size(struct pablo_mmio *base, u32 width, u32 height)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_INPUT_IMG_HSIZE, width);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_INPUT_IMG_VSIZE, height);
...
void is_scaler_set_djag_src_size(struct pablo_mmio *base, u32 pos_x, u32 pos_y, u32 width, u32 height)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_SRC_HPOS, pos_x);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_SRC_VPOS, pos_y);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_PS_SRC_POS, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_SRC_HSIZE, width);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_SRC_VSIZE, height);
...
void is_scaler_set_djag_hf_enable(struct pablo_mmio *base, u32 hf_enable, u32 hf_scaler_enable)
{
	...
	if (hf_enable && !djag_enable)
		...
		val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_ENABLE_A, hf_enable);
...
{
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_RECOM_WEIGHT, hf_cfg->hf_weight);
}
...
static void is_scaler_set_djag_hf_img_size(struct pablo_mmio *base, u32 width, u32 height)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_INPUT_IMG_HSIZE, width);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_INPUT_IMG_VSIZE, height);
...
static void is_scaler_set_djag_hf_radial_center(struct pablo_mmio *base, u32 pos_x, u32 pos_y)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_RADIAL_CENTER_X, pos_x);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_RADIAL_CENTER_Y, pos_y);
...
static void is_scaler_set_djag_hf_radial_binning(struct pablo_mmio *base, u32 pos_x, u32 pos_y)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_RADIAL_BINNING_X, pos_x);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_RECOM_RADIAL_BINNING_Y, pos_y);
...
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_RECOM_RADIAL_BIQUAD_FACTOR_A,
		MCSC_F_YUV_DJAG_RECOM_RADIAL_BIQUAD_FACTOR_A, factor_a);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_RECOM_RADIAL_BIQUAD_FACTOR_B,
		MCSC_F_YUV_DJAG_RECOM_RADIAL_BIQUAD_FACTOR_B, factor_b);
}

static void is_scaler_set_djag_hf_radial_gain(struct pablo_mmio *base,
	u32 enable, u32 increment)
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_RECOM_RADIAL_GAIN_ENABLE,
		MCSC_F_YUV_DJAG_RECOM_RADIAL_GAIN_ENABLE, enable);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_RECOM_RADIAL_GAIN_INCREMENT,
		MCSC_F_YUV_DJAG_RECOM_RADIAL_GAIN_INCREMENT, increment);
}

static void is_scaler_set_djag_hf_radial_biquad_scal_shift_adder(struct pablo_mmio *base,
	u32 value)
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_RECOM_RADIAL_BIQUAD_SCAL_SHIFT_ADDER,
		MCSC_F_YUV_DJAG_RECOM_RADIAL_BIQUAD_SCAL_SHIFT_ADDER, value);
}
/*
#if IS_ENABLED(USE_DJAG_COEFF_CONFIG)
static void is_scaler_set_djag_scaler_h_coef(struct __iomem *base_addr,
	...
	struct mcsc_v_coef *v_coef_cfg, enum mcsc_plane_type plane)
{
	...
	u32 reg_offset = 8, field_offset = 4;
...
void is_scaler_set_djag_dst_size(struct pablo_mmio *base, u32 width, u32 height)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_DST_HSIZE, width);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_PS_DST_VSIZE, height);
...
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_H_RATIO,
		MCSC_F_YUV_DJAG_PS_H_RATIO, hratio);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_V_RATIO,
		MCSC_F_YUV_DJAG_PS_V_RATIO, vratio);
}

void is_scaler_set_djag_init_phase_offset(struct pablo_mmio *base, u32 h_offset, u32 v_offset)
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_H_INIT_PHASE_OFFSET,
		MCSC_F_YUV_DJAG_PS_H_INIT_PHASE_OFFSET, h_offset);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_V_INIT_PHASE_OFFSET,
		MCSC_F_YUV_DJAG_PS_V_INIT_PHASE_OFFSET, v_offset);
}

void is_scaler_set_djag_round_mode(struct pablo_mmio *base, u32 round_enable)
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_ROUND_MODE,
		MCSC_F_YUV_DJAG_PS_ROUND_MODE, round_enable);
}
...
void is_scaler_set_djag_strip_config(struct pablo_mmio *base, u32 output_id, u32 pre_dst_h, u32 start_pos_h)
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_STRIP_PRE_DST_SIZE,
		MCSC_F_YUV_DJAG_PS_STRIP_PRE_DST_SIZE_H, pre_dst_h);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_PS_STRIP_IN_START_POS,
		MCSC_F_YUV_DJAG_PS_STRIP_IN_START_POS_H, start_pos_h);
}
	...
	const struct djag_scaling_ratio_depended_config *djag_tune)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_XFILTER_DEJAGGING_WEIGHT0,
		djag_tune->xfilter_dejagging_coeff_cfg.xfilter_dejagging_weight0);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_XFILTER_DEJAGGING_WEIGHT1,
		djag_tune->xfilter_dejagging_coeff_cfg.xfilter_dejagging_weight1);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_XFILTER_HF_BOOST_WEIGHT,
		djag_tune->xfilter_dejagging_coeff_cfg.xfilter_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_CENTER_HF_BOOST_WEIGHT,
		djag_tune->xfilter_dejagging_coeff_cfg.center_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DIAGONAL_HF_BOOST_WEIGHT,
		djag_tune->xfilter_dejagging_coeff_cfg.diagonal_hf_boost_weight);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_CENTER_WEIGHTED_MEAN_WEIGHT,
	...
	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_1X5_MATCHING_SAD,
		djag_tune->thres_1x5_matching_cfg.thres_1x5_matching_sad);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_1X5_ABSHF,
		djag_tune->thres_1x5_matching_cfg.thres_1x5_abshf);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_THRES_1X5_MATCHING, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_SHOOTING_LLCRR,
		djag_tune->thres_shooting_detect_cfg.thres_shooting_llcrr);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_SHOOTING_LCR,
		djag_tune->thres_shooting_detect_cfg.thres_shooting_lcr);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_SHOOTING_NEIGHBOR,
		djag_tune->thres_shooting_detect_cfg.thres_shooting_neighbor);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_THRES_SHOOTING_DETECT_0, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_SHOOTING_UUCDD,
		djag_tune->thres_shooting_detect_cfg.thres_shooting_uucdd);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_THRES_SHOOTING_UCD,
		djag_tune->thres_shooting_detect_cfg.thres_shooting_ucd);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_MIN_MAX_WEIGHT,
		djag_tune->thres_shooting_detect_cfg.min_max_weight);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_THRES_SHOOTING_DETECT_1, val);

	MCSC_SET_F(base, MCSC_R_YUV_DJAG_LFSR_SEED_0,
		MCSC_F_YUV_DJAG_LFSR_SEED_0, djag_tune->lfsr_seed_cfg.lfsr_seed_0);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_LFSR_SEED_1,
		MCSC_F_YUV_DJAG_LFSR_SEED_1, djag_tune->lfsr_seed_cfg.lfsr_seed_1);
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_LFSR_SEED_2,
		MCSC_F_YUV_DJAG_LFSR_SEED_2, djag_tune->lfsr_seed_cfg.lfsr_seed_2);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_0,
		djag_tune->dither_cfg.dither_value[0]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_1,
		djag_tune->dither_cfg.dither_value[1]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_2,
		djag_tune->dither_cfg.dither_value[2]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_3,
		djag_tune->dither_cfg.dither_value[3]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_4,
		djag_tune->dither_cfg.dither_value[4]);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_DITHER_VALUE_04, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_5,
		djag_tune->dither_cfg.dither_value[5]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_6,
		djag_tune->dither_cfg.dither_value[6]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_7,
		djag_tune->dither_cfg.dither_value[7]);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_VALUE_8,
		djag_tune->dither_cfg.dither_value[8]);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_DITHER_VALUE_58, val);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_SAT_CTRL,
		djag_tune->dither_cfg.sat_ctrl);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_THRES,
		djag_tune->dither_cfg.dither_thres);
	MCSC_SET_R(base, MCSC_R_YUV_DJAG_DITHER_THRES, val);

	MCSC_SET_F(base, MCSC_R_YUV_DJAG_CP_HF_THRES,
		MCSC_F_YUV_DJAG_CP_HF_THRES, djag_tune->cp_cfg.cp_hf_thres);

	val = 0;
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_CP_ARBI_MAX_COV_OFFSET,
		djag_tune->cp_cfg.cp_arbi_max_cov_offset);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_CP_ARBI_MAX_COV_SHIFT,
		djag_tune->cp_cfg.cp_arbi_max_cov_shift);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_CP_ARBI_DENOM,
		djag_tune->cp_cfg.cp_arbi_denom);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_CP_ARBI_MODE,
...
void is_scaler_set_djag_dither_wb(struct pablo_mmio *base, struct djag_wb_thres_cfg *djag_wb, u32 wht, u32 blk)
{
	...
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_WHITE_LEVEL, wht);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_BLACK_LEVEL, blk);
	val = MCSC_SET_V(base, val, MCSC_F_YUV_DJAG_DITHER_WB_THRES, djag_wb->dither_wb_thres);
...
{
	return MCSC_GET_F(base, MCSC_R_YUV_DJAG_CTRL, MCSC_F_YUV_DJAG_PS_STRIP_ENABLE);
}
...
void is_scaler_set_djag_strip_enable(struct pablo_mmio *base, u32 output_id, u32 enable)
{
	MCSC_SET_F(base, MCSC_R_YUV_DJAG_CTRL,
		MCSC_F_YUV_DJAG_PS_STRIP_ENABLE, enable);
}
	...
	u32 *payload_size)
{
	...
	is_scaler_set_rdma_format(base, hw_id, hf_param->dma_format);
	...
	is_scaler_set_djag_hf_enable(base, hf_enable, 0);

	/* todo for HF */
	is_scaler_set_djag_hf_radial_center(base, 0, 0);
	is_scaler_set_djag_hf_radial_binning(base, 0, 0);
	is_scaler_set_djag_hf_radial_biquad_factor(base, 0, 0);
	is_scaler_set_djag_hf_radial_gain(base, 0, 0);
	is_scaler_set_djag_hf_radial_biquad_scal_shift_adder(base, 0);
...
const struct pmio_field_desc *mcsc_hw_g_field_descs(void)
{
	return mcsc_field_descs;
}

unsigned int mcsc_hw_g_num_field_descs(void)
{
	return ARRAY_SIZE(mcsc_field_descs);
}

const struct pmio_access_table *mcsc_hw_g_access_table(int type)
{
	switch (type) {
	case 0:
		return &mcsc_volatile_ranges_table;
	case 1:
		return &mcsc_wr_noinc_ranges_table;
	default:
		return NULL;
	};
