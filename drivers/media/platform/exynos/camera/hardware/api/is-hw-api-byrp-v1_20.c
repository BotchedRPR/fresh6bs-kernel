...
static void byrp_hw_s_path(struct pablo_mmio *base, u32 set_id)
{
	u32 val = 0;
	...
	u32 *sbwc_en, u32 *payload_size)
{
	...
	*payload_size = 0;
	...
	if (!IS_ENABLED(BYRP_DDK_LIB_CALL)) {
		...
		val = 0;
		val = BYRP_SET_V(base, val, BYRP_F_BYR_BLCNONBYRZSL_PERIODIC_GAINS_EN, 1);
		val = BYRP_SET_V(base, val, BYRP_F_BYR_BLCNONBYRZSL_PERIODIC_OFFSETS_BEFORE_GAIN_EN, 0);
		val = BYRP_SET_V(base, val, BYRP_F_BYR_BLCNONBYRZSL_PERIODIC_OFFSETS_AFTER_GAIN_EN, 0);
		BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_BYR_BLCNONBYRZSL_CONFIG, val);

		val = 0;
		for (i = 0; i < 30; ++i) {
			val = BYRP_SET_V(base, val, BYRP_F_BYR_BLCNONBYRZSL_PERIODIC_GAINS_0_0, 1024);
			val = BYRP_SET_V(base, val, BYRP_F_BYR_BLCNONBYRZSL_PERIODIC_GAINS_0_1, 1024);
