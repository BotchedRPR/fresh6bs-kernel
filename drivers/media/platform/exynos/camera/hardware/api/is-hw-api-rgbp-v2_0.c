...
#include "sfr/is-sfr-rgbp-v2_0.h"
...
unsigned int rgbp_hw_is_occurred1(unsigned int status, enum rgbp_event_type1 type)
{
	...
	case INTR1_VOTFLOSTFLUSH_INT:
		mask = 1 << INTR1_RGBP_VOTFLOSTFLUSH_INT;
		break;
	case INTR1_VOTF0RDMALOSTCONNECTION_INT:
		mask = 1 << INTR1_RGBP_VOTF0RDMALOSTCONNECTION_INT;
		break;
	case INTR1_VOTF0RDMALOSTFLUSH_INT:
		mask = 1 << INTR1_RGBP_VOTF0RDMALOSTFLUSH_INT;
		break;
	case INTR1_VOTF1WDMALOSTFLUSH_INT:
		mask = 1 << INTR1_RGBP_VOTF1WDMALOSTFLUSH_INT;
		break;
	case INTR1_ERR1:
...
int rgbp_hw_rdma_create(struct is_common_dma *dma, void *base, u32 input_id)
{
	...
	case RGBP_RDMA_RGB_EVEN:
		...
		break;
	case RGBP_RDMA_RGB_ODD:
		dma->reg_ofs = RGBP_R_RGB_RDMAREPRGBODD_EN;
		dma->field_ofs = RGBP_F_RGB_RDMAREPRGBODD_EN;
