...
#include "sfr/is-sfr-common-dma-v2.h"
...
static int is_hw_dma_set_format(struct is_common_dma *dma, u32 format, u32 format_type)
{
	...
	case DMA_FMT_BAYER:
		if (test_bit(format, &dma->available_bayer_format_map)) {
			...
			err_hw("invalid format_type[%d] fomat:[%d] available:[%llu]",
				format_type, format, dma->available_bayer_format_map);
			ret = DMA_OPS_ERROR;
		}
		break;
	case DMA_FMT_YUV:
		if (test_bit(format, &dma->available_yuv_format_map)) {
			...
			err_hw("invalid format_type[%d] fomat:[%d] available:[%llu]",
				format_type, format, dma->available_yuv_format_map);
			ret = DMA_OPS_ERROR;
		}
		break;
	case DMA_FMT_RGB:
		...
		break;
	default:
		err_hw("invalid format_type[%d]", format_type);
		return -EINVAL;
	}
...
static int is_hw_dma_set_msb_align(struct is_common_dma *dma, u32 msb_align)
{
	void __iomem *base = __is_hw_dma_get_base_addr(dma);
...
static int is_hw_dma_set_comp_align(struct is_common_dma *dma, u32 comp_align_type)
{
	switch (comp_align_type) {
	case SBWC_ALIGN_TYPE_32B:
		SET_F(dma->base, DMA_R_COMP_CTRL, DMA_F_COMP_SBWC_64B_ALIGN, 0);
		SET_F(dma->base, DMA_R_COMP_CTRL, DMA_F_COMP_SBWC_128B_ALIGN, 0);
		break;
	case SBWC_ALIGN_TYPE_64B:
		SET_F(dma->base, DMA_R_COMP_CTRL, DMA_F_COMP_SBWC_64B_ALIGN, 1);
		SET_F(dma->base, DMA_R_COMP_CTRL, DMA_F_COMP_SBWC_128B_ALIGN, 0);
		break;
	case SBWC_ALIGN_TYPE_128B:
		SET_F(dma->base, DMA_R_COMP_CTRL, DMA_F_COMP_SBWC_64B_ALIGN, 0);
		SET_F(dma->base, DMA_R_COMP_CTRL, DMA_F_COMP_SBWC_128B_ALIGN, 1);
		break;
	default:
		break;
	}
...
static int is_hw_dma_set_comp_quality(struct is_common_dma *dma, enum lossy_comp_quality_type quality_control)
{
	void __iomem *base = __is_hw_dma_get_base_addr(dma);

	if (quality_control >= LOSSY_COMP_QUALITY_TYPE_MAX) {
		warn_hw("SBWC 2.7 does not support lossy32ByteNum :%d", quality_control);
		return DMA_OPS_ERROR;
	}

	/* 0 : Quality prioritzed mode(default), 1: normal mode  */
	SET_R(base, DMA_R_COMP_LOSSY_QUALITY_CONTROL, quality_control);

	return DMA_OPS_SUCCESS;
}

static int is_hw_dma_set_img_addr_36b(struct is_common_dma *dma, dma_addr_t *addr, u32 plane,
				int buf_idx, u32 num_buffers)
{
	int i = 0;
	void __iomem *base = __is_hw_dma_get_base_addr(dma);

	do {
		SET_R(base, DMA_R_IMG_BASE_ADDR_1P_FRO_00 +
			 (plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i,
				(u32)((addr[i + buf_idx] & MSB_32BITS_MASK) >> LSB_BIT));

		SET_R(base, DMA_R_IMG_BASE_ADDR_1P_FRO_00_LSB_4B +
			 (plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i,
				addr[i + buf_idx] & LSB_4BITS_MASK);

		dbg_dma(4, "image addr[%d]:0x%llx msb:0x%x lsb:0x%x", i + buf_idx, addr[i + buf_idx],
				(u32)((addr[i + buf_idx] & MSB_32BITS_MASK) >> LSB_BIT),
				(u32)(addr[i + buf_idx] & LSB_4BITS_MASK));
	} while (++i < num_buffers);

	return DMA_OPS_SUCCESS;
}

static int is_hw_dma_set_header_addr_36b(struct is_common_dma *dma, dma_addr_t *addr, u32 plane,
				int buf_idx, u32 num_buffers)
{
	int i = 0;
	void __iomem *base = __is_hw_dma_get_base_addr(dma);
	...
	do {
		SET_R(base, DMA_R_HEADER_BASE_ADDR_1P_FRO_00 +
			 (plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i,
				(u32)((addr[i + buf_idx] & MSB_32BITS_MASK) >> LSB_BIT));

		SET_R(base, DMA_R_HEADER_BASE_ADDR_1P_FRO_00_LSB_4B +
			 (plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i,
				addr[i + buf_idx] & LSB_4BITS_MASK);

		dbg_dma(4, "header addr[%d]:0x%llx msb:0x%x lsb:0x%x", i + buf_idx, addr[i + buf_idx],
				(u32)((addr[i + buf_idx] & MSB_32BITS_MASK) >> LSB_BIT),
				(u32)(addr[i + buf_idx] & LSB_4BITS_MASK));
	} while (++i < num_buffers);
...
static const struct is_common_dma_ops is_hw_dma_ops = {
	...
	.dma_set_comp_64b_align		= is_hw_dma_set_comp_align,
	.dma_set_comp_error		= is_hw_dma_set_comp_error,
	.dma_set_comp_rate		= is_hw_dma_set_comp_rate,
	.dma_set_comp_quality		= is_hw_dma_set_comp_quality,
	...
	.dma_set_img_addr		= is_hw_dma_set_img_addr_36b,
	.dma_set_header_addr		= is_hw_dma_set_header_addr_36b,
...
static int pmio_dma_set_msb_align(struct is_common_dma *dma, u32 msb_align)
{
	void __iomem *base = __pmio_dma_get_base_addr(dma);
	u32 val;
	unsigned int corex_offset = GET_COREX_OFFSET(dma->set_id);
...
static int pmio_dma_set_comp_quality(struct is_common_dma *dma, enum lossy_comp_quality_type quality_control)
{
	void __iomem *base = __pmio_dma_get_base_addr(dma);
	u32 val;
	unsigned int corex_offset = GET_COREX_OFFSET(dma->set_id);
	...
	/* 0 : Quality prioritzed mode(default), 1: normal mode  */
	val = PMIO_GET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_COMP_LOSSY_QUALITY_CONTROL].sfr_offset);
	val = SET_V(val, DMA_F_COMP_LOSSY_QUALITY_CONTROL, quality_control);
	PMIO_SET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_COMP_LOSSY_QUALITY_CONTROL].sfr_offset, val);
...
static int pmio_dma_set_img_addr(struct is_common_dma *dma, dma_addr_t *addr, u32 plane, int buf_idx, u32 num_buffers)
{
	...
	void __iomem *base = __pmio_dma_get_base_addr(dma);
	unsigned int corex_offset = GET_COREX_OFFSET(dma->set_id);
	u32 dva_high, dva_low;

	do {
		dva_high = (u32)DVA_36BIT_HIGH(addr[i + buf_idx]);
		dva_low = (u32)DVA_36BIT_LOW(addr[i + buf_idx]);

		PMIO_SET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_IMG_BASE_ADDR_1P_FRO_00].sfr_offset +
			 ((plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i) * 4, dva_high);

		PMIO_SET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_IMG_BASE_ADDR_1P_FRO_00_LSB_4B].sfr_offset +
			 ((plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i) * 4, dva_low);

		dbg_dma(4, "image addr[%d]: %pad, high: 0x%x, low: 0x%x", i + buf_idx,
						&addr[i + buf_idx], dva_high, dva_low);
	} while (++i < num_buffers);
	...
	u32 plane, int buf_idx, u32 num_buffers)
{
	...
	do {
		...
		PMIO_SET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_HEADER_BASE_ADDR_1P_FRO_00_LSB_4B].sfr_offset +
			 ((plane * (IS_MAX_FRO + IS_MAX_FRO_LSB)) + i) * 4, dva_low);

		dbg_dma(4, "header addr[%d]: %pad, high: 0x%x low: 0x%x", i + buf_idx,
						&addr[i + buf_idx], dva_high, dva_low);
	} while (++i < num_buffers);
...
static int pmio_dma_get_enable(struct is_common_dma *dma, u32 *enable)
{
	...
	*enable = PMIO_GET_R(base, dma->reg_ofs + dma_regs[DMA_R_EN].sfr_offset);
...
int pmio_dma_set_corex_id(struct is_common_dma *dma, int set_id)
{
	dma->set_id = set_id;
	return DMA_OPS_SUCCESS;
}
...
int is_hw_dma_get_rgb_format(u32 bitwidth, u32 plane, u32 order)
{
	...
	case DMA_INOUT_BIT_WIDTH_32BIT:
		...
		case DMA_INOUT_ORDER_ABGR:
			...
			break;
		default:
			err_hw("output order error - (%d/%d)", order, bitwidth);
			return -EINVAL;
		}
		break;
	/* bitwidth : 10 bit */
	...
	default:
		err_hw("output bitwidth error - (%d/%d)", order, bitwidth);
		return -EINVAL;
	}
...
int is_hw_dma_get_yuv_format(u32 bitwidth, u32 format, u32 plane, u32 order)
{
	...
	case DMA_INOUT_BIT_WIDTH_8BIT:
		...
		case DMA_INOUT_FORMAT_YUV420:
			...
			case DMA_INOUT_ORDER_CrCb:
				...
				break;
			default:
				err_hw("output order error - (%d/%d/%d/%d)", bitwidth, format, order, plane);
				return -EINVAL;
			}
			break;
		...
		case DMA_INOUT_FORMAT_YUV444:
			...
			case 3:
				...
				break;
			default:
				err_hw("img plane error - (%d/%d/%d/%d)", bitwidth, format, order, plane);
				return -EINVAL;
			}
			break;
		...
		case DMA_INOUT_FORMAT_RGB:
			...
			break;
		default:
			err_hw("output format error - (%d/%d/%d/%d)", bitwidth, format, order, plane);
			return -EINVAL;
		}
		break;
	/* bitwidth : 10 bit */
	case DMA_INOUT_BIT_WIDTH_10BIT:
		...
		case DMA_INOUT_FORMAT_YUV444:
			...
			break;
		default:
			err_hw("img format error - (%d/%d/%d/%d)", bitwidth, format, order, plane);
			return -EINVAL;
		}
		break;
	/* bitwidth : 16 bit */
	case DMA_INOUT_BIT_WIDTH_16BIT:
		...
		break;
	default:
		err_hw("output bitwidth error - (%d/%d/%d/%d)", bitwidth, format, order, plane);
		return -EINVAL;
	}
	if (ret == -EINVAL)
		err_hw("get yuv format error - (%d/%d/%d/%d)", bitwidth, format, order, plane);
...
u32 is_hw_dma_get_payload_stride(u32 comp_sbwc_en, u32 bit_depth, u32 width,
	u32 comp_align_type, u32 lossy_byte32num,
	u32 block_width, u32 block_height)
{
	u32 payload_stride = 0;
	u32 compressed_data = 0;
	...
	if (comp_sbwc_en == COMP) {
		...
		if (block_width == 256 && block_height == 1) {
			/* Bayer 256x1 */
			payload_stride =
				 (((width + 255) / 256) * (256 * 1 * ALIGN(bit_depth, 2))) / 8;
		...
		case 8:
			...
			break;
		case 10:
			if (comp_align_type == SBWC_ALIGN_TYPE_32B)
				compressed_data = 10;
			else if (comp_align_type == SBWC_ALIGN_TYPE_64B)
				compressed_data = 12;
			else /* SBWC_ALIGN_TYPE_128B */
				compressed_data = 16;
			break;
		case 12:
			compressed_data = 12;
			break;
		default:
			err_hw("not suppored bit_depth :%d", bit_depth);
			break;
		}
		payload_stride = (((width + 31) / 32) * (32 * 4 * compressed_data)) / 8;
		...
		case SBWC_ALIGN_TYPE_64B:
			...
			break;
		case SBWC_ALIGN_TYPE_128B:
			payload_stride = ALIGN(payload_stride, 128);
			break;
		default:
			break;
		}
		...
		if (block_width == 256 && block_height == 1) {
			/* 256x1 */
			payload_stride =  ((width / block_width) * block_width) +
						(((width % block_width) * bit_depth) / 8);
		} else {
			/* 32x4 */
			if (bit_depth < 12 && comp_align_type == SBWC_ALIGN_TYPE_32B)
				compressed_data = 96;
			else
				compressed_data = 128;

			payload_stride =  ((width + 31) / 32) * (compressed_data);
		}
		...
		case SBWC_ALIGN_TYPE_128B:
			payload_stride = ALIGN(payload_stride, 128);
			err_hw("sbwc lossy not support comp_128b_align (%d)",
				comp_align_type);
			break;
		default:
			break;
		...
		err_hw("not suppored comp type :%d", comp_sbwc_en);
	}
