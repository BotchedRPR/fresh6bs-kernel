...
static int __nocfi is_hw_lme_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = CALL_HW_HELPER_OPS(hw_ip, alloc_iqset, lme_hw_get_reg_cnt());
	if (ret)
...
static void is_hw_lme_set_i_subdev_config(struct pablo_internal_subdev *sd)
{
	...
	sd->size[0] = ALIGN((sd->width * 2), 32) * sd->height;
}
...
static int is_hw_lme_deinit(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	CALL_HW_HELPER_OPS(hw_ip, deinit, instance, &hw_lme->lib[instance]);
	...
	struct lme_param_set *param_set, u32 instance, u32 is_reprocessing, u32 id, u32 set_id)
{
	...
	case LME_RDMA_CACHE_IN_0:
		if (is_reprocessing) {
			...
			sdbg_hw(4, "[HW][%s] MF STILL CUR cmd: %d, input_dva[0]: 0x%x\n",
	...
	case LME_RDMA_CACHE_IN_1:
		if (is_reprocessing) {
			input_dva = param_set->prev_input_dva;
			cmd = param_set->dma.prev_input_cmd;
			sdbg_hw(4, "[HW][%s] MF STILL REF cmd: %d, input_dva[0]: 0x%x\n",
				hw_ip, __func__, cmd, input_dva[0]);
		} else {
			input_dva = param_set->cur_input_dva;
			cmd = param_set->dma.cur_input_cmd;
	...
	sdbg_hw(4, "[HW][%s] RDMA id: %d, cmd: %d, input_dva[0]: 0x%x, input_dva[1]: 0x%x\n",
	...
	struct lme_param_set *param_set, u32 instance, u32 id, u32 set_id)
{
	...
	sdbg_hw(4, "[HW][%s] WDMA id: %d, cmd: %d, output_dva[0]: 0x%x, output_dva[1]: 0x%x\n",
						...
						u32 instance, struct lme_param_set *param_set)
{
	...
	for (buf_id = 0; buf_id < LME_INTERNAL_BUF_ID_MAX; buf_id++) {
		...
		if (buf_id == LME_INTERNAL_BUF_MBMV_IN)
			dst_dva = hw_lme->mbmv0_dva;
		else if (buf_id == LME_INTERNAL_BUF_MBMV_OUT)
			dst_dva = hw_lme->mbmv1_dva;

		snprintf(name, sizeof(name), "LME_MBMV%d", buf_id);

		CALL_HW_OPS(hw_ip->hardware, dma_cfg, name, hw_ip,
		...
		msdbg_hw(4, "[MBMV%d]cmd:%d, dva[0]:0x%x, frame_dva[0]:%pad\n",
	...
	ulong hw_map)
{
	...
	cmd_cur_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lme_cur", hw_ip,
	...
	sdbg_hw(4, "[HW][%s][CUR_IN] cmd: %d, input_dva: 0x%x, dvaddr_buffer[0]: %pad\n",
	...
	if (is_reprocessing && !param_set->dma.prev_input_cmd) {
		...
		cmd_prev_input = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lme_prev", hw_ip,
					frame, cur_idx, frame->num_buffers,
	...
	if (!is_reprocessing && !param_set->dma.prev_input_cmd) {
		...
		sdbg_hw(4, "[HW][%s] VIDEO 1st frame prev_input_dva = cur_input_dva = 0x%x\n",
			hw_ip, __func__, param_set->cur_input_dva[0]);
	}

	sdbg_hw(4, "[HW][%s][PREV_IN] cmd: %d, input_dva: 0x%x, lemdsTargetAddress[0]: %pad\n",
		hw_ip, __func__, param_set->dma.prev_input_cmd,
		param_set->prev_input_dva[0], &frame->lmesTargetAddress[0]);

	cmd_output = CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lme_pure", hw_ip,
			frame, cur_idx, frame->num_buffers,
	...
	sdbg_hw(4, "[HW][%s][MV_OUT] cmd: %d, output_dva: 0x%x, lmecTargetAddress[0]: %pad\n",
		hw_ip, __func__, param_set->dma.output_cmd, param_set->output_dva[0], &frame->lmecTargetAddress[0]);

	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "lme_pure", hw_ip,
			frame, cur_idx,
			&param_set->dma.output_cmd,
			param_set->dma.output_plane,
	...
	sdbg_hw(4, "[HW][%s][MV_OUT_KVA] output_kva_motion_pure[0]: 0x%lx, lmecKTargetAddress[0]: 0x%lx\n",
		hw_ip, __func__, param_set->output_kva_motion_pure[0], frame->lmecKTargetAddress[0]);

	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "lme_sad", hw_ip,
		frame, cur_idx, frame->num_buffers,
	...
	sdbg_hw(4, "[HW][%s][SAD_OUT] cmd: %d, output_dva_sad[0]: 0x%x, lmesadTargetAddressa[0]:%pad,\n",
		hw_ip, __func__,
		param_set->dma.sad_output_cmd, param_set->output_dva_sad[0], &frame->lmesadTargetAddress[0]);

	CALL_HW_OPS(hw_ip->hardware, dma_cfg_kva64, "lme_proc", hw_ip,
			frame, cur_idx,
	...
	sdbg_hw(4, "[HW][%s][RTA_MV_OUT_KVA] output_kva_motion_processed[0]: 0x%lx, lmemKTargetAddress[0]: 0x%lx\n",
		hw_ip, __func__, param_set->output_kva_motion_processed[0], frame->lmemKTargetAddress[0]);
...
config:
	...
	CALL_HW_HELPER_OPS(hw_ip, set_rta_regs, instance, set_id, false, frame, NULL);
...
shot_fail_recovery:
	CALL_HW_HELPER_OPS(hw_ip, restore, instance, &hw_lme->lib[instance]);
	...
	u32 instance, u32 fcount, u32 size, ulong addr)
{
	...
	return CALL_HW_HELPER_OPS(hw_ip, get_cap_meta, instance,
				&hw_lme->lib[instance], fcount, size, addr);
}
