...
/* Read the djag_en from setfile */
	...
	is_scaler_set_djag_scaling_ratio(hw_ip->pmio, hratio, vratio);
...
#endif
	is_scaler_set_djag_tuning_param(hw_ip->pmio, djag_tuneset);
#if defined(USE_YUV_RANGE_BY_ISP)
...
#endif
	is_scaler_set_djag_dither_wb(hw_ip->pmio, djag_wb, white, black);
	...
	if (djag_en && use_out_crop) {
		...
		is_scaler_set_djag_strip_config(hw_ip->pmio, output_id, temp_stripe_pre_dst_x, temp_stripe_start_pos_x);
