...
static inline void is_hw_cstat_frame_start(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (start_fcount >= hw_fcount) {
		mserr_hw("ignore invalid cstat_frame_start(P[F%d]->N[F%d])",
			instance, hw_ip, start_fcount, hw_fcount);
		return;
	}
...
static inline void is_hw_cstat_frame_end(struct is_hw_ip *hw_ip, u32 instance, bool frame_done)
{
	...
	u32 start_fcount, end_fcount;
	struct is_framemgr *framemgr;
	struct is_frame *frame;
	unsigned long flags;
	u32 edge_score = 0, buf_id;
	struct pablo_crta_buf_info stat_buf[PABLO_CRTA_BUF_MAX - PABLO_CRTA_BUF_PRE_THUMB] = { 0, };
	struct pablo_crta_buf_info shot_buf = { 0, };

	/* start_fcount -> end_fcount */
	start_fcount = atomic_read(&hw->start_fcount);
	end_fcount = atomic_read(&hw->end_fcount);
	if (end_fcount >= start_fcount) {
		mserr_hw("ignore invalid cstat_frame_end(P[F%d]->N[F%d])",
			instance, hw_ip, end_fcount, start_fcount);
		return;
	}
	atomic_set(&hw->end_fcount, start_fcount);
	...
	if (IS_ENABLED(CRTA_CALL)) {
		...
		if (test_and_clear_bit(HW_OVERFLOW_RECOVERY, &hw_ip->state)) {
			...
			for (buf_id = PABLO_CRTA_BUF_PRE_THUMB; buf_id < PABLO_CRTA_BUF_MAX; buf_id++)
				CALL_CRTA_BUFMGR_OPS(hw->bufmgr[instance], get_process_buf, buf_id,
					start_fcount, &stat_buf[buf_id - PABLO_CRTA_BUF_PRE_THUMB]);
		}

		ret = CALL_ADT_MSG_OPS(hw->icpu_adt, send_msg_cstat_frame_end, instance,
					(void *)hw_ip, NULL,
					start_fcount, &shot_buf,
					(PABLO_CRTA_BUF_MAX - PABLO_CRTA_BUF_PRE_THUMB), stat_buf, edge_score);
		if (ret)
			mserr_hw("icpu_adt send_msg_cstat_frame_end fail", instance, hw_ip);
	} else {
		CALL_HW_OPS(hardware, dbg_trace, hw_ip, start_fcount, DEBUG_POINT_FRAME_END);
...
static int is_hw_cstat_isr1(u32 hw_id, void *data)
{
	...
	struct is_hardware *hardware = hw_ip->hardware;
	ulong int1, int2;
	u32 instance, hw_fcount, start_fcount, end_fcount;
	...
	while (int_status) {
		...
		case CSTAT_FS:
			if (line) {
				...
				end_fcount = atomic_read(&hw->end_fcount);
				if (end_fcount >= start_fcount)
					mserr_hw("ignore invalid config_lock(P[F%d]->N[F%d])",
						instance, hw_ip, end_fcount, start_fcount);
				else
...
static inline void is_hw_cstat_cdaf(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	struct pablo_crta_buf_info cdaf_buf = { 0, };
	...
	ret = CALL_ADT_MSG_OPS(hw->icpu_adt, send_msg_cstat_cdaf_end, instance,
		(void *)hw_ip, NULL, hw_fcount, 1, &cdaf_buf);
	if (ret)
...
static int is_hw_cstat_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = CALL_ADT_MSG_OPS(hw->icpu_adt, register_response_msg_cb, instance,
		PABLO_HIC_CSTAT_FRAME_START, is_hw_cstat_frame_start_callback);
	if (ret) {
		mserr_hw("icpu_adt register_response_msg_cb fail", instance, hw_ip);
		goto err_adt_start;
	}

	ret = CALL_ADT_MSG_OPS(hw->icpu_adt, register_response_msg_cb, instance,
		PABLO_HIC_CSTAT_FRAME_END, is_hw_cstat_frame_end_callback);
	if (ret) {
		mserr_hw("icpu_adt register_response_msg_cb fail", instance, hw_ip);
		goto err_adt_end;
	}
	...
	ret = CALL_ADT_MSG_OPS(hw->icpu_adt, register_response_msg_cb, instance,
		PABLO_HIC_CSTAT_CDAF_END, is_hw_cstat_cdaf_callback);
	if (ret) {
		mserr_hw("icpu_adt register_response_msg_cb fail", instance, hw_ip);
		goto err_adt_cdaf;
	}
...
err_regs_alloc:
	CALL_ADT_MSG_OPS(hw->icpu_adt, register_response_msg_cb, instance,
...
static void is_hw_cstat_s_rdma_byr(void *data, unsigned long id, struct is_frame *frame)
{
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_byr_in",
		...
		struct cstat_param_set *p_set)
{
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_drc",
			hw_ip, frame, b_idx, frame->num_buffers,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_lme_ds0",
			hw_ip, frame, 0, 1,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_lme_ds1",
			hw_ip, frame, 0, 1,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_fdpig",
			hw_ip, frame, 0, 1,
	...
	CALL_HW_OPS(hw_ip->hardware, dma_cfg, "cstat_cds",
			hw_ip, frame, 0, 1,
		...
		struct cstat_param_set *p_set)
{
	...
	for (buf_id = PABLO_CRTA_BUF_PRE_THUMB; buf_id < PABLO_CRTA_BUF_MAX; buf_id++) {
		...
		dma_out = cstat_hw_s_stat_cfg(buf_id - PABLO_CRTA_BUF_CDAF, buf_info.dva_cstat, p_set);
		if (!dma_out)
			CALL_CRTA_BUFMGR_OPS(hw->bufmgr[instance], put_buf, &buf_info);
	}
}
...
static int is_hw_cstat_trigger(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	case VOTF:
		...
		if (!test_bit(HW_CONFIG, &hw_ip->state)) {
			...
			/* 1. sw trigger & enable hw before sensor on */
			is_hw_cstat_s_global_enable(hw_ip, true, hw->hw_fro_en);

			/* 2-1. disable corex because aeb short sensor is not on from first frame */
			/* 2-2. ignore hw trigger until next configuration */
			if (test_bit(HW_SUSPEND, &hw_ip->state))
				cstat_hw_s_corex_enable(base, false);
			else
				cstat_hw_s_corex_type(base, COREX_IGNORE);
		} else if (test_and_clear_bit(HW_SUSPEND, &hw_ip->state)) {
			/* 1. sw trigger before aeb short sensor on*/
			cstat_hw_s_corex_enable(base, true);
			/* 2. ignore hw trigger until next configuration */
			cstat_hw_s_corex_type(base, COREX_IGNORE);
		} else {
			/* 1. hw trigger after next configuration */
	...
	ulong hw_map)
{
	...
	if (hw_ip->frame_type == LIB_FRAME_HDR_SHORT && !test_bit(HW_CONFIG, &hw_ip->state))
	...
	/* Shot */
	if (!IS_ENABLED(CRTA_CALL) || unlikely(test_bit(hw_ip->id, &debug_iq)))
...
static void cstat_hw_g_pcfi(struct is_hw_ip *hw_ip, struct is_frame *frame,
...
static void pablo_hw_cstat_query(struct is_hw_ip *ip, u32 instance, u32 type, void *in, void *out)
{
	...
	case PABLO_QUERY_GET_PCFI:
		cstat_hw_g_pcfi(ip, (struct is_frame *)in, (struct pablo_rta_frame_info *)out);
		break;
	default:
		break;
	}
