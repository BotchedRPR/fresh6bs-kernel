...
static int sensor_self_start(void)
{
	...
	ret = is_sensor_front_start(sensor, 0, 1);
	if (ret) {
...
#define PHY_ADDR_SD_DESKEW_CON0	0x140
#define PHY_ADDR_SD_DESKEW_CON2	0x148
#define PHY_ADDR_SD_CRC_CON0	0x160
...
static void __skew_code_sweep(u32 num_skew_code, u32 skew_test_time,
...
static void skew_code_sweep(u32 num_skew_code, u32 skew_test_time,
			struct is_device_csi *csi, struct v4l2_subdev *sd_csi,
			struct v4l2_subdev *sd_mod, void __iomem *regs)
{
	struct phy_setfile sf_info;
	...
	__skew_code_sweep(num_skew_code, skew_test_time, csi, sd_csi, sd_mod,
			regs, SKEW_CAL_CLK_START_POS);

	/* Sweep SKEW_CAL_DATA_CODE */
	__skew_code_sweep(num_skew_code, skew_test_time, csi, sd_csi, sd_mod,
			regs, SKEW_CAL_DATA_START_POS);
...
static void crc_code_sweep(u32 num_crc_code, u32 crc_test_time,
			struct is_device_csi *csi, struct v4l2_subdev *sd_csi,
			struct v4l2_subdev *sd_mod, void __iomem *regs)
{
	struct phy_setfile sf_info;
...
static int pablo_test_set_phy_tune(const char *val, const struct kernel_param *kp)
{
	...
	void (*run_code_sweep)(u32, u32, struct is_device_csi*, struct v4l2_subdev*,
	...
	if (!num_test_code) {
		...
		run_code_sweep(num_test_code, test_time, csi, sd_csi, sd_mod, regs);
	}
