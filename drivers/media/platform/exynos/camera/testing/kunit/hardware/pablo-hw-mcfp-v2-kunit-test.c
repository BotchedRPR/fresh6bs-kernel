...
static void pablo_hw_mcfp_shot_kunit_test(struct kunit *test)
{
	...
	set_bit(IS_SUBDEV_OPEN, &hw_mcfp->subdev[instance][MCFP_SUBDEV_STILL].state);
	set_bit(IS_SUBDEV_OPEN, &hw_mcfp->subdev[instance][MCFP_SUBDEV_VIDEO].state);
...
static void pablo_hw_mcfp_frame_ndone_kunit_test(struct kunit *test)
{
	...
	ret = CALL_HWIP_OPS(hw_ip, frame_ndone, &test_ctx.frame, instance, type);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
