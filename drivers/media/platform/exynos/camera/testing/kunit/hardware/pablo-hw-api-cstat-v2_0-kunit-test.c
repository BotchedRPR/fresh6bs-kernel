...
#include "hardware/sfr/is-sfr-cstat-v2_0.h"
...
static void pablo_hw_api_cstat_hw_is_occurred_kunit_test(struct kunit *test)
{
	...
	ret = pablo_run_cstat_hw_is_occurred(LIC_ERR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);
	...
	ret = pablo_run_cstat_hw_is_occurred(CDAF_DONE, CSTAT_CDAF);
	KUNIT_EXPECT_TRUE(test, ret);
...
static void pablo_hw_api_cstat_hw_s_cds_dma_kunit_test(struct kunit *test)
{
	enum cstat_dma_id dma_id;
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		dma_out->width = test_config[test_idx].cds.w;
		dma_out->height = test_config[test_idx].cds.h;
		...
		param.output_dva_cds[0] = 0x19860218;
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_YUV_WDMACDS0_EN, CSTAT_F_YUV_WDMACDS0_EN);
		...
		/* MSB << 4 | LSB */
		set_val = (set_val << 4) | CSTAT_GET_F_COREX(ctx->test_addr,
					CSTAT_R_YUV_WDMACDS0_IMG_BASE_ADDR_1P_FRO0_LSB_4B,
					CSTAT_F_YUV_WDMACDS0_IMG_BASE_ADDR_1P_FRO0_LSB_4B);

		expected_val = param.output_dva_cds[0];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
	}
}
...
static void pablo_hw_api_cstat_hw_s_fdpig_wdma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(ctx->test_addr, CSTAT_R_RGB_WDMAFDPIG_EN, CSTAT_F_RGB_WDMAFDPIG_EN);
		expected_val = dma_out->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
		...
		/* MSB << 4 | LSB */
		set_val = (set_val << 4) | CSTAT_GET_F_COREX(ctx->test_addr,
					CSTAT_R_RGB_WDMAFDPIG_IMG_BASE_ADDR_1P_FRO0_LSB_4B,
					CSTAT_F_RGB_WDMAFDPIG_IMG_BASE_ADDR_1P_FRO0_LSB_4B);
...
static void pablo_hw_api_cstat_hw_s_ds_cfg_kunit_test(struct kunit *test)
{
	...
	for (dma_id = 0; dma_id < CSTAT_DMA_NUM; dma_id++) {
		for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
			...
			case CSTAT_W_FDPIG:
				dma_out = &param.dma_output_fdpig;
				dma_out->dma_crop_width = test_config[test_idx].fdpig.w;
				dma_out->dma_crop_height = test_config[test_idx].fdpig.h;
				dma_out->width = test_config[test_idx].fdpig.w;
				dma_out->height = test_config[test_idx].fdpig.h;
				break;
			case CSTAT_W_CDS0:
				dma_out = &param.dma_output_cds;
				dma_out->dma_crop_width = test_config[test_idx].cds.w;
				dma_out->dma_crop_height = test_config[test_idx].cds.h;
				dma_out->width = test_config[test_idx].cds.w;
				dma_out->height = test_config[test_idx].cds.h;
				break;
			default:
	...
	dma_out->cmd = DMA_OUTPUT_COMMAND_ENABLE;
	ret = cstat_hw_s_ds_cfg(ctx->test_addr, CSTAT_W_CDS0, &size_cfg, &param);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
...
static struct kunit_case pablo_hw_api_cstat_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_cstat_hw_s_cds_dma_kunit_test),
