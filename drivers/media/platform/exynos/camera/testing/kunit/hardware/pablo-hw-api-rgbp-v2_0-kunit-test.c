...
#include "hardware/sfr/is-sfr-rgbp-v2_0.h"
...
static void pablo_hw_api_rgbp_hw_s_path_kunit_test(struct kunit *test)
{
	...
	rgbp_hw_s_path(test_ctx.pmio, set_id, &config);
...
static void pablo_hw_api_rgbp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	rgbp_hw_s_core(test_ctx.pmio, num_buffers, set_id, &config, &test_ctx.param_set);
...
static void pablo_hw_api_rgbp_hw_s_rgb_rdma_format_kunit_test(struct kunit *test)
{
	...
	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_ABGR8888);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_ABGR8888;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_ARGB8888);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_ARGB8888;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_BGRA8888);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_BGRA8888;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_RGBA1010102);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_RGBA1010102;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_ABGR1010102);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_ABGR1010102;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_ARGB1010102);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_ARGB1010102;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	pablo_kunit_rgbp_hw_s_rgb_rdma_format(test_ctx.pmio, DMA_FMT_RGB_BGRA1010102);
	set_val = RGBP_GET_F(test_ctx.pmio, RGBP_R_RGB1P_FORMAT, RGBP_F_RDMA_RGB1P_FORMAT);
	expected_val = RGBP_DMA_FMT_RGB_BGRA1010102;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static void pablo_hw_api_rgbp_hw_g_input_dva_kunit_test(struct kunit *test)
{
	...
	id = RGBP_RDMA_BYR;
	rgbp_hw_g_input_dva(&test_ctx.param_set, instance, id, &cmd);

	id = RGBP_RDMA_RGB_EVEN;
	rgbp_hw_g_input_dva(&test_ctx.param_set, instance, id, &cmd);

	id = RGBP_RDMA_RGB_ODD;
	rgbp_hw_g_input_dva(&test_ctx.param_set, instance, id, &cmd);
...
static void pablo_hw_api_rgbp_hw_g_output_dva_kunit_test(struct kunit *test)
{
	...
	id = RGBP_WDMA_HF;
	rgbp_hw_g_output_dva(&test_ctx.param_set, instance, id, &cmd);
...
static void pablo_hw_api_rgbp_hw_s_rdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_rdma_addr(&test_ctx.dma, &addr, plane,
...
static void pablo_hw_api_rgbp_hw_s_wdma_addr_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_wdma_addr(&test_ctx.dma, &addr, plane,
			num_buffers, buf_idx, comp_sbwc_en, payload_size);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_rgbp_hw_s_yuvsc_scaling_ratio_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_SC_H_RATIO, RGBP_F_YUV_SC_H_RATIO), 1);
	KUNIT_EXPECT_EQ(test, RGBP_GET_F(test_ctx.pmio, RGBP_R_YUV_SC_V_RATIO, RGBP_F_YUV_SC_V_RATIO), 2);
}
...
static void pablo_hw_api_rgbp_hw_s_dma_init_kunit_test(struct kunit *test)
{
	...
	ret = rgbp_hw_s_rdma_init(hw_ip, &test_ctx.dma, &test_ctx.param_set,
			/* instance, */ enable, in_crop_size_x, cache_hint, &sbwc_en, &payload_size);
	KUNIT_EXPECT_EQ(test, ret, 1);

	ret = rgbp_hw_s_wdma_init(hw_ip, &test_ctx.dma, &test_ctx.param_set,
			instance, enable, in_crop_size_x, cache_hint, &sbwc_en, &payload_size);
	KUNIT_EXPECT_EQ(test, ret, 1);
}
