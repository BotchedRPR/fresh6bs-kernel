...
static void pablo_hw_api_pdp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	u32 sensor_type, path, en_votf, num_buffers;
	bool enable;
	...
	pablo_pdp_check_pd_size(test, pd_width, pd_height, pd_hwformat);
...
{
	u32 mux = 0, img_vc, af_vc;

	pdp_hw_g_input_vc(mux, &img_vc, &af_vc);
	KUNIT_EXPECT_EQ(test, img_vc, (u32)0);
	KUNIT_EXPECT_EQ(test, af_vc, (u32)2);

	mux = 1;
	pdp_hw_g_input_vc(mux, &img_vc, &af_vc);
	KUNIT_EXPECT_EQ(test, img_vc, (u32)1);
	KUNIT_EXPECT_EQ(test, af_vc, (u32)3);
}
...
static void pablo_hw_api_pdp_hw_s_input_mux_kunit_test(struct kunit *test)
{
	...
	if (IS_ENABLED(CONFIG_PDP_V5_0)) {
		val = pdp->mux_val[pdp->csi_ch] + NUM_CSI_LINK * (pdp->img_vc / NUM_DMA_VC);
		KUNIT_EXPECT_EQ(test, val, mux_val[pdp->id]);
	} else {
