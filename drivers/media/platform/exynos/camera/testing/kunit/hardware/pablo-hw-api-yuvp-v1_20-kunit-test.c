...
#include "hardware/sfr/is-sfr-yuvp-v1_20.h"
#include "pmio.h"

static struct yuvp_test_ctx {
	void *test_addr;
...
static void pablo_hw_api_yuvp_hw_is_occurred0_kunit_test(struct kunit *test)
{
	...
	u32 err_interrupt_list[] = {
		INTR0_YUVP_CMDQ_ERROR_INT,
		INTR0_YUVP_C_LOADER_ERROR_INT,
		INTR0_YUVP_COREX_ERROR_INT,
		INTR0_YUVP_CINFIFO_0_OVERFLOW_ERROR_INT,
		INTR0_YUVP_CINFIFO_0_OVERLAP_ERROR_INT,
		INTR0_YUVP_CINFIFO_0_PIXEL_CNT_ERROR_INT,
		INTR0_YUVP_CINFIFO_0_INPUT_PROTOCOL_ERROR_INT,
		INTR0_YUVP_COUTFIFO_0_PIXEL_CNT_ERROR_INT,
		INTR0_YUVP_COUTFIFO_0_INPUT_PROTOCOL_ERROR_INT,
		INTR0_YUVP_COUTFIFO_0_OVERFLOW_ERROR_INT,
		INTR0_YUVP_COUTFIFO_1_PIXEL_CNT_ERROR_INT,
		INTR0_YUVP_COUTFIFO_1_INPUT_PROTOCOL_ERROR_INT,
		INTR0_YUVP_COUTFIFO_1_OVERFLOW_ERROR_INT,
		INTR0_YUVP_OTF_SEQ_ID_ERROR_INT,
	};

	status = 1 << INTR0_YUVP_FRAME_START_INT;
	ret = yuvp_hw_is_occurred0(status, INTR_FRAME_START);
	KUNIT_EXPECT_EQ(test, ret, status);

	status = 1 << INTR0_YUVP_FRAME_END_INT;
	ret = yuvp_hw_is_occurred0(status, INTR_FRAME_END);
	KUNIT_EXPECT_EQ(test, ret, status);

	status = 1 << INTR0_YUVP_COREX_END_INT_0;
	ret = yuvp_hw_is_occurred0(status, INTR_COREX_END_0);
	KUNIT_EXPECT_EQ(test, ret, status);

	status = 1 << INTR0_YUVP_COREX_END_INT_1;
...
static void pablo_hw_api_yuvp_hw_is_occurred1_kunit_test(struct kunit *test)
{
	...
	int test_idx;
	u32 err_interrupt_list[] = {
		INTR1_YUVP_DTP_DBG_CNT_ERR,
		INTR1_YUVP_YUVNR_DBG_CNT_ERR,
		INTR1_YUVP_YUV422TO444_DBG_CNT_ERR,
		INTR1_YUVP_YUV2RGB_DBG_CNT_ERR,
		INTR1_YUVP_DEGAMMA_DBG_CNT_ERR,
		INTR1_YUVP_INVCCM_DBG_CNT_ERR,
		INTR1_YUVP_DRCDSTR_DBG_CNT_ERR,
		INTR1_YUVP_DRCTMC_DBG_CNT_ERR,
		INTR1_YUVP_CCM_DBG_CNT_ERR,
		INTR1_YUVP_RGB_GAMMA_DBG_CNT_ERR,
		INTR1_YUVP_PRC_DBG_CNT_ERR,
		INTR1_YUVP_RGB2YUV_DBG_CNT_ERR,
		INTR1_YUVP_SHARPEN_DBG_CNT_ERR,
		INTR1_YUVP_OETF_GAMMA_DBG_CNT_ERR,
		INTR1_YUVP_YUV444TO422_DBG_CNT_ERR,
		INTR1_YUVP_DITHER_DBG_CNT_ERR,
		INTR1_YUVP_SYNCFIFO_SEG_DBG_CNT_ERR,
		INTR1_YUVP_CLAHE_HIST_ERR,
	};
...
static void pablo_hw_api_yuvp_hw_s_reset_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, ret, YUVP_TRY_COUNT + 1); /* timeout */
}
...
static void pablo_hw_api_yuvp_hw_s_coutfifo_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_IP_USE_OTF_PATH,
	...
	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO0_INTERVALS,
			YUVP_F_YUV_COUTFIFO0_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO1_INTERVALS,
			YUVP_F_YUV_COUTFIFO1_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO0_INTERVAL_VBLANK,
			YUVP_F_YUV_COUTFIFO0_INTERVAL_VBLANK);
	expected_val = VBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO1_INTERVAL_VBLANK,
			YUVP_F_YUV_COUTFIFO1_INTERVAL_VBLANK);
	expected_val = VBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_R(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO0_ENABLE);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO0_CONFIG,
			YUVP_F_YUV_COUTFIFO0_DEBUG_EN);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_R(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO1_ENABLE);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_COUTFIFO1_CONFIG,
...
static void pablo_hw_api_yuvp_hw_s_cinfifo_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_CINFIFO0_INTERVALS,
			YUVP_F_YUV_CINFIFO0_INTERVAL_HBLANK);
	expected_val = HBLANK_CYCLE;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_R(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_YUV_CINFIFO0_ENABLE);
	expected_val = 1;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = YUVP_GET_F(test_ctx.pmio,
			GET_COREX_OFFSET(set_id) + YUVP_R_YUV_CINFIFO0_CONFIG,
...
static void pablo_hw_api_yuvp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	expected_val = INT0_YUVP_EN_MASK;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
	...
	set_val = YUVP_GET_R(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_SECU_CTRL_SEQID);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static void pablo_hw_api_yuvp_hw_wdma_create_kunit_test(struct kunit *test)
{
	...
	for (test_idx = YUVP_WDMA_Y; test_idx < YUVP_WDMA_MAX; test_idx++) {
		dma_id = test_idx;
...
static void pablo_hw_api_yuvp_hw_s_sharpen_size_kunit_test(struct kunit *test)
{
	u32 set_id = COREX_DIRECT;
	u32 start_pos = 0;
	u32 frame_width = 0;
	...
	yuvp_hw_s_sharpen_size(test_ctx.pmio, set_id, start_pos, frame_width, dma_width,
...
static void pablo_hw_api_yuvp_hw_s_input_path_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_F(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_IP_USE_OTF_PATH,
...
static void pablo_hw_api_yuvp_hw_s_demux_enable_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_R(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_CHAIN_DEMUX_ENABLE);
	expected_val = type;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static void pablo_hw_api_yuvp_hw_s_mux_dtp_kunit_test(struct kunit *test)
{
	...
	set_val = YUVP_GET_R(test_ctx.pmio, GET_COREX_OFFSET(set_id) + YUVP_R_CHAIN_MUX_SELECT);
	expected_val = type;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static struct kunit_case pablo_hw_api_yuvp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_yuvp_hw_s_sharpen_size_kunit_test),
...
static int __pablo_hw_api_yuvp_pmio_init(struct kunit *test)
{
	...
	if (IS_ERR(test_ctx.pmio)) {
		err("failed to init yuvp PMIO: %d", PTR_ERR(test_ctx.pmio));
		return -ENOMEM;
	}
...
static void pablo_hw_api_yuvp_kunit_test_exit(struct kunit *test)
{
	...
	memset(&test_ctx, 0, sizeof(struct yuvp_test_ctx));
}
