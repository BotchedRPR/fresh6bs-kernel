...
#include "hardware/sfr/is-sfr-mcsc-v11_0.h"
...
static void pablo_hw_api_mcsc_set_poly_scaler_coef_kunit_test(struct kunit *test)
{
	...
	val = MCSC_GET_F(tctx->pmio, MCSC_R_YUV_POLY_SC4_UV_V_COEFF_4_23,
...
static void pablo_hw_api_mcsc_set_hwfc_config_kunit_test(struct kunit *test)
{
	...
	val = MCSC_GET_F(tctx->pmio, MCSC_R_YUV_HWFC_IMAGE_HEIGHT,
		MCSC_F_YUV_HWFC_IMAGE_HEIGHT_A);
	KUNIT_EXPECT_EQ(test, height, val);

	output_id = MCSC_OUTPUT4;
...
struct kunit_suite pablo_hw_api_mcsc_kunit_test_suite = {
	.name = "pablo-hw-api-mcsc-v11_0-kunit-test",
