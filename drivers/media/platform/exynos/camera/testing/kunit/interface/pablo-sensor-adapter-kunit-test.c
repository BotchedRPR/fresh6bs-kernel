...
static void pablo_sensor_adt_v1_kunit_test_exit(struct kunit *test)
{
	...
	if (sensor_peri->actuator) sensor_peri->actuator->ixc_ops = pablo_sensor_adt_test_ctx.act_ixc_ops;
	if (sensor_peri->flash) sensor_peri->flash->ixc_ops = pablo_sensor_adt_test_ctx.fl_ixc_ops;
	if (sensor_peri->aperture) sensor_peri->aperture->ixc_ops = pablo_sensor_adt_test_ctx.apt_ixc_ops;
	if (sensor_peri->eeprom) sensor_peri->eeprom->ixc_ops = pablo_sensor_adt_test_ctx.ep_ixc_ops;
