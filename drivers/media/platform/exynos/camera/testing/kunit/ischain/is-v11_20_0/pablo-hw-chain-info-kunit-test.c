...
static void pablo_hw_chain_info_get_hw_ip_kunit_test(struct kunit *test)
{
	...
	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_PAF1);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[1]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_PAF2);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[2]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_3AA0);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[4]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_3AA1);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[5]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_3AA2);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[6]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_LME0);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[8]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_YPP);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[9]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_MCSC0);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[10]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_BYRP);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[11]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_RGBP);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[12]);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_ip, DEV_HW_MCFP);
	KUNIT_EXPECT_TRUE(test, hw_ip == &hw->hw_ip[13]);
...
static void pablo_hw_chain_info_get_ctrl_kunit_test(struct kunit *test)
{
	...
	value = false;
	CALL_HW_CHAIN_INFO_OPS(hw, get_ctrl, 0, HW_G_CTRL_FRM_DONE_WITH_DMA, (void *)&value);
