...
static void pablo_hw_chain_info_get_iommu_mem_kunit_test(struct kunit *test)
{
	...
	mem = CALL_HW_CHAIN_INFO_OPS(hw, get_iommu_mem, GROUP_ID_LME0);
	mem_tmp = &(pablo_iommu_group_get(1))->mem;
	KUNIT_EXPECT_TRUE(test, mem == mem_tmp);

	mem = CALL_HW_CHAIN_INFO_OPS(hw, get_iommu_mem, GROUP_ID_RGBP);
	mem_tmp = &(pablo_iommu_group_get(2))->mem;
	KUNIT_EXPECT_TRUE(test, mem == mem_tmp);

	mem = CALL_HW_CHAIN_INFO_OPS(hw, get_iommu_mem, GROUP_ID_YUVP);
	mem_tmp = &(pablo_iommu_group_get(3))->mem;
	KUNIT_EXPECT_TRUE(test, mem == mem_tmp);
