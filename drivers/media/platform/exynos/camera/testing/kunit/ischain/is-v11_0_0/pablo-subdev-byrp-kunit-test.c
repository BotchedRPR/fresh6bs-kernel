...
static void pablo_subdev_byrp_tag_votf_kunit_test(struct kunit *test)
{
	int ret;
	const struct is_subdev_ops *ops = test_ctx.ops;
	struct is_subdev *subdev = &test_ctx.subdev;
	...
	/* votf in */
	frame->out_node.sframe[0].id = IS_LVN_BYRP_HDR;
	set_bit(IS_GROUP_VOTF_INPUT, &idi->group_byrp.state);
	__set_test_vector_cropRegion(node->input.cropRegion);

	/* otf out */
	set_bit(IS_GROUP_OTF_OUTPUT, &idi->group_byrp.state);
	__set_test_vector_cropRegion(node->output.cropRegion);
	node->pixelformat = V4L2_PIX_FMT_NV21;

	/* byrp votf is not properly implemented yet */
	ret = ops->tag(subdev, idi, frame, node);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
	clear_bit(IS_GROUP_VOTF_INPUT, &idi->group_byrp.state);
	frame->out_node.sframe[0].id = 0;
}

static void pablo_subdev_byrp_tag_votf_negative_kunit_test(struct kunit *test)
{
	int ret;
	const struct is_subdev_ops *ops = test_ctx.ops;
	struct is_subdev *subdev = &test_ctx.subdev;
...
static struct kunit_case pablo_subdev_byrp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_subdev_byrp_tag_votf_kunit_test),
	KUNIT_CASE(pablo_subdev_byrp_tag_votf_negative_kunit_test),
