...
static void timer_func(struct timer_list *data)
{
	...
	ret = itf->send_message_isr(test, NULL, NULL, ICPU_MSG_PRIO_NORMAL, 1, send_buf);
	KUNIT_EXPECT_EQ(test, ret, 0);

	complete(&timer_done);
}

static void pablo_icpu_itf_send_message_isr_kunit_test(struct kunit *test)
{
	int ret;
...
static struct kunit_case pablo_icpu_itf_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_icpu_itf_send_message_isr_kunit_test),
