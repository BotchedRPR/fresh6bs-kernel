...
int is_buf_prepare(struct vb2_buffer *vb)
{
	...
	if (IS_ENABLED(DMABUF_CONTAINER)) {
		...
		/* Unmerged meta plane */
		ret = vbuf->ops->dbufcon_kmap(vbuf, num_i_planes, pablo_itf_get_icpu_dev());
		if (ret) {
	...
	if (test_bit(IS_QUEUE_NEED_TO_KMAP, &queue->state)) {
		...
		if (vbuf->num_merged_dbufs) {
			for (i = 0; i < num_i_planes; i++)
				vbuf->ops->dbufcon_kmap(vbuf, i, NULL);
		} else {
			for (i = 0; i < num_i_planes; i++)
	...
	if (IS_ENABLED(CONFIG_PABLO_ICPU))
		dva_meta = vbuf->dva_ext[pos_meta_p];
	else
		dva_meta = 0;
...
int is_start_streaming(struct vb2_queue *vbq, unsigned int count)
{
	...
	if (iq->buf_rdycount &&
		...
		err("[%s] need at least %u buffers", iq->name, iq->buf_rdycount);
		return -EINVAL;
	}
...
int is_vidioc_prepare_buf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	...
	ret = pablo_vb2_prepare_buf(vbq, b);
	if (ret) {
...
int is_vidioc_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	case V4L2_CID_IS_S_IRTA_RESULT_BUF:
		idi = GET_DEVICE_ISCHAIN(ivc);
		pablo_interface_irta_result_buf_set(idi->pii, a->value);
		break;
	case V4L2_CID_IS_S_IRTA_RESULT_IDX:
		idi = GET_DEVICE_ISCHAIN(ivc);
		pablo_interface_irta_result_fcount_set(idi->pii, a->value);
		break;
	case V4L2_CID_IS_S_CRTA_START:
		core = is_get_is_core();
		bufmgr = &core->resourcemgr.crta_bufmgr[instance];
		icpu_adt = pablo_get_icpu_adt();

		ret = CALL_CRTA_BUFMGR_OPS(bufmgr, get_free_buf, PABLO_CRTA_BUF_PCSI, 0, true, &buf_info);
		if (ret) {
			mverr("failed to get_free_buf(PCSI)", ivc, iv);
			return ret;
		} else {
			ret = CALL_ADT_MSG_OPS(icpu_adt, send_msg_start, instance, &buf_info);
			if (ret) {
				mverr("failed to send_msg_start", ivc, iv);
				return ret;
			}
		}
		break;
