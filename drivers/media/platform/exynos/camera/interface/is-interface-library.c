					...
					struct is_interface_hwip *itf_ip)
{
	...
	for (i = 0; i < INTR_HWIP_MAX; i++) {
		itf_ip->handler[i].valid = false;

		lib_support.intr_handler_taaisp[handler_id][i]
			= (struct hwip_intr_handler *)&itf_ip->handler[i];

		hw_ip->intr_handler[i] = (struct hwip_intr_handler *)&itf_ip->handler[i];
	}
}
...
{
	if (!lib_support.minfo) {
		lib_support.itfc = itfc;
		lib_support.minfo = itfc->minfo;
		lib_support.pdev = pdev;
	}
}
...
int is_log_write(const char *str, ...)
{
	...
	*((int *)(ctrl_kva)) = (lib->log_ptr - debug_kva);
	...
	is_memlog_ddk("[%d] %s", cpu, lib->string);
	...
	u32 type, const char *name)
{
	...
	info_lib("memory block %s - kva: %p, dva: %pad, size: %zu\n",
			mblk->name, (void *)mblk->kva_base,
...
static int mblk_dva(struct lib_mem_block *mblk, ulong kva, u32 *dva)
{
	...
	if ((kva < mblk->kva_base) || (kva >= (mblk->kva_base + mblk->pb->size))) {
		err_lib("invalid DVA request - kva: %p", (void *)kva);
		*dva = 0;
		WARN_ON(1);
...
ulong get_reg_addr(u32 id)
{
	...
	if (!hw_ip) {
		err_lib("invalid hw_id (%d) ", hw_id);
		return 0;
	}

	reg_addr = (ulong)hw_ip->regs[REG_SETA];
...
void is_flush_ddk_thread(void)
{
	...
	for (i = 0; i < TASK_INDEX_MAX; i++) {
		if (lib->task_taaisp[i].task) {
			...
			if (ret) {
				err_lib("kthread_stop fail (%d): exit_code(%d), exit_state(%d)",
					ret,
...
static int __nocfi pablo_lib_heap_mem_alloc_dynamic(struct is_mem *mem, struct is_minfo *minfo, int type)
{
	...
	if (type == IS_BIN_LIB_HINT_DDK) {
		...
		heap_size = env_args[0];
		if (heap_size == 0) {
			warn("DDK heap size is zero");
			return 0;
		}

		if (minfo->kvaddr_heap_ddk) {
			info_lib("DDK heap is already allocated(addr:%p), use it", (void *)minfo->kvaddr_heap_ddk);
			return 0;
		}
		...
		info_lib("memory_alloc(DDK heap)(V/S): %p/0x%x", (void *)minfo->kvaddr_heap_ddk, heap_size);
	} else if (type == IS_BIN_LIB_HINT_RTA) {
		((rta_get_env_func_t)RTA_GET_ENV_ADDR)((void *)&env_args);
		heap_size = env_args[0];
		if (heap_size == 0) {
			warn("RTA heap size is zero");
			return 0;
		}

		if (minfo->kvaddr_heap_rta) {
			info_lib("RTA heap is already allocated(addr:%p), use it", (void *)minfo->kvaddr_heap_rta);
			return 0;
		}
		...
		info_lib("memory_alloc(RTA heap)(V/S): 0x%p/0x%x", (void *)minfo->kvaddr_heap_rta, heap_size);
	}

	return 0;
}

static int pablo_lib_heap_mem_free(struct is_minfo *minfo)
{
	if (!IS_ENABLED(DYNAMIC_HEAP_FOR_DDK_RTA))
		return 0;
...
void is_load_clear(void)
{
	...
	if (!lib->binary_load_flg) {
		info_lib("%s: binary is already cleared\n", __func__);
		return;
	}

	lib->binary_load_flg = false;

	info_lib("binary clear done");
...
int is_set_config(int target_hw_id, int instance, u32 fcount, void *config)
{
	...
	hw_ip = CALL_HW_CHAIN_INFO_OPS(&core->hardware, get_hw_ip, target_hw_id);
	if (!hw_ip) {
...
int is_dump_regs_array(int target_hw_id, int instance, u32 fcount, struct cr_set *regs, u32 regs_size)
{
	...
	return CALL_HWIP_OPS(hw_ip, dump_regs, instance, fcount, regs, regs_size, IS_REG_DUMP_TO_ARRAY);
}
...
#if IS_ENABLED(CONFIG_PABLO_KUNIT_TEST)
void set_os_system_funcs_for_vra(os_system_func_t *funcs)
{
	funcs[0] = (os_system_func_t)is_alloc_vra;
	funcs[1] = (os_system_func_t)is_free_vra;
	funcs[2] = (os_system_func_t)is_alloc_vra_net_array;
	funcs[3] = (os_system_func_t)is_free_vra_net_array;
	funcs[4] = (os_system_func_t)is_dva_vra;
	funcs[5] = (os_system_func_t)is_dva_vra_net_array;
	funcs[6] = (os_system_func_t)is_kva_vra;
	funcs[7] = (os_system_func_t)is_inv_vra;
	funcs[8] = (os_system_func_t)is_clean_vra;
	funcs[9] = (os_system_func_t)is_inv_vra_net_array;
}

#define MAX_OF_OSF	106
static os_system_func_t osf[MAX_OF_OSF];
os_system_func_t pablo_kunit_get_os_system_func(int type, int index)
{
	if (index >= MAX_OF_OSF)
		return NULL;

	memset((void *)osf, 0, array_size(sizeof(os_system_func_t), MAX_OF_OSF));

	if (type == KUNIT_TEST_LIB_ISP)
		set_os_system_funcs(osf);
	else if (type == KUNIT_TEST_LIB_RTA)
		set_os_system_funcs_for_rta(osf);
	else if (type == KUNIT_TEST_LIB_VRA)
		set_os_system_funcs_for_vra(osf);
...
int __nocfi is_load_ddk_bin(int loadType)
{
	...
	ret = pablo_lib_heap_mem_alloc_dynamic(mem, minfo, IS_BIN_LIB_HINT_DDK);
	if (ret) {
...
int __nocfi is_load_rta_bin(int loadType)
{
	...
	ret = pablo_lib_heap_mem_alloc_dynamic(mem, minfo, IS_BIN_LIB_HINT_RTA);
	if (ret) {
...
int pablo_lib_init_reserv_mem(struct reserved_mem *rmem)
{
	struct vm_struct *vm;
	unsigned int npages;
	int i;
	...
	if (vm->size < LIB_SIZE) {
		probe_err("insufficient bin rmem (0x%lx < 0x%x)", vm->size, LIB_SIZE);
		return -ENOMEM;
	}
...
int pablo_lib_init_mem(struct is_mem *mem)
{
	...
	if (!pablo_bin_vm.phys_addr) {
		...
		ret = pablo_alloc_n_map(mem, &pablo_bin_vm, PAGE_KERNEL_EXEC);
		if (ret) {
	...
	if (!IS_ENABLED(DYNAMIC_HEAP_FOR_DDK_RTA)) {
		...
		ret = pablo_alloc_n_map(mem, &pablo_heap_vm, PAGE_KERNEL);
		if (ret) {
		...
		ret = pablo_alloc_n_map(mem, &pablo_heap_rta_vm, PAGE_KERNEL);
		if (ret) {
...
int __nocfi pablo_lib_register_sensor_itf(void *sensor_itf)
{
	...
	is_fpsimd_get_func();
	ret = register_sensor_itf((void *)sensor_itf);
	is_fpsimd_put_func();
	if (ret < 0) {
		err("register_sensor_itf(lib) failed(%d)", ret);
		goto p_err;
	}
	...
	is_fpsimd_put_func();
	if (ret < 0) {
		err("register_sensor_itf(rta) failed(%d)", ret);
		goto p_err;
	}
#endif
