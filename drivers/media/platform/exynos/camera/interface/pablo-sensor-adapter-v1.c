...
{
	int ret;
	u32 i, expo, tgain, again, dgain, interval, ctrl_end;
	...
	for (i = 0; i < 2; i++) {
		long_term_expo_mode.expo[i] = expo;
		long_term_expo_mode.tgain[i] = tgain;
		long_term_expo_mode.again[i] = again;
		long_term_expo_mode.expo[i] = expo;
	}
	long_term_expo_mode.frm_num_strm_off_on_interval = interval;
...
static int pablo_sensor_adt_get_sensor_info(struct pablo_sensor_adt *adt, struct pablo_crta_sensor_info *pcsi)
{
	...
	pcsi->mono_en = 0; /* 0: normal, 1: mono */
	...
	if (pcsi->actuator_available) {
		...
		sensor_itf->actuator_itf_ops.get_status(sensor_itf, &status, &pcsi->temperature_valid, &pcsi->temperature,
		&pcsi->voltage_valid, &pcsi->voltage);
	}
	...
	/* pdaf info */
	sensor_itf->csi_itf_ops.get_vc_dma_buf_info(sensor_itf,
		VC_BUF_DATA_TYPE_GENERAL_STAT1, &buf_info);
	pcsi->pdaf_info.sensor_mode = buf_info.sensor_mode;
	pcsi->pdaf_info.width = buf_info.width;
	pcsi->pdaf_info.height = buf_info.height;
