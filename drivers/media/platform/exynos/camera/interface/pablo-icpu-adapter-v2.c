...
static struct cmd_info hic_cmd_info[PABLO_HIC_MAX] = {
	{ "PABLO_HIC_OPEN", ICPU_MSG_PRIO_NORMAL, true, 5, },
	{ "PABLO_HIC_PUT_BUF", ICPU_MSG_PRIO_NORMAL, false, 5, },
	{ "PABLO_HIC_SET_SHARED_BUF_IDX", ICPU_MSG_PRIO_NORMAL, false, 3, },
	{ "PABLO_HIC_START", ICPU_MSG_PRIO_NORMAL, true, 3, },
	{ "PABLO_HIC_SHOT", ICPU_MSG_PRIO_NORMAL, false, 6, },
	{ "PABLO_HIC_CSTAT_FRAME_START", ICPU_MSG_PRIO_NORMAL, false, 4, },
	{ "PABLO_HIC_CSTAT_CDAF_END", ICPU_MSG_PRIO_NORMAL, false, 0, },
	{ "PABLO_HIC_PDP_STAT0_END", ICPU_MSG_PRIO_NORMAL, false, 0, },
	{ "PABLO_HIC_PDP_STAT1_END", ICPU_MSG_PRIO_NORMAL, false, 0, },
	{ "PABLO_HIC_CSTAT_FRAME_END", ICPU_MSG_PRIO_NORMAL, false, 0, },
	{ "PABLO_HIC_STOP", ICPU_MSG_PRIO_NORMAL, true, 2, },
	{ "PABLO_HIC_CLOSE", ICPU_MSG_PRIO_NORMAL, true, 2, },
};

static struct cmd_info ihc_cmd_info[PABLO_IHC_MAX] = {
	{ "PABLO_HIC_CONTROL_SENSOR", ICPU_MSG_PRIO_NORMAL, false, 1, },
};
...
static void __pablo_icpu_adt_wake_up_wait_q(struct block_msg_wait_q *wait_q)
{
	...
	wake_up_interruptible(&wait_q->head);
}
...
static inline int __send_message_rsp(struct cmd_info *info, u32 *data, void *sender, void *cookie,
	struct block_msg_wait_q *wait_q)
{
	...
	if (!ret && info->block) {
		wait = wait_event_interruptible_timeout(wait_q->head,
			refcount_read(&wait_q->state), TIMEOUT_MSG);

		if (wait <= 0) {
			merr_adt("wait msg(%s) timeout (%ld)", wait_q->instance,
...
static inline int __send_message_isr_rsp(struct cmd_info *info, u32 *data, void *sender, void *cookie)
{
	return ITF_API(__itf, send_message_isr, sender, cookie, __pablo_icpu_adt_handle_response_msg,
			info->prio, info->data_len, data);
}
					...
					u32 hw_id, u32 rep_flag, u32 position, u32 f_type)
{
	...
	/* send msg */
	ret = __send_message_rsp(&hic_cmd_info[cmd], data, icpu_adt, NULL,
	...
	u32 fcount, struct pablo_crta_buf_info *shot_buf, struct pablo_crta_buf_info *pcfi_buf)
{
	...
	/* send msg */
	if (in_interrupt()) {
		hic_cmd_info[cmd].block = false;
		ret = __send_message_isr_rsp(&hic_cmd_info[cmd], data, cb_caller, cb_ctx);
	} else {
		hic_cmd_info[cmd].block = true;
		ret = __send_message_rsp(&hic_cmd_info[cmd], data, cb_caller, cb_ctx,
			&adt->cb_handler[cmd].wait_q[instance]);
	}
	...
	void *cb_caller, void *cb_ctx, u32 fcount, u32 stat_num, struct pablo_crta_buf_info *stat_buf)
{
	...
	/* BUF_CDAF dva */
	__pablo_icpu_adt_set_dva(&data[data_offset], &data[data_offset + 1], stat_buf[0].dva);
	data_offset += 2;
	/* TODO: set BUF_PDAF_TAIL dva */
	__pablo_icpu_adt_set_dva(&data[data_offset], &data[data_offset + 1], 0);
	data_offset += 2;
	...
	u32 stat_num, struct pablo_crta_buf_info *stat_buf, u32 edge_score)
{
	...
	data_offset += 2;
	/* BUF_PRE_THUMB, BUF_AE_THUMB, BUF_AWB_THUMB, BUF_RGBY_HIST */
	dva_num = PABLO_CRTA_BUF_CDAF_MW - PABLO_CRTA_BUF_PRE_THUMB;
	for (dva_idx = 0; dva_idx < dva_num; dva_idx++) {
		i = dva_idx * 2 + data_offset;
		__pablo_icpu_adt_set_dva(&data[i], &data[i + 1], stat_buf[dva_idx].dva);
	}
	data_offset = dva_idx * 2 + data_offset;
	/* edge score */
	data[data_offset++] = edge_score;
	/* BUF_CDAF_MW */
	__pablo_icpu_adt_set_dva(&data[data_offset], &data[data_offset + 1], stat_buf[dva_idx].dva);
	data_offset += 2;
...
static int pablo_icpu_adt_send_msg_stop(struct pablo_icpu_adt *icpu_adt, u32 instance)
{
	int ret;
	u32 data[MBOX_HIC_PARAM] = { 0, };
	struct icpu_adt_v2 *adt;
