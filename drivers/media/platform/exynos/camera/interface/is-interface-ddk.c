	...
	u32 instance_id, void *data)
{
	...
	case LIB_EVENT_FRAME_START_ISR:
		...
		if (sysfs_debug->pattern_en && (hw_ip->id == DEV_HW_3AA0 ||
			...
			if (IS_ERR_OR_NULL(device->sensor->subdev_csi)) {
				mserr_hw("subdev_csi is NULL", instance_id, hw_ip);
				return;
			}

			csi = v4l2_get_subdevdata(device->sensor->subdev_csi);
	...
	case LIB_EVENT_FRAME_END:
		...
		if (frame && frame->result) {
			...
			CALL_HW_CHAIN_INFO_OPS(hardware, get_ctrl, hw_ip->id,
					HW_G_CTRL_FRM_DONE_WITH_DMA, (void *)&frame_done);
			if (frame_done) {
