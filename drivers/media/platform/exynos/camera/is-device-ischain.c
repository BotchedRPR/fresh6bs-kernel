	...
	struct is_vender *vender)
{
	...
	setfile_addr = device->minfo->kvaddr_setfile[position];
...
static void is_ischain_calc_plane_dva(u32 planes, dma_addr_t dva[], u32 pixelformat,
				u32 width, u32 height, dma_addr_t target_addr[])
{
	int i, j;
	...
	dma_addr_t target_addr[])
{
	...
	if (frame) {
		...
		if (__is_repeating_same_frame(ldr_frame))
	...
	u32 instance)
{
	...
	device->minfo		= is_get_is_minfo();
	device->is_region	= (struct is_region *)(device->minfo->kvaddr_region
	...
	is_subdev_probe(&device->m0p, instance, ENTRY_M0P, "M0P", pablo_get_is_subdev_mcsp_ops());
	is_subdev_probe(&device->m1p, instance, ENTRY_M1P, "M1P", pablo_get_is_subdev_mcsp_ops());
	is_subdev_probe(&device->m2p, instance, ENTRY_M2P, "M2P", pablo_get_is_subdev_mcsp_ops());
	is_subdev_probe(&device->m3p, instance, ENTRY_M3P, "M3P", pablo_get_is_subdev_mcsp_ops());
	is_subdev_probe(&device->m4p, instance, ENTRY_M4P, "M4P", pablo_get_is_subdev_mcsp_ops());
	is_subdev_probe(&device->m5p, instance, ENTRY_M5P, "M5P", pablo_get_is_subdev_mcsp_ops());
...
static int is_ischain_open(struct is_device_ischain *device)
{
	...
	if (device->pii)
		pablo_interface_irta_open(device->pii);
...
static int is_ischain_close(struct is_device_ischain *device)
{
	...
	/* subdev close */
	if (IS_ENABLED(SOC_VRA)) {
		CALL_HW_CHAIN_INFO_OPS(device->hardware, get_ctrl, 0,
...
static int is_ischain_stop(struct is_device_ischain *device)
{
	...
	ret = CALL_ADT_MSG_OPS(icpu_adt, send_msg_stop, device->instance);
	if (ret) {
		merr("failed to call send_msg_stop of icpu_adt(%d)", device, ret);
		return ret;
	}
