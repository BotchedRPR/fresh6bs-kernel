...
{
	return sprintf(buffer, "Pablo active memory: %d\n", atomic_read(&mem_stats_sz_active));
}
...
static int is_dbufcon_kmap(struct is_vb2_buf *vbuf, u32 plane, struct device *dev_ext)
{
	int ret;
	struct vb2_buffer *vb = &vbuf->vb.vb2_buf;
...
skip_dbufcon_release:
	for (b = 0, p = adj_plane; b < count; b++, p += num_planes) {
		...
		if (dev_ext) {
			vbuf->atch_ext[p] = CALL_DMABUF_OPS(vbuf, attach, vbuf->dbuf[p], dev_ext);
			if (IS_ERR(vbuf->atch_ext[p])) {
				ret = PTR_ERR(vbuf->atch_ext[p]);
				vbuf->atch_ext[p] = NULL;
				goto err_map_ext;
			}

			vbuf->sgt_ext[p] = CALL_DMABUF_OPS(vbuf, map_attachment, vbuf->atch_ext[p], DMA_BIDIRECTIONAL);
			if (IS_ERR(vbuf->sgt_ext[p])) {
				ret = PTR_ERR(vbuf->sgt_ext[p]);
				vbuf->sgt_ext[p] = NULL;
				goto err_map_ext;
			}

			vbuf->dva_ext[p] = sg_dma_address(vbuf->sgt_ext[p]->sgl);
			if (IS_ERR_VALUE(vbuf->dva_ext[p])) {
				ret = (int)vbuf->dva_ext[p];
				vbuf->dva_ext[p] = 0;
				goto err_map_ext;
			}
		}
...
err_get_dbuf:
	b_err = b;
	for (b = 0, p = adj_plane; b < b_err; b++, p += num_planes) {
		if (vbuf->sgt_ext[p] && vbuf->atch_ext[p])
			CALL_DMABUF_OPS(vbuf, unmap_attachment, vbuf->atch_ext[p], vbuf->sgt_ext[p], DMA_BIDIRECTIONAL);

		if (vbuf->atch_ext[p])
			CALL_DMABUF_OPS(vbuf, detach, vbuf->dbuf[p], vbuf->atch_ext[p]);
...
static void is_dbufcon_kunmap(struct is_vb2_buf *vbuf, u32 plane)
{
	...
	for (b = 0, p = adj_plane; b < num_merged; b++, p += num_planes) {
		...
		if (!IS_ERR_OR_NULL((void *) vbuf->dva_ext[p])) {
			CALL_DMABUF_OPS(vbuf, unmap_attachment, vbuf->atch_ext[p], vbuf->sgt_ext[p], DMA_BIDIRECTIONAL);
			CALL_DMABUF_OPS(vbuf, detach, vbuf->dbuf[p], vbuf->atch_ext[p]);
			vbuf->dva_ext[p] = 0;
		}
