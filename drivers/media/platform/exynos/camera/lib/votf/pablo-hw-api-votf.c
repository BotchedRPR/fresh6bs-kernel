...
u32 camerapp_hw_votf_wrapper_reset(void __iomem *base_addr)
{
	...
	is_hw_set_reg(base_addr, &votf_wrapper_regs[VOTF_WRAPPER_SWRST], 0x11);

	do {
		reset_count++;
		if (reset_count > 10000)
			return reset_count;
	} while (is_hw_get_reg(base_addr, &votf_wrapper_regs[VOTF_WRAPPER_SWRST]) != 0);
	...
	u32 *value, u32 *state)
{
	...
	if (module == C2SERV) {
		...
		*value = (en << 0) | (id << 1) | (sel << 5);
		...
		is_hw_set_reg(base_addr, &c2serv_regs[C2SERV_R_C2COM_DEBUG], *value);
		*state = is_hw_get_reg(base_addr, &c2serv_regs[C2SERV_R_C2COM_DEBUG_DOUT]);
	...
	int trs_ip, int trs_id, int tws_ip, int tws_id)
{
	...
	if (module == C2SERV) {
		...
		is_hw_set_reg(base_addr, &c2serv_regs[C2SERV_R_VOTF_PKT_DATA], value);
...
void camerapp_hw_votf_sfr_dump(void __iomem *base_addr, int module, int module_type)
{
	if (module == C2SERV) {
		switch (module_type) {
		case M0S4:
			is_hw_dump_regs(base_addr, c2serv_m0s4_regs,
					C2SERV_M0S4_REG_CNT);
			break;
		case M2S2:
			is_hw_dump_regs(base_addr, c2serv_m2s2_regs,
					C2SERV_M2S2_REG_CNT);
			break;
		case M3S1:
			is_hw_dump_regs(base_addr, c2serv_m3s1_regs,
					C2SERV_M3S1_REG_CNT);
			break;
		case M16S16:
			is_hw_dump_regs(base_addr, c2serv_m16s16_regs,
		...
		is_hw_dump_regs(base_addr, c2agent_m6s4_regs,
				C2AGENT_M4S6_REG_CNT);
	}
}

void camerapp_hw_votf_set_mcsc_hwfc(void __iomem *base_addr, u32 mcsc_output)
{
	switch (mcsc_output) {
	case 0:
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS0_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS1_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS2_ENABLE], 0x0);
		break;
	case 1:
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS3_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS4_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS5_ENABLE], 0x0);
		break;
	case 2:
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS6_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS7_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS8_ENABLE], 0x0);
		break;
	case 3:
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS9_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS10_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS11_ENABLE], 0x0);
		break;
	case 4:
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS12_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS13_ENABLE], 0x0);
		is_hw_set_reg(base_addr, &c2serv_m16s16_regs[C2SERV_R_M16S16_TWS14_ENABLE], 0x0);
		break;
	default:
		pr_err("%s: [VOTF] invalid MCSC output!\n", __func__);
		break;
	}
}
