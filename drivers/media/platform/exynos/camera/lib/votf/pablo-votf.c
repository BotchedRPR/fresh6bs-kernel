...
void __iomem *votf_addr_temp[VOTF_DEV_NUM][IP_MAX];
void votfitf_kunit_mock_init(struct kunit *test)
{
	u32 dev_id, ip;
	...
	for (dev_id = 0; dev_id < VOTF_DEV_NUM; dev_id++) {
		...
		for (ip = 0; ip < IP_MAX; ip++) {
			votf_addr_temp[dev_id][ip] = votfdev[dev_id]->votf_addr[ip];
			votfdev[dev_id]->votf_addr[ip] = kunit_kzalloc(test, SZ_64K, 0);
			KUNIT_ASSERT_NOT_ERR_OR_NULL(test, votfdev[dev_id]->votf_addr[ip]);
		}
	}
}
KUNIT_EXPORT_SYMBOL(votfitf_kunit_mock_init);

void votfitf_kunit_mock_deinit(struct kunit *test)
{
	u32 dev_id, ip;
	...
	for (dev_id = 0; dev_id < VOTF_DEV_NUM; dev_id++) {
		...
		for (ip = 0; ip < IP_MAX; ip++) {
			kunit_kfree(test, votfdev[dev_id]->votf_addr[ip]);
			votfdev[dev_id]->votf_addr[ip] = votf_addr_temp[dev_id][ip];
		}
	}
}
KUNIT_EXPORT_SYMBOL(votfitf_kunit_mock_deinit);
#endif
...
{
	return get_votf_dev_by_ip(vinfo);
}
KUNIT_EXPORT_SYMBOL(get_votf_dev_by_ip_kunit_wrapper);
...
{
	return search_ip_idx(vinfo);
}
KUNIT_EXPORT_SYMBOL(search_ip_idx_kunit_wrapper);
...
int votf_set_mcsc_hwfc(struct votf_info *vinfo, u32 output_id)
{
	...
	camerapp_hw_votf_set_mcsc_hwfc(votf->votf_addr[ip], output_id);

	return 0;
}
EXPORT_SYMBOL_GPL(votf_set_mcsc_hwfc);
...
int votfitf_create_link(int src_ip, int dst_ip)
{
	...
	if (!votf) {
		votf_err("[IP0x%x]Failed to get votf dev", src_ip);
		return -EINVAL;
	}
	...
	if (!camerapp_check_votf_ring(votf->votf_addr[src_ip_idx], src_module)) {
...
int votfitf_check_invalid_state(struct votf_info *s_vinfo, struct votf_info *d_vinfo)
{
	...
	if ((tws_state & (GENMASK(3, 0))) == VOTF_WAIT_TOKEN_ACK ||
		...
		is_debug_s2d(true, "VOTF invalid state");
	}
...
int votf_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev, "%s: votf%d driver is used %s interface(%d)\n", __func__,
			votf->id,
	...
	for (ip = 0; ip < IP_MAX; ip++) {
		if (votf->votf_table[TWS][ip][0].use ||
			...
			if (votf->use_axi)
				pkv_iommu_dma_reserve_iova_map(dev, addr, 0x10000);
