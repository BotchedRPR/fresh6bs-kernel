...
static void sc_ext_show_task_dma(struct sc_ext_buf *buf, char *str)
{
	...
	for (i = 0; i < buf->count; i++)
		pr_info("   Plane[%d] addr = (base %pad + offset %#x)\n",
...
static void show_mscl_buf(struct mscl_buffer *buf, char *str)
{
	...
	if (buf->count > MSCL_MAX_PLANES) {
		pr_err("buf->count(%d) is larger than MAX(%d)\n",
			buf->count, MSCL_MAX_PLANES);
		return;
	}
		...
		struct sc_ext_buf *buf, int width, int height, int span)
{
	...
	if (width > span) {
		dev_err(xdev->dev, "%s: size is invalid(width(%d) > span(%d))\n",
			__func__, width, span);
		return -EINVAL;
	}
...
err_buf_range:
	dev_err(xdev->dev, "Error for buf %d, size %lu, %d x %d (span:%d)\n",
