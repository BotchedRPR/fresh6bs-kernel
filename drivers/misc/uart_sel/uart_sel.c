...
#include "../../soc/samsung/cpif/mcu_ipc.h"	/* TODO */
#include "uart_sel.h"
...
static void uart_dir_work(void)
{
	...
	if (info_value != cp_mbox_extract_sr(switch_data->mbx_ap_united_status,
				switch_data->sbi_uart_noti_mask, switch_data->sbi_uart_noti_pos)) {
		usel_err("%s: change uart state to %s\n", __func__,
			info_value ? "CP" : "AP");

		cp_mbox_update_sr(switch_data->mbx_ap_united_status, info_value,
			switch_data->sbi_uart_noti_mask, switch_data->sbi_uart_noti_pos);
		if (cp_mbox_extract_sr(switch_data->mbx_ap_united_status,
			switch_data->sbi_uart_noti_mask, switch_data->sbi_uart_noti_pos))
			cp_mbox_set_interrupt(0, switch_data->int_uart_noti);
	}
#endif
}
...
void cp_recheck_uart_dir(void)
{
	...
	mbx_uart_noti = cp_mbox_extract_sr(switch_data->mbx_ap_united_status,
			switch_data->sbi_uart_noti_mask, switch_data->sbi_uart_noti_pos);
...
#if IS_ENABLED(CONFIG_MUIC_NOTIFIER)
	switch_data->attached_dev = attached_dev;
#endif
	return 0;
}
#endif
