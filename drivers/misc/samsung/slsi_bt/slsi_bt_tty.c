...
long slsi_bt_tty_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	 ...
	 * btcli is not working with this proxy code. This code is standard,
	 * but it is not needed in the current solution using fixed
	 * configuration for uart. If there is no problem with using btcli,
	 * refer to this.
