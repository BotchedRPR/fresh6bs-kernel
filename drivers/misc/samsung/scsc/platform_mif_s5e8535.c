...
#if !defined(CONFIG_SOC_S5E8535)
#error Target processor CONFIG_SOC_S5E8535 not selected
#endif
...
module_param_cb(qos_info, &domain_id_ops, NULL, 0644);
...
{
	unsigned int ka_addr = PMU_BOOT_RAM_START, ka_addr_end;
	unsigned int val;

	if (platform->ka_patch_fw && !fw_compiled_in_kernel)
		ka_addr_end = ka_addr + platform->ka_patch_len;
	else
		ka_addr_end = ka_addr + ARRAY_SIZE(ka_patch);

	SCSC_TAG_INFO(PLAT_MIF, "Print KARAM area START:%d END:%d\n", ka_addr, ka_addr_end);
	...
	while (ka_addr < ka_addr_end) {
		...
		ka_addr += (unsigned int)sizeof(uint32_t);
	}
...
void platform_wlbt_karamdump(struct scsc_mif_abs *interface)
{
        struct platform_mif *platform = platform_mif_from_mif_abs(interface);
...
#define MEMLOG_CHECK(x) do { \
	...
	MEMLOG_CHECK(exynos_hvc(HVC_FID_SET_BAAW_WINDOW,
				1,
				((u64)(WLBT_DBUS_BAAW_1_START >> 12) << 32) | (WLBT_DBUS_BAAW_1_END >> 12),
				(paddr >> 12),
...
#else
	MEMLOG_CHECK(regmap_write(platform->dbus_baaw, BAAW1_D_WLBT_START, WLBT_DBUS_BAAW_1_START >> 12));
	MEMLOG_CHECK(regmap_write(platform->dbus_baaw, BAAW1_D_WLBT_END, WLBT_DBUS_BAAW_1_END >> 12));
	MEMLOG_CHECK(regmap_write(platform->dbus_baaw, BAAW1_D_WLBT_REMAP, paddr >> 12));
...
#define CHECK(x) do { \
	...
	if ((id & CHIP_VERSION_ID_IP_ID) >> CHIP_VERSION_ID_IP_ID_SHIFT == 0xA8)
		id = (id & ~CHIP_VERSION_ID_IP_ID) | (CHIP_IP_ID_8535 << CHIP_VERSION_ID_IP_ID_SHIFT);
	...
	CHECK(exynos_hvc(HVC_FID_SET_BAAW_WINDOW,
			 0,
			 ((u64)(WLBT_DBUS_BAAW_0_START >> 12) << 32) | (WLBT_DBUS_BAAW_0_END >> 12),
			 (platform->mem_start >> 12),
			 WLBT_BAAW_ACCESS_CTRL));
...
#else
	CHECK(regmap_write(platform->dbus_baaw, BAAW0_D_WLBT_START, WLBT_DBUS_BAAW_0_START >> 12));
	CHECK(regmap_write(platform->dbus_baaw, BAAW0_D_WLBT_END, WLBT_DBUS_BAAW_0_END >> 12));
	CHECK(regmap_write(platform->dbus_baaw, BAAW0_D_WLBT_REMAP, platform->mem_start >> 12));
	...
	CHECK(regmap_write(platform->pbus_baaw, BAAW_C_WLBT_REMAP_1, WLBT_MAILBOX_AP_WLBT_WL >> 12));
...
struct scsc_mif_abs *platform_mif_create(struct platform_device *pdev)
{
	...
	platform_if->wlbt_karamdump = platform_wlbt_karamdump;
