...
#error Target processor CONFIG_SOC_S5E9935 not selected
#endif
...
struct platform_mif {
	...
	/* State variable to control suspend / resume callbacks to
	* enable disable PCIE link*/
	bool wlbt_is_on;

	struct scsc_wake_lock pcie_wake_lock;
};
...
irqreturn_t platform_wlbt_wakeup_handler(int irq, void *data)
{
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "irq : %d value %d gpio %d\n", irq, wake_val, platform->gpio_num);

	/* gpio_num = 1 - WLBT requests for PCIE link off */
	/* gpio_num = 0 - WLBT requests for PCIE link on */
...
static int platform_mif_parse_qos(struct platform_mif *platform, struct device_node *np)
{
	...
	for (i = 0; i < len / 4; i++) {
		SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "QoS Table[%d] mif : %u int : %u cl0 : %u cl1: %u cl2: %u\n", i,
...
struct qos_table platform_mif_pm_qos_get_table(struct platform_mif *platform, enum scsc_qos_config config)
{
	...
	case SCSC_QOS_MIN:
		...
		table.freq_cl2 = platform->qos[0].freq_cl2;
		break;
	...
	case SCSC_QOS_MED:
		...
		table.freq_cl2 = platform->qos[1].freq_cl2;
		break;
	...
	case SCSC_QOS_MAX:
		...
		table.freq_cl2 = platform->qos[2].freq_cl2;
		break;
...
static int platform_mif_pm_qos_add_request(struct scsc_mif_abs *interface, struct scsc_mifqos_request *qos_req, enum scsc_qos_config config)
{
	...
	if ((!qos_req->cpu_cluster0_policy) || (!qos_req->cpu_cluster1_policy) || (!qos_req->cpu_cluster2_policy)) {
	...
	ret = freq_qos_tracer_add_request(&qos_req->cpu_cluster2_policy->constraints, &qos_req->pm_qos_req_cl2, FREQ_QOS_MIN, 0);
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "PM QoS add request cl2. Setting freq_qos_add_request %d", ret);
#else
	...
	pm_qos_add_request(&qos_req->pm_qos_req_cl2, PM_QOS_CLUSTER2_FREQ_MIN, table.freq_cl2);
#endif
	return 0;
}
...
static int platform_mif_pm_qos_update_request(struct scsc_mif_abs *interface, struct scsc_mifqos_request *qos_req, enum scsc_qos_config config)
{
	...
	freq_qos_update_request(&qos_req->pm_qos_req_cl2, table.freq_cl2);
#else
	...
	pm_qos_update_request(&qos_req->pm_qos_req_cl2, table.freq_cl2);
#endif
...
static int platform_mif_pm_qos_remove_request(struct scsc_mif_abs *interface, struct scsc_mifqos_request *qos_req)
{
	...
	pm_qos_remove_request(&qos_req->pm_qos_req_cl2);
#endif
...
static int platform_mif_reset(struct scsc_mif_abs *interface, bool reset)
{
	...
	if (enable_platform_mif_arm_reset) {
		if (!reset) { /* Release from reset */
			...
			wake_lock(&platform->pcie_wake_lock);

			gpio_set_value(platform->pmic_gpio, 1);

			pcie_mif_poweron(platform->pcie);
			...
			platform->wlbt_is_on = false;

			wake_unlock(&platform->pcie_wake_lock);
		}
	} else
...
static __iomem void *platform_get_scoreboard(struct scsc_mif_abs *interface)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	return pcie_get_scoreboard(platform->pcie);
}

static int platform_get_scoreboard_ref(struct scsc_mif_abs *interface, __iomem void *addr, uintptr_t *addr_ref)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	return pcie_get_scoreboard_ref(platform->pcie, addr, addr_ref);
}
...
struct scsc_mif_abs *platform_mif_create(struct platform_device *pdev)
{
...
#endif
	platform_if->get_scoreboard = platform_get_scoreboard;
	platform_if->get_scoreboard_ref = platform_get_scoreboard_ref;
	...
	wake_lock_init(NULL,&platform->pcie_wake_lock.ws, "pcie_mif_poweron");
...
//	bool wake_val;
	...
	/* Ignore the suspend call if WLBT is disabled */
	if (platform->wlbt_is_on == false)
		return 0;

//	wake_val = gpio_get_value(platform->gpio_num);
//	if (wake_val == PCIE_LINK_ON) {
//			SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "PCIE_LINK is ON, ignore the suspend");
//			return -EBUSY;
//	}
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "suspending PCIe");
	/* Power off pcie */
	pcie_mif_poweroff(platform->pcie);
	return r;
}
...
void platform_mif_resume(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "resuming PCIe");
	/* WLBT migth be off - WAKE == 1.. but for simplicitiy enable
	 * PCIE on each resume */
	/* Power on pcie */
	pcie_mif_poweron(platform->pcie);
