...
#error Target processor CONFIG_SOC_S5E9815 not selected
#endif
...
#endif
	struct regmap *pbus_reg; /* 0x15A00000  */
...
static void wlbt_karam_dump(struct platform_mif *platform)
{
	...
	regmap_write(platform->pbus_reg, 0x0, 0x1);

	while (ka_addr < ka_array_size) {
		regmap_read(platform->pbus_reg, ka_addr, &val);
		SCSC_TAG_INFO(PLAT_MIF, "0x%08x: 0x%08x\n", ka_addr, val);
		ka_addr += (unsigned int)sizeof(ka_patch[0]);
	}

	regmap_write(platform->pbus_reg, 0x0, 0x0);
}
#endif
...
#define CHECK(x) do { \
	...
	CHECK(regmap_write(platform->wlbt_remap, PROC_RMP_BOOT, WLBT_DBUS_BAAW_0_START >> 12));
	regmap_read(platform->wlbt_remap, PROC_RMP_BOOT, &val);
...
static int platform_mif_pmu_reset_release(struct scsc_mif_abs *interface)
{
	...
	/* init sequence
	 ...
	 DELAY 3
	 SYSTEM_OUT[20] = 1
	 WAIT VGPIO_TX_MONITOR[12] = 1
	 DELAY 0X3E8 (1000)
	 WLBT_CONFIGUREATION[0] = 1
	 WAIT WLBT_STATUS[4] = 1
	 WLBT_CTRL_NS[8] = 0
	 WLBT_CTRL_NS[7] = 1
	 */
