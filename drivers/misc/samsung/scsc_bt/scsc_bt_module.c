 ...
 * Default value is 16 * 1024 (MXLOG_TRANSPORT_BUF_LENGTH)
 */
static int fw_log_circ_buf_size = 16 * 1024;
/* MUST less than equal 1017 (BSMHCP_MAX_PACKET_SIZE - L2CAP_HEADER_SIZE)
 * Default value is 1017 (BSMHCP_MAX_PACKET_SIZE - L2CAP_HEADER_SIZE)
 */
...
static void bt_fw_log(struct scsc_service_client *client, size_t length, u32 level, const void *message)
{
	...
	/* Ensure fw_acl_read_state is cleared before reading the fw logs */
	mb();

	if (bt_service.fw_log.observers_registered == true &&
	    (bt_service.fw_log.circ_buf.buf != NULL && bt_service.fw_log.fw_acl_read != NULL) &&
		...
		if (bt_service.fw_log.fw_acl_read_state == FW_READ_ENABLE &&
			...
			bt_service.fw_log.fw_acl_read_state |= FW_READ_THRESHOLD;
...
static void slsi_sm_bt_service_cleanup_interrupts(void)
{
	...
	if (bt_module_irq_mask & 1 << irq_num++)
		scsc_service_mifintrbit_unregister_tohost(bt_service.service,
            bt_service.bsmhcp_protocol->header.info_bg_to_ap_int_src, SCSC_MIFINTR_TARGET_WPAN);
	if (bt_module_irq_mask & 1 << irq_num++)
...
static int slsi_sm_bt_service_init_interrupts(void) {
	...
	irq_ret = scsc_service_mifintrbit_register_tohost(bt_service.service,
	    scsc_bt_shm_irq_handler, NULL, SCSC_MIFINTR_TARGET_WPAN);
	if (irq_ret < 0)
		return irq_ret;
...
static int slsi_sm_bt_service_cleanup(void)
{
	...
	if (NULL != bt_service.service) {
		...
		/* Release the fw_acl_read for btsnoop */
		...
		if (bt_service.bhcd_start_ref != 0) {
			scsc_mx_service_mifram_free(
				bt_service.service,
				bt_service.bhcd_start_ref);
			bt_service.bhcd_start_ref = 0;
		}
#else
...
int slsi_sm_bt_service_start(void)
{
	...
	bt_service.service = scsc_mx_service_open_boot_data(
		common_service.maxwell_core,
		SCSC_SERVICE_ID_BT,
		&mx_bt_client,
		&err,
	...
	kfree(boot_data);
#else
	bt_service.service = scsc_mx_service_open(
		common_service.maxwell_core,
		SCSC_SERVICE_ID_BT,
		&mx_bt_client,
		&err);
...
static int slsi_bt_service_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "  mailbox_iso_free_read_scan    = %u\n",
		bt_service.mailbox_iso_free_read_scan);
	...
	for (index = 0; index < BSMHCP_DATA_BUFFER_TX_ISO_SIZE; index++)
		iso_allocated_text[index] = bt_service.iso_allocated[index] ? '1' : '0';
					   ...
					   const struct kernel_param *kp)
{
	...
	if (!ret) {
		...
		/* Enable fwsnoop, if filter is fwsnoop canned set */
		if ((firmware_btlog_enables1_high == 0x04000000 ||
			firmware_btlog_enables1_high == 0x05000000 ||
			firmware_btlog_enables1_high == 0x06000000) &&
		    firmware_btlog_enables1_low == 0x00000000 &&
		    firmware_btlog_enables0_high == 0x00000000 &&
		    firmware_btlog_enables0_low == 0x00000000)
			scsc_bt_fw_log_update_filter(1,
				firmware_btlog_enables1_high, firmware_btlog_enables1_low,
				firmware_btlog_enables0_high, firmware_btlog_enables0_low);
		/* Disable fwsnoop, if filter is not fwsnoop canned set */
		else
			scsc_bt_fw_log_update_filter(0,
				firmware_btlog_enables1_high, firmware_btlog_enables1_low,
				firmware_btlog_enables0_high, firmware_btlog_enables0_low);
	}
