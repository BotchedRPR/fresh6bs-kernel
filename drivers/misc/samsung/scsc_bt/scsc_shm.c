...
{
	/* Read index before writing contents at that index. */
	int head = bt_service.fw_log.circ_buf.head;

	if (CIRC_SPACE(head, bt_service.fw_log.circ_buf.tail, bt_service.fw_log.circ_buf_size) >= 1) {
		/* Insert one item into the buffer */
		bt_service.fw_log.circ_buf.buf[head] = data;
		/* Finish writing descriptor before incrementing head. */
		bt_service.fw_log.circ_buf.head = (head + 1) & (bt_service.fw_log.circ_buf_size - 1);
		return 1;
	} else {
		SCSC_TAG_ERR(BT_H4, "No more space in circ_buf\n");
		return 0;
	}
}

static uint8_t fw_log_circ_buf_pop(void)
{
	uint8_t data = 0;

	/* Read index before reading contents at that index. */
	int tail = bt_service.fw_log.circ_buf.tail;

	if (CIRC_CNT(bt_service.fw_log.circ_buf.head, tail, bt_service.fw_log.circ_buf_size) >= 1) {
		/* Extract one item from the buffer */
		data = bt_service.fw_log.circ_buf.buf[tail];
		/* Finish reading descriptor before incrementing tail. */
		bt_service.fw_log.circ_buf.tail = (tail + 1) & (bt_service.fw_log.circ_buf_size - 1);
	} else {
		SCSC_TAG_ERR(BT_H4, "No more data in circ_buf\n");
	}

	return data;
}

static size_t fw_log_circ_buf_remove(size_t len)
{
	size_t cnt = 0;
	size_t i = 0;
	size_t head_len = 0;
	...
	cnt = CIRC_CNT(bt_service.fw_log.circ_buf.head, bt_service.fw_log.circ_buf.tail,
		       bt_service.fw_log.circ_buf_size);
	len = min(cnt, len);

	spin_lock(&bt_service.fw_log.read_lock);
	/* Check the state is in release */
	if (bt_service.fw_log.circ_buf.buf == NULL || bt_service.fw_log.fw_acl_read == NULL) {
			cnt = 0;
		} else {
			/* Remove binary FW log until remained space of circ_buf is larger than len */
			while (consumed < len) {
				head_len = bt_service.fw_log.circ_buf.buf[bt_service.fw_log.circ_buf.tail];
				for (i = consumed; i <= head_len + consumed; i++) {
					fw_log_circ_buf_pop();
				}
				consumed = i;
		}
	...
	spin_unlock(&bt_service.fw_log.read_lock);
...
size_t scsc_bt_fw_log_circ_buf_write(uint8_t level, uint8_t *data, size_t len)
{
	...
	cnt = CIRC_SPACE(bt_service.fw_log.circ_buf.head, bt_service.fw_log.circ_buf.tail,
			 bt_service.fw_log.circ_buf_size);
	if (cnt <= len) {
		removed = fw_log_circ_buf_remove(len * FW_LOG_REMOVE_MULTIPLIER);
		if (removed <= len) {
			SCSC_TAG_ERR(BT_H4, "Failed to override fw_log_circ_buf\n");
			return 0;
		}
	}

	spin_lock(&bt_service.fw_log.write_lock);
	/* Check the state is in release */
	if (bt_service.fw_log.circ_buf.buf == NULL || bt_service.fw_log.fw_acl_read == NULL) {
		cnt = 0;
	} else {
		/* Write binary FW log to circ_buf */
		cnt = fw_log_circ_buf_push((uint8_t)(len & 0x00ff) - 1);
		cnt += fw_log_circ_buf_push(level);

		for (i = 0; i < len - FW_LOG_HEADER_SIZE; i++) {
			cnt += fw_log_circ_buf_push(data[i]);
		}
	}
	spin_unlock(&bt_service.fw_log.write_lock);

	return cnt;
}

size_t scsc_bt_fw_log_circ_buf_read(uint8_t *data, size_t len)
{
	size_t cnt = 0;
	size_t i = 0;
	size_t head_len = 0;
	...
	if (bt_service.fw_log.circ_buf.buf == NULL || bt_service.fw_log.fw_acl_read == NULL) {
		...
		/* Read binary FW log from circ_buf */
		head_len = bt_service.fw_log.circ_buf.buf[bt_service.fw_log.circ_buf.tail];
		do {
			for (i = consumed; i <= head_len + consumed; i++) {
				data[i] = fw_log_circ_buf_pop();
			}
			consumed = i;
			head_len = bt_service.fw_log.circ_buf.buf[bt_service.fw_log.circ_buf.tail];
...
void scsc_bt_fw_log_update_filter(u8 trigger, u32 bt1_high, u32 bt1_low, u32 bt0_high, u32 bt0_low)
{
	/* Trigger ON */
	if (trigger == 1) {
		if (!bt_service.fw_log.observers_registered) {
			mxlogger_register_global_observer_class("FW_LOG", RELAXED_OBSERVERS);
			bt_service.fw_log.observers_registered = true;
			if (bt_service.fw_log.fw_acl_read_state < FW_READ_ENABLE)
				bt_service.fw_log.fw_acl_read_state |= FW_READ_ENABLE;
		}
		SCSC_TAG_INFO(BT_COMMON,
			"fwsnoop enabled: circ_buf_size(%lu), circ_buf_threshold(%lu), fwsnoop_level(%08X)\n",
			bt_service.fw_log.circ_buf_size, bt_service.fw_log.circ_buf_threshold, bt1_high);
	/* Trigger OFF */
	} else if (trigger == 0) {
		if (bt_service.fw_log.observers_registered) {
			mxlogger_unregister_global_observer("FW_LOG");
			bt_service.fw_log.observers_registered = false;
			if (bt_service.fw_log.fw_acl_read_state & FW_READ_ENABLE)
				bt_service.fw_log.fw_acl_read_state ^= FW_READ_ENABLE;
		}
	...
	if (bt_service.service) {
		bt_service.bsmhcp_protocol->header.btlog_enables0_low = bt0_low;
		bt_service.bsmhcp_protocol->header.btlog_enables0_high = bt0_high;
		bt_service.bsmhcp_protocol->header.btlog_enables1_low = bt1_low;
		bt_service.bsmhcp_protocol->header.btlog_enables1_high = bt1_high;
...
static void scsc_bt_shm_irq_handler(int irqbit, void *data)
{
	...
	if (scsc_read_data_available() ||
		...
		if (!wake_lock_active(&bt_service.read_wake_lock))
			wake_lock(&bt_service.read_wake_lock);
...
/* FW log to btsnoop */
static bool scsc_read_bt_fw_data_available(void)
{
	if ((bt_service.fw_log.observers_registered == true &&
	     (bt_service.fw_log.fw_acl_read_state & FW_READ_ENABLE &&
	     bt_service.fw_log.fw_acl_read_state & (FW_READ_HCI_EVENT | FW_READ_THRESHOLD)) &&
	     CIRC_CNT(bt_service.fw_log.circ_buf.head, bt_service.fw_log.circ_buf.tail,
	              bt_service.fw_log.circ_buf_size) >= bt_service.fw_log.circ_buf_threshold) ||
	     bt_service.fw_log.fw_acl_read_state & FW_READ_VSC)
		return true;
...
{
	/* Wake the reader operation */
	if (waitqueue_active(&bt_service.read_wait) &&
	    (scsc_read_bt_fw_data_available() ||
	     atomic_read(&bt_service.error_count) != 0 ||
	    bt_service.bsmhcp_protocol->header.panic_deathbed_confession)) {
		if (!wake_lock_active(&bt_service.read_wake_lock))
			wake_lock(&bt_service.read_wake_lock);
...
static ssize_t scsc_hci_fw_log_read(char __user *buf, size_t len)
{
	ssize_t ret = 0;
	ssize_t consumed = 0;
	size_t copy_len = 0;
	...
	if (ret == 0 && bt_service.read_offset >= h4_read_data_header_len && (len - consumed)) {
		/* Calculate the amount of data that can be transferred */
		copy_len = min((bt_service.fw_log.fw_acl_read_len - (bt_service.read_offset - h4_read_data_header_len)),
			       (len - consumed));

		/* Copy the data to the user buffer */
		ret = copy_to_user(&buf[consumed],
				   &bt_service.fw_log.fw_acl_read[bt_service.read_offset - h4_read_data_header_len],
				   copy_len);
		...
		if (ret == 0) {
			...
			/* Have all data been copied to the userspace buffer */
			if (bt_service.read_offset == (h4_read_data_header_len + bt_service.fw_log.fw_acl_read_len)) {
				/* All good - h4_hci_fw_log read operation is completed */
				bt_service.read_offset = 0;
				bt_service.read_operation = BT_READ_OP_NONE;
			}
		} else {
...
static ssize_t scsc_bt_shm_h4_read_continue(char __user *buf, size_t len)
{
	...
	/* Is a fw_acl_read read operation ongoing */
	if (bt_service.read_operation == BT_READ_OP_FW_LOG_DATA) {
		SCSC_TAG_DEBUG(BT_H4, "BT_READ_OP_FW_LOG_DATA\n");

		/* Copy data into the userspace buffer */
		ret = scsc_hci_fw_log_read(buf, len);
		/* Reset the interrupt to 0 or 1 */

		if (bt_service.read_operation == BT_READ_OP_NONE) {
			/* All done - reset the read interrupt to read the new FW log */
			if (bt_service.fw_log.fw_acl_read_state & FW_READ_VSC)
				bt_service.fw_log.fw_acl_read_state &= (FW_READ_ENABLE | FW_READ_VSC);
			else
				bt_service.fw_log.fw_acl_read_state &= FW_READ_ENABLE;
		}
		/* Is a HCI event read operation ongoing */
	} else if (bt_service.read_operation == BT_READ_OP_HCI_EVT) {
		...
		if (bt_service.read_operation == BT_READ_OP_NONE) {
			...
			if (bt_service.read_index == bt_service.mailbox_hci_evt_read) {
				...
				if (bt_service.fw_log.observers_registered == true &&
				    bt_service.fw_log.fw_acl_read_state == FW_READ_ENABLE)
					srvman_wake_up_mxlog_thread_for_fwsnoop(bt_service.service);
			}
		}
...
static ssize_t scsc_bt_shm_h4_read_hci_evt(char __user *buf, size_t len)
{
	...
	while (bt_service.read_operation == BT_READ_OP_NONE &&
		...
		if (td->event_type == BSMHCP_EVENT_TYPE_CONNECTED) {
			...
			SCSC_TAG_DEBUG(BT_H4,
				       "connected (hci_connection_handle=0x%03x, state=%u)\n",
				       td->hci_connection_handle,
...
static bool scsc_rx_read_connection_active(uint16_t hci_connection_handle, uint8_t disconnected)
{
	...
	if (bt_service.connection_handle_list[hci_connection_handle].state == CONNECTION_ACTIVE) {
		...
		if (disconnected) {
			SCSC_TAG_DEBUG(BT_H4,
				       "CONN disconnected (hci_connection_handle=0x%03x, state=%u)\n",
				       hci_connection_handle,
...
static ssize_t scsc_bt_circ_h4_read_fw_log_data(char __user *buf, size_t len)
{
	ssize_t ret = 0;
	ssize_t consumed = 0;
	ssize_t read_len = 0;
	...
	/*
	 * FW log is delivered to the Host,
	 * when accumlated FW log exceeds the threshold or when HCI_EVENT is received from FW
	 */
	while (bt_service.read_operation == BT_READ_OP_NONE &&
	       scsc_read_bt_fw_data_available()) {
		/* Set read_operation for FW_LOG and interrupt as reading */
		bt_service.fw_log.fw_acl_read_state |= FW_READ_BUSY;
		bt_service.read_operation = BT_READ_OP_FW_LOG_DATA;

		/*
		 * Fully generate the H4 header + HCI_COMMAND_COMPLETE_EVENT data header +
		...
		if (bt_service.fw_log.fw_acl_read_state & FW_READ_VSC) {
			...
			h4_read_data_header[2] = HCI_EVENT_COMMAND_COMPLETE_LEN;
			bt_service.fw_log.fw_acl_read[0] = 0x01;
			bt_service.fw_log.fw_acl_read[1] = (u8)(HCI_VSC_SET_FW_LOG_BTSNOOP & 0x00ff);
			bt_service.fw_log.fw_acl_read[2] = (u8)((HCI_VSC_SET_FW_LOG_BTSNOOP & 0xff00) >> 8);
			bt_service.fw_log.fw_acl_read[3] = 0x00;
			bt_service.fw_log.fw_acl_read_len = HCI_EVENT_COMMAND_COMPLETE_LEN;
			bt_service.fw_log.fw_acl_read_state ^= FW_READ_VSC;

			/*
			* Fully generate the H4 header + ACL data header + L2CAP data header
			* regardless of the available amount of user memory
			*/
		} else {
			read_len = scsc_bt_fw_log_circ_buf_read(bt_service.fw_log.fw_acl_read + L2CAP_HEADER_SIZE,
							bt_service.fw_log.circ_buf_threshold);

			if (read_len == 0)
				SCSC_TAG_ERR(BT_H4, "Failed to read the message of FW log from circ_buf\n");

			h4_read_data_header_len = H4DMUX_HEADER_ACL;
			h4_read_data_header[H4_HEADER_INDEX] = HCI_ACLDATA_PKT;
			HCI_ACL_DATA_SET_CON_HDL(h4_read_data_header, 0x2EEF, 0x02, 0x00);
			HCI_ACL_DATA_SET_LENGTH(h4_read_data_header, (read_len + L2CAP_HEADER_SIZE));
			HCI_L2CAP_DATA_SET_LENGTH(bt_service.fw_log.fw_acl_read, read_len);
			HCI_L2CAP_DATA_SET_CON_HDL(bt_service.fw_log.fw_acl_read, 0x0000);
			bt_service.fw_log.fw_acl_read_len = read_len + L2CAP_HEADER_SIZE;
		}

		/* Start to read fw_acl_read */
		ret = scsc_hci_fw_log_read(buf, len);
		if (ret > 0) {
			/* All good - Update our consumed information */
			consumed += ret;
...
ssize_t scsc_bt_shm_h4_queue_sync_helper(char __user *buf, size_t len)
{
	...
	while (bt_service.data_paused) {
		while (mailbox_hci_evt_read != bt_service.mailbox_hci_evt_write) {
			...
			if (td->event_type & BSMHCP_EVENT_TYPE_CONNECTED &&
			    (conn_hdl_acl == td->hci_connection_handle || conn_hdl_iso == td->hci_connection_handle)) {
				/* Update the connection table to mark it as active */
				bt_service.connection_handle_list[td->hci_connection_handle].state = CONNECTION_ACTIVE;
				bt_service.connection_handle_list[td->hci_connection_handle].remaining_length = 0;
...
ssize_t scsc_bt_shm_h4_read(struct file *file, char __user *buf, size_t len, loff_t *offset)
{
	...
	while (0 == atomic_read(&bt_service.error_count) && 0 == consumed) {
		...
		SCSC_TAG_DEBUG(BT_H4,
			"read_operation=%u, hci_event_paused=%u, data_paused=%u, fw_acl_read_state=%u\n",
			bt_service.read_operation,
			bt_service.hci_event_paused,
			bt_service.data_paused,
			bt_service.fw_log.fw_acl_read_state);
		...
		/* Eighth: process any pending FW log report that needs to be sent to userspace */
		res = scsc_bt_circ_h4_read_fw_log_data(&buf[consumed], len - consumed);
		if (res < 0) {
			ret = res;
			break;
		}
	...
	if (!gen_bg_int && !gen_fg_int)
		if (wake_lock_active(&bt_service.read_wake_lock))
			wake_unlock(&bt_service.read_wake_lock);

	if (BT_READ_OP_STOP != bt_service.read_operation && BT_READ_OP_FW_LOG_DATA != bt_service.read_operation)
		...
		if (ret == 0) {
			...
			if ((length + bt_service.h4_write_offset) >= H4DMUX_HEADER_HCI &&
				...
				if ((hci_pkt_len + 1) <= (length + bt_service.h4_write_offset)) {
					/* If HCI_VSC_SET_FW_LOG_BTSNOOP command is received */
					if (HCI_COMMAND_GET_OPCODE(h4_write_buffer) == HCI_VSC_SET_FW_LOG_BTSNOOP &&
					    h4_write_buffer[3] == HCI_VSC_SET_FW_LOG_BTSNOOP_LEN) {
						/* Send HCI_VSC_COMPLETE_FW_LOG_BTSNOOP */
						bt_service.fw_log.fw_acl_read_state |= FW_READ_VSC;
						scsc_bt_fw_log_state_handler();

						scsc_bt_fw_log_update_filter(h4_write_buffer[4],
							HCI_FW_LOG_GET_FILTER(&h4_write_buffer[5]),
							HCI_FW_LOG_GET_FILTER(&h4_write_buffer[9]),
							HCI_FW_LOG_GET_FILTER(&h4_write_buffer[13]),
							HCI_FW_LOG_GET_FILTER(&h4_write_buffer[17]));
...
{
	/* Init scsc_bt_fw_log */
	memset(&bt_service.fw_log, 0, sizeof(struct scsc_bt_fw_log));
	bt_service.fw_log.circ_buf_size = len;
	bt_service.fw_log.circ_buf_threshold = threshold;

	/* Init spin lock for circ_buf */
	spin_lock_init(&bt_service.fw_log.write_lock);
	spin_lock_init(&bt_service.fw_log.read_lock);

	/* Allocate the memeory for circ_buf */
	bt_service.fw_log.circ_buf.buf = kmalloc(len, GFP_KERNEL);

	if (bt_service.fw_log.circ_buf.buf == NULL) {
		SCSC_TAG_ERR(BT_COMMON, "Failed to alloc circ_buf memory\n");
	}

	/* Allocate the memeory for fw_acl_read */
	bt_service.fw_log.fw_acl_read = kmalloc(threshold + L2CAP_HEADER_SIZE, GFP_KERNEL);

	if (bt_service.fw_log.fw_acl_read == NULL) {
		SCSC_TAG_ERR(BT_COMMON, "Failed to alloc fw_acl_read memory\n");
	}

	/* Init FW_LOG observer */
	bt_service.fw_log.observers_registered = false;
}
...
void scsc_bt_fw_log_release(void)
{
	...
	bt_service.fw_log.fw_acl_read_state = FW_READ_DISABLE;

	spin_lock(&bt_service.fw_log.read_lock);
	/* Release the memeory for fw_acl_read */
	if (bt_service.fw_log.fw_acl_read != NULL) {
		kfree(bt_service.fw_log.fw_acl_read);
		bt_service.fw_log.fw_acl_read = NULL;
	}
	spin_unlock(&bt_service.fw_log.read_lock);
	...
	if (bt_service.fw_log.circ_buf.buf != NULL) {
		kfree(bt_service.fw_log.circ_buf.buf);
		bt_service.fw_log.circ_buf.buf = NULL;
	}
	spin_unlock(&bt_service.fw_log.write_lock);
