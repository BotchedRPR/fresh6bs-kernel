...
static struct ifconn_notifier_ops notifier_ops;

void ifconn_notifier_set_ops(struct ifconn_notifier_ops *manager_ops)
{
	notifier_ops.get_muic_cable_type = manager_ops->get_muic_cable_type;
	notifier_ops.get_pd_con_state = manager_ops->get_pd_con_state;
}
EXPORT_SYMBOL_GPL(ifconn_notifier_set_ops);
			     ...
			     ifconn_notifier_t listener, ifconn_notifier_t src)
{
	...
	if (pnoti[src].sent[listener] || pnoti[src].sent[IFCONN_NOTIFY_ALL]) {
		...
		if (listener == IFCONN_NOTIFY_BATTERY) {
			if (notifier_ops.get_pd_con_state()) {
				...
				template->cable_type = notifier_ops.get_muic_cable_type();
				template->event = notifier_ops.get_muic_cable_type();
			}
...
static int __init ifconn_notifier_init(void)
{
	...
	for(i = 0; i < IFCONN_NOTIFY_MAX; i++) {
		BLOCKING_INIT_NOTIFIER_HEAD(&(pnoti[i].notifier_call_chain));
	}

	return ret;
}
static void __exit ifconn_notifier_exit(void)
{
	pr_info("%s,\n", __func__);
}

device_initcall(ifconn_notifier_init);
module_exit(ifconn_notifier_exit);

MODULE_DESCRIPTION("ifconn notifier");
