...
#ifdef CONFIG_SENSORS_SSP
		ssp_set_wakeup_by_me(chub->ssp_data, true);
#endif
	}
...
#ifdef CONFIG_SENSORS_SSP
			ssp_set_wakeup_by_me(chub->ssp_data, false);
#endif
			return IRQ_HANDLED;
		}
...
#else
#ifdef CONFIG_SENSORS_SSP
		if (data->ssp_data->panic_enable == 1)
			return NOTIFY_OK;
		else
#endif
...
static int contexthub_ipc_probe(struct platform_device *pdev)
{
	...
	/* rdx ramdump, 230317 by hyunjong.yim */	
//	chub_dbg_register_dump_to_dss(chub->sram_phys, chub->sram_size);
	chub_dbg_register_dump_to_dss(chub->sram_phys, SZ_2M + 0x8000);
