...
#define USE_IMGLOADER	1
#define USE_SSPLOADER	2
#define CONTEXTHUB_FW_LOADER		USE_SSPLOADER
#endif
				     ...
				     enum ipc_region reg)
{
	...
	int ret = 0;
	void *chub_addr = NULL;
	const char* fw_path =  "/etc/firmware";
#ifdef CONFIG_SENSORS_SSP
	struct path path;
	int fw_ret = 0;
	int firm = 0;
...
#ifdef CONFIG_SENSORS_SSP
		fw_ret = kern_path(fw_path, LOOKUP_FOLLOW, &path);
		if(fw_ret) {
			nanohub_dev_err(chub->dev, "File not exist in %s\n", fw_path);
			ret = 2;
		} else {
			path_put(&path);
	...
	if(entry->size > CHUB_SRAM_IMAGE_SIZE) {
		memcpy_toio(chub_addr, entry->data, CHUB_SRAM_IMAGE_SIZE);
		memcpy_toio(chub_addr + LPD_CHUB_IMAGE_START_OFFSET, 
			entry->data + LPD_CHUB_IMAGE_START_OFFSET, entry->size - LPD_CHUB_IMAGE_START_OFFSET);
	} else {
	...
	nanohub_dev_info(chub->dev, "%s: bl:%d, bin(size:%d) on %lx %lx\n",
			 __func__, reg == IPC_REG_BL, (int)entry->size,
			 (unsigned long)ipc_get_base(reg), (unsigned long)chub_addr);
		     ...
		     enum chub_err_type err)
{
...
#ifdef CONFIG_SENSORS_SSP
	if ((chub->ssp_data->panic_enable == 1) && !skip_panic) {
		chub_dbg_dump_hw(chub, err);
	...
	/* image download */
	if (force_load) {
#if (CONTEXTHUB_FW_LOADER == USE_IMGLOADER)
