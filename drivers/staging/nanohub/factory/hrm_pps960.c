...
#define HRM_SPEC_LENGTH        228
#define HRM_TRIM_LENGTH        24

#define HRM_SPEC_BUFFER_LENGTH        57
#define HRM_TRIM_BUFFER_LENGTH        6
...
static int hrm_open_eol_data(struct ssp_data *data)
{
    ...
    iRet = file_manager_read(data, EOL_DATA_FILE_PATH, (char *)&eol_data,  HRM_EOL_DATA_SIZE * sizeof(u32), 0);
...
{
    int iRet = 0;
    file_manager_write(data, EOL_DATA_FILE_PATH, (char *)&data->buf[HRM_RAW_FAC_SENSOR], HRM_EOL_DATA_SIZE * sizeof(u32), 0);
...
static int hrm_open_xtalk2_data(struct ssp_data *data)
{
    ...
    iRet = file_manager_read(data, XTALK2_DATA_FILE_PATH, (char *)&xtalk2_data,  HRM_XTALK2_DATA_SIZE * sizeof(u32), 0);
...
static int save_hrm_xtalk2_data(struct ssp_data *data)
{
    ...
    file_manager_write(data, XTALK2_DATA_FILE_PATH, (char *)&data->buf[HRM_RAW_FAC2_SENSOR], HRM_XTALK2_DATA_SIZE * sizeof(u32), 0);
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    count = snprintf(buf, PAGE_SIZE,
        "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
            ...
            data->buf[HRM_RAW_FAC_SENSOR].hrm_eol_data[24],
            data->buf[HRM_RAW_FAC_SENSOR].hrm_eol_data[25],
            data->buf[HRM_RAW_FAC_SENSOR].hrm_eol_data[26],
            data->buf[HRM_RAW_FAC_SENSOR].hrm_eol_data[27]);
...
//    set_hrm_calibration(data);
    ...
    count = snprintf(buf, PAGE_SIZE,
        "%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",
            ...
            data->buf[HRM_RAW_FAC2_SENSOR].hrm_xtalk2_data[24],
            data->buf[HRM_RAW_FAC2_SENSOR].hrm_xtalk2_data[25],
            data->buf[HRM_RAW_FAC2_SENSOR].hrm_xtalk2_data[26],
            data->buf[HRM_RAW_FAC2_SENSOR].hrm_xtalk2_data[27]);
    mutex_unlock(&data->sysfs_op_mtx);
...
static ssize_t hrm_eol_spec_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    uint32_t sbuf[HRM_SPEC_BUFFER_LENGTH] = {0,};
    struct ssp_data *data = dev_get_drvdata(dev);

    ssp_send_read_factory_cmd(data, HRM_RAW_FAC_SENSOR, HRM_CMD_EOL_SPEC, (char *)sbuf, sizeof(sbuf), 1000, HRM_SPEC_LENGTH, __func__);
    ...
    return snprintf(buf, PAGE_SIZE, "%d,"\
                            ...
                            "PD Dark1, %d, %d, PD Dark2, %d, %d, PD Dark3, %d, %d, PD Dark4, %d, %d, "\
                            ...
                            sbuf[45], sbuf[46], sbuf[47], sbuf[48], sbuf[49], sbuf[50], sbuf[51], sbuf[52], sbuf[53], sbuf[54], sbuf[55],
                            sbuf[56]);
}
    ...
    struct device_attribute *attr, char *buf)
{
    ...
    ssp_send_read_factory_cmd(data, HRM_RAW_FAC_SENSOR, HRM_CMD_XTALK_SPEC, (char *)sbuf, sizeof(sbuf), 1000, HRM_SPEC_LENGTH, __func__);
...
static ssize_t hrm_trim_check_show(struct device *dev,
    struct device_attribute *attr, char *buf)
{
    uint32_t trim[HRM_TRIM_BUFFER_LENGTH] = {0,};
    struct ssp_data *data = dev_get_drvdata(dev);

    ssp_send_read_factory_cmd(data, HRM_RAW_FAC_SENSOR, HRM_CMD_TRIM_CHECK, (char *)trim, sizeof(trim), 1000, HRM_TRIM_LENGTH, __func__);
