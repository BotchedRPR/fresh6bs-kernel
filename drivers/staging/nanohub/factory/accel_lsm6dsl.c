...
#define CALIBRATION_FILE_PATH	"/efs/FactoryApp/accel_cal_data"
...
void print_acc_esn_data(struct ssp_data *data)
{
	...
	nanohub_info("[SSP]:%s-acc_esn from MCU,size=%d\n",	__func__, sizeof(data->acc_esn));

	for(i = 0 ; i < sizeof(data->acc_esn) - 15; i += 16)
		nanohub_info("SSP:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,0x%02x,"
	...
	nanohub_info("SSP:0x%02x,0x%02x,0x%02x", esn[80], esn[81], esn[82]);
}
