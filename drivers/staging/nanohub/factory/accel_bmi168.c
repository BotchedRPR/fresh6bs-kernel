...
int bmi168_accel_open_calibration(struct ssp_data *data)
{
	int iRet = 0;
	mm_segment_t old_fs;
...
int bmi168_set_accel_cal(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
	s16 accel_cal[3];
	...
	pr_info("[SSP] %s: Set accel cal data %d, %d, %d\n", __func__,
	...
	struct device_attribute *attr, char *buf)
{
	...
	shift_ratio[0] = (s16)((chTempBuf[1] << 8) + chTempBuf[0]);
	shift_ratio[1] = (s16)((chTempBuf[3] << 8) + chTempBuf[2]);
	shift_ratio[2] = (s16)((chTempBuf[5] << 8) + chTempBuf[4]);
	result = chTempBuf[6];

	pr_info("[SSP] %s - %d, %d, %d, %d\n", __func__,
		result, shift_ratio[0], shift_ratio[1], shift_ratio[2]);
...
void initialize_bmi168_accel_factorytest(struct ssp_data *data)
{
	sensors_register(data->acc_device, data, acc_attrs,
		"accelerometer_sensor");
}

void remove_bmi168_accel_factorytest(struct ssp_data *data)
{
