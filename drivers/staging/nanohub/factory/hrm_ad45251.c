...
static int hrm_open_eol_data(struct ssp_data *data)
{
	...
	iRet = vfs_read(cal_filp, (char *)&eol_data, 16 * sizeof(u32), &cal_filp->f_pos);
...
static int save_hrm_eol_data(struct ssp_data *data)
{
	...
	iRet = vfs_write(cal_filp, (char *)&data->buf[HRM_RAW_FAC_SENSOR], 16 * sizeof(u32), &cal_filp->f_pos);
...
int ad45251_set_hrm_calibration(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
...
int ad45251_hrm_open_calibration(struct ssp_data *data)
{
	int iRet = 0;
...
void initialize_ad45251_hrm_factorytest(struct ssp_data *data)
{
	sensors_register(data->hrm_device, data, hrm_attrs,
		"hrm_sensor");
}

void remove_ad45251_hrm_factorytest(struct ssp_data *data)
{
