...
int ak09918c_mag_open_calibration(struct ssp_data *data)
{
	...
	iRet = file_manager_read(data, CALIBRATION_FILE_PATH, (char *)&data->magcal,  sizeof(data->magcal), 0);
...
{
	int ret = 0;
	char *fp = (char *)(&data->magcal);
	int size = sizeof(data->magcal);
	int offset = 0;

	if (size > 1024) {
		ssp_errf("[SSP]  save mag cal data too large size %d", size);
		return -1;
	}

	while (size) {
		if (size > 512) {
			file_manager_write(data, CALIBRATION_FILE_PATH, fp+offset, 512, offset);
			offset += 512;
			size -= 512;
		} else {
			file_manager_write(data, CALIBRATION_FILE_PATH, fp+offset, size, offset);
			size = 0;
		}
...
static DEVICE_ATTR(cal_data, 0440, magnetic_cal_data_show, NULL);
