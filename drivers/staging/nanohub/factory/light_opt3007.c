...
int opt3007_light_open_calibration(struct ssp_data *data)
{
    ...
    ret = file_manager_read(data, CALIBRATION_FILE_PATH, (char *)&data->lightcal,  3 * sizeof(data->lightcal), 0);
...
static int light_do_calibrate(struct ssp_data *data, int iEnable)
{
    ...
    send_buf[2] = (int)data->lightcal.curr_lux;
       ...
       iRet =  file_manager_write(data, CALIBRATION_FILE_PATH, (char *)&data->lightcal, 3 * sizeof(int), 0);
