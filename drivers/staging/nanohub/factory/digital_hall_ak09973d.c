...
void digital_hall_send_uevent(struct ssp_data *data)
{
	...
	if (data->buf[DIGITAL_HALL_EVENT_SENSOR_15].direction == 0) {
		return;
	}

	sprintf(sw_int, "SW_INT=%d", data->buf[DIGITAL_HALL_EVENT_SENSOR_15].direction);
	sprintf(hw_int, "HW_INT=%d", data->buf[DIGITAL_HALL_EVENT_SENSOR_15].interrupt);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	msg->cmd = MSG2SSP_SET_BEZEL_TEST_ONOFF;
    msg->length = 1;
    ...
    *msg->buffer = data->wheel_test;
    msg->free_buffer = 1;

	ret = ssp_send_command(data, msg, 0);
    if (ret != SUCCESS)
        pr_err("[SSP] %s - fail %d\n", __func__, ret);
    else
        pr_info("[SSP] %s - %d\n", __func__, data->wheel_test);
...
static ssize_t digital_hall_test_start_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct ssp_data *data = dev_get_drvdata(dev);
	struct ssp_msg *msg;	
	...
	if(data->wheel_test == false) {
		return snprintf(buf, PAGE_SIZE, "%d\n", test_ready);
	}

retries:
	msg = kzalloc(sizeof(*msg), GFP_KERNEL);
	msg->cmd = MSG2SSP_GET_BEZEL_TEST_READY;
	msg->length = 1;
	msg->options = AP2HUB_READ;
	msg->buffer = &test_ready;
	msg->free_buffer = 0;
...
static DEVICE_ATTR(test_start, S_IRUGO | S_IWUSR | S_IWGRP, digital_hall_test_start_show, digital_hall_test_start_set);
