...
#define CHUB_LOG_BUF_SIZE 	(SZ_1M)
...
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
enum {
	SSP_UB_CON_CONNECTED = PANEL_EVENT_UB_CON_STATE_CONNECTED,
	SSP_UB_CON_DISCONNECTED = PANEL_EVENT_UB_CON_STATE_DISCONNECTED,
...
#define HRM_EOL_DATA_SIZE		28
#define HRM_XTALK2_DATA_SIZE	28
...
#define SENSORHUB_REG_DUMP_SIZE		(127) //DATA_PACKET_SIZE//1365//300

#ifdef CONFIG_SENSORS_SSP_LUCKY
#define SENSOR_STUCK_INFO_SIZE 52
#else
#define SENSOR_STUCK_INFO_SIZE 64
#endif
#define LIB_PROTOCOL_VERSION_SIZE 41

#define SENSOR_CAL_FILE_SIZE		63
#define FM_BUFFER_SIZE			2048

#define WRISTUP_CPU_DEFAULT_FREQ 0        /* 0 GHz */
#define WRISTUP_CPU_MIN_FREQ     1200000  /* 1.2 GHz */
#define WRISTUP_PERIOD_US        500      /* 500 ms */
...
enum {
	...
	DIGITAL_HALL_EVENT_SENSOR_15,	/* [8]=256 */
	LIGHT_SENSOR,                   /* [9]=512 */
	...
	SH_SENSOR_LIGHT_CCT,            /* [29]=536870912 */
...
struct sensor_value {
	union {
		...
		struct  /* cct type, can be changed */
		{
			u32 raw_lux;
			u32 ch0_16ms;
			u32 ch0_891us;
...
#define NUM_OF_CHB_GPR (17)
#define HARDFAULT_INFO_LEN (13)
struct ssp_chub_crash_mini_dump {
	u8 month;
	u8 day;
	u8 hour;
	u8 min;
	u8 sec;
	u32 gpr_dump[NUM_OF_CHB_GPR];
	u32 hardfault_info[HARDFAULT_INFO_LEN];
};
...
struct ssp_data {
	...
	struct workqueue_struct *post_suspend_ts_sync_wq;
	struct workqueue_struct *chub_log_dump_wq;
	struct workqueue_struct *bigdata_wq;
	...
	struct work_struct work_chub_log_dump;
	struct work_struct work_chub_dumpstate;
	struct work_struct work_bigdata;
	struct work_struct work_bigdata_info;
	struct delayed_work wristup_boost_release_work;
	...
	bool bHRMShutdown;
	bool bWakeupByMe;
	bool bSavePostSuspendTime;
	bool bIsWheel;
	bool bIsSleep;

	int aiCheckStatus[SENSOR_MAX];
	u8 acc_esn[ACC_ESN_SIZE];
	...
	char uSsrStuckInfo[SENSOR_STUCK_INFO_SIZE];
	struct ssp_chub_crash_mini_dump crash_mini_dump;
	...
	struct mutex chub_log_dump_mutex;
	struct mutex chub_dram_dump_mutex;
	struct mutex scontext_mutex;
...
#if IS_ENABLED(CONFIG_SEC_PANEL_NOTIFIER_V2)
	struct panel_event_bl_data panel_event_data;
	char ub_con_state;
	char las_copr_status;
#endif
...
#endif
	u8 bypass_logcnt;
	u8 lib_version[LIB_PROTOCOL_VERSION_SIZE];
	u8 bin_type;

	char* chub_log_dump_buf;
	struct class *ssp_mmap_class;
	dev_t ssp_mmap_dev_num;
	dev_t ssp_mmap_dev_num1;
	unsigned int ssp_mmap_dev_major;
	unsigned int ssp_mmap_dev_major1;
	struct device *ssp_mmap_dev;
	struct cdev ssp_mmap_cdev;
	struct cdev ssp_mmap_cdev1;

	struct completion fm_read_done;
	struct completion fm_write_done;
	int fm_tx_buffer_length;
	char fm_rx_buffer[FM_BUFFER_SIZE];
	char fm_tx_buffer[FM_BUFFER_SIZE];

	bool wristup_boost_initialized;
	struct freq_qos_request cpu_cluster_qos;

	u16 msgBeforWakeup[SH_LIBRARY_MAX+1];
	char *pMsgBeforWakeup;
	u32 msgBeforWakeupCnt;
	u32 wrongWakeupCnt;
	int mday;
	u64 prevTick;
	u16 prevWristUpCnt;
...
void ssp_timestamp_suspend_resume_sync_cmd(struct ssp_data *);
...
int initialize_chub_log_dump_node(struct ssp_data *);
...
int sensors_class_init(void);
void  sensors_class_exit(void);
...
void report_hallic_data(struct ssp_data *data, struct sensor_value *hidata, int sensor_id);
...
int ssp_regulator_ctrl(struct regulator *reg, bool enable, int on_delay_ms);
#ifdef CONFIG_SENSORS_SSP_LUCKY
static inline unsigned int get_hrm_vendor(struct ssp_data *data)  {data->hrm_vendor = 1; return 1;}
#else
