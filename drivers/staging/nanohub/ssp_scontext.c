...
void remove_ssp_big(struct ssp_data *data)
{
	...
	if (!PTR_ERR_OR_ZERO(data->big_events.library_data)) {
		...
		data->big_events.library_data = NULL;
...
void ssp_read_big_library_task(struct work_struct *work)
{
	...
	while (residue > 0) {
		...
		ret = ssp_send_command(big->data, msg, 1000);		
		...
		//memcpy((big->data->big_events.library_data + pos), msg->buffer, msg->length);
			...
			const char *buf, int count)
{
	...
	if (buf[2] < MSG2SSP_AP_STATUS_LCD_ON ||
	    ...
	    buf[2] != MSG2SSP_AP_STATUS_HRM_SHUTDOWN)) {
					...
					const char *buf, int count)
{
	...
	} else if (buf[0] == MSG2SSP_INST_LIBRARY_GETVALUE_8 && buf[1] == 0x5 && buf[2] == 0x1) {
		if (ssp_big_queue_work(data, buf[3], false)) {
			return SUCCESS;			
		}
		ssp_errf("BIG - request big data : 0x%x !!", buf[3]);			
