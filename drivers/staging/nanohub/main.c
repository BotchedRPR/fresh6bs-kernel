...
static inline void mcu_wakeup_mailbox_set_value(struct nanohub_data *data,
					     int val)
{
...
static inline void mcu_wakeup_mailbox_get_locked(struct nanohub_data *data,
					      int priority_lock)
{
	...
	if (timeout <= 0) {
		...
		if (!timeout && !priority_lock) {
			...
			nanohub_info("wakeup: timeout:%d/%d, err_cnt:%d, \
				...
				nanohub_irq1_fired(data), current->__state,
				    ...
				    u8 interrupt, bool mask)
{
	...
	do {
		...
		if (ret) {
			nanohub_err("%s: interrupt %d %smask failed: ret=%d\n",
		...
		ret =
		    nanohub_comms_tx_rx_retrans(data, cmd,
						...
						false, 10, 0);
		release_wakeup(data);
		nanohub_debug("%smasking interrupt %d, ret=%d, mask_ret=%d\n",
			mask ? "" : "un",
...
static inline int nanohub_wakeup_lock(struct nanohub_data *data, int mode)
{
	...
	mcu_wakeup_mailbox_set_value(data, mode != LOCK_MODE_IO_BL);
				    ...
				    const char *buf, size_t count)
{
	...
	if (ret) {
		nanohub_dev_err(dev, "nanohub_download_app(%s): err=%d\n",
			buffer, ret);
...
static int nanohub_create_devices(struct nanohub_data *data)
{
	...
	for (i = 0; i < ID_NANOHUB_MAX; ++i) {
		...
		if (IS_ERR(io->dev)) {
			ret = PTR_ERR(io->dev);
			nanohub_err("nanohub: device_create failed for %s; err=%d\n",
			       names[i], ret);
			goto fail_dev;
		}
	}
...
int nanohub_suspend(struct iio_dev *iio_dev)
{
	...
	if (!ret) {
		...
		ret = -EBUSY;
		nanohub_dev_info(&iio_dev->dev,
			 "%s: failed to suspend: IRQ1=%d, state=%d\n",
			 __func__, nanohub_irq1_fired(data),
			 nanohub_get_state(data));
		nanohub_wakeup_unlock(data);
	} else {
		nanohub_dev_info(&iio_dev->dev, "%s: could not take wakeup lock\n",
			 __func__);
	}
