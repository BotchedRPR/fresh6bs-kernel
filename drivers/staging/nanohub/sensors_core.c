...
#else
void *sensors_register(struct device *dev, void *drvdata,
	struct device_attribute *attributes[], char *name)
{
	int ret = 0;
	...
	if (!sensors_class) {
		sensors_class = class_create(THIS_MODULE, "sensors");
		if (IS_ERR(sensors_class))
			return (void *)0; // PTR_ERR(sensors_class);
	}
	...
	pr_err("[SENSORS CORE] device_create2 : 2 [%s:0x%x]\n", name, dev);
...
EXPORT_SYMBOL_GPL(sensors_class_init);
...
EXPORT_SYMBOL_GPL(sensors_class_exit);
