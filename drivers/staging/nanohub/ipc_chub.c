...
static void *ipc_set_map(char *sram_base)
{
	...
	CSP_PRINTF_INFO("dump       (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_DUMP), ipc_get_size(IPC_REG_DUMP));
	CSP_PRINTF_INFO("bl         (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_BL), ipc_get_size(IPC_REG_BL));
	CSP_PRINTF_INFO("os         (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_OS), ipc_get_size(IPC_REG_OS));
	CSP_PRINTF_INFO("ipc        (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_IPC), ipc_get_size(IPC_REG_IPC));
	CSP_PRINTF_INFO("sensormap  (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_IPC_SENSORINFO), ipc_get_size(IPC_REG_IPC_SENSORINFO));
	CSP_PRINTF_INFO("persistbuf (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_PERSISTBUF), ipc_get_size(IPC_REG_PERSISTBUF));
	CSP_PRINTF_INFO("log        (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_LOG), ipc_get_size(IPC_REG_LOG));
	CSP_PRINTF_INFO("ram        (0x%-5x %6x)\n",
			ipc_get_offset(IPC_REG_RAM), ipc_get_size(IPC_REG_RAM));
	return &ipc.ipc_map;
}
