...
static void cipc_func_handle_irq(int evt, void *priv)
{
	...
	case IRQ_EVT_C2A_INT:
		if (atomic_read(&chub->irq1_apInt) == C2A_OFF) {
			atomic_set(&chub->irq1_apInt, C2A_ON);
	...
	case IRQ_EVT_A2C_LPD_END:
		chub->lpd_flag = true;
		wake_up(&chub->lpd_end_wait);
		break;
#endif
	default:
		if (evt < IRQ_EVT_CHUB_MAX) {        
#ifdef CONFIG_SENSORS_SSP
			void *raw_rx_buf = 0;
			u32 size = 0;

			if (contexthub_get_token(chub)) {
				dev_warn(chub->dev, "%s: get token\n", __func__);
				return;
			}
			//raw_rx_buf = ipc_read_data(IPC_DATA_C2A, &size);
			raw_rx_buf = cipc_read_data(CIPC_REG_DATA_CHUB2AP, &size);
            
			//nanohub_info("%s: evt %d, size %lu, buf %p", __func__, evt, size, raw_rx_buf);

			if (size > 0 && raw_rx_buf) {
				clear_err_cnt(chub, CHUB_ERR_READ_FAIL);
				//contexthub_read_process(ipc_rx_buf, (void *)raw_rx_buf, size);
				memcpy_fromio(ipc_rx_buf, (void *)raw_rx_buf, size);
				ssp_handle_recv_packet(chub->ssp_data, ipc_rx_buf,size);
			}
			contexthub_put_token(chub);
			...
			nanohub_dev_err(chub->dev, "%s: invalid %d event",
				 __func__, evt);
			return;            
		}
	};
...
int contexthub_get_sensortype(struct contexthub_ipc_info *chub, char *buf)
{
	...
	if (ipc_have_sensor_info(sensor_map)) {
		pack->num_os = chub->os_name[os_name_idx] - '0';
			       ...
			       enum mailbox_event event)
{
	...
	if (need_ipc) {
		...
		case MAILBOX_EVT_DFS_GOVERNOR:
			...
			break;
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
		/* TODO: add LPD ON/OFF event */
		case MAILBOX_EVT_START_LPD:
			ipc_write_hw_value(IPC_VAL_HW_DEBUG, get_time_lpd());
			ret = cipc_add_evt(CIPC_REG_EVT_AP2CHUB, IRQ_EVT_A2C_LPD_START);
			break;
		case MAILBOX_EVT_STOP_LPD:
			ret = cipc_add_evt(CIPC_REG_EVT_AP2CHUB, IRQ_EVT_A2C_LPD_END);
			break;
#endif
		default:
			/* handle ipc utc */
			 ...
			 uint8_t *tx, int length, int timeout)
{
	...
	if (!IS_ENABLED(CONFIG_SENSOR_DRV)) {
		if (atomic_read(&chub->chub_sleep)) {
			nanohub_dev_warn(chub->dev, "%s: CHUB in suspend, ignoring\n", __func__);
			contexthub_put_token(chub);
			return 0;
		}
	}
...
#endif
#ifdef CONFIG_SENSORS_SSP
	chub->ssp_data = ssp_device_probe(chub->dev);
	if(IS_ERR(chub->ssp_data)) {
		nanohub_dev_info(chub->dev, "[Chub] ssp_probe failed \n");
