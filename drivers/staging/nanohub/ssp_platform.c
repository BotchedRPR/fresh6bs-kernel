...
void ssp_handle_recv_packet(void *ssp_data, char *packet, int packet_len)
{
	...
	ssp_set_wakeup_by_me(data, false);
}
...
int get_sensorhub_dram_size(void)
{
	return get_chub_rmem_size();
}

void *get_sensorhub_dram_ptr(void)
{
    return get_chub_dram_ptr();
}

void ssp_dump_write_file(void *ssp_data, int sec_time, int reason, void *sram_buf, int sram_size, u32 *gpr, int num_gpr, u32 *hardfault_info)
{
	struct ssp_data *data = ssp_data;
	char dump_info[20] = {0,};
	...
	write_ssp_dump_file(data, dump_info, sram_buf, sram_size, reason, gpr, num_gpr, hardfault_info);
...
void ssp_set_logbuf_write_index(void *ssp_data, unsigned int index)
{
	struct contexthub_ipc_info *chub = ((struct ssp_data *)ssp_data)->platform_data;

	nanohub_info("[SSP]: %s,index=%d\n", __func__, index);
	
	chub->chub_rt_log.write_index = index;
}
...
void ssp_set_wakeup_by_me(void *ssp_data, bool flag)
{
	struct ssp_data *data = ssp_data;
	data->bWakeupByMe = flag;
}

void ssp_chub_dump_store(void *ssp_data)
{
	struct contexthub_ipc_info *chub = ((struct ssp_data *)ssp_data)->platform_data;
	chub_dbg_dump_hw(chub, 0xFF);
}

u32 ssp_get_wakeup_by_chub_cnt(void *ssp_data)
{
	struct contexthub_ipc_info *chub = ((struct ssp_data *)ssp_data)->platform_data;

	return chub->wakeup_by_chub_cnt;
}
