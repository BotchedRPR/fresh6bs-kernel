...
static void print_cipc_map(void)
{
	...
	/* step: debug log out */
	CIPC_PRINT(">>> CIPC map info(addr_info): magic:%s, user_cnt:%d\n",
...
static int cipc_set_map(int master)
{
	...
	for (userid = 0; userid < CIPC_USER_MAX; userid++, cipc_user_ival++) {
		...
		if (cipc_user_ival->reg) {
			...
			if (master)
				CIPC_USER_STRNCPY(user_map->evt.magic, CIPC_MAGIC_USER_EVT, sizeof(CIPC_MAGIC_USER_EVT), 1, 0);
			...
			for (j = 0; j < CIPC_USER_DATA_MAX; j++) {
				if (user_info->map_info.data_size[j]) {
					...
					if (master) {
						CIPC_USER_STRNCPY(ipc_data->magic, CIPC_MAGIC_USER_DATA,
							  sizeof(CIPC_MAGIC_USER_DATA), 1, 0);
			...
			if (master)
				CIPC_USER_MEMCPY(&user_map->magic[0], CIPC_MAGIC_USER, sizeof(CIPC_MAGIC_USER), 1, 0);
			...
			remain = 0;
			if ((userid == CIPC_USER_CHUB2AP) || (userid == CIPC_USER_CHUB2AP)) {
	...
	if (master) {
		CIPC_USER_MEMCPY(&cipc_map->magic[0], CIPC_MAGIC, sizeof(CIPC_MAGIC), 1, 0);
		print_cipc_map();
	}
	return 0;
}
			...
			int *size)
{
	...
	for (i = 0; i < CIPC_USER_MAX; i++, user_info++) {
		...
		if (user_info->map_info.owner == owner) {
			...
			//CIPC_PRINT
			//    ("%s loop(%d-%s) find: user-magic:%s, owner_id:%d, owner(addr:+%x, size:%d), cur-user(addr:+%x, size:%d)\n",
	...
	if (onwer_start_off) {
		...
		*size = owner_size + (IPC_ALIGN_SIZE - owner_size % IPC_ALIGN_SIZE);
		...
		//CIPC_PRINT
		//    ("%s done: magic(cipc:%s) owner_id:%d, addr:+%x(+%x), size:%d(%d)\n",
...
int cipc_reset_map(void)
{
	...
	for (i = 0; i < cipc.user_cnt; i++) {
		...
		user = cipc_get_base(reg);
		if (user)
			CIPC_USER_STRNCPY(user->magic, CIPC_MAGIC_USER,
					  sizeof(CIPC_MAGIC_USER), 1, 0);

		/* step: set evt magic */
		reg = user_info->map_info.evt_reg;
		evt = cipc_get_base(reg);
		if (evt)
			CIPC_USER_STRNCPY(evt->magic, CIPC_MAGIC_USER_EVT,
					  sizeof(CIPC_MAGIC_USER_EVT), 1, 0);
		...
		for (j = 0; j < user_info->map_info.data_pool_cnt; j++) {
			...
			if (data) {
				...
				CIPC_USER_STRNCPY(data->magic, CIPC_MAGIC_USER_DATA,
						  sizeof(CIPC_MAGIC_USER_DATA), 1, 0);
			}
		}
		/* step: set cipc magic */
		CIPC_USER_STRNCPY(cipc_map->magic, CIPC_MAGIC,
						  sizeof(CIPC_MAGIC), 1, 0);
	}
	is_cipc_valid();
	return 0;
}
...
static void ipc_print_databuf(enum cipc_region reg)
{
	...
	CIPC_PRINT("%s: data(0x%x,%s): eq:%d dq:%d, size:%d, cnt:%d\n",
	...
	if (__ipc_queue_cnt(&ipc_data->ctrl.qctrl, ipc_data->ctrl.cnt))
		for (i = 0; i < ipc_data->ctrl.cnt; i++) {
			...
			CIPC_PRINT("%s:ch%d(addr:%p, size:0x%x)\n", __func__, i,
...
{
	CIPC_PRINT("MB_SFR : mcutrl - 0x%x\n",
		   (unsigned int)CIPC_RAW_READL(base + REG_MAILBOX_MCUCTL));
	CIPC_PRINT("MB_SFR0: 0x%x 0x%x 0x%x 0x%x 0x%x\n",
	...
	CIPC_PRINT("MB_SFR1: 0x%x 0x%x 0x%x 0x%x 0x%x\n",
...
void cipc_dump(enum cipc_user_id id)
{
	...
	for (i = (reg + 1); i < (reg + 3); i++) {
		CIPC_PRINT("%s: reg:%d, is_evt:%d, is_data:%d, is_user:%d\n",
