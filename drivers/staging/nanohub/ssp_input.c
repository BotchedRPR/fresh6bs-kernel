...
void report_hallic_data(struct ssp_data *data, struct sensor_value *hidata, int sensor_id)
{
	if (hidata->direction == 0) {
		...
		if (sensor_id == DIGITAL_HALL_EVENT_SENSOR_15) {
			data->buf[DIGITAL_HALL_EVENT_SENSOR_15].direction = hidata->direction;
		...
		} else if(sensor_id == DIGITAL_HALL_EVENT_SENSOR) {
			data->buf[DIGITAL_HALL_EVENT_SENSOR].direction = hidata->direction;

			input_report_rel(data->hallic_input_dev, REL_HWHEEL, hidata->direction * BZ_SHORT);
			input_sync(data->hallic_input_dev);

			ssp_wake_lock_timeout(data->ssp_wake_lock, msecs_to_jiffies(600));// 500

			pr_info("[SSP]%s,[%d][%d]-[%d:%d]\n", __func__, hidata->direction * BZ_SHORT, hidata->interrupt, data->irq_state, data->irq_cnt);
		}

		if (data->bWakeupByMe) {
			print_current_time("wheel(S)", false);
			data->bSavePostSuspendTime = true;
			data->bIsWheel = true;
		} else {
			print_current_time("wheel(W)", false);
		}
	}
}
#endif
...
int initialize_input_dev(struct ssp_data *data)
{
	...
	__set_bit(REL_HWHEEL, data->hallic_input_dev->relbit);
	...
	input_set_capability(data->hallic_input_dev, EV_REL, REL_HWHEEL);
