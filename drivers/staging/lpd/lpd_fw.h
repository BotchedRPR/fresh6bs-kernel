...
#define LPD_FW_DRAM_SIZE			0x01000000
...
/* reserved 256kbytes for lpd command */
#define LPD_CMD_RMEM_SIZE	0x40000
...
enum lpd_aux_event {
	IPC_LPD_EVENT_TIME_UPDATE = 0,
	IPC_LPD_EVENT_WRIST_UP,
	IPC_LPD_EVENT_WRIST_DOWN,
	IPC_LPD_EVENT_WRIST_UP_TIMEOUT,
};
...
struct lpd_meta_data {
	struct lpd_sensor_meta sensor_meta;		/* sensor data */
	...
	struct lpd_time_meta time_meta;
	struct lpd_debug_meta debug_meta;
};

#define TOTAL_STRUCT_SIZE (sizeof(struct lpd_meta_data) + sizeof(struct lpd_fw_info)+ \
