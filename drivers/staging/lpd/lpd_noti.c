				    ...
				    unsigned long action, void *data)
{
	...
	lpd_err("==============LPD LOG END===================\n");
	if (IS_LPD_SRAM_AVAILABLE(lpd))
		memlog_do_dump(lpd->debug_info.mlog.memlog_sram, MEMLOG_LEVEL_EMERG);
#endif
	return 0;
}
		...
		unsigned long action, void *nb_data)
{
	...
	case LPD_NOTIFIER_CONFIG:
		...
		if (lpd->enable) {
			dpu_meta = (struct lpd_dpu_meta *)nb_data;
			if (dpu_meta == NULL) {
				lpd_err("null dpu_meta from dpu driver\n");
				return NOTIFY_STOP;
			}
#if defined(LPD_SRAM_POWER_GATING)
			memcpy(&lpd->lpd_meta.dpu_meta, dpu_meta, sizeof(struct lpd_dpu_meta));
#else
			lpd_update_dpu_data(lpd, dpu_info);
			lpd_init_logbuf(lpd);
#endif
			lpd_print_meta_info(lpd);
		}
