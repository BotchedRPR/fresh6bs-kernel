...
static unsigned int hard_reset_keys[] = { KEY_POWER, KEY_APPSELECT };
...
static int load_pmic_key_info(void)
{
	...
	if (dtname[0] == '\0')
		return -1;
#else
	const char *dtname = "s2mpw03-keys";
#endif
			   ...
			   unsigned long type, void *data)
{
	...
	if (code == KEY_POWER){
		pr_info("%s: KEY_POWER KEY(%d) %s\n", __func__ , code, pressed ? "pressed" : "released");
		for (i = 0; i < ARRAY_SIZE(hard_reset_keys); i++) {
			if (code == keys_info[i].keycode) {
				pmic_key_is_pwron = pressed;
			}
		}
	}
...
#else
	hold_time = ktime_set(15, 0); /* 15 seconds */
#endif
