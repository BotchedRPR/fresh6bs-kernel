...
static int32_t sv_sha_init(unsigned int offset, unsigned int data_len)
{
	...
	dcache_clean_inval_poc((unsigned long)h_ctx, (unsigned long)(h_ctx + sizeof(sha_ctx)));
	...
	dcache_inval_poc((unsigned long)h_ctx, (unsigned long)(h_ctx + sizeof(sha_ctx)));

	memset(phys_to_virt((phys_addr_t)sig_buf), 0, SIGNFIELD_SIZE);
	memcpy(phys_to_virt((phys_addr_t)sig_buf), sv_virt + offset, data_len);
	dcache_clean_inval_poc((unsigned long)phys_to_virt((phys_addr_t)sig_buf), (unsigned long)phys_to_virt((phys_addr_t)sig_buf + SIGNFIELD_SIZE));
...
static int sv_sha_update(unsigned int offset, unsigned int data_len)
{
	...
	dcache_clean_inval_poc((unsigned long)h_ctx, (unsigned long)(h_ctx + sizeof(sha_ctx)));
	dcache_clean_inval_poc((unsigned long)phys_to_virt((phys_addr_t)data_buf), (unsigned long)phys_to_virt((phys_addr_t)data_buf + data_len));
...
static int sv_sha_final(void)
{
	...
	dcache_clean_inval_poc((unsigned long)h_ctx, (unsigned long)(h_ctx + sizeof(sha_ctx)));
	dcache_clean_inval_poc((unsigned long)phys_to_virt((phys_addr_t)sha_buf), (unsigned long)phys_to_virt((phys_addr_t)sha_buf + SHA_DIGEST_LEN + RBCNTINFO_SIZE));
	...
	dcache_inval_poc((unsigned long)phys_to_virt((phys_addr_t)sha_buf), (unsigned long)phys_to_virt((phys_addr_t)sha_buf + SHA_DIGEST_LEN + RBCNTINFO_SIZE));
...
static int sv_sig_check_with_hash(void)
{
	...
	dcache_inval_poc((unsigned long)phys_to_virt((phys_addr_t)final_buf), (unsigned long)phys_to_virt((phys_addr_t)final_buf + SHA_DIGEST_LEN));
	...
	dcache_clean_inval_poc((unsigned long)s_ctx, (unsigned long)(s_ctx + sizeof(sb_ctx)));
...
static int sv_sig_check(uint32_t data_len)
{
	...
	dcache_clean_inval_poc((unsigned long)s_ctx, (unsigned long)(s_ctx + sizeof(sb_ctx)));
	dcache_clean_inval_poc((unsigned long)phys_to_virt((phys_addr_t)input_buf), (unsigned long)phys_to_virt((phys_addr_t)input_buf + data_len));
...
int __read_mostly wirelessd;

module_param(wirelessd, int, 0440);
...
static long svsmc_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	ret = copy_from_user(&i_info, ioargp, sizeof(smc_ioctl_info));
	if (ret) {
		pr_err("%s: data copy failed\n", __func__);
		goto out;
	}

	if ((input_buf + TOTAL_BUF_SIZE) < (i_info.offset + i_info.data_len)) {
		pr_err("%s: offset is buffer overflow\n", __func__);
		goto out;
	}
