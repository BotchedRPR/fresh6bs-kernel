...
struct crash_key user_crash_key_combination[] = {
	{KEY_APPSELECT, CRASH_COUNT_FIRST},
	{KEY_POWER, CRASH_COUNT_SECOND},
	{KEY_APPSELECT, CRASH_COUNT_THIRD},
};
...
static bool is_available_crash_key_user;
...
static void reset_count(void)
{
	...
	is_available_crash_key_user = false;
	check_count = 0;
	SEC_LOG("");
}
...
static void increase_count(void)
{
	if (check_count < get_count_for_panic()){
		if (check_count == 0 && check_step == 0){
			is_available_crash_key_user = true;
			hrtimer_start(&crash_key_user_timer,
				limit_time, HRTIMER_MODE_REL);
			pr_info("%s : hrtimer_start\n", "sec_upload");
		}
		check_count++;
	}
	else if (is_available_crash_key_user)
		panic("User Crash Key");
	else{
		pr_info("%s: Crash key user time expired or Last key is released. crash key reset\n", "sec_upload");
		reset_count();
		reset_step();
	}
...
{
	is_available_crash_key_user = false;
	pr_err("Crash key user time expired\n");
