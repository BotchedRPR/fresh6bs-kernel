...
#if IS_ENABLED(CONFIG_BATTERY_SAMSUNG)
static bool sec_check_reboot_charging(void)
{
	bool ac, usb, wireless;
	bool ret;

	sec_set_reboot_magic(SEC_REBOOT_LPM, SEC_REBOOT_END_OFFSET, 0xFF);
	ac = psy_get_online("ac");
	usb = psy_get_online("usb");
	wireless = psy_get_online("wireless");
	ret = ac || usb || wireless;
	pr_info("[%s] reboot_charging(%d), AC[%d], USB[%d], WPC[%d]\n",
		__func__, ret, ac, usb, wireless);
	return ret;
}
#else
static bool sec_check_reboot_charging(void)
...
static void sec_power_off(void)
{
	...
	reboot_charging = sec_check_reboot_charging();
	...
	while (1) {
		...
		if (poweroff_try >= 5) {
			...
			regmap_write(pmureg, panic_inform, SEC_RESET_REASON_PKEY_HOLD);
