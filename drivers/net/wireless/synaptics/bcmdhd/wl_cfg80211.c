...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
		if (cfg->wiphy_lock_held) {
			printf("XXX %s %d wiphy_lock_held, delay\n", __func__, __LINE__);
			schedule_delayed_work(&cfg->remove_iface_work, 0);
		} else
#endif /* LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0) */
		{
			WL_DBG_MEM(("Did not Schedule remove iface work\n"));
			/* Remove interface except for primary ifidx */
			printf("XXX %s %d no wiphy_lock_held, REMOVE NOW\n", __func__, __LINE__);
	...
	s32 *dbm)
{
	...
	if (NULL == dbm) {
		...
		err = -2;
	} else if (NULL == (cfg = wiphy_priv(wiphy))) {
		WL_ERR(("wiphy=0x%p, invalid cfg!\n", wiphy));
		err = -3;
	} else if (NULL == cfg->wdev) {
		WL_ERR(("wiphy=0x%p, cfg=0x%p, invalid wdev!\n", wiphy, cfg));
		err = -4;
	} else if (NULL == (ndev = bcmcfg_to_prmry_ndev(cfg))) {
	...
	struct mgmt_frame_regs * flags)
{
	 ...
	 *         for these subtypes
	 */
	WL_ERR(("dev_type=0x%X(mcast=0x%X), itf_type=0x%X(mcast=0x%X)\n",
...
#if defined(WIPHY_DYNAMIC_UPDATE)
	...
	bool notify, bool user_enforced, int revinfo)
{
	...
	if (ret < 0) {
		...
		/* set 802.11 mode
		   It's safe to call WLC_UP once 'wl country' can execute successful */
		...
		int gain, int eirp)
{
	...
	if (NULL == regd) {
		...
		if (2500 < freq_start) {
			// Non-2.4G ignore the BW40 flag
			interest_flag &= ~CONST_SYNA_REG_FLAG_FILTER_EXTRA_MASK;
		}
		freq_start -= 10;
		freq_end   += 10;
		...
		if (freq_diff < band_width) {
			if (band_width) {
				WL_ERR(("*Warning, freq_range=%d-%d=%d < max_bw=%d, force minimial\n",
				        freq_end, freq_start, freq_diff, band_width));
			}
			band_width = freq_diff;
		}
		...
		WL_DBG(("rule[%d]: start_freq_khz=%d, end_freq_khz=%d, "
		        "max_banddwidth_khz=%d, flags=0x%X %s%s%s%s%s%s%s%s%s%s%s%s\n",
		        index,
		        ...
		        reg_flags,
		        (IEEE80211_CHAN_DISABLED & reg_flags)?"DISABLED ":"",
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0))
		        (IEEE80211_CHAN_NO_IR & reg_flags)?"PASSIVE ":"",
#else /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)) */
		        "",
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 14, 0)) */
		        (IEEE80211_CHAN_RADAR & reg_flags)?"RADAR ":"",
		        (IEEE80211_CHAN_NO_HT40PLUS & reg_flags)?"NO_HT40PLUS ":"",
		        (IEEE80211_CHAN_NO_HT40MINUS & reg_flags)?"NO_HT40MINUS ":"",
		        (IEEE80211_CHAN_NO_OFDM & reg_flags)?"NO_OFDM ":"",
		        (IEEE80211_CHAN_NO_80MHZ & reg_flags)?"NO_80MHz ":"",
		        (IEEE80211_CHAN_NO_160MHZ & reg_flags)?"NO_160MHz ":"",
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
		        (IEEE80211_CHAN_INDOOR_ONLY & reg_flags)?"INDOOR ":"",
#else /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0)) */
		        "",
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0)) */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 2, 0))
		        (IEEE80211_CHAN_IR_CONCURRENT & reg_flags)?"IR_CON ":"",
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0))
		        (IEEE80211_CHAN_GO_CONCURRENT & reg_flags)?"GO_CON ":"",
#else /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)) */
		        "",
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 15, 0)) */
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0))
		        (IEEE80211_CHAN_NO_20MHZ & reg_flags)?"NO_20MHz ":"",
		        (IEEE80211_CHAN_NO_10MHZ & reg_flags)?"NO_10MHz ":""
#else /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0)) */
		        "",
		        ""
#endif /* (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 16, 0)) */
...
int wl_get_bss_info(struct bcm_cfg80211 *cfg, struct net_device *dev, const u8 *mac)
{
	...
	*(u32 *) cfg->extra_buf = htod32(WL_EXTRA_BUF_MAX);
	...
	if (err) {
		...
		rate = dtoh32(rate);
		snprintf(rate_str, sizeof(rate_str), "%d", (rate/2)%10000u);
	}
...
wl_handle_link_down(struct bcm_cfg80211 *cfg, wl_assoc_status_t *as)
{
	...
	if (memcmp(as->curbssid, as->addr, ETHER_ADDR_LEN) != 0) {
		if (	// some disconnect early before sending event case will
			...
			|| (0 == memcmp(&cfg->start_roam_addr, as->addr, ETHER_ADDR_LEN))
#endif // WLFBT
		   ) {
...
#ifdef WLFBT
		/* When roaming,After WLC_E_AUTH SUCCESS,fw will clear PENDING_AUTH flag,starting ASSOC REQ.
		 * But During this time,if device received special packets,such as PS_POLL,BLOCKACK,BLOCKACK_REQ
		 * coming from origin router(due to two routers in the same band),fw will send DEAUTH packets
		 * to origin router and sendup WLC_E_DEAUTH to driver,because the WLC_E_DEAUTH event came earlier
		 * than WLC_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,it will cause uplayer thinking the roaming was fail.
		 * In fw logical,when roaming complete,it must wait for the first beacon frame of the roamed router,
		 * then will sendup WLC_E_JOIN/WLC_E_BSSID/WLE_E_ROAM event to driver.
		 * So do the patch below :
		 * 1, After driver received WLC_E_AUTH SUCCESS event,save the roamed addr.
		 * 2, Between WLC_E_AUTH SUCCESS and WLC_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,if received WLC_E_DEAUTH,
		 *    check the addr whether the same with the saved roamed addr,if not,just ingore this event.
		 * 3, After driver received WLE_E_JOIN/WLC_E_BSSID/WLC_E_ROAM,clear the saved roamed addr.
		 */
		...
		event_param_t  *event_param, int need_enqueue)
{
	...
	if (NULL == cfg) {
		WL_ERR(("*invalid cfg!\n"));
		return BCME_BADARG;
	} else if (NULL == event_param) {
		WL_ERR(("*invalid event_param!\n"));
		return BCME_BADARG;
	}
	if (NULL == event_param->ndev) {
		if (NULL == (wdev = cfg->wdev)) {
			WL_ERR(("*invalid ndev!\n"));
			return BCME_BADARG;
		}
	} else if (NULL == (wdev = event_param->ndev->ieee80211_ptr)) {
		if (NULL == (wdev = cfg->wdev)) {
			WL_ERR(("*invalid wdev!\n"));
			return BCME_BADARG;
		}
	}
	if (NULL == (wiphy = wdev->wiphy)) {
		WL_ERR(("*invalid wiphy!\n"));
		return BCME_BADARG;
	} else {
	...
	s32 bw_cap_5g, s32 bw_cap_6g)
{
	...
	for (i = 0; i < dtoh32(list_count); i++) {
		...
		if (CHSPEC_IS2G(chspec) && (channel >= CH_MIN_2G_CHANNEL) &&
			...
			ht160_allowed = WL_BW_CAP_80MHZ(bw_cap_6g);
		}
#endif /* CFG80211_6G_SUPPORT */
		else if (
#ifdef WL_6G_BAND
			...
			*/
			...
			ht160_allowed = WL_BW_CAP_80MHZ(bw_cap_5g);
		} else {
			...
			if (0 != min_ch) {
				...
				for (j = 0; j < array_size; j++) {
					if (   (min_ch <= band_chan_arr[j].hw_value)
						...
						WL_DBG(("    side_band, CH[%d]=%d, mask=0x%X, value=0x%X, chspec=0x%X, "
						        "channel=%d, flags=0x%X %s%s%s%s%s%s%s%s%s%s%s%s\n",
						        j, band_chan_arr[j].hw_value, mask, value,
						        chspec, channel, flags,
...
static s32 __wl_update_wiphybands(struct bcm_cfg80211 *cfg, bool notify)
{
	...
	if (   (NULL == bands[IEEE80211_BAND_2GHZ])
	    && (NULL == bands[IEEE80211_BAND_5GHZ])
#ifdef CFG80211_6G_SUPPORT
	    && (NULL == bands[IEEE80211_BAND_6GHZ])
#endif /* CFG80211_6G_SUPPORT */
	   ) {
...
static s32 __wl_cfg80211_up(struct bcm_cfg80211 *cfg)
{
	...
	*/
	...
	if (NULL == global_regd_self) {
		...
		global_regd_self = KVZALLOC(regd_len, GFP_ATOMIC);
		if (global_regd_self) {
...
static void wl_cfg80211_wbtext_update_rcc(struct bcm_cfg80211 *cfg, struct net_device *dev)
{
	...
	if (!req_sent) {
		if ((cap_ie = bcm_parse_tlvs(conn_info->resp_ie, conn_info->resp_ie_len,
			...
			if (unlikely(0 > ret)) {
				WL_ERR(("skip since fail to get wnm for rcc update, ret=%d\n", ret));
			} else if (0 == (WL_WNM_BSSTRANS & wnm)) {
...
bool
wl_cfg80211_check_is2G20(struct net_device *ndev) {
	...
	u32 event_type, const wl_event_msg_t *e, void *data)
{
	...
	if (dhdp->roam_event_rx) {
		...
		WL_INFORM(("Roaming %d status %d reason %d freq %d chanspec %x"MACDBG,
			event_type, status, reason, freq, chanspec?(*chanspec):0x00, MAC2STRDBG(curbssid)));
		...
		if (freq && notify_channel)
			nla_put(skb, BRCM_ROAMING_STATUS_ATTR_CHANNEL, sizeof(struct ieee80211_channel), notify_channel);
		if (curbssid)
...
wl_cfg80211_btc_mode(struct bcm_cfg80211 *cfg, bool on)
{
	...
	if (NULL == (ndev = bcmcfg_to_prmry_ndev(cfg))) {
