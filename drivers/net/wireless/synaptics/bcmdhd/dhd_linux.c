...
extern int dhd_get_monitor_data(dhd_pub_t *dhdp, void *pktbuf,
		struct sk_buff *skb, void *pktdata, uint pktlen);
#else
int dhd_get_monitor_data(dhd_pub_t *dhdp, void *pktbuf, struct sk_buff *skb,
		void *pktdata, uint pktlen)
{
...
static int dhd_pm_callback(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	...
	DHD_ERROR(("%s: Enter with action=0x%X\n", __FUNCTION__, (int)action));
	...
	DHD_ERROR(("%s: Exit with ret=%d, suspend=%d\n", __func__, ret, suspend));
...
dhd_bssidx2bssid(dhd_pub_t *dhdp, int idx)
{
	 ...
	 */
	if (idx >= DHD_MAX_IFS) {
		if (idx < (2*DHD_MAX_IFS)) {
			idx -= DHD_MAX_IFS;
			if (dhd->iflist[idx]) {
				return dhd->iflist[idx]->mac_addr;
			} else {
				return NULL;
			}
		} else {
...
BCMFASTPATH(__dhd_sendpkt)(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
...

	{
...
#endif /* !PKTPRIO_OVERRIDE */
		{
			...
			if (PKTPRIO(pktbuf) != PRIO_8021D_BE && PKTPRIO(pktbuf) != PRIO_8021D_BK) {
				skb_orphan(pktbuf);
			}
#endif
#else
...
#ifndef DHD_NO_ORPHAN_FOR_ALL_AC
	if (PKTPRIO(skb) != PRIO_8021D_BE && PKTPRIO(skb) != PRIO_8021D_BK) {
		skb_orphan(skb);
	} else
#endif
	if (skb->sk) {
	...
	while (1) {
		if (down_interruptible(&tsk->sema) == 0) {
			...
			while (skb) {
				...
				if (skb) {
					uint8 *pptr = PKTDATA(pub->osh, skb);
					/* we are IP packet at this point 'dhd_rxf_thread' */
					// uint8 *pMAC = pptr;
					...
					if (is_rtp) {
						pr_err("%s-%d: RX ptr=0x%p,protocol=0x%04X  RTP:tag=0x%X,"
						       "SSRC=0x%X,seq=0x%X(%d),time=0x%x(%d)\n",
						       __FUNCTION__, __LINE__, skb, proto,
						       tag, rtp_ssrc, rtp_seq, rtp_seq, rtp_time, rtp_time);
						if ((previous_rtp_seq + 1) != rtp_seq) {
							pr_err("%s-%d: ***Warning, previous_RTP_seq=0x%X(%d),"
							       " current_seq=0x%X(%d)\n",
							       __FUNCTION__, __LINE__, previous_rtp_seq,
							       previous_rtp_seq, rtp_seq, rtp_seq);
						}
						previous_rtp_seq = rtp_seq;
					} else {
						pr_err("%s-%d: RX ptr=0x%p,protocol=0x%04X,tag=0x%X\n",
						       __FUNCTION__, __LINE__, skb, proto, tag);
					}
				}
#endif /* SYNA_RTP_DUMP */
...
dhd_del_monitor_if(dhd_info_t *dhd)
{
	...
	if (FW_SUPPORTED((&dhd->pub), monitor) && (dhd->pub.up)) {
		...
		ret = dhd_iovar(&dhd->pub, 0, "scansuppress", (char *)&scan_suppress,
				sizeof(scan_suppress), NULL, 0, TRUE);
		if (ret < 0) {
			DHD_ERROR(("%s: scansuppress set failed, ret=%d(ignore when remove)\n", __FUNCTION__, ret));
		}
	}
#endif /* WL_CFG80211_MONITOR */
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc, void *data_buf)
{
	...
	if (WLC_SET_GMODE == ioc->cmd) {
		...
		} else if ((0 == strncmp("mode_reqd", data_buf, 10))
#if defined(WL_CFG80211)
...
#ifdef DHD_UPDATE_INTF_MAC
int
dhd_event_iflink(struct dhd_info *dhdinfo, wl_event_msg_t *event, wl_event_data_if_t *ifevent, char *name, uint8 *mac)
{
	dhd_if_event_t *if_event;
...
/* xxx andrey tmp fix for dk8000 build error  */
	...
	defined(DISABLE_WL_FRAMEBURST_2G20_TDLS)
...
int dhd_ioctl_entry_local(struct net_device *net, wl_ioctl_t *ioc, int cmd)
{
	...
	ifidx = dhd_net2idx(dhd, net);
	if (ifidx == DHD_BAD_IF) {
		DHD_ERROR(("%s bad ifidx", __FUNCTION__));
		DHD_ERROR((" cmd:%d buf:%s\n",
			ioc->cmd, ioc->buf?(char*)ioc->buf:""));
		return -ENODEV;
	}
