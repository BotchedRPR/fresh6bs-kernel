...
/* XXX Bootloader Instruction Definition for 43711 */
#define ARMCR4_BOOTLOADER_INSTRUCTION          0xB83EF000
...
static bool dhdsdio_is_rom_protect_set(dhd_bus_t *bus);
#if defined(FW_SIGNATURE)
static bool dhdsdio_is_vector_protect_set(dhd_bus_t *bus);
...
static int dhdsdio_read_fwstatus(dhd_bus_t *bus, bcm_fwsign_verif_status_t *status);
#ifdef BL_SIM_OTP
...
dhdsdio_sr_init(dhd_bus_t *bus)
{
	...
	if (!(wkctl & val)) {
		DHD_ERROR(("%s: Set wake up ctrl failed. SBSDIO_FUNC1_WAKEUPCTRL=%08x, expect %08x\n",
...
dhdsdio_clk_kso_enab(dhd_bus_t *bus, bool on)
{
	...
	if ((try_cnt > CUSTOM_MAX_KSO_ATTEMPTS) || timedout)  {
		DHD_ERROR(("%s> op:%s, ERROR: try_cnt:%d, tout:%d rd_val:%x, ERR:%x \n",
			__FUNCTION__, (on ? "KSO_SET" : "KSO_CLR"), try_cnt, timedout, rd_val, err));
	}
...
dhdsdio_clk_devsleep_iovar(dhd_bus_t *bus, bool on)
{
	...
	if (on == TRUE) {
		...
		if (err == 0) {
			...
			DHD_ERROR(("%s: ExitSleep sleepcsr: 0x%x\n", __FUNCTION__, csr));
...
dhdsdio_read_bl_cons_log(dhd_bus_t *bus)
{
	...
	while (!dhd_timeout_expired(&tmo)) {
		if (cons_addr == 0) {
			...
			ret =  dhdsdio_membytes(bus, FALSE, BL_CONS_ADDR_REG,
				(uint8 *)&cons_addr, sizeof(cons_addr));
			if (ret != BCME_OK) {
				DHD_ERROR(("%s: error %d on reading %zu membytes at 0x%08x\n",
					__FUNCTION__, ret, sizeof(cons_addr), BL_CONS_ADDR_REG));
			} else {
				if (cons_addr != 0) {
					...
					if (cons_addr == 0xffffffff) {
						dhdsdio_membytes(bus, TRUE, BL_CONS_ADDR_REG,
								(uint8 *)&cons_addr, sizeof(cons_addr));
			...
			bootrom_ret =  dhdsdio_membytes(bus, FALSE, BL_CONS_ADDR_REG,
				(uint8 *)&cons_addr, sizeof(cons_addr));
			if (bootrom_ret != BCME_OK) {
				DHD_ERROR(("%s: error %d on reading %zu membytes at 0x%08x\n",
					__FUNCTION__, bootrom_ret,
					sizeof(cons_addr), BL_CONS_ADDR_REG));
				return BCME_OK;
			} else {
	...
	if (enter) {
		...
		if (!si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) {
...
#if defined(FW_SIGNATURE)
			if ((dhdsdio_is_vector_protect_set(bus) == TRUE) ||
					bus->fwsig_filename[0] != 0) {
				DHD_ERROR(("%s: Secure Boot %s, Signature File %s "
						"Entering Secure Boot Mode \n",
						__FUNCTION__, ((vector_protect == TRUE)?
						"Enabled":"Disabled"),
						bus->fwsig_filename));
				if ((bcmerror =
			...
			if (do_wr_flops) {
				...
				if (!dhdsdio_is_rom_protect_set(bus) && bcmerror == BCME_OK) {
					uint32 tmp;
...
static bool dhdsdio_is_rom_protect_set(dhd_bus_t *bus)
{
	...
	if ((cr4regs = si_setcore(bus->sih, ARMCR4_CORE_ID, 0)) != NULL) {
		mpu_ctrl_reg = R_REG(bus->dhd->osh, ARM_CR4_REG(cr4regs, mpucontrol));
		DHD_INFO(("%s: reg:0x%x val:0x%x\n",
			__FUNCTION__, (uint)OFFSETOF(cr4regs_t, mpucontrol), mpu_ctrl_reg));
	}

	rom_protect = ((mpu_ctrl_reg & (1 << ROM_PROTECT_BIT)) >> ROM_PROTECT_BIT);
...
static bool dhdsdio_is_vector_protect_set(dhd_bus_t *bus)
{
	...
	vector_protect = ((mpu_ctrl_reg & (1 << VECTOR_PROTECT_BIT)) >> VECTOR_PROTECT_BIT);

#ifdef BL_RAM_BOOT
	/* 4612 Sboot on RAM */
	return TRUE;
#endif /* BL_RAM_BOOT */
	return vector_protect;
}
...
#else
	/* Run Bootloader from DHD */
	bus->resetinstr = ARMCR4_BOOTLOADER_INSTRUCTION;
#endif /* BL_RAM_BOOT */
...
dhdsdio_verify_sig_file_size(dhd_bus_t *bus, char *path)
{
	...
	/* Fetch File Size */
	sig_act_size = dhd_os_get_image_size(filep);
...
#ifdef DHD_METADATA_DOWNLOAD
	if ((metadata.enabled == TRUE) && (metadata.sign_file.type == SIGNATURE_FILE)) {
...
static int
dhdsdio_read_fwstatus(dhd_bus_t *bus, bcm_fwsign_verif_status_t *status)
{
	int ret = BCME_OK;
...
#if defined(FW_SIGNATURE)
	if (vector_protect == TRUE) {
		bcm_fwsign_verif_status_t status;

		if (dhdsdio_read_fwstatus(bus, &status) == BCME_OK) {
...
dhd_is_metadata_present(dhd_bus_t *bus, char *fwpath)
{
	...
	file = dhd_os_open_image1(bus->dhd, fwpath);
	if (!file) {
		ret = BCME_ERROR;
		goto exit;
	}
	...
	if (val & 0x4) {
		for (i = 0; i < 41; i++) {
			printk("glom %02d: %d, free time %d, free one pkt time %d\n",
				...
				sdio_transfer_glom_list[i][1] ? (sdio_transfer_glom_list[i][1] / sdio_transfer_glom_list[i][0] / i) : 0);
