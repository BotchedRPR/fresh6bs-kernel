...
void slsi_rx_scan_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	mgmt_len = fapi_get_mgmtlen(skb) > 0 ? fapi_get_mgmtlen(skb) : 0;
	if (mgmt_len > offsetof(struct ieee80211_mgmt, u.probe_resp.variable)) {
		ie_len = mgmt_len - offsetof(struct ieee80211_mgmt, u.probe_resp.variable);
	} else {
		SLSI_NET_ERR(dev, "Invalid ie_len = %d - %d\n", mgmt_len,
			     ...
			     struct slsi_acs_chan_info *ch_info)
{
	...
	if (acs_selected_channels->ch_width == 80) {
		...
		if (all_bss_load || min_avg_chan_utilization <= 512) {
			...
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_ap].chan + 4;
...
int slsi_is_40mhz_6gchan(u8 pri_channel, u8 last_channel)
{
	int slsi_40mhz_chan[15] = {3, 19, 35, 51, 67, 83, 99, 115, 131, 147, 163, 179, 195, 211, 227};
	int i;
...
int slsi_is_80mhz_6gchan(u8 pri_channel, u8 last_channel)
{
	int slsi_80mhz_chan[14] = {7, 23, 39, 55, 71, 87, 103, 119, 135, 151, 167, 183, 199, 215};
	int i;
...
int slsi_is_160mhz_6gchan(u8 pri_channel, u8 last_channel)
{
	int slsi_160mhz_chan[7] = {15, 47, 79, 111, 143, 175, 207};
	int i;
...
int slsi_set_6g_auto_channel(struct slsi_dev *sdev, struct netdev_vif *ndev_vif,
			     ...
			     struct slsi_acs_chan_info *ch_info)
{
	int i = 0, avg_load = 0, total_num_ap = 0;
	bool all_bss_load = true, none_bss_load = true;
	int min_num_ap = INT_MAX, min_avg_chan_utilization = INT_MAX;
	int ch_idx_min_load = 0, ch_idx_min_ap = 0;
	int min_avg_chan_utilization_20 = INT_MAX, min_num_ap_20 = INT_MAX;
	...
	if (acs_selected_channels->ch_width == 160) {
		for (i = 0; i < ch_list_len; i++) {
			if (i + 7 >= ch_list_len)
				continue;
			if (!ch_info[i].chan || !ch_info[i + 1].chan || !ch_info[i + 2].chan || !ch_info[i + 3].chan ||
			    !ch_info[i + 4].chan || !ch_info[i + 5].chan || !ch_info[i + 6].chan || !ch_info[i + 7].chan)
				continue;
			if (slsi_is_160mhz_6gchan(ch_info[i].chan, ch_info[i + 7].chan)) {
				int j;

				for (j = i; j < i + 8; j++) {
					avg_load += ch_info[j].avg_chan_utilization;
					total_num_ap += ch_info[j].num_ap;
				}
				...
				if (avg_load < min_avg_chan_utilization) {
					...
					ch_idx_min_load = i;
					   ...
					   (ch_info[ch_idx_min_load].num_ap + ch_info[ch_idx_min_load + 1].num_ap +
						ch_info[ch_idx_min_load + 2].num_ap +
						ch_info[ch_idx_min_load + 3].num_ap + ch_info[ch_idx_min_load + 4].num_ap +
						ch_info[ch_idx_min_load + 5].num_ap + ch_info[ch_idx_min_load + 6].num_ap +
						ch_info[ch_idx_min_load + 7].num_ap)) {
					ch_idx_min_load = i;
				}
				if (total_num_ap < min_num_ap) {
					min_num_ap = total_num_ap;
		...
		if (all_bss_load || min_avg_chan_utilization <= 1024) {
			acs_selected_channels->pri_channel = ch_info[ch_idx_min_load + 1].chan;
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_load + 1].chan + 16;
			acs_selected_channels->vht_seg1_center_ch = ch_info[ch_idx_min_load].chan + 14;
		} else if (none_bss_load || min_avg_chan_utilization > 1024) {
			acs_selected_channels->pri_channel = ch_info[ch_idx_min_ap + 1].chan;
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_ap + 1].chan + 16;
			acs_selected_channels->vht_seg1_center_ch = ch_info[ch_idx_min_ap].chan + 14;
		}
	...
	if (acs_selected_channels->ch_width == 80) {
		for (i = 0; i < ch_list_len; i++) {
			...
			if (!ch_info[i].chan || !ch_info[i + 1].chan || !ch_info[i + 2].chan || !ch_info[i + 3].chan)
				continue;
			if (slsi_is_80mhz_6gchan(ch_info[i].chan, ch_info[i + 3].chan)) {
				avg_load = ch_info[i].avg_chan_utilization + ch_info[i + 1].avg_chan_utilization +
					   ch_info[i + 2].avg_chan_utilization + ch_info[i + 3].avg_chan_utilization;
				total_num_ap = ch_info[i].num_ap + ch_info[i + 1].num_ap + ch_info[i + 2].num_ap +
						   ch_info[i + 3].num_ap;
		...
		if (all_bss_load || min_avg_chan_utilization <= 512) {
			acs_selected_channels->pri_channel = ch_info[ch_idx_min_load + 1].chan;
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_load + 1].chan + 8;
			acs_selected_channels->vht_seg0_center_ch = ch_info[ch_idx_min_load].chan + 6;
		} else if (none_bss_load || min_avg_chan_utilization > 512) {
			acs_selected_channels->pri_channel = ch_info[ch_idx_min_ap + 1].chan;
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_ap + 1].chan + 8;
			acs_selected_channels->vht_seg0_center_ch = ch_info[ch_idx_min_ap].chan + 6;
		}
	...
	if (acs_selected_channels->ch_width == 40) {
		for (i = 0; i < ch_list_len; i++) {
			if (!ch_info[i].chan || i + 1 >= ch_list_len || !ch_info[i + 1].chan)
				continue;
			if (slsi_is_40mhz_6gchan(ch_info[i].chan, ch_info[i + 1].chan)) {
				avg_load = ch_info[i].avg_chan_utilization + ch_info[i + 1].avg_chan_utilization;
				total_num_ap = ch_info[i].num_ap + ch_info[i + 1].num_ap;
				if (avg_load < min_avg_chan_utilization) {
					min_avg_chan_utilization = avg_load;
		...
		if (all_bss_load || min_avg_chan_utilization <= 256) {
			acs_selected_channels->pri_channel = ch_info[ch_idx_min_load + 1].chan;
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_load].chan;
		} else if (none_bss_load || min_avg_chan_utilization > 256) {
			acs_selected_channels->pri_channel = ch_info[ch_idx_min_ap + 1].chan;
			acs_selected_channels->sec_channel = ch_info[ch_idx_min_ap].chan;
		}
