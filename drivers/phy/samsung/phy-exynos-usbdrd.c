...
static int exynos_usbdrd_get_phyinfo(struct exynos_usbdrd_phy *phy_drd)
{
	...
	if (!of_property_read_u32(dev->of_node, "low_power_call", &value)) {
		phy_drd->usbphy_info.low_power_call = value ? true : false;
		dev_info(dev, "<<< low_power_call = %d\n", phy_drd->usbphy_info.low_power_call);
	} else {
		dev_err(dev, "can't get low_power_call\n");
		phy_drd->usbphy_info.low_power_call = false;
	}
							...
							int phy_state)
{
	...
	if (!hs_tune_param) {
		dev_info(phy_drd->dev, "%s hs_tune_pamam is NULL. Check tune val in dt ", __func__);
		return;
	}
...
extern void get_exynos_abox_device(void);
extern int exynos_usbdrd_phy_set_common(struct exynos_usbdrd_phy *phy_drd,
					enum phy_mode mode, int submode);
#endif

int exynos_usbdrd_phy_set_mode(struct phy *phy, enum phy_mode mode, int submode)
{
	struct phy_usb_instance *inst = phy_get_drvdata(phy);
	struct exynos_usbdrd_phy *phy_drd = to_usbdrd_phy(inst);
	int ret = 0;

	if (mode < PHY_MODE_BUS_SUSPEND)
		inst->phy_cfg->phy_set(phy_drd, mode, NULL);
#ifdef CONFIG_SND_EXYNOS_USB_AUDIO_GIC
	else
		ret = exynos_usbdrd_phy_set_common(phy_drd, mode, submode);
#endif

	return ret;
}
EXPORT_SYMBOL_GPL(exynos_usbdrd_phy_set_mode);
...
int exynos_usbdrd_phy_bc_check(struct phy *phy)
{
	struct phy_usb_instance *inst = phy_get_drvdata(phy);
	struct exynos_usbdrd_phy *phy_drd = to_usbdrd_phy(inst);
	enum exynos_usb_bc chg_port;

	dev_info(phy_drd->dev, ">> Check the PHY BC\n");
	/* check the data contact detection */
	phy_exynos_usb3p1_bc_data_contact_detect(&phy_drd->usbphy_info);

	chg_port = phy_exynos_usb3p1_bc_battery_charger_detection(&phy_drd->usbphy_info);
	switch (chg_port) {
	case BC_SDP:
		dev_info(phy_drd->dev, "SDP(Standard  Downstream Port: Normal USB Host\n");
		goto USB;
	case BC_DCP:
		dev_info(phy_drd->dev, "DCP(Dedicated Charging Port: TA\n");
		goto TA;
	case BC_CDP:
		dev_info(phy_drd->dev, "CDP(Charging Downstream Port: High Power Host\n");
		goto USB;
	default:
		dev_info(phy_drd->dev, "Unknown cable type\n");
		goto unknown;
	}
unknown:
TA:
...
EXPORT_SYMBOL_GPL(exynos_usbdrd_phy_bc_check);
...
static struct phy_ops exynos_usbdrd_phy_ops = {
	...
	.set_mode	= exynos_usbdrd_phy_set_mode,
	.owner		= THIS_MODULE,
};
