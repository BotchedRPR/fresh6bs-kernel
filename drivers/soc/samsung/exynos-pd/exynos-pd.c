...
#endif
	if (pd->rgt) {
		pr_info("%s [%s] : regulator enabled\n", EXYNOS_PD_PREFIX, pd->name);
		ret = regulator_enable(pd->rgt);
		if (ret)
			pr_err("%s [%s] : regulator enable fail.\n", EXYNOS_PD_PREFIX, pd->name);
	}
}
...
#endif
	if (pd->rgt) {
		pr_info("%s [%s] : regulator disabled\n", EXYNOS_PD_PREFIX, pd->name);
		ret = regulator_disable(pd->rgt);
		if (ret)
			pr_err("%s [%s] : regulator disable fail.\n", EXYNOS_PD_PREFIX, pd->name);
	}
}
...
static bool exynos_pd_power_down_ok_dispaud(void)
{
	bool disp_ret = true;
	bool aud_ret = true;
...
static void of_get_power_down_ok(struct exynos_pm_domain *pd)
{
	...
	if (ret)
		...
		case PD_OK_USB:
			...
			break;
		case PD_OK_DISPAUD:
			pd->power_down_ok = exynos_pd_power_down_ok_dispaud;
			break;
		default:
			break;
		}
...
static int exynos_pd_resume_early(struct device *dev)
{
	...
	if (of_property_read_bool(dev->of_node, "pd-always-on")) {
		...
		dev_info(dev, EXYNOS_PD_PREFIX "    %-9s - %s\n", data->genpd.name,
				"on,  always");
#if defined(CONFIG_SOC_S5E9935)
		if (!strcmp(data->genpd.name, "pd_m2m") &&
...
static int exynos_pd_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_bool(np, "rgt-supply");
	if (ret) {
		pd->rgt = devm_regulator_get(&pdev->dev, "rgt");
		if (IS_ERR(pd->rgt)) {
			dev_dbg(&pdev->dev, "Error getting regulator: %ld\n",
					PTR_ERR(pd->rgt));
			goto defer_err;
		}
		pr_info("%s [%s] : regulator get\n", EXYNOS_PD_PREFIX, pd->name);
		if (pd->rgt) {
			pr_info("%s [%s] : regulator enabled\n", EXYNOS_PD_PREFIX, pd->name);
			ret = regulator_enable(pd->rgt);
			if (ret)
	...
	if (ret) {
		dev_err(dev, EXYNOS_PD_PREFIX "%s: cmu_init fail: %s, ret:%d\n",
				__func__, pd->name, ret);
		return ret;
	}
...
#if defined(CONFIG_SOC_S5E9935)
	if (!strcmp(pd->name, "pd_m2m") && (exynos_soc_info.main_rev == 1)) {
		pd->skip_idle_ip = false;
		pd->idle_ip_index = exynos_get_idle_ip_index(pd->name, 1);
	}
#endif
