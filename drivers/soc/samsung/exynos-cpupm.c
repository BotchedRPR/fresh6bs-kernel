...
static void wakeup_mask_free(void)
{
	if (wm_config) {
...
{
	struct device_node *root_dn, *wm_dn, *ewm_dn, *dn;
	int count, i;

	root_dn = of_get_child_by_name(cpupm_dn, "wakeup-mask");
	if (!root_dn) {
		pr_warn("wakeup-mask is omitted in device tree\n");
		return;
	}

	wm_dn = of_get_child_by_name(root_dn, "wakeup-masks");
	if (!wm_dn) {
		pr_warn("wakeup-masks is omitted in device tree\n");
		return;
	}

	ewm_dn = of_get_child_by_name(root_dn, "eint-wakeup-masks");
	if (!ewm_dn) {
	...
	for_each_child_of_node(wm_dn, dn) {
		struct wakeup_mask *wm = &wm_config->wakeup_masks[i++];

		of_property_read_u32(dn, "mask-reg-offset", &wm->mask_reg_offset);
		of_property_read_u32(dn, "stat-reg-offset", &wm->stat_reg_offset);
		of_property_read_u32(dn, "mask", &wm->mask);
	}

	/* Parse eint-wakeup-masks node */
	count = of_get_child_count(ewm_dn);
	...
	i = 0;
	for_each_child_of_node(ewm_dn, dn) {
