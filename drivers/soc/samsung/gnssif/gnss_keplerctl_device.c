...
static int kepler_release_reset(struct gnss_ctl *gc)
{
	...
	ret = gc->pmu_ops->req_baaw(gc);
	if (ret) {
		gif_err("req_baaw error! %d\n", ret);
		return ret;
	}
...
static int kepler_power_on(struct gnss_ctl *gc)
{
	...
	if (gc->irq_gnss_active.registered) {
...
static int kepler_resume(struct gnss_ctl *gc)
{
	...
	gnss_pmu_read(EXYNOS_PMU_GNSS_IN, &gnss_in);
	gif_info("GNSS_IN: 0x%08X", gnss_in);
