				   ...
				   unsigned int trans_type)
{
	...
	char temp_buf[SZ_128];
#endif
#if IS_ENABLED(CONFIG_SEC_DEBUG_AUTO_COMMENT) || IS_ENABLED(CONFIG_SEC_DEBUG_EXTRA_INFO)
	/* SEC DEBUG FEATURE */
	...
	if (p_itmon_history) {
		...
		if (strcmp(info->port, "See M_NODE of Raw registers") == 0)
			port = "MIDNODE";
		else
			port = info->port;
		...
		snprintf(p_itmon_history->data[p_itmon_history->idx], sizeof(p_itmon_history->data[0]),
				...
				port, info->master ? info->master : "",
				info->dest ? info->dest : NO_NAME,
				info->target_addr,
				itmon_errcode_short[info->errcode]);
