...
int itmon_history_max_size;
int itmon_history_idx;
char (*itmon_history)[ITMON_HISTROY_STRING_SIZE];
int *itmon_history_idx_base_addr;
...
{
	if (itmon_history_idx_base_addr)
		__raw_writel(HISTORY_MAGIC, &itmon_history_idx_base_addr[HISTORY_MAGIC_OFFSET]);
}
...
void itmon_print_history(void)
{
	...
	if (itmon_history) {
		if (strlen(itmon_history[itmon_history_idx]) != 0)
			for (idx = itmon_history_idx; idx < itmon_history_max_size; idx++)
				pr_info("%s/", itmon_history[idx]);
		else if (itmon_history_idx == 0)
			return;
		for (idx = 0; idx < itmon_history_idx-1; idx++)
			pr_cont("%s/", itmon_history[idx]);
		if (strlen(itmon_history[itmon_history_idx-1]) != 0)
			pr_cont("%s\n", itmon_history[itmon_history_idx-1]);
	}
}
				   ...
				   unsigned int trans_type)
{
	...
	if (itmon_history) {
		snprintf(itmon_history[itmon_history_idx], sizeof(itmon_history[0]),
		...
		itmon_history_idx = (itmon_history_idx + 1) % itmon_history_max_size;
		__raw_writel(itmon_history_idx, &itmon_history_idx_base_addr[HISTORY_IDX_OFFSET]);
	}
}
...
static int itmon_history_rmem_setup(struct itmon_dev *itmon)
{
	...
	itmon_history_idx_base_addr = (int *)vaddr;
	itmon_history_idx_base_addr[HISTORY_IDX_OFFSET] = -1;
	itmon_history = (char (*)[50])(vaddr + sizeof(int) * HISTORY_OFFSET_MAX);
	itmon_history_max_size = rmem->size / ITMON_HISTROY_STRING_SIZE;
	itmon_history_idx_base_addr[HISTORY_SIZE_OFFSET] = itmon_history_max_size;
	itmon_history_idx = 0;
