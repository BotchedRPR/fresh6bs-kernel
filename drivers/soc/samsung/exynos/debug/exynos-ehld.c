...
struct ehld_dev {
	...
	void __iomem			*reg_instret[3];
	void __iomem			*reg_instrun[3];
...
static int ehld_init_dt_parse(struct device_node *np)
{
	...
	ehld->reg_instret[2] = of_iomap(np, 4);
	if (ehld->reg_instret[2])
		ehld_info(1, "ehld: instret reg[2] address registered\n");

	ehld->reg_instrun[2] = of_iomap(np, 5);
	if (ehld->reg_instrun[2])
		ehld_info(1, "ehld: instrun reg[2] address registered\n");
	...
	for_each_possible_cpu(cpu) {
		...
		if (cpu < 4) {
			...
			ctrl->reg_instret = ehld->reg_instret[2];
			ctrl->reg_instrun = ehld->reg_instrun[2];
		}

		if (!ctrl->reg_instret || !ctrl->reg_instrun) {
			ehld_err(1, "ehld: no ppc sfr for cpu%d\n", cpu);
			return -ENOMEM;
		}
