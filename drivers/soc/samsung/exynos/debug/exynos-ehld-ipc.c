...
#include <soc/samsung/exynos/exynos-ehld.h>
...
int adv_tracer_ehld_set_enable(int en, bool log)
{
	struct adv_tracer_ipc_cmd cmd;
	int ret = 0;
...
int adv_tracer_ehld_send_cmd(u32 ccmd, u32 val1, u32 val2, u32 ns_time, bool async)
{
	struct adv_tracer_ipc_cmd cmd;
	int ret = 0;

	cmd.cmd_raw.cmd = eEHLD_IPC_CMD_SEND_CMD;
	cmd.buffer[1] = ccmd;
	cmd.buffer[2] = val1;
	cmd.buffer[3] = val2;

	if (ns_time) {
		ret = adv_tracer_ipc_send_data_polling_timeout
				(plugin_ehld.ehld_dev->id, &cmd, ns_time);
	} else {
		if (async)
			ret = adv_tracer_ipc_send_data_async(plugin_ehld.ehld_dev->id, &cmd);
		else
			ret = adv_tracer_ipc_send_data(plugin_ehld.ehld_dev->id, &cmd);
	}
	if (ret < 0) {
		pr_err("ehld-ipc: failed to send cmd: 0x%x/0x%x\n", ccmd, val1);
		return ret;
	}

	pr_info("ehld-ipc: send cmd: 0x%x/0x%x\n", ccmd, val1);
