...
struct memlog_obj_prv {
	...
	struct bin_attribute obj_dumpstate_ba;
...
struct memlog_prv {
	...
	struct list_head list;
	struct kobject *kobj;
	struct kobject *kobj_dumpstate;
	struct list_head obj_dumpstate_file_list;
	struct mutex obj_dumpstate_file_lock;
...
struct memlogger {
	...
	void *rmem_base_vaddr;
	phys_addr_t rmem_free_base;
	struct device *dev;
};
...
static inline void memlog_getnstimeofday(struct rtc_time *tm)
{
				...
				void *src, size_t size)
{
	...
	if (!obj || !obj->enabled || (log_level > obj->log_level) ||
				...
				void *src, size_t size, bool sync)
{
	...
	if (!obj || !obj->enabled || prvobj->is_dead || !main_desc.mem_to_file_allow)
		return 0;
...
static ssize_t memlog_obj_dumpstate_read(struct file *file,
					struct kobject *kobj,
				   ...
				   const char *name, bool uaccess)
{
	...
	sysfs_attr_init(&prvobj->obj_dumpstate_ba.attr);
	prvobj->obj_dumpstate_ba.attr.mode = 0664;
	prvobj->obj_dumpstate_ba.attr.name = name;
	prvobj->obj_dumpstate_ba.size = prvobj->obj.size + PAGE_SIZE;
	prvobj->obj_dumpstate_ba.read = memlog_obj_dumpstate_read;
	...
	if (prvobj->obj.log_type == MEMLOG_TYPE_STRING) {
		ret = sysfs_create_bin_file(prvdesc->kobj_dumpstate, &prvobj->obj_dumpstate_ba);
		if (ret)
			goto out_fail_obj_dumpstate;
	}
...
static void memlog_obj_remove_sysfs(struct memlog_obj_prv *prvobj)
{
	...
	if (prvobj->obj.log_type == MEMLOG_TYPE_STRING) {
		...
		sysfs_remove_bin_file(prvdesc->kobj_dumpstate, &prvobj->obj_dumpstate_ba);
	}
	kobject_put(prvobj->kobj);
}
...
void memlog_obj_set_sysfs_mode(struct memlog_obj *obj, bool uaccess)
{
	...
	ret = memlog_obj_create_sysfs(prvdesc, prvobj, (const char *)prvobj->obj_name, uaccess);
...
static void *memlog_alloc_coherent(size_t size, dma_addr_t *dma_handle)
{
	struct reserved_mem *rmem;
	...
	end_paddr = rmem->base + rmem->size;
	if (end_paddr - main_desc.rmem_free_base < size) {
		dev_err(main_desc.dev, "%s: there is no free memory\n", __func__);
		return NULL;
	}

	base = main_desc.rmem_base_vaddr + (main_desc.rmem_free_base - rmem->base);
	*dma_handle = (dma_addr_t)main_desc.rmem_free_base;
	main_desc.rmem_free_base += size;
	main_desc.rmem_free_base = (main_desc.rmem_free_base + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
	return base;
}
					...
					const char *name)
{
	...
	if (unlikely(!desc)) {
		dev_err(main_desc.dev, "%s: no desc\n", __func__);
	...
	ret = memlog_obj_create_sysfs(prvdesc, prvobj, name, false);
	if (ret) {
	...
	case MEMLOG_TYPE_DIRECT:
		if (flag & MEMLOG_PROPERTY_NONCACHABLE) {
			prvobj->obj.vaddr = memlog_alloc_coherent(size, &prvobj->obj.paddr);
			if (prvobj->obj.vaddr)
					...
					u32 uflag)
{
	...
	if (main_desc.mem_constraint) {
		if (file_obj && file_obj->size > SZ_32K)
			memlog_reset_file_obj_vmem(file_obj, SZ_32K);
		if (size > SZ_32K)
			size = SZ_32K;
	}
...
void memlog_unregister(struct memlog *desc)
{
	...
	kobject_put(prvdesc->kobj_dumpstate);
					...
					struct memlog **desc)
{
	...
	*desc = NULL;
	...
	prvdata->kobj_dumpstate = kobject_create_and_add(dev_name, main_desc.obj_kobj);
	if (!prvdata->kobj_dumpstate) {
	...
	mutex_init(&prvdata->obj_dumpstate_file_lock);
	INIT_LIST_HEAD(&prvdata->obj_list);
	INIT_LIST_HEAD(&prvdata->obj_dumpstate_file_list);
...
static void *memlog_rmem_set_vmap(struct reserved_mem *rmem)
{
...
static int memlog_init_rmem(struct device *dev)
{
	...
	main_desc.rmem_base_vaddr = memlog_rmem_set_vmap(rmem);
	if (!main_desc.rmem_base_vaddr) {
		dev_err(dev, "%s: fail to vmap\n", __func__);
		ret = -ENOMEM;
		goto out;
	}

	main_desc.rmem = rmem;
	main_desc.rmem_free_base = rmem->base;
...
static void *memlog_copy_objlog_to_buf(struct memlog_obj_prv *prvobj, void *buf,
					size_t *total_copy_size)
{
	...
	*total_copy_size = 0;
	...
	if ((size_t)eob > curr_ptr_before_memcpy) {
		...
		memcpy(buf, (void *)curr_ptr_before_memcpy, copy_size);
		*total_copy_size = copy_size;
	}

	copy_size = (size_t)curr_ptr_before_memcpy - (size_t)prvobj->obj.vaddr;
	memcpy(buf + *total_copy_size, prvobj->obj.vaddr, copy_size);
	*total_copy_size += copy_size;
	...
	*total_copy_size -= overwritten_data_size;

	return buf + overwritten_data_size;
}

static void memlog_copied_data_to_kmsg(char *log, size_t size, size_t max)
{
	char *token;
...
static inline void set_null_at_the_end_of_string(char *buf, size_t size)
{
	buf[(int)size - 1] = '\0';
}

void memlog_copy_log_to_kmsg(struct memlog_obj *obj, size_t size)
{
	void *data_buf;
	void *copied_data;
	size_t total_copy_size;
	struct memlog *desc = (struct memlog *)obj->parent;
	struct memlog_obj_prv *prvobj = obj_to_data(obj);

	if (obj->log_type != MEMLOG_TYPE_STRING) {
		const char *type =  memlog_get_typename(obj->log_type);

		dev_err(main_desc.dev, "%s: %s, %s's type is %s(not printf type)\n",
				       __func__, desc->dev_name, prvobj->obj_name, type);
		return;
	}

	if (!preemptible())
		dev_warn(main_desc.dev,
			"%s: unpreemtible(this use GFP_KERNEL) preempt_count(%d) mask I(%d)\n",
			__func__, preempt_count(), irqs_disabled());

	data_buf = vzalloc(prvobj->obj.size);
	if (!data_buf) {
		dev_err(main_desc.dev, "%s: fail to vzalloc\n", __func__);
		return;
	}

	copied_data = memlog_copy_objlog_to_buf(prvobj, data_buf, &total_copy_size);
	set_null_at_the_end_of_string(data_buf, prvobj->obj.size);
	dev_info(main_desc.dev, "%s: %s, %s log flush to kernel start\n",
			       __func__, desc->dev_name, prvobj->obj_name);
	memlog_copied_data_to_kmsg(copied_data, total_copy_size, size);
	dev_info(main_desc.dev, "%s: %s, %s log flush to kernel done\n",
			       __func__, desc->dev_name, prvobj->obj_name);
	vfree(data_buf);
}
EXPORT_SYMBOL(memlog_copy_log_to_kmsg);
...
static size_t memlog_dumpstate_copy_data(struct memlog_obj_prv *prvobj, void *buf, size_t max)
{
	...
	if ((max - n) >= total_copy_size) {
		memcpy(buf + n, copied_data, total_copy_size);
		n += total_copy_size;
	}
	vfree(data_buf);
					...
					char *buf, loff_t off, size_t count)
{
	struct memlog_ba_file_data *data;
	struct memlog_obj_prv *prvobj = container_of(attr,
						     struct memlog_obj_prv, obj_dumpstate_ba);
	struct memlog_prv *prvdesc = desc_to_data(prvobj->obj.parent);

	data = memlog_ba_search_file(&prvdesc->obj_dumpstate_file_list,
				     &prvdesc->obj_dumpstate_file_lock, file, current->pid);
	...
	if (data == NULL) {
		...
		data->size = memlog_dumpstate_copy_data(prvobj, data->data, size - 1);
		data->file = file;
		data->pid = current->pid;
		memlog_ba_file_add(&prvdesc->obj_dumpstate_file_list,
				   &prvdesc->obj_dumpstate_file_lock, data);
	}
	...
	if (data->size > off) {
		...
		memlog_ba_file_delete(&prvdesc->obj_dumpstate_file_list,
				      &prvdesc->obj_dumpstate_file_lock, data);
		vfree(data->data);
		kfree(data);
		count = 0;
