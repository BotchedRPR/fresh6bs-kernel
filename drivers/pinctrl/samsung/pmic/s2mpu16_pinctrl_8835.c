...
static const struct pinconf_generic_params pmic_gpio_bindings[] = {
	...
	{"pmic-gpio,drive-strength",	PMIC_GPIO_CONF_DRIVE_STRENGTH,	1},
};
...
static int pmic_gpio_read(const struct pmic_gpio_state *state, const uint8_t addr)
{
	...
	ret = s2mpu16_read_reg(state->i2c, addr, &val);
	if (ret)
		dev_err(state->dev, "[SUB_PMIC] %s: read %#x failed\n", __func__, addr);
	else
		ret = val;
...
static void vgpio_write(struct pmic_gpio_state *state, const uint32_t pin, uint32_t val)
{
	u32 reg;
	u32 offset = pin / 4;
...
{
	return s2mpu16_write_reg(state->i2c, addr, val);
}
...
{
	return (pin != GPIO_HV_IDX) ? (S2MPU16_GPIO_GPIO_SET0 + pin) : S2MPU16_GPIO_GPIO_SET_HV;
}

static uint8_t get_status_reg(const uint32_t pin)
{
	return (pin / GPIO_RANGE) + S2MPU16_GPIO_GPIO_STATUS1;
}
...
static int get_pmic_gpio_info(struct pmic_gpio_state *state, const uint32_t pin)
{
	...
	const uint8_t reg_status = get_status_reg(pin);
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: (%#02hhx:%#02hhx), (%#02hhx:%#02hhx)\n",
					...
					uint32_t pin, unsigned long *config)
{
	...
	*config = pinconf_to_config_packed(param, arg);
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: param(%#x), arg(%#x)\n", __func__, pin, param, arg);
...
{
	struct pmic_gpio_state *state = pinctrl_dev_get_drvdata(pctldev);
	const struct pmic_gpio_pad *pad = pctldev->desc->pins[pin].drv_data;
	const uint8_t reg_set = get_set_reg(pin);
	...
	if (ret < 0) {
		dev_err(state->dev, "[SUB_PMIC] %s: pmic_gpio_write fail\n", __func__);
		return ret;
	}
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: %s\n", __func__, pin, buf);
...
static int pmic_gpio_direction_input(struct gpio_chip *chip, uint32_t pin)
{
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: ret(%#x)\n", __func__, pin, ret);
...
static int pmic_gpio_direction_output(struct gpio_chip *chip, uint32_t pin, int val)
{
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: val(%#x), ret(%#x)\n", __func__, pin, val, ret);
...
static int pmic_gpio_get(struct gpio_chip *chip, uint32_t pin)
{
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: DAT(%s)\n",
...
static void pmic_gpio_set(struct gpio_chip *chip, uint32_t pin, int value)
{
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: Set DAT(%s_%#x)\n",
...
static int pmic_gpio_set_config(struct gpio_chip *chip, uint32_t pin, unsigned long config)
{
	...
	dev_err(state->dev, "[SUB_PMIC] %s: pin%d: config(%#x)\n", __func__, pin, config);
			      ...
			      struct pmic_gpio_pad *pad, const uint32_t pin)
{
	...
	dev_info(state->dev, "[SUB_PMIC] %s: pin%d: %s(%#x), MODE(%s), "
...
static int pmic_gpio_parse_dt(const struct s2mpu16_dev *iodev, struct pmic_gpio_state *state)
{
	...
	gpio_np = of_find_node_by_name(mfd_np, "s2mpu16-gpio");
	if (!gpio_np) {
...
static int pmic_set_vgpio(struct pmic_gpio_state *state)
{
	/* VGPIO setting by AP */
