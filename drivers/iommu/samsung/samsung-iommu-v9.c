...
{
	u32 ctrl_val = CTRL_MMU_ENABLE | CTRL_INT_ENABLE;
...
static int samsung_sysmmu_attach_dev(struct iommu_domain *dom, struct device *dev)
{
	...
	dev_info(dev, "attached with pgtable %pK\n", &domain->page_table);
					...
					int pmmu_index)
{
	...
	int i, readsize, cnt, ret, num_stream_table, num_ptlb;
	...
	ret = of_property_read_u32_index(dev->of_node, num_stream_table_name, 0, &num_stream_table);
	if (ret) {
		dev_err(dev, "failed to init number of stream_table\n");
		return ret;
	}
	drvdata->num_stream_table[pmmu_index] = num_stream_table;

	ret = of_property_read_u32_index(dev->of_node, num_ptlb_name, 0, &num_ptlb);
	if (ret) {
		dev_err(dev, "failed to init number of ptlb\n");
		return ret;
	}
	drvdata->num_ptlb[pmmu_index] = num_ptlb;

	for (i = 0; i < cnt; i++) {
		if (cfg[i].index >= num_stream_table) {
			dev_err(dev, "invalid index %d is ignored. (max:%d)\n",
				cfg[i].index, num_stream_table);
...
static int sysmmu_parse_dt(struct device *sysmmu, struct sysmmu_drvdata *data)
{
	...
	num_stream_table = devm_kcalloc(sysmmu, num_pmmu, sizeof(*num_stream_table), GFP_KERNEL);
	if (!num_stream_table)
		return -ENOMEM;
	data->num_stream_table = num_stream_table;

	num_ptlb = devm_kcalloc(sysmmu, num_pmmu, sizeof(*num_ptlb), GFP_KERNEL);
	if (!num_ptlb)
		return -ENOMEM;
	data->num_ptlb = num_ptlb;
