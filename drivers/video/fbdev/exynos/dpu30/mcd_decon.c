...
void print_win_config_info(struct decon_device *decon, struct decon_reg_data *reg_data)
{
	...
	for (i = 0; i < decon->dt.max_win + 2; i++) {
		dpp_config = &reg_data->dpp_config[i];
		...
		if ((dpp_config->state != DECON_WIN_STATE_BUFFER) && (dpp_config->state != DECON_WIN_STATE_COLOR)
			&& (dpp_config->state != DECON_WIN_STATE_CURSOR))
			continue;

		fmt_info = dpu_find_fmt_info(dpp_config->format);

		decon_info("idx: %02d, %s src: %04d %04d %04d %04d, dst: %04d %04d %04d %04d, rot: %s, comp:%01d, fmt: %s, eq: 0x%08x, blend: %d, alpha: %d\n",
			...
			dpp_config->dpp_parm.eq_mode, dpp_config->blending, dpp_config->plane_alpha);
