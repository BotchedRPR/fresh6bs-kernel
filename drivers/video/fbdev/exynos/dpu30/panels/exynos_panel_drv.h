...
enum {
	USDM_PANEL_ADAPTER_WQ_VSYNC = 0,
	USDM_PANEL_ADAPTER_WQ_FSYNC = 1,
	USDM_PANEL_ADAPTER_WQ_DISPON = 2,
	USDM_PANEL_ADAPTER_WQ_PANEL_PROBE = 3,
	MAX_USDM_PANEL_ADAPTER_WQ,
};

struct usdm_panel_adapter_wq {
	int index;
...
struct exynos_panel_ops {
	...
	int (*reset_disable_panel)(struct exynos_panel_device *panel);
...
struct exynos_panel_device {
	...
	struct usdm_panel_adapter_wq wqs[MAX_USDM_PANEL_ADAPTER_WQ];
...
#if IS_ENABLED(CONFIG_USDM_PANEL)
static inline struct dsim_device *get_dsim_device_from_exynos_panel(struct exynos_panel_device *panel)
{
	return panel ? get_dsim_drvdata(panel->id) : NULL;
}
...
#define EXYNOS_PANEL_IOC_RESET_DISABLE	_IOW('P', 33, u32)
#endif
