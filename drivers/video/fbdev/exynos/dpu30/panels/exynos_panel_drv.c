		...
		struct device_node *np)
{
	...
	DPU_INFO_PANEL("LCD size(%dx%d), DDI type(%d)\n", res[0], res[1],
			lcd_info->ddi_type);
...
//minwoo7945.kim
	...
	if (!found) {
		DPU_INFO_PANEL("panel ops is not found in panel list\n");
		return -EINVAL;
	}
#else
	panel->ops = &panel_s6e36w4x01_ops;
	panel->id_index = 0;
#endif
	return 0;
}
...
static void exynos_panel_find_id(struct exynos_panel_device *panel)
{
	...
	DPU_INFO_PANEL("np value in find id = %x", np);
	if (!np) {
		DPU_INFO_PANEL("panel is not matched\n");
		return;
	}
