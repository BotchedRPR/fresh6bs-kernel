...
void decon_dpp_stop(struct decon_device *decon, bool do_reset)
{
	...
	for (i = 0; i < decon->dt.dpp_cnt; i++) {
		if (test_bit(i, &decon->prev_used_dpp) &&
			...
			v4l2_subdev_call(sd, core, ioctl, DPP_STOP, &rst);
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
	if (decon->res.pinctrl && decon->res.disp_rstn_ret_off) {
		if (pinctrl_select_state(decon->res.pinctrl,
					decon->res.disp_rstn_ret_off)) {
			decon_err("failed to turn off disp rst retention\n");
		}
	}
#endif
...
static int decon_enable(struct decon_device *decon)
{
	...
	//fix the bug: enter into recovery sysmmu error
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
#if IS_ENABLED(CONFIG_USDM_PANEL)
	decon->lpd_dpu_state = DPU_STATE_ON;
	lpd_notifier_call(LPD_NOTIFIER_STOP, &decon->lpd_dpu_state);
#else
	...
	DO_ONCE(decon_pd_booton_rel, "pd-disp");

#if IS_ENABLED(CONFIG_EXYNOS_LPD)
#if IS_ENABLED(CONFIG_USDM_PANEL)
	lpd_notifier_call(LPD_NOTIFIER_STOP_POST, &decon->lpd_dpu_state);
#else
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
#if IS_ENABLED(CONFIG_USDM_PANEL)
	decon->lpd_dpu_state = DPU_STATE_DOZE;
	lpd_notifier_call(LPD_NOTIFIER_STOP, &decon->lpd_dpu_state);
#else
	lpd_notifier_call(LPD_NOTIFIER_STOP, NULL);
#endif
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
	if(state == DECON_STATE_DOZE_SUSPEND){
		if (decon->res.pinctrl && decon->res.disp_rstn_ret_on) {
			if (pinctrl_select_state(decon->res.pinctrl,
						decon->res.disp_rstn_ret_on)) {
				decon_err("failed to turn on disp rst retention\n");
			}
		}
	}
	else {
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
#if IS_ENABLED(CONFIG_USDM_PANEL)
	decon->lpd_dpu_state = DPU_STATE_OFF;
	lpd_notifier_call(LPD_NOTIFIER_STOP, &decon->lpd_dpu_state);
#else
	lpd_notifier_call(LPD_NOTIFIER_STOP, NULL);
#endif
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
#if IS_ENABLED(CONFIG_USDM_PANEL)
	decon->lpd_dpu_state = DPU_STATE_DOZE_SUSPEND;
	lpd_notifier_call(LPD_NOTIFIER_CONFIG, &decon->lpd_dpu);
	lpd_notifier_call(LPD_NOTIFIER_START_PRE, &decon->lpd_dpu_state);
#else
	lpd_notifier_call(LPD_NOTIFIER_CONFIG, &decon->lcd_info->id);
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
#if IS_ENABLED(CONFIG_USDM_PANEL)
	lpd_notifier_call(LPD_NOTIFIER_START, &decon->lpd_dpu_state);
#else
		...
		struct decon_reg_data *regs, int win_num, bool on)
{
	...
	ret = dma_buf_vmap(regs->dma_buf_data[win_num][mp_idx].dma_buf, &map);
	if (ret) {
		decon_err("failed to vmap buffer [%x]\n", ret);
		return -EINVAL;
	}
	video_meta = (struct exynos_video_meta *)map.vaddr;
	 ...
	 */
	if (hdr_cmp == 0) {
		dma_buf_vunmap(regs->dma_buf_data[win_num][mp_idx].dma_buf, &map);
		return 0;
	}
#if defined(CONFIG_EXYNOS_DISPLAYPORT)
	ret = v4l2_subdev_call(decon->displayport_sd, core, ioctl,
		...
		struct decon_win_config_data *win_data)
{
	...
	if (IS_DECON_OFF_STATE(decon) ||
#if defined(CONFIG_EXYNOS_READ_ESD_SOLUTION) || IS_ENABLED(CONFIG_USDM_PANEL)
...
static int decon_fb_alloc_memory(struct decon_device *decon, struct decon_win *win)
{
	...
	if (!dma_heap) {
		dev_err(decon->dev, "dma_heap_find() faile\n");
	...
	if (ret) {
		dev_err(decon->dev,"failed to vmap buffer [%x]\n", ret);
		return -EINVAL;
	}
	vaddr = map.vaddr;
...
#if IS_ENABLED(CONFIG_EXYNOS_LPD)
static int decon_set_panel_meta(struct exynos_panel_info *lcd_info, struct lpd_panel_meta *panel_meta)
{
	if (lcd_info == NULL) {
		decon_err("null lcd_info\n");
		return -EINVAL;
	}

	if (panel_meta == NULL) {
		decon_err("null panel_meta\n");
		return -EINVAL;
	}

	panel_meta->id = lcd_info->id;
	panel_meta->xres = lcd_info->xres;
	panel_meta->yres = lcd_info->yres;
	panel_meta->fps = lcd_info->fps;

	decon_info("panel_meta->id : %x\n", panel_meta->id);

	decon_info("panel_meta->xres: %d, panel_meta->yres: %d, panel_meta->fps: %d\n",
		panel_meta->xres, panel_meta->yres, panel_meta->fps);

	return 0;
}

static int decon_set_dsim_meta(struct exynos_panel_info *lcd_info, struct lpd_dsim_meta *dsim_meta)
{
	if (lcd_info == NULL) {
		decon_err("null lcd_info\n");
		return -EINVAL;
	...
	dsim_meta->hs_clk  = lcd_info->hs_clk;
	dsim_meta->p = lcd_info->dphy_pms.p;
	dsim_meta->m = lcd_info->dphy_pms.m;
	dsim_meta->s = lcd_info->dphy_pms.s;
	dsim_meta->k = lcd_info->dphy_pms.k;

	decon_info("dsim_meta->hs_clk: %d, dsim_meta->p: %d, dsim_meta->m: %d, dsim_meta->s: %d, dsim_meta->: %d\n",
		dsim_meta->hs_clk, dsim_meta->p, dsim_meta->m, dsim_meta->s, dsim_meta->k);

	return 0;

}

static int decon_set_lpd_meta(struct decon_device *decon)
{
	int ret = 0;
	struct lpd_dpu_meta *dpu_meta;
	...
	dpu_meta = &decon->lpd_dpu;
	lcd_info = decon->lcd_info;

	ret = decon_set_panel_meta(lcd_info, &dpu_meta->panel_meta);
	if (ret) {
		decon_err("failed to set lpd panel meta\n");
		return ret;
	}

	ret = decon_set_dsim_meta(lcd_info, &dpu_meta->dsim_meta);
	if (ret) {
		decon_err("failed to set lpd dsim meta\n");
		return ret;
	}
...
MODULE_SOFTDEP("pre: acpm-mfd-bus s2mpw03_mfd s2mpw03-regulator");
