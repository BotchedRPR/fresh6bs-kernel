...
#include <soc/samsung/exynos/exynos-mcinfo.h>
...
{
	exynos_acpm_tmu_instance_update(instance->cdev->id, instance->trip,
...
void exynos_tmu_callback_hotplug(char cpuhp_name[], bool hotplug)
{
	...
	list_for_each_entry(data, &dtm_dev_list, node) {
		if (!strncmp(data->tmu_name, cpuhp_name, 3))
			break;
	}
	if (hotplug) {
...
static void exynos_tmu_callback_cdev_update(u32 cdev_list)
{
	...
	for (i = 0; i < NOTI_STATUS_CDEV_UPDATE_NUM; i++) {
		if (0x1 << i & cdev_list) {
			...
			list_for_each_entry(instance, &tzd->thermal_instances,
					tz_node) {
				if (instance->cdev) {
					cdev = instance->cdev;
...
static void exynos_tmu_callback_pi_trace(u32 tz_list)
{
	struct exynos_tmu_data *data;
	struct thermal_zone_device *tzd;
	s64 k_p = 0, k_i = 0, k_d = 0;
	s64 power_range = 0, err_integral = 0, err = 0;
	int i;

	for (i = 0; i < NOTI_STATUS_PI_TRACE_NUM; i++) {
		if (!(0x1 << i & tz_list))
			continue;
		...
		exynos_acpm_tmu_get_pi_params(data->id,
				TMU_DATA_TRACE_ERR, &err);
		exynos_acpm_tmu_get_pi_params(data->id,
				TMU_DATA_TRACE_ERR_INTEGRAL,
				&err_integral);
		exynos_acpm_tmu_get_pi_params(data->id,
				TMU_DATA_TRACE_P, &k_p);
		exynos_acpm_tmu_get_pi_params(data->id,
				TMU_DATA_TRACE_I, &k_i);;
		exynos_acpm_tmu_get_pi_params(data->id,
				TMU_DATA_TRACE_D, &k_d);
		exynos_acpm_tmu_get_pi_params(data->id,
				TMU_DATA_TRACE_POWER_RANGE,
				&power_range);

		trace_thermal_exynos_power_allocator_pid(tzd, (s32)err,
				err_integral, k_p, k_i,
				k_d, power_range);
	}
}

static void exynos_tmu_callback_power_trace(u32 tz_list)
{
	struct exynos_tmu_data *data;
	struct thermal_zone_device *tzd;
	u64 power_range, max_power, control_temp;
	int temperature, i;

	for (i = 0; i < NOTI_STATUS_POWER_TRACE_NUM; i++) {
		if (!(0x1 << i & tz_list))
			continue;
		...
		exynos_acpm_tmu_get_pi_params(data->id,
				...
				&control_temp);
		exynos_acpm_tmu_get_tmu_data(data->id, TMU_DATA_TEMPERATURE,
				&temperature);

		trace_thermal_exynos_power_allocator(tzd, (u32)power_range,
				(u32)max_power, temperature,
				(u32)control_temp - temperature);
	}
}
...
static void exynos_tmu_esca_callback_work(struct kthread_work *work)
{
	...
	partial_state = (state[NOTI_STATUS_DATA_POS1] & NOTI_STATUS_PI_TRACE_MASK) >>
			NOTI_STATUS_PI_TRACE_SHIFT;
	if (partial_state)
		exynos_tmu_callback_pi_trace(partial_state);

	partial_state = (state[NOTI_STATUS_DATA_POS1] & NOTI_STATUS_POWER_TRACE_MASK) >>
		NOTI_STATUS_POWER_TRACE_SHIFT;
	if (partial_state)
		exynos_tmu_callback_power_trace(partial_state);
}
...
// WATCH(Morion) use one cluster with name "CPU"
		if (!strncasecmp(data->tmu_name, "CPU", THERMAL_NAME_LENGTH)) {
...
static ssize_t
polling_stop_show(struct device *dev, struct device_attribute *devattr,
		char *buf)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct exynos_tmu_data *data = platform_get_drvdata(pdev);
...
static ssize_t
polling_stop_store(struct device *dev, struct device_attribute *devattr,
		const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct exynos_tmu_data *data = platform_get_drvdata(pdev);
		       ...
		       char *buf)
{
	...
	int emergency_frequency = data->emergency_frequency;

	if (data) {
#if IS_ENABLED(CONFIG_EXYNOS_ESCA_THERMAL)
		exynos_acpm_tmu_get_tmu_data(data->id, TMU_DATA_EMERGENCY_FREQ,
				&emergency_frequency);
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCA_THERMAL)
	exynos_acpm_tmu_set_tmu_data(data->id, TMU_DATA_EMERGENCY_FREQ, 0,
			data->emergency_frequency);
#endif
...
#define create_s32_boost_param_attr(name, pos)					\
	static ssize_t								\
	name##_show(struct device *dev, struct device_attribute *devattr, 	\
			char *buf)							\
	{									\
		...
		s64 name;								\
		\
		exynos_acpm_tmu_get_boost_param(data->id, pos, &name);			\
		\
		data->boost_param.name = (s32)name;					\
		\
	...
	{									\
		...
		return -EINVAL;						\
		\
		exynos_acpm_tmu_set_boost_param(data->id, pos, value);		\
...
static DEVICE_ATTR(polling_stop, S_IWUSR | S_IRUGO, polling_stop_show,
		polling_stop_store);
...
static void exynos_tmu_set_tmu_data(struct exynos_tmu_data *data)
{
	...
	if (data->limited_frequency) {
		exynos_acpm_tmu_set_tmu_data(data->id,
				...
				0, data->limited_frequency);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_LIMITED_THRESHOLD,
				0, data->limited_threshold);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_LIMITED_THRESHOLD_REL,
				0, data->limited_threshold_release);
	} else if (data->limited_frequency_2) {
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_LIMITED_FREQ,
				0, data->limited_frequency);
		exynos_acpm_tmu_set_tmu_data(data->id,
		...
		exynos_acpm_tmu_set_tmu_data(data->id,
				...
				0, data->limited_frequency_2);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_LIMITED_THRESHOLD2,
				0, data->limited_threshold_2);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_LIMITED_THRESHOLD_REL2,
				0, data->limited_threshold_release_2);
	}

	if (data->emergency_frequency) {
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_EMERGENCY_FREQ,
				0, data->emergency_frequency);
	}

	if (data->emergency_frequency_1) {
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_EMERGENCY_FREQ1,
				0, data->emergency_frequency_1);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_EMERGENCY_FREQ2,
				0, data->emergency_frequency_2);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_EMERGENCY_FREQ3,
				0, data->emergency_frequency_3);
	}
...
static void exynos_tmu_set_pi_params(struct exynos_tmu_data *data)
{
	...
	if (data->use_pi_thermal) {
		...
		exynos_acpm_tmu_set_tmu_data(data->id,
				...
				0, params->polling_delay_on);
		exynos_acpm_tmu_set_tmu_data(data->id,
				TMU_DATA_POLLING_DELAY_OFF,
				0, params->polling_delay_off);
	}
}
