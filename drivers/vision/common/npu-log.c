...
struct npu_log npu_log = {
	...
	.npu_dvfs_array_obj = NULL,
	.npu_scheduler_array_obj = NULL,
...
static DEFINE_SPINLOCK(fw_profile_lock);
...
static void npu_log_rmemlog(struct npu_device *npu_dev)
{
	...
	if (npu_log.npu_array_file_obj) {
		npu_log.npu_dvfs_array_obj = memlog_alloc_array(npu_log.memlog_desc_array, LOG_UNIT_NUM,
		...
		npu_log.npu_scheduler_array_obj = memlog_alloc_array(npu_log.memlog_desc_array, LOG_UNIT_NUM,
		...
		npu_log.npu_protodrv_array_obj = memlog_alloc_array(npu_log.memlog_desc_array, LOG_UNIT_NUM,
		...
		npu_log.npu_hwdev_array_obj = memlog_alloc_array(npu_log.memlog_desc_array, LOG_UNIT_NUM,
...
#if IS_ENABLED(CONFIG_EXYNOS_MEMORY_LOGGER)
inline void npu_log_dvfs_set_data(int id, int from, int to)
{
	struct npu_log_dvfs npu_log_dvfs;

	npu_log_dvfs.id = id;
	npu_log_dvfs.from = from;
	npu_log_dvfs.to = to;

	if (npu_log.npu_dvfs_array_obj)
		memlog_write_array(npu_log.npu_dvfs_array_obj, MEMLOG_LEVEL_CAUTION, &npu_log_dvfs);
}

inline void npu_log_scheduler_set_data(struct npu_device *device)
{
	struct npu_scheduler_info *info;
	struct npu_log_scheduler npu_log_scheduler;

	info = device->sched;

	npu_log_scheduler.temp = 0;
	npu_log_scheduler.load = ((info->idle_load & 0xFFFF) << 16) | (info->fps_load & 0xFFFF);

	if (npu_log.npu_scheduler_array_obj)
		memlog_write_array(npu_log.npu_scheduler_array_obj, MEMLOG_LEVEL_CAUTION, &npu_log_scheduler);
}

inline void npu_log_protodrv_set_data(void *req, int type, bool act)
{
	struct npu_frame *req_frame = NULL;
	struct npu_nw *req_nw = NULL;
	struct npu_log_protodrv npu_log_protodrv;

	switch (type) {
	case PROTO_DRV_REQ_TYPE_FRAME:
		req_frame = req;
		if (act) {
			npu_log_protodrv.uid_cmd = req_frame->uid << 16 | (u16)req_frame->cmd;
			npu_log_protodrv.param0 = req_frame->frame_id;
			npu_log_protodrv.param1 = 0;
		} else {
			npu_log_protodrv.uid_cmd = req_frame->uid << 16 | (u16)req_frame->cmd;
			npu_log_protodrv.param0 = req_frame->frame_id;
			npu_log_protodrv.param1 = req_frame->result_code;
		}
		break;
	case PROTO_DRV_REQ_TYPE_NW:
		req_nw = req;
		if (act) {
			npu_log_protodrv.uid_cmd = req_nw->uid << 16 | (u16)req_nw->cmd;
			npu_log_protodrv.param0 = req_nw->param0;
			npu_log_protodrv.param1 = req_nw->param1;
		} else {
			npu_log_protodrv.uid_cmd = req_nw->uid << 16 | (u16)req_nw->cmd;
			npu_log_protodrv.param0 = req_nw->param0;
			npu_log_protodrv.param1 = req_nw->result_code;
		}
		break;
	}

	if (npu_log.npu_protodrv_array_obj)
		memlog_write_array(npu_log.npu_protodrv_array_obj, MEMLOG_LEVEL_CAUTION, &npu_log_protodrv);
}

inline void npu_log_hwdev_set_data(int hid)
{
	struct npu_hw_device *hwdev;
	struct npu_log_hwdev npu_log_hwdev;
	...
	if (hid) {
		...
		npu_log_hwdev.status = hwdev->status;
	} else {
		npu_log_hwdev.hid = 0xffffffff;
		npu_log_hwdev.status = 0;
	}

	if (npu_log.npu_hwdev_array_obj)
		memlog_write_array(npu_log.npu_hwdev_array_obj, MEMLOG_LEVEL_CAUTION, &npu_log_hwdev);
}
...
inline void npu_log_protodrv_set_data(void *req, int type, bool act) {}
inline void npu_log_scheduler_set_data(struct npu_device *device) {}
inline void npu_log_hwdev_set_data(int id, int cmd) {}
#endif
...
void npu_fw_profile_init(char *buf_addr, const size_t size)
{
	...
	fw_profile.st_buf = buf_addr;
	buf_addr[0] = '\0';
	buf_addr[size - 1] = '\0';
	fw_profile.st_size = size;
	fw_profile.wr_pos = 0;
	fw_profile.rp_pos = 0;
	fw_profile.line_cnt = 0;
...
void npu_fw_profile_deinit(void)
{
	...
	/* Wake-up readers and preserve some time to flush */
	wake_up_all(&fw_profile.wq);
	msleep(NPU_STORE_LOG_FLUSH_INTERVAL_MS);

	npu_info("fw_profile memory deinitializing : %pK -> NULL\n", fw_profile.st_buf);
	spin_lock_irqsave(&fw_profile_lock, intr_flags);
	fw_profile.st_buf = NULL;
	fw_profile.st_size = 0;
	fw_profile.wr_pos = 0;
	fw_profile.line_cnt = 0;
...
static int npu_fw_profile_fops_open(struct inode *inode, struct file *file)
{
	int				ret = 0;
...
static int npu_fw_profile_fops_close(struct inode *inode, struct file *file)
{
	struct npu_store_log_read_obj	*robj;
...
static ssize_t __npu_store_log_fops_read(struct npu_store_log_read_obj *robj, char *outbuf, const size_t outlen)
{
	...
	npu_dbg("end: %zu  copy_len: %zu  wr_pos: %zu  read_pos: %zu\n", buf_end, copy_len, npu_log.wr_pos, robj->read_pos);
	...
	npu_dbg("Buf = %pK, Read pos = %zu, Read len = %zu\n", npu_log.st_buf, robj->read_pos, copy_len);
...
int npu_fw_profile_store(char *strRep, int nSize)
{
	...
	if (nSize >= (int)remain) {
		...
		fw_profile.line_cnt = fw_profile.wr_pos;
		fw_profile.wr_pos = 0;
		remain = fw_profile.st_size;
		buf = fw_profile.st_buf;
		fw_profile.last_dump_line_cnt++;
	}

	memcpy(buf, strRep, nSize);
	remain -= nSize;
	wr_len += nSize;
	npu_dbg("fw_profile nSize : %d,\t remain = %zu\n", nSize, remain);

	/* Update write position */
	fw_profile.wr_pos = (fw_profile.wr_pos + wr_len) % fw_profile.st_size;
	fw_profile.st_buf[fw_profile.wr_pos] = '\0';
...
static ssize_t __npu_fw_profile_fops_read(struct npu_store_log_read_obj *robj, char *outbuf, const size_t outlen)
{
	size_t	copy_len, buf_end;

	/* Copy data to temporary buffer*/
	if (fw_profile.st_buf) {
		buf_end = (robj->read_pos > fw_profile.wr_pos) ? fw_profile.line_cnt : fw_profile.wr_pos;
		copy_len = min(outlen, buf_end - robj->read_pos);
		memcpy(outbuf, fw_profile.st_buf + robj->read_pos, copy_len);
	} else
		copy_len = 0;

	if (copy_len > 0) {
		robj->read_pos = (robj->read_pos + copy_len) % fw_profile.st_size;
		if (fw_profile.line_cnt == robj->read_pos) {
			fw_profile.line_cnt = 0;
			robj->read_pos = 0;
			memset(&fw_profile.st_buf[fw_profile.wr_pos], '\0', (fw_profile.st_size - fw_profile.wr_pos));
		}
	}
...
static ssize_t npu_fw_profile_fops_read(struct file *file, char __user *outbuf, size_t outlen, loff_t *loff)
{
	struct npu_store_log_read_obj	*robj;
	ssize_t				ret, copy_len;
	size_t				tmp_buf_len;
	...
	while (ret == 0) {	/* ret = 0 on timeout */
		/* TODO: Accessing npu_log.wr_pos outside of spinlock is potentially dangerous */
		ret = wait_event_interruptible_timeout(fw_profile.wq, robj->read_pos !=  fw_profile.wr_pos, 1 * HZ);
		if (ret == -ERESTARTSYS) {
			ret = 0;
			goto err_exit;
		}
	...
	copy_len = __npu_fw_profile_fops_read(robj, tmp_buf, tmp_buf_len);
	spin_unlock_irqrestore(&fw_profile_lock, intr_flags);
...
const struct file_operations npu_fw_profile_fops = {
	.owner = THIS_MODULE,
	.open = npu_fw_profile_fops_open,
	.release = npu_fw_profile_fops_close,
	.read = npu_fw_profile_fops_read,
};
...
static int npu_store_log_dump(const size_t dump_size)
{
	...
	if (total > 0) {
		...
		pos = 0;
		st = dump_buf;
		npu_err("---------- Start NPU log dump (len = %zu) ---------------\n", total);
		while (pos < total) {
			if (dump_buf[pos] == '\n') {
		...
		if (st != dump_buf + pos) {
			...
			dump_buf[pos - 1] = '\0';
			npu_err("%s <<No Line Ending>>\n", st);
		}
		npu_err("---------- End of NPU log dump ---------------\n");
	}
...
void npu_log_on_error(void)
{
	...
	if (ret)
		npu_err("npu_store_log_dump() failed: ret = %d\n", ret);
}
