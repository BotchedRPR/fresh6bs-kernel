...
static void dump_auto_sleep_thread(const struct auto_sleep_thread *ctx)
{
	...
	npu_dbg("state = running(%d) stopped(%d)\n",
		task_is_running(ctx->thread_ref),
		task_is_stopped(ctx->thread_ref));
#else
	npu_dbg("state = %ld\n", ctx->thread_ref->state);
#endif
	npu_dbg("pid = %u\n", ctx->thread_ref->pid);
	npu_dbg("tgid = %u\n", ctx->thread_ref->tgid);
...
#define IDIOT_TESTCASE_IMPL "asthread.idiot"
#include "idiot-def.h"
#endif
