 ...
 */
static int is_frame_stucked_result_code(const npu_errno_t result_code)
{
...
{
	return is_frame_stucked_result_code(req_frame->frame.result_code);
}
...
{
	if (unlikely(npu_buffer_q_get(&target->frame) == 0)) {
		return 0;	/* Empty */
	} else {
...
static int npu_queue_op_put_result(const struct proto_req_frame *src)
{
	...
	if (likely(result->result_code == NPU_ERR_NO_ERROR)) {
...
static int npu_protodrv_handler_frame_free(void)
{
	...
	while ((entry_lsm = proto_frame_lsm.lsm_get_entry(FREE)) != NULL) {
		/* Is request available ? */
		if (likely(npu_queue_op_get_request_pair(entry_lsm) != 0)) {
			/* Save the request and put it to REQUESTED state */
...
static int npu_protodrv_handler_frame_completed(void)
{
	...
	LSM_FOR_EACH_ENTRY_IN(proto_frame_lsm, COMPLETED, entry,
		...
		/* Time keeping */
		npu_ufnotice("(COMPLETED)frame: cmd(%u) / req_id(%d) / result(%u/0x%08x) NPU-TIMESTAT:%s\n",
		...
		if (unlikely(entry->frame.result_code != NPU_ERR_NO_ERROR))
			fw_will_note(FW_LOGSIZE);
		/* Write back result */
		if (likely(npu_queue_op_put_result(entry) > 0)) {
			...
			if (unlikely(is_stucked_req_frame(entry))) {
				...
				npu_warn("call set_emergency_err_from_req_frame()\n");
			...
			if (unlikely(session_ref_entry == NULL)) {
				...
				case NPU_NW_CMD_STREAMON:
					...
					if (likely(entry->nw.result_code == NPU_ERR_NO_ERROR)) {
						...
						npu_uerr(
							"complete STREAM_ON, with error(%u/0x%08x) : keep current s_state (%d)\n",
							&entry->nw,
							entry->nw.result_code,
							entry->nw.result_code,
							session_ref_entry
	...
	struct npu_device *device = npu_proto_drv.npu_device;
	{
		...
		LSM_FOR_EACH_ENTRY_IN(proto_frame_lsm, state, entry,
			if (unlikely(is_timedout(entry, NPU_PROTODRV_TIMEOUT_MAP[state][frame_req_type].timeout_ns, now))) {
				...
				fw_will_note(FW_LOGSIZE);
				proto_drv_frame_dump(&entry->frame);
				npu_session_dump(entry->frame.session);
...
static int retrive_lsm_stat(struct lsm_state_stat *stat, const struct lsm_getinfo_ops *getinfo_ops)
{
	...
	for (i = 0; i < all_entry_num; ++i) {
		...
		if (unlikely(qtype < 0 || qtype >= LSM_LIST_TYPE_INVALID)) {
		...
		if (unlikely(ts_entry_idx < 0 || ts_entry_idx >= LSM_ELEM_STATUS_TRACK_LEN)) {
...
static size_t stat_to_string(const struct lsm_state_stat *stat, char *outbuf, const size_t len)
{
	...
	int i, ret;
	struct timespec64 ts_avg_curr, ts_avg_init, ts_max_curr, ts_max_init;
	...
	for (i = 0; i < LSM_LIST_TYPE_INVALID; ++i) {
		...
		ts_avg_curr = ns_to_timespec64(stat->avg_curr[i]);
		ts_avg_init = ns_to_timespec64(stat->avg_init[i]);
		ts_max_curr = ns_to_timespec64(stat->max_curr[i]);
		ts_max_init = ns_to_timespec64(stat->max_init[i]);
		ret = scnprintf(outbuf, remain_len,
				"Average curr age %lld.%09ldns, init age %lld.%09ldns\n"
				" Max curr age %lld.%09ldns - entry at [%pK]\n"
				" Max init age %lld.%09ldns - entry at [%pK]\n",
...
static ssize_t proto_drv_dump_status(char *outbuf, const size_t len)
{
	...
	if (likely(ret > 0)) {
		...
		npu_err("dump error %d in output writing\n", ret);
		return -EFAULT;
	}
	if (unlikely(current_state != PROTO_DRV_STATE_OPENED)) {
		/* Protodrv is not operational. Just print current state */
		goto ok_exit;
	}

	/* Collect statistics */
	if (unlikely(retrive_lsm_stat(&frame_stat, &proto_frame_lsm_getinfo_ops) != 0)) {
		npu_err("fail in retrive_lsm_stat(FRAME)");
		return -EFAULT;
	}
	if (unlikely(retrive_lsm_stat(&nw_stat, &proto_nw_lsm_getinfo_ops) != 0)) {
...
int proto_drv_dump_debug_open(struct inode *inode, struct file *file)
{
	...
	npu_dbg("complete: [%lu] bytes output.\n", dump_data->len);
	return 0;
...
ssize_t proto_drv_dump_debug_read(struct file *file, char __user *outbuf, size_t outlen, loff_t *loff)
{
	...
	if (unlikely(ul_ret != 0)) {
		npu_err("error on copy_to_user: [%lu]\n", ul_ret);
		return -EFAULT;
	}
	dump_data->cur += copy_len;
	npu_dbg("complete, [%lu] bytes are read\n", copy_len);
	return copy_len;
}
...
int proto_drv_probe(struct npu_device *npu_device)
{
	...
	if (ret)
		probe_err("error in npu_debug_register\n");
