...
static int npu_qos_freq_qos_add_request(void)
{
	...
	if (!qos_setting->cl2_policy) {
		qos_setting->cl2_policy = cpufreq_cpu_get(8);
		if (qos_setting->cl2_policy) {
...
int npu_qos_probe(struct npu_system *system)
{
	...
	__npu_pm_qos_add_request(&qos_setting->npu_qos_req_npu0,
			PM_QOS_NPU0_THROUGHPUT, 0);
	__npu_pm_qos_add_request(&qos_setting->npu_qos_req_npu1,
			PM_QOS_NPU1_THROUGHPUT, 0);
#else
...
#ifdef CONFIG_SOC_S5E9935
	__npu_pm_qos_add_request(&qos_setting->npu_qos_req_npu0_max,
			PM_QOS_NPU0_THROUGHPUT_MAX,
			PM_QOS_NPU0_THROUGHPUT_MAX_DEFAULT_VALUE);
	__npu_pm_qos_add_request(&qos_setting->npu_qos_req_npu1_max,
...
#ifdef CONFIG_SOC_S5E9935
	qos_lock.npu_freq_maxlock = PM_QOS_NPU0_THROUGHPUT_MAX_DEFAULT_VALUE;
#else
	...
	qos_setting->skip_max_noti = false;
#ifdef CONFIG_SOC_S5E9935
	__npu_pm_qos_add_notifier(PM_QOS_NPU0_THROUGHPUT_MAX, &qos_setting->npu_qos_max_nb);
#else
...
#ifdef CONFIG_SOC_S5E9935
	__npu_pm_qos_update_request(&qos_setting->npu_qos_req_npu0, 0);
	__npu_pm_qos_update_request(&qos_setting->npu_qos_req_npu1, 0);
#else
...
#ifdef CONFIG_SOC_S5E9935
	__npu_pm_qos_update_request(&qos_setting->npu_qos_req_npu0_max,
			PM_QOS_NPU0_THROUGHPUT_MAX_DEFAULT_VALUE);
	__npu_pm_qos_update_request(&qos_setting->npu_qos_req_npu1_max,
			PM_QOS_NPU1_THROUGHPUT_MAX_DEFAULT_VALUE);
#else
...
npu_s_param_ret npu_qos_param_handler(struct npu_session *sess, struct vs4l_param *param)
{
	...
	case NPU_S_PARAM_QOS_NPU:
		qos_setting->req_npu_freq = (param->offset != NPU_QOS_DEFAULT_VALUE) ?
			param->offset : 0;
#ifdef CONFIG_SOC_S5E9935
		__req_param_qos(sess->uid, param->target, &qos_setting->npu_qos_req_npu0,
				qos_setting->req_npu_freq);
		__req_param_qos(sess->uid, param->target, &qos_setting->npu_qos_req_npu1,
				qos_setting->req_npu_freq);
#else
...
#ifdef CONFIG_SOC_S5E9935
		qos_setting->req_npu_freq = (param->offset != NPU_QOS_DEFAULT_VALUE) ?
			param->offset : PM_QOS_NPU0_THROUGHPUT_MAX_DEFAULT_VALUE;
		__req_param_qos(sess->uid, param->target, &qos_setting->npu_qos_req_npu0_max,
				qos_setting->req_npu_freq);
		__req_param_qos(sess->uid, param->target, &qos_setting->npu_qos_req_npu1_max,
				qos_setting->req_npu_freq);
#else
		qos_setting->req_npu_freq = (param->offset != NPU_QOS_DEFAULT_VALUE) ?
...
#ifdef CONFIG_SOC_S5E9935
	case NPU_S_PARAM_QOS_NPU0_MIN_PRESET:
		if (!npu_qos_preset_is_valid_value(param->offset))
			goto ok_preset_exit;
	...
	case NPU_S_PARAM_QOS_NPU1_MIN_PRESET:
		if (!npu_qos_preset_is_valid_value(param->offset))
			goto ok_preset_exit;
	...
	case NPU_S_PARAM_QOS_NPU0_MAX_PRESET:
		if (!npu_qos_preset_is_valid_value(param->offset))
			goto ok_preset_exit;
	...
	case NPU_S_PARAM_QOS_NPU1_MAX_PRESET:
		if (!npu_qos_preset_is_valid_value(param->offset))
			goto ok_preset_exit;

		qos_setting->skip_max_noti = true;
		qos_setting->req_npu_freq = (param->offset != NPU_QOS_DEFAULT_VALUE) ?
			param->offset : PM_QOS_NPU1_THROUGHPUT_MAX_DEFAULT_VALUE;
		__req_param_qos(sess->uid, param->target, &qos_setting->npu_qos_req_npu1_max,
				qos_setting->req_npu_freq);
		goto ok_preset_exit;
	...
	case NPU_S_PARAM_CPU_AFF:
		...
		for (i = 0; i < npu_system->irq_num; i++) {
