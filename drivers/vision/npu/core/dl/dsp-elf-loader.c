...
static void *__dsp_elf32_get_ptr(struct dsp_elf32 *elf, void *src, size_t size)
{
	if ((src < (void *)elf->data) ||
		...
		dsp_err("invalid ptr access\n");
		return NULL;
	}

	if (src + size < src) {
		dsp_err("ptr overflow\n");
		return NULL;
	}
...
static int __dsp_elf32_get_symtab(int idx, struct dsp_elf32 *elf)
{
	...
	if (!shdr) {
		dsp_err("shdr is NULL\n");
		return -1;
	}

	if (!shdr->sh_entsize) {
		dsp_err("sh_entsize must not be zero\n");
		return -1;
	}
	...
	struct dsp_list_head *TCMb_local, struct dsp_list_head *SFRw)
{
	...
	if (strncmp("DMb", mem_name, 3) == 0) {
		...
		if (strlen(local_name) == 13 &&
			strncmp(".thread_local", local_name, 13) == 0) {
			dsp_dbg("sec : %d to TCMb_local\n", idx);
			__dsp_elf32_idx_node_push_back(idx, TCMb_local);
		} else {
			dsp_dbg("sec : %d to TCMb\n", idx);
	...
	struct dsp_list_head *TCMb_local, struct dsp_list_head *SFRw)
{
	...
	if (head) {
		...
		if (ret == -1) {
			dsp_err("Failed rela node push back\n");
			return -1;
		}
	}
	return 0;
...
static int __dsp_elf32_get_extern_sym(struct dsp_elf32 *elf)
{
	...
	dsp_dbg("Elf32 get extern symbols\n");
...
int dsp_elf32_load(struct dsp_elf32 *elf, struct dsp_dl_lib_file *file)
{
	...
	for (idx = 0; idx < elf->hdr->e_shnum; idx++) {
		...
		if (shdr->sh_type == 2) {
			...
			ret = __dsp_elf32_get_mem_rela(idx, sec_name, elf,
					...
					&elf->SFRw.rela);
			if (ret == -1) {
				dsp_err("Failed to get mem rela\n");
				return -1;
			}
		}
	}
	...
	if (ret == -1) {
		dsp_err("Failed to get bss symhash\n");
		return -1;
	}

	ret = __dsp_elf32_get_extern_sym(elf);
	if (ret == -1) {
		dsp_err("Failed to get extern sym\n");
		return -1;
	}
...
void dsp_elf32_hdr_print(struct dsp_elf32 *elf)
{
	...
	dsp_dump("ABI VERSION : %d\n", header->e_ident[EI_ABIVERSION]);
	dsp_dump("OBJ Type : %s\n", elf_type[header->e_type]);
	dsp_dump("Machine id : %#x\n", header->e_machine);
	dsp_dump("Elf version : %d\n", header->e_version);
	dsp_dump("Entry point : %#x\n", header->e_entry);
	dsp_dump("Program header address : %#x\n", header->e_phoff);
	dsp_dump("Section header address : %#x\n", header->e_shoff);
	dsp_dump("Flag : %d\n", header->e_flags);
	dsp_dump("Header Size : %d\n", header->e_ehsize);
	dsp_dump("Program header size : %d\n", header->e_phentsize);
	dsp_dump("Program header entry : %d\n", header->e_phnum);
	dsp_dump("Section header size : %d\n", header->e_shentsize);
	dsp_dump("Section header entry : %d\n", header->e_shnum);
	dsp_dump("Section header string table index : %d\n",
		header->e_shstrndx);
}
...
{
	dsp_dump(DL_BORDER);
	dsp_dump("Elf32 mem : DMb_local(%u)\n",
...
{
	dsp_dump(DL_BORDER);
	dsp_dump("Elf32 mem : TCMb_local(%u)\n",
	...
	struct dsp_lib **libs, int libs_size)
{
	...
	for (idx = 0; idx < libs_size; idx++) {
		if (!libs[idx]->elf) {
			...
			dsp_dbg("Load ELF for library %s\n",
				libs[idx]->name);
