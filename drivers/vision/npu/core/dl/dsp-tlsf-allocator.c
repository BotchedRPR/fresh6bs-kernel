...
int dsp_tlsf_insert_block(struct dsp_tlsf_mem *mem, struct dsp_tlsf *tlsf)
{
	...
	dsp_dbg("TLSF insert block\n");

	if (mem->size & (tlsf->align - 1)) {
		dsp_err("size(0x%zx) align error\n", mem->size);
		return -1;
	}
	...
	struct dsp_tlsf *tlsf)
{
	...
	if (tlsf_idx.sl == -1) {
		tlsf_idx.fl = __find_bit_upper(tlsf->fl, __get_fl_size(tlsf),
				tlsf_idx.fl + 1);
		if (tlsf_idx.fl == -1) {
			dsp_dbg("Cannot find block due to fl\n");
			return -1;
		}
		...
		tlsf_idx.sl = __find_bit_upper(tlsf->sl[tlsf_idx.fl],
				TLSF_SL_SIZE, 0);
		if (tlsf_idx.sl == -1) {
			dsp_dbg("Cannot find block due to sl\n");
			return -1;
		}
	}

	dsp_dbg("Find block fl(%d), sl(%d)\n", tlsf_idx.fl, tlsf_idx.sl);

	head = __get_head_from_idx(tlsf, &tlsf_idx);
	if (head->num <= 0) {
		dsp_err("Block head is empty\n");
		return -1;
	}
	...
	size_t size, unsigned int align)
{
	...
	size = size & ~(align - 1);
	dsp_dbg("Tlsf max size aligned : %zu\n", size);
...
int dsp_tlsf_free(struct dsp_tlsf_mem *mem, struct dsp_tlsf *tlsf)
{
	...
	if (mem->type == MEM_EMPTY) {
		dsp_err("struct dsp_tlsf_mem is already empty\n");
		return -1;
	}
	...
	if (dsp_tlsf_is_prev_empty(merge)) {
		...
		if (!merge) {
			dsp_err("struct dsp_tlsf_mem Merge failed\n");
			return -1;
		}
	}
