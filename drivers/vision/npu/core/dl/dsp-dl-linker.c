...
void dsp_link_info_print(struct dsp_link_info *info)
{
	...
	dsp_dbg("text start address : 0x%lx\n", info->text);
	dsp_dbg("text size : %zu(0x%x)\n", info->text_size,
		(unsigned int)info->text_size);
	dsp_dbg("DMb start address : 0x%lx\n", info->DMb);
	dsp_dbg("DMb size : %zu(0x%x)\n", info->DMb_size,
		(unsigned int)info->DMb_size);
	dsp_dbg("DMb_local start address : 0x%lx\n", info->DMb_local);
	dsp_dbg("DMb_local size : %zu(0x%x)\n", info->DMb_local_size,
		(unsigned int)info->DMb_local_size);
	dsp_dbg("DRAMb start address : 0x%lx\n", info->DRAMb);
	dsp_dbg("DRAMb size : %zu(0x%x)\n", info->DRAMb_size,
		(unsigned int)info->DRAMb_size);
	dsp_dbg("TCMb start address : 0x%lx\n", info->TCMb);
	dsp_dbg("TCMb size : %zu(0x%x)\n", info->TCMb_size,
		(unsigned int)info->TCMb_size);
	dsp_dbg("TCMb_local start address : 0x%lx\n", info->TCMb_local);
	dsp_dbg("TCMb_local size : %zu(0x%x)\n", info->TCMb_local_size,
		(unsigned int)info->TCMb_local_size);
	dsp_dbg("SFRw start address : 0x%lx\n", info->SFRw);
	dsp_dbg("SFRw size : %zu(0x%x)\n", info->SFRw_size,
		(unsigned int)info->SFRw_size);

	dsp_dbg("Section address\n");
	...
	dsp_list_for_each(node, &info->reloc_sym) {
		...
		dsp_info("Symbol(%s) value(0x%x) addr(0x%x)\n",
	...
	char *kernel_name)
{
	...
	dsp_dbg("sh ndx: %u, sec addr: %lu, sym value: %u, align value: %u\n",
		sym->st_shndx, sec_addr, sym->st_value, sym_align);
	dsp_info("kernel name(%s) value(0x%x) addr(0x%x)\n",
	...
	struct dsp_lib **common_libs, int common_size)
{
	...
	for (idx = 0; idx < common_size; idx++) {
		...
		dsp_dbg("Find %s in common lib %s\n",
	...
	struct dsp_lib **common_libs, int common_size)
{
	...
	if (ret == -1) {
		...
		if (ruleidx >= rules->cnt || ruleidx >= RULE_MAX) {
			dsp_err("invalid ruleidx(%u/%d)\n",
					ruleidx, rules->cnt);
			return -1;
		}
		...
		ret = dsp_hash_get(&rela_info->elf->symhash, sym_str,
				(void **)&real_sym);
		if (ret == -1) {
			dsp_dbg("Sym(%s) is external variable\n", sym_str);
			...
			if (ret == -1) {
				dsp_err("Symbol(%s) is not found\n",
					sym_str);
				return -1;
			}
		} else {
			dsp_dbg("Sym(%s) is internal variable\n", sym_str);
			real_lib = lib;
			sym_info = rela_info;
		}
	...
	struct dsp_elf32_rela *rela)
{
	...
	if ((reloc_data > (elf->data + elf->size)) ||
			(reloc_data < elf->data)) {
		dsp_err("reloc_data is out of range(%#lx/%#zx)\n",
				(unsigned long)(reloc_data - elf->data),
				elf->size);
		return -1;
	}
	dsp_dbg("reloc_data : %#lx/%#zx\n",
			(unsigned long)(reloc_data - elf->data), elf->size);
	dsp_dbg("item_bit : %d, cnt : %d, align : %d\n",
			item_bit, item_cnt, item_align);
	dsp_dbg("cont bit sz : %d, item reversed : %d",
...
int dsp_linker_init(struct dsp_dl_lib_file *file)
{
	...
	if (ret == -1) {
		dsp_err("Failed to import reloc rule list\n");
		dsp_dl_free(rules);
		return -1;
	}
