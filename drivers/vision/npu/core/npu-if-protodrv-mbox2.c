...
#else
				profiler[PROFILE_DD_NPU_HARDWARE].duration += frame.duration;
#endif
		...
		*target = msgid_claim_get_ref(pool, msgid, PROTO_DRV_REQ_TYPE_FRAME);
		...
		if (likely(*target != NULL)) {
			...
			if ((*target)->is_fence) {
				...
				npu_hw_time = -1;
				dsp_hw_time = -1;
				frame_time = PWM_COUNTER_TOTAL_USEC((*target)->pwm_start_time - pwm_end_time);
				...
				npu_dbg("get kpi uid %d(%d), msgid %d, frame id %d, index %d, frame 0x%p\n",
					session->uid, (*target)->uid, msgid, (*target)->frame_id,
					(*target)->output->index, (*target));
				npu_dbg("kpi fence hw time NPU %d, DSP %d, Firmware = %d, Firmware_time_stamp = %d\n", npu_hw_time, dsp_hw_time, frame_time, frame.duration);
			} else {
#endif
				if (profiler[PROFILE_QBUF].node != NULL) {
					profiler[PROFILE_QBUF].node[0].duration = frame.now - profiler[PROFILE_QBUF].node[0].duration;
