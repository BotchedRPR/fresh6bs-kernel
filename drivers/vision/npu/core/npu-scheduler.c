...
static int npu_scheduler_init_dt(struct npu_scheduler_info *info)
{
	...
	for (i = 0; i < count; i += 2) {
		...
		if (!strcmp(dinfo->name, "NPU0") || !strcmp(dinfo->name, "NPU1")) {
			int core_idx = (strcmp(dinfo->name, "NPU0")) ? 1 : 0;
			int t_cnt;

			info->dvfs_NPU_freq[core_idx][0] = dinfo->max_freq;
			info->dvfs_NPU_freq_cnt[core_idx] = 1;
			f = dinfo->max_freq;
			...
			while (1) {
				...
				if (IS_ERR(opp)) {
					...
					info->dvfs_NPU_freq[core_idx][info->dvfs_NPU_freq_cnt[core_idx]] = f;
					dev_pm_opp_put(opp);
				}
				info->dvfs_NPU_freq_cnt[core_idx] = info->dvfs_NPU_freq_cnt[core_idx] + 1;

				if (f == dinfo->min_freq)
					break;
			}
			for (t_cnt = 0; t_cnt < info->dvfs_NPU_freq_cnt[core_idx]; t_cnt++) {
				probe_info("npu freq table[%d] [%d]->%d", core_idx, t_cnt, info->dvfs_NPU_freq[core_idx][t_cnt]);
			}
#endif
#if IS_ENABLED(CONFIG_NPU_USE_PI_DTM)
			info->pid_max_clk = (info->pid_max_clk < dinfo->max_freq) ? dinfo->max_freq : info->pid_max_clk;
#endif
		}
...
#if IS_ENABLED(CONFIG_NPU_USE_PI_DTM) || IS_ENABLED(CONFIG_NPU_USE_IFD)
static int npu_scheduler_get_lut_idx(struct npu_scheduler_info *info, int clk, int core_idx)
{
	int lut_idx;

	// 0 == Max clk index
	// info->dvfs_NPU_freq_cnt - 1 == Min clk index
	for (lut_idx = 0; lut_idx < info->dvfs_NPU_freq_cnt[core_idx]; lut_idx++) {
		if (clk >= info->dvfs_NPU_freq[core_idx][lut_idx])
			return lut_idx;
	}
	return lut_idx;
}
static int npu_scheduler_get_clk(struct npu_scheduler_info *info, int lut_idx, int core_idx)
{
	// info->dvfs_NPU_freq[0] == Max clk
	// info->dvfs_NPU_freq[info->dvfs_NPU_freq_cnt - 1] == Min clk
	if (lut_idx < 0)
		lut_idx = 0;
	else if (lut_idx > (info->dvfs_NPU_freq_cnt[core_idx] - 1))
		lut_idx = info->dvfs_NPU_freq_cnt[core_idx] -1;

	return info->dvfs_NPU_freq[core_idx][lut_idx];
}
#endif
#if IS_ENABLED(CONFIG_NPU_USE_PI_DTM)
bool npu_scheduler_get_DTM_flag(void)
{
	...
	return (info->curr_thermal >= DTM_NM_LIMIT_TH) ? TRUE : FALSE;
}
...
static void npu_scheduler_DTM(struct npu_scheduler_info *info)
{
	...
	if ((info->mode == NPU_PERF_MODE_NORMAL ||
		...
		if (npu_thermal >= DTM_NM_LIMIT_TH) {
			qt_freq = npu_scheduler_get_clk(info, npu_scheduler_get_lut_idx(info, DTM_NM_LIMIT_FREQ, 0), 0);
			npu_dbg("NORMAL mode DTM set freq : %d\n", qt_freq);
		}
		else {
			qt_freq = npu_scheduler_get_clk(info, 0, 0);
		}
		if (info->dtm_prev_freq != qt_freq) {
			mutex_lock(&info->exec_lock);
			list_for_each_entry(d, &info->ip_list, ip_list) {
				if (!strcmp("NPU0", d->name) || !strcmp("NPU1", d->name)) {
					info->dtm_prev_freq = npu_scheduler_set_freq(d, &d->qos_req_max, qt_freq);
				}
			}
			mutex_unlock(&info->exec_lock);
		}
	...
	//PID MODE Loop
		...
		else if (info->dtm_curr_freq < PID_MIN_FREQ)
			info->dtm_curr_freq = PID_MIN_FREQ;

		qt_freq = npu_scheduler_get_clk(info, npu_scheduler_get_lut_idx(info, info->dtm_curr_freq, 0), 0);
...
void npu_scheduler_qbuf_DVFS(struct npu_session *session)
{
	...
	list_for_each_entry(d, &info->ip_list, ip_list) {
		if ((!strcmp("NPU0", d->name) || !strcmp("NPU1", d->name)) && (dvfs_info->hids & NPU_HWDEV_ID_NPU)) {
			...
			dvfs_info->prev_freq = npu_scheduler_set_freq(d, &d->qos_req_min, cur_req_freq);
...
void npu_scheduler_dqbuf_DVFS(struct npu_session *session)
{
	...
	list_for_each_entry(d, &info->ip_list, ip_list) {
		if (((!strcmp("NPU0", d->name) || !strcmp("NPU1", d->name)) && (dvfs_info->hids & NPU_HWDEV_ID_NPU)) ||
			(!strcmp("DSP", d->name) && (dvfs_info->hids & NPU_HWDEV_ID_DSP))) {
			int core_idx = (strcmp(d->name, "NPU0")) ? 1 : 0;
			...
			if (dvfs_info->load > (1 << 10))	{	// (1 << 10) = 100% [measure / target]
				...
				while(1) {
					temp_freq = npu_scheduler_get_clk(info,
										npu_scheduler_get_lut_idx(info, dvfs_info->prev_freq, core_idx) - clk_idx, core_idx);
					...
					if ((temp_load > dvfs_info->load || dvfs_info->prev_freq >= d->max_freq) ||
						(clk_idx >= info->dvfs_NPU_freq_cnt[core_idx]))
						break;
					else
						clk_idx++;
				}
				dvfs_info->dvfs_clk_dn_delay = 0;
					...
					temp_freq = npu_scheduler_get_clk(info,
										npu_scheduler_get_lut_idx(info, dvfs_info->prev_freq, core_idx) + 1, core_idx);
					temp_load = (temp_freq << 10) / dvfs_info->prev_freq;
					...
					if (dvfs_info->dvfs_clk_dn_delay < 2)
	...
	case NPU_DVFS_INFO:
		{
			...
			i += scnprintf(buf + i, PAGE_SIZE - i, "IDX: [Linked] hid buf_size prev_f tpf load [NPU_OP/DSP_OP] Q[%d]\n", t_cnt);


			//mutex_lock(&g_npu_scheduler_info->dvfs_info_lock);
			list_for_each_entry(dvfs_info, &g_npu_scheduler_info->dsi_list, list) {
				if (dvfs_info->is_linked) {
					i += scnprintf(buf + i, PAGE_SIZE - i, "%d: %s %d %d %d %llu [%d/%d]\n"
						, dvfs_info_cnt
			...
			//mutex_unlock(&g_npu_scheduler_info->dvfs_info_lock);
		}
		break;
	...
	case NPU_DVFS_INFO_LIST:
		{
			...
			i += scnprintf(buf + i, PAGE_SIZE - i, "IDX: [Ln] hid buf_size prev_f tpf load [NPU_OP/DSP_OP] Q[%d] time\n", t_cnt);

			//mutex_lock(&g_npu_scheduler_info->dvfs_info_lock);
			list_for_each_entry(dvfs_info, &g_npu_scheduler_info->dsi_list, list) {
				i += scnprintf(buf + i, PAGE_SIZE - i, "%d: [%d] %s %d %d %d %llu [%d/%d] %lld\n"
					, dvfs_info_cnt++
					, dvfs_info->is_linked
					, (dvfs_info->hids == NPU_HWDEV_ID_NPU) ? "NPU" : "DSP"
					, (int)dvfs_info->buf_size
