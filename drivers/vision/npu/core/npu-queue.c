...
int npu_queue_qbuf(struct npu_queue *queue, struct vs4l_container_list *c)
{
	...
	struct vb_bundle *tmp_invb, *tmp_otvb;
	...
	invb = NULL;
	list_for_each_entry(tmp_invb, &inq->queued_list, queued_entry) {
		if (tmp_invb->state == VB_BUF_STATE_QUEUED) {
			invb = tmp_invb;
			otvb = NULL;
			list_for_each_entry(tmp_otvb, &otq->queued_list, queued_entry) {
				if (tmp_otvb->state == VB_BUF_STATE_QUEUED &&
						tmp_otvb->clist.index == invb->clist.index) {
					otvb = tmp_otvb;
					break;
				}
			}
