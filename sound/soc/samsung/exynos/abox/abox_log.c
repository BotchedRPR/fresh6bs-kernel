...
#define LAST_BUFF_SZ 960

struct ABOX_LOG_BUFFER *g_alogbuffer;
char *last_alog;

char *get_last_abox_log(void)
{
	int idx = 0;
	int tmp_idx = 0;
	int tmp_sz = 0;
	...
	if (!abox_is_on()) {
		pr_err("%s is skipped due to no power\n", __func__);
		return NULL;
	}
	...
	idx = g_alogbuffer->index_writer;

	if (idx < LAST_BUFF_SZ) {
		tmp_idx = g_alogbuffer->size - LAST_BUFF_SZ + idx;
		tmp_sz = LAST_BUFF_SZ - idx;
		memcpy(last_alog, g_alogbuffer->buffer + tmp_idx, tmp_sz);
		memcpy(last_alog + tmp_sz, g_alogbuffer->buffer, idx);
	} else {
		tmp_idx = idx - LAST_BUFF_SZ;
		memcpy(last_alog, g_alogbuffer->buffer + tmp_idx, LAST_BUFF_SZ);
	}

	return last_alog;

}
EXPORT_SYMBOL(get_last_abox_log);
...
#if IS_ENABLED(CONFIG_SND_SOC_SAMSUNG_AUDIO)
	if (id == 0)
		last_alog = devm_kzalloc(dev, LAST_BUFF_SZ, GFP_ATOMIC);
#endif
	spin_unlock_irqrestore(&abox_log_register_lock, flags);
