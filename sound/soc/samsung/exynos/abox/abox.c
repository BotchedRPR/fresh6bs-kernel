		...
		phys_addr_t addr, size_t bytes, void *area, int prot)
{
	...
	ret = iommu_map(data->iommu_domain, iova, addr, bytes, prot);
	if (ret < 0) {
		...
		struct abox_data *data, ABOX_IPC_MSG *msg)
{
	...
	case ABOX_REPORT_FAULT:
	{
		...
		abox_failsafe_report(dev, true);
#if IS_ENABLED(CONFIG_SND_SOC_SAMSUNG_AUDIO)
		lalog = get_last_abox_log();
		abox_err(dev, "==== last abox log start ====\n");
		if (lalog) {
			pr_info("%s\n", lalog);
		} else {
			abox_err(dev, "no lalog\n");
		}
		abox_err(dev, "==== last abox log end ====\n");
#endif
		break;
	}
...
static void abox_cleanup_sifs_cnt_val(struct abox_data *data)
{
	...
	for (i = ARRAY_SIZE(data->sifs_cnt_dirty) - 1; i >= 0; i--) {
		...
		writel_relaxed(ABOX_SIFS_CNT_VAL_MASK(i),
				b + ABOX_SPUS_CTRL_SIFS_CNT(i));
		abox_dbg(dev, "sifs%d_cnt_val: %#x\n", i, readl_relaxed(b +
				ABOX_SPUS_CTRL_SIFS_CNT(i)));
		/* enable rdma11 */
		...
		udelay(1);
		/* clear sifs_cnt_val */
		writel(0, b + ABOX_SPUS_CTRL_SIFS_CNT(i));
		abox_dbg(dev, "sifs%d_cnt_val: %#x\n", i, readl_relaxed(b +
				ABOX_SPUS_CTRL_SIFS_CNT(i)));
		/* disable rdma11 */
...
static void abox_qos_request_add(struct abox_data *data, int cpu_id)
{
	struct cpufreq_policy *cpufreq_policy;

	cpufreq_policy = cpufreq_cpu_get(cpu_id);
	if (cpufreq_policy) {
		freq_qos_tracer_add_request(&cpufreq_policy->constraints,
				&data->max_qos_req, FREQ_QOS_MAX, INT_MAX);
		cpufreq_cpu_put(cpufreq_policy);
	} else {
		abox_err(data->dev, "abox: cpu_cluster0_policy is NULL");
	}
	abox_info(data->dev, "%s : %d\n", __func__, cpu_id);
}

static void abox_update_qos_request(struct abox_data *data, s32 freq)
{
	if (freq_qos_update_request(&data->max_qos_req, freq) < 0)
		abox_info(data->dev, "Failed to update max freq constraint\n");
...
static int abox_enable(struct device *dev)
{
	...
	abox_update_qos_request(data, 1200000);
...
static int abox_disable(struct device *dev)
{
	...
	abox_update_qos_request(data, INT_MAX);
...
static int samsung_abox_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_string(np, "samsung,modify-slog-size",
			&data->modify_slog_size);
	if (ret < 0)
		data->modify_slog_size = "16"; /* default slog size */
	abox_info(dev, "modify_slog_size : %s\n", data->modify_slog_size);
...
static int samsung_abox_remove(struct platform_device *pdev)
{
	...
	freq_qos_tracer_remove_request(&data->max_qos_req);
