...
unsigned int abox_dma_read_pointer(struct abox_dma_data *data)
{
		...
		struct snd_pcm_substream *substream)
{
	...
	pointer = abox_dma_progress(data->cmpnt) ? abox_dma_read_pointer(data) : 0;
	abox_dbg(dev, "%s: pointer=%#x\n", __func__, pointer);
...
static const struct snd_kcontrol_new abox_dma_hw_params_controls[] = {
...
int abox_dma_add_hw_params_controls(struct snd_soc_component *cmpnt)
{
	return snd_soc_add_component_controls(cmpnt, abox_dma_hw_params_controls,
			ARRAY_SIZE(abox_dma_hw_params_controls));
}
...
static const struct snd_kcontrol_new abox_dma_func_controls[] = {
...
int abox_dma_add_func_controls(struct snd_soc_component *cmpnt)
{
	return snd_soc_add_component_controls(cmpnt, abox_dma_func_controls,
			ARRAY_SIZE(abox_dma_func_controls));
}
...
static SOC_VALUE_ENUM_SINGLE_DECL(abox_ddma_src_enum, DMA_REG_CTRL,
		ABOX_DMA_DEBUG_SRC_L, ABOX_DMA_DEBUG_SRC_MASK >> ABOX_DMA_DEBUG_SRC_L,
...
static int abox_ddma_probe(struct snd_soc_component *cmpnt)
{
	int ret;
