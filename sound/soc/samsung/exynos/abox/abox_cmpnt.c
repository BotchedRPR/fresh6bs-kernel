		...
		unsigned int channels, enum ABOX_CONFIGMSG configmsg)
{
	...
	case SET_SIFS0_FORMAT:
		...
		/* Set STMIX format */
		regmap_update_bits(regmap, ABOX_SPUS_CTRL_STMIX_FORMAT,
...
static int companion_mode_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = cmpnt->dev;
	...
	abox_dbg(dev, "%s: %u\n", __func__, data->companion_mode);

	item = snd_soc_enum_val_to_item(e, data->companion_mode);
	ucontrol->value.enumerated.item[0] = item;

	return 0;
}

static int companion_mode_put_ipc(struct device *dev, enum companion_mode mode)
{
	struct abox_data *data = dev_get_drvdata(dev);
	ABOX_IPC_MSG msg;
	struct IPC_SYSTEM_MSG *system_msg = &msg.msg.system;
	...
	msg.ipcid = IPC_SYSTEM;
	system_msg->msgtype = ABOX_COMPANION_MODE;
	system_msg->param1 = data->companion_mode = mode;
	return abox_request_ipc(dev, msg.ipcid, &msg, sizeof(msg), 0, 0);
}

static int companion_mode_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *cmpnt = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = cmpnt->dev;
	...
	return companion_mode_put_ipc(dev, mode);
}
...
static const char * const companion_mode_enum_texts[] = {
	"DISABLE",
	"ENABLE",
};

static const unsigned int companion_mode_enum_values[] = {
	MODE_DISABLE,
	MODE_ENABLE,
};
SOC_VALUE_ENUM_SINGLE_DECL(companion_mode_enum, SND_SOC_NOPM, 0, 0,
		companion_mode_enum_texts, companion_mode_enum_values);
...
static const struct snd_kcontrol_new cmpnt_controls[] = {
	...
	SOC_VALUE_ENUM_EXT("Companion Mode", companion_mode_enum,
			companion_mode_get, companion_mode_put),
	...
	SOC_SINGLE_EXT("NSRC0 Bridge", ABOX_ROUTE_CTRL2,
			ABOX_NSRC_CONNECTION_TYPE_L(0), 1, 0,
			nsrc_bridge_get, nsrc_bridge_put),
	SOC_SINGLE_EXT("NSRC1 Bridge", ABOX_ROUTE_CTRL2,
			ABOX_NSRC_CONNECTION_TYPE_L(1), 1, 0,
			nsrc_bridge_get, nsrc_bridge_put),
	SOC_SINGLE_EXT("NSRC2 Bridge", ABOX_ROUTE_CTRL2,
			ABOX_NSRC_CONNECTION_TYPE_L(2), 1, 0,
			nsrc_bridge_get, nsrc_bridge_put),
	SOC_SINGLE_EXT("NSRC3 Bridge", ABOX_ROUTE_CTRL2,
			ABOX_NSRC_CONNECTION_TYPE_L(3), 1, 0,
			nsrc_bridge_get, nsrc_bridge_put),
	SOC_SINGLE_EXT("NSRC4 Bridge", ABOX_ROUTE_CTRL2,
...
static enum abox_dai get_source_dai_id(struct abox_data *data, enum abox_dai id)
{
	...
	case ABOX_RDMA6_BE:
	case ABOX_RDMA7_BE:
		idx = id - ((id < ABOX_RDMA4_BE) ? ABOX_RDMA4 : ABOX_RDMA4_BE);
	...
	case ABOX_RDMA10_BE:
	case ABOX_RDMA11_BE:
		idx = id - ((id < ABOX_RDMA8_BE) ? ABOX_RDMA8 : ABOX_RDMA8_BE);
		...
		const struct snd_pcm_hw_params *params)
{
	...
	abox_info(adev, "%s - %u, %x, %u \n", __func__, rate, format, channels);
		...
		unsigned int tgt_width, int tune)
{
	...
	abox_info(dev, "%s(%d, %d, %d, %uHz, %uHz, %ubit, %d)\n", __func__,
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out0_enum,
		ABOX_SPUS_CTRL_FC0, ABOX_FUNC_CHAIN_SRC_OUT_L(0),
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out1_enum,
		ABOX_SPUS_CTRL_FC0, ABOX_FUNC_CHAIN_SRC_OUT_L(1),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out2_enum,
		ABOX_SPUS_CTRL_FC0, ABOX_FUNC_CHAIN_SRC_OUT_L(2),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out3_enum,
		ABOX_SPUS_CTRL_FC0, ABOX_FUNC_CHAIN_SRC_OUT_L(3),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out4_enum,
		ABOX_SPUS_CTRL_FC1, ABOX_FUNC_CHAIN_SRC_OUT_L(4),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out5_enum,
		ABOX_SPUS_CTRL_FC1, ABOX_FUNC_CHAIN_SRC_OUT_L(5),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out6_enum,
		ABOX_SPUS_CTRL_FC1, ABOX_FUNC_CHAIN_SRC_OUT_L(6),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out7_enum,
		ABOX_SPUS_CTRL_FC1, ABOX_FUNC_CHAIN_SRC_OUT_L(7),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out8_enum,
		ABOX_SPUS_CTRL_FC2, ABOX_FUNC_CHAIN_SRC_OUT_L(8),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out9_enum,
		ABOX_SPUS_CTRL_FC2, ABOX_FUNC_CHAIN_SRC_OUT_L(9),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out10_enum,
		ABOX_SPUS_CTRL_FC2, ABOX_FUNC_CHAIN_SRC_OUT_L(10),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_AUTODISABLE_DECL(spus_out11_enum,
		ABOX_SPUS_CTRL_FC2, ABOX_FUNC_CHAIN_SRC_OUT_L(11),
		spus_outx_mask, spus_outx_texts, spus_outx_values);
...
static SOC_VALUE_ENUM_SINGLE_DECL(spusm_enum, ABOX_ROUTE_CTRL1,
...
static const unsigned int spust_mask =
		ABOX_ROUTE_SPUSM_MASK >> ABOX_ROUTE_SPUSM_L;
static SOC_VALUE_ENUM_SINGLE_DECL(spust_enum, ABOX_ROUTE_CTRL2,
...
static SOC_ENUM_SINGLE_DECL(sifs1_enum, ABOX_SPUS_CTRL1,
		ABOX_SIFS_OUT_SEL_L(0), sifsx_texts);
...
static SOC_ENUM_SINGLE_DECL(sifst_enum, ABOX_SPUS_CTRL2,
...
static const enum abox_dai nsrcx_dai[] = {
	...
	ABOX_SIFS3, ABOX_SIFS4, ABOX_SIFS5,
	ABOX_UAIF0, ABOX_UAIF1, ABOX_UAIF2, ABOX_UAIF3,
	ABOX_UAIF4, ABOX_UAIF5, ABOX_UAIF6,
	ABOX_SPDY,
};
...
static SOC_ENUM_SINGLE_DECL(sifm7_enum, ABOX_SPUM_CTRL0,
...
static SOC_ENUM_SINGLE_DECL(sifms_enum, ABOX_SPUM_CTRL1,
		...
		unsigned int sbank_size)
{
	...
	abox_info(cmpnt->dev, "%s(t: %d, s: %x)\n", __func__, time, sbank_size);
...
int abox_cmpnt_reset_cnt_val(struct abox_data *data, enum abox_dai id)
{
	...
	if (src_id < ABOX_SIFS0 || src_id > ABOX_SIFS5)
		return -EINVAL;
...
int abox_cmpnt_restore(struct device *adev)
{
	...
	if (data->companion_mode)
		companion_mode_put_ipc(adev, data->companion_mode);
