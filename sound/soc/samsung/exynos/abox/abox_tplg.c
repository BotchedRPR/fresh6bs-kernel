...
{
	struct abox_tplg_kcontrol_data *kdata, *ret = NULL;
...
static void abox_tplg_add_kcontrol_data(struct abox_tplg_kcontrol_data *kdata)
{
	mutex_lock(&kcontrol_mutex);
		...
		struct abox_tplg_pipelines *pls, int pl_id)
{
	...
	abox_dbg(dev, "%s:pl_id(%d) <-> count(%d)\n", __func__, pl_id, pls->count);
		...
		struct snd_soc_tplg_ctl_hdr *hdr)
{
	...
	abox_tplg_add_kcontrol_data(kdata);
		...
		struct snd_soc_tplg_pcm *pcm, struct snd_soc_dai *dai)
{
	...
	aaudio = abox_tplg_get_bool(&pcm->priv, ABOX_TKN_AAUDIO);

	dev_platform = abox_vdma_register_component(dev, dai_drv->id,
			dai_drv->name, aaudio, &playback, &capture);
...
static bool abox_tplg_is_in_dapm_context(struct snd_soc_component *cmpnt, const char *widget_name)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(cmpnt);
	...
	full_name = kasprintf(GFP_KERNEL, "%s %s", cmpnt->name_prefix, widget_name);

	for_each_card_widgets(cmpnt->card, w) {
		if (w->dapm == dapm && !strcmp(w->name, full_name)) {
			ret = true;
			break;
		}
	}
...
static int abox_tplg_add_route_mux(struct snd_soc_component *cmpnt,
		struct abox_tplg_widget_data *wdata, const char *source)
{
	...
	if (abox_tplg_is_in_dapm_context(cmpnt, source)) {
		dapm = snd_soc_component_get_dapm(cmpnt);
		route.sink = tplg_w->name;
	} else {
		dapm = &cmpnt->card->dapm;
		route.sink = wdata->w->name;
	}
		...
		struct abox_tplg_widget_data *wdata)
{
	...
	struct soc_enum *e = (struct soc_enum *)kcontrol_new->private_value;
	int i;
	 ...
	 */
	for (i = 0; i < e->items; i++)
		abox_tplg_add_route_mux(cmpnt, wdata, e->texts[i]);
}
		...
		struct snd_soc_tplg_private *priv)
{
	...
	for (i = 0; ret >= 0; i++) {
		...
		ret = abox_add_extra_firmware(dev, data, name, area, offset, changeable);
	}
...
int abox_tplg_load_extra_firmware(struct snd_soc_component *cmpnt, const char *fw_name)
{
	...
	if (fw) {
		abox_info(dev, "%s already loaded\n", fw_name);
		return 0;
	}

	ret = firmware_request_nowarn(&fw, fw_name, dev);
	if (ret < 0) {
		abox_err(dev, "%s request failed\n", fw_name);
		return ret;
	}

	ret = snd_soc_tplg_component_load(cmpnt, &abox_tplg_ops, fw);
	if (ret < 0) {
		abox_err(dev, "%s load failed\n", fw_name);
		release_firmware(fw);
		fw = NULL;
		return ret;
	}

	abox_info(dev, "%s loaded\n", fw_name);
...
int abox_tplg_probe(struct snd_soc_component *cmpnt)
{
	...
	for (tplg_fw = abox_tplg_fw; tplg_fw - abox_tplg_fw < ARRAY_SIZE(abox_tplg_fw); tplg_fw++) {
		...
		ret = request_firmware_direct(&tplg_fw->fw, tplg_fw->fw_name, dev);
		...
		ret = snd_soc_tplg_component_load(cmpnt, &abox_tplg_ops, tplg_fw->fw);
		if (ret < 0)
			goto err_load;
	}
...
err_ipc:
	snd_soc_tplg_component_remove(cmpnt);
err_load:
	for (tplg_fw = abox_tplg_fw; tplg_fw - abox_tplg_fw < ARRAY_SIZE(abox_tplg_fw); tplg_fw++) {
		release_firmware(tplg_fw->fw);
		tplg_fw->fw = NULL;
