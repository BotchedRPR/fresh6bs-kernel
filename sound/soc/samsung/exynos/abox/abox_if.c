		...
		struct snd_pcm_hw_params *hw_params, struct snd_soc_dai *dai)
{
	...
	if (data->slave) {
		...
		abox_info(dev, "%s - mclk set rate\n", __func__);
		ret = clk_set_rate(data->clk_mclk, 24576000);
		if (ret < 0) {
			abox_err(dev, "mclk set error: %d\n", ret);
			return ret;
		}
	}
...
static int abox_if_set_sysclk(struct snd_soc_dai *dai,
		int clk_id, unsigned int freq, int dir)
{
	...
	if (freq != bclk) {
		abox_err(dev, "Invalid CLK Config freq(%d): %u\n", clk_id, freq);
		freq = bclk;
	}

	abox_register_bclk_usage(dev, abox_data, dai_id, rate, channels, width);

	ret = clk_set_rate(data->clk_bclk, freq);
	if (ret < 0) {
		abox_err(dev, "bclk set error: %d\n", ret);
		return ret;
	}

	abox_info(dev, "%s : rate=%u, width=%u, channel=%u, bclk=%lu\n",
			__func__, rate, width, channels, clk_get_rate(data->clk_bclk));
	return 0;
}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	data->config[reg] = val;
	if (dai_id == ABOX_SPDY) {
		abox_info(dev, "skip to bclk in abox spdy\n");
		return 0;
	}
		...
		struct snd_pcm_hw_params *params)
{
	...
	if (dai->driver->symmetric_rate && dai->rate && dai->rate != rate)
		rate = dai->rate;
	if (dai->driver->symmetric_channels && dai->channels &&
			dai->channels != channels)
		channels = dai->channels;
	if (dai->driver->symmetric_sample_bits && dai->sample_bits) {
