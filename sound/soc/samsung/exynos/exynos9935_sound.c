...
/*
 *  Sound Card Driver for Exynos9935
 *
...
#if IS_ENABLED(CONFIG_SND_SOC_CS40L26)
...
static struct snd_soc_dai_link *get_dai_link(struct snd_soc_card *card, int id)
{
	struct snd_soc_dai_link *dai_link;
	int i;
...
static struct snd_soc_dai *get_codec_dai(struct snd_soc_card *card, int id)
{
	struct snd_soc_dai_link *dai_link;
	struct snd_soc_dai_link_component *codec;

	dai_link = get_dai_link(card, id);
	if (!dai_link)
		return NULL;

	codec = asoc_link_to_codec(dai_link, 0);
	if (!codec)
		return NULL;

	return snd_soc_find_dai_with_mutex(codec);
}
...
static int cs40l26_hw_params(struct snd_pcm_substream *substream,
			       struct snd_pcm_hw_params *params)
{
	struct snd_soc_pcm_runtime *rtd = asoc_substream_to_rtd(substream);
	struct snd_soc_dai *codec_dai;
...
static const struct snd_soc_ops cs40l26_ops = {
	.hw_params = cs40l26_hw_params,
};
...
static void cs40l26_request_firmware_work_func(struct work_struct *work)
{
#if IS_ENABLED(CONFIG_SND_SOC_CS40L26)
	...
	codec_dai = get_codec_dai(card, CS40L26_DAI_ID);
	if (!codec_dai)
		return;

	dev = codec_dai->dev;
	cs40l26 = dev_get_drvdata(dev->parent);
	...
	if (ret < 0) {
		dev_info(dev, "retry request firmware\n");
		schedule_delayed_work(to_delayed_work(work), msecs_to_jiffies(1000));
	}
#endif
}
static DECLARE_DELAYED_WORK(cs40l26_request_firmware_work, cs40l26_request_firmware_work_func);
...
static void exynos_register_card_work_func(struct work_struct *work)
{
	...
	schedule_delayed_work(&cs40l26_request_firmware_work, 0);
}
	...
	for_each_child_of_node(np, dai) {
		...
		if (!link->codecs) {
			...
			if (link->codecs) {
				...
				} else if (strstr(link->codecs[0].dai_name, "cs40l26")) {
					link->id = CS40L26_DAI_ID;
					link->ops = &cs40l26_ops;
				}
			}
		}
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos9935-sound");
