...
u32 vts_set_baaw(void __iomem *sfr_base, u64 base, u32 size)
{
	...
	/* set VTS BAAW config */
	/* ALIVE SFR */
	writel(0x40300000, sfr_base + 0x0);
	writel(0x40400000, sfr_base + 0x4);
	writel(0x12900000, sfr_base + 0x8);
	writel(0x80000003, sfr_base + 0xC);

	/* DRAM */
	pr_info("[vts] %s: %d start_addr:0x%x\n", __func__, __LINE__, VTS_BAAW_BASE);
	pr_info("[vts] %s: %d end_addr:0x%x\n", __func__, __LINE__, (VTS_BAAW_BASE + aligned_size));
	pr_info("[vts] %s: %d remapped_addr:0x%x\n", __func__, __LINE__, aligned_base);
