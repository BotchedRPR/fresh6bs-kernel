...
int aud9001x_pmic_read_reg(u8 slave, u8 reg, u8 *dest)
{
	int ret = 0;

	ret = exynos_acpm_read_reg(codec_np, 0, slave, reg, dest);
...
int aud9001x_pmic_write_reg(u8 slave, u8 reg, u8 value)
{
	int ret = 0;

	ret = exynos_acpm_write_reg(codec_np, 0, slave, reg, value);
...
int aud9001x_pmic_update_reg(u8 slave, u8 reg, u8 value, u8 mask)
{
	int ret = 0;

	ret = exynos_acpm_update_reg(codec_np, 0, slave, reg, value, mask);
...
static int aud9001x_reg_hw_write(void *context, unsigned int reg,
		unsigned int value)
{
	return aud9001x_pmic_write_reg(CODEC_ADDR, (u8)reg, (u8)value);
}

static int aud9001x_reg_hw_read(void *context, unsigned int reg,
		unsigned int *value)
{
	return aud9001x_pmic_read_reg(CODEC_ADDR, (u8)reg, (u8 *)value);
}
...
void aud9001x_adc_digital_mute(struct aud9001x_priv *aud9001x, bool on)
{
	...
	if (on)
		regmap_update_bits(aud9001x->regmap, AUD9001X_46_ADC1,
...
static void aud9001x_dac_soft_mute(struct aud9001x_priv *aud9001x, bool on)
{
	...
	if (on)
		regmap_update_bits(aud9001x->regmap, AUD9001X_50_DAC1,
...
static void aud9001x_adc_mute_work(struct work_struct *work)
{
	...
	regmap_read(aud9001x->regmap, AUD9001X_6C_SW_RESERV, &chop_val);
...
static void aud9001x_chk_clk(struct aud9001x_priv *aud9001x)
{
	unsigned int lrclk, bclk, temp;

	regmap_read(aud9001x->regmap, AUD9001X_67_CED_CHK1, &temp);
	regmap_read(aud9001x->regmap, AUD9001X_68_CED_CHK2, &lrclk);
	regmap_read(aud9001x->regmap, AUD9001X_69_CED_CHK3, &bclk);

	lrclk += ((temp & 0xF) << 8);
	dev_info(aud9001x->dev, "%s called, LRCLK: 0x%04X, BCLK: 0x%02X\n",
			__func__, lrclk, bclk);
}
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		regmap_update_bits(aud9001x->regmap, AUD9001X_40_PD,
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* DMIC1 IO Enable */
		regmap_update_bits(aud9001x->regmap, AUD9001X_0F_IO_CTRL1,
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		/* DCT Clock Invert */
		regmap_update_bits(aud9001x->regmap, AUD9001X_72_CLK3_COD,
				CLK_DA_INV_MASK, CLK_DA_INV_MASK);
		break;
	case SND_SOC_DAPM_POST_PMU:
		/* Cap Init */
		regmap_write(aud9001x->regmap, AUD9001X_DB_CTRL_SPKS1, 0xA8);
		break;
	case SND_SOC_DAPM_PRE_PMD:
		break;
	case SND_SOC_DAPM_POST_PMD:
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		if (aud9001x->regulator_count2 == 0) {
			aud9001x->regulator_count2 = 1;
			ret = regulator_enable(aud9001x->vdd2);
			dev_info(codec->dev, "%s enable vdd2: %d\n",
					__func__, aud9001x->regulator_count2);
			aud9001x_usleep(1000);
		}
		/* Boost Control Enable */
		regmap_update_bits(aud9001x->regmap, AUD9001X_94_BST_CTL,
	...
	case SND_SOC_DAPM_POST_PMU:
		/* SPK APW On */
		regmap_update_bits(aud9001x->regmap, AUD9001X_18_PWAUTO_DA,
		...
		/* DCT Pre-Charge Off */
		regmap_update_bits(aud9001x->regmap, AUD9001X_14_PD_DA1,
				EN_DCT_PREQ_MASK, 0);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		if (aud9001x->regulator_count2 == 1) {
			aud9001x->regulator_count2 = 0;
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	aud9001x_pmic_read_reg(PMIC_ADDR, PMIC_DMIC_BIAS_REG, &dmic_bias);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value)
		aud9001x_pmic_update_reg(PMIC_ADDR, PMIC_DMIC_BIAS_REG,
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	regmap_read(aud9001x->regmap, AUD9001X_FC_AVC_PARA28, &val);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (value)
		regmap_update_bits(aud9001x->regmap, AUD9001X_FC_AVC_PARA28,
		...
		struct snd_soc_dai *dai)
{
	...
	case BIT_RATE_16:
		/* I2S 16bit setting */
		regmap_update_bits(aud9001x->regmap, AUD9001X_42_IF1_FORM2,
				I2S1_DL_MASK, I2S1_DL_16BIT << I2S1_DL_SHIFT);

		/* I2S 32fs setting */
		regmap_update_bits(aud9001x->regmap, AUD9001X_43_IF1_FORM3,
...
static void aud9001x_i2c_parse_dt(struct aud9001x_priv *aud9001x)
{
	...
	unsigned int dmic_osr;
	int ret;

	codec_np = np;

	/* DMIC Clk */
	ret = of_property_read_u32(dev->of_node, "dmic-clk", &dmic_osr);
	if (!ret) {
		if (dmic_osr == AUD9001X_DMIC_CLK_1_536MHZ)
			aud9001x->dmic_osr = OSR32; // 32*fs (@48kHz, 1.536 Mhz)
		else if (dmic_osr == AUD9001X_DMIC_CLK_3_072MHZ)
			aud9001x->dmic_osr = OSR64; // 64*fs (@48kHz, 3.072Mhz)
	}

	dev_info(dev, "Property 'dmic-clk' %u", aud9001x->dmic_osr);
...
static void aud9001x_register_initialize(void *context)
{
	...
	regmap_write(aud9001x->regmap, AUD9001X_80_PDB_ACC1, 0x02);
	aud9001x_usleep(15000);

	/* DCT pre-charge On */
	regmap_write(aud9001x->regmap, AUD9001X_14_PD_DA1, 0x20);

	/* IO Setting for DMIC */
	regmap_write(aud9001x->regmap, AUD9001X_0F_IO_CTRL1, 0x00);
	regmap_write(aud9001x->regmap, AUD9001X_40_PD, 0x58);

	/* I2S interface setting */
	regmap_write(aud9001x->regmap, AUD9001X_41_IF1_FORM1, 0x00);
	regmap_write(aud9001x->regmap, AUD9001X_42_IF1_FORM2, 0x10);
	regmap_write(aud9001x->regmap, AUD9001X_43_IF1_FORM3, 0x20);
	regmap_write(aud9001x->regmap, AUD9001X_44_IF1_FORM4, 0xE4);
	regmap_write(aud9001x->regmap, AUD9001X_45_IF1_FORM5, 0x10);

	/* DMIC control setting */
	regmap_write(aud9001x->regmap, AUD9001X_49_DMIC_CTR1, 0x46);
	regmap_write(aud9001x->regmap, AUD9001X_4A_DMIC_CTR2, 0x20);
	regmap_update_bits(aud9001x->regmap, AUD9001X_4A_DMIC_CTR2,
			DMIC_OSR_MASK, aud9001x->dmic_osr << DMIC_OSR_SHIFT);

	/* ADC digital volume setting */
	regmap_write(aud9001x->regmap, AUD9001X_4B_AVOLL1_SUB, 0x54);
	regmap_write(aud9001x->regmap, AUD9001X_4C_AVOLR1_SUB, 0x54);
	...
	/* AVC LR Setting */
	regmap_update_bits(aud9001x->regmap, AUC9001X_2B_AVC_Para40,
			MUTE_CH_SEL_MASK, MUTE_CH_SEL_L_AND_R << MUTE_CH_SEL_SHIFT);

	/* DAC digital volume setting */
	regmap_write(aud9001x->regmap, AUD9001X_51_DVOLL, 0x54);
	regmap_write(aud9001x->regmap, AUD9001X_52_DVOLR, 0x54);

	/* DAC, ADC digital mute */
	regmap_write(aud9001x->regmap, AUD9001X_50_DAC1, 0x02);
	regmap_write(aud9001x->regmap, AUD9001X_46_ADC1, 0x0D);
...
static void aud9001x_regulators_enable(struct snd_soc_component *codec)
{
	...
	if (aud9001x->regulator_count == 0) {
		...
		ret = regulator_enable(aud9001x->vdd);
		aud9001x_usleep(1000);
...
static void aud9001x_regmap_sync(struct device *dev)
{
	...
	unsigned int reg[AUD9001X_MAX_REGISTER] = {0,};
	int i;
	...
	for (i = 0; i < AUD9001X_REGCACHE_SYNC_END_REG; i++)
		if (aud9001x_readable_register(dev, i) &&
			...
			regmap_read(aud9001x->regmap, i, &reg[i]);
	...
	for (i = 0; i < AUD9001X_REGCACHE_SYNC_END_REG ; i++)
		if (aud9001x_writeable_register(dev, i) &&
				(!aud9001x_volatile_register(dev, i)))
			regmap_write(aud9001x->regmap, i, reg[i]);
...
static void aud9001x_reg_restore(struct snd_soc_component *codec)
{
	...
	regmap_update_bits(aud9001x->regmap, AUD9001X_80_PDB_ACC1,
