...
/*
 *  Sound Card Driver for Exynos8535
 *
...
static struct madera_drvdata exynos_drvdata;
			       ...
			       struct snd_pcm_hw_params *params)
{
	...
	for_each_rtd_codec_dais(rtd, i, codec_dai) {
		ret = snd_soc_component_set_sysclk(codec_dai->component,
				...
				SND_SOC_CLOCK_IN);
		if (ret < 0)
			dev_err(codec_dai->dev, "%s: set codec sysclk failed: %d\n",
					codec_dai->name, ret);
	}
...
static int exynos_late_probe(struct snd_soc_card *card)
{
	struct madera_drvdata *drvdata = card->drvdata;
	struct snd_soc_pcm_runtime *rtd;
	struct snd_soc_dai *aif_dai, *amp_dai, *dai;
	struct snd_soc_dapm_context *dapm;
	struct snd_soc_component *amp;
	const char *name;
	int ret, i;
...
static int exynos_sound_probe(struct platform_device *pdev)
{
	...
	for_each_child_of_node(np, dai) {
		...
		if (!link->codecs) {
			...
			if (link->codecs && strstr(link->codecs[0].dai_name, "cs35l41")) {
...
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:exynos8535-sound");
