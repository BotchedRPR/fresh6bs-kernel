...
/* This feature is that EMS uses cpuset cgroup instead of cpuctl cgroup
 * For compatibility, cgroup index convert from cpuset to cpuctl
 */
#define EMS_USE_CPUSET_CGROUP
#endif
...
#ifdef EMS_USE_CPUSET_CGROUP
int cgroup_cpuset_to_cpu_idx(int cpuset_idx)
{
	int cpu_idx = CGROUP_ROOT;
	...
	case 0:
		...
		break;
	case 1:
		cpu_idx = CGROUP_FOREGROUND;
		break;
	case 2:
		cpu_idx = CGROUP_BACKGROUND;
		break;
	case 3:
		cpu_idx = CGROUP_SYSTEM_BACKGROUND;
		break;
	case 4: // RESTRCITED
		cpu_idx = CGROUP_ROOT;
		break;
	case 5:
		cpu_idx = CGROUP_TOPAPP;
		break;
	case 6:
		cpu_idx = CGROUP_CAMERA_DAEMON;
		break;
	default:
		cpu_idx = CGROUP_ROOT;
		break;
...
int cpuctl_task_group_idx(struct task_struct *p)
{
	...
	idx = css->id - 1;
#ifdef EMS_USE_CPUSET_CGROUP
	idx = cgroup_cpuset_to_cpu_idx(idx);
#endif
	rcu_read_unlock();
...
bool ems_is_rt_policy(int policy)
{
	return policy == SCHED_FIFO || policy == SCHED_RR;
}

bool ems_is_fair_policy(int policy)
{
	return policy == SCHED_NORMAL || policy == SCHED_BATCH;
}

bool ems_binder_supported_policy(int policy)
{
	return ems_is_fair_policy(policy) || ems_is_rt_policy(policy);
}
